  ...
  final class ParseTreeDetailViewModelTests: XCTestCase {
    func testSelectingNodePublishesDetailAndHexSlice() async throws {
      ...
      let annotation = PayloadAnnotation(
        ...
        byteRange: node.header.payloadRange.lowerBound..<(node.header.payloadRange.lowerBound + 4))
    ...
    func testSelectingAnnotationUpdatesHighlight() async throws {
      ...
      let provider = HexSliceProviderStub(
        result: HexSlice(
          offset: node.header.payloadRange.lowerBound, bytes: Data(repeating: 0x00, count: 16)))
      let annotationA = PayloadAnnotation(
        label: "fieldA", value: "value",
        byteRange: node.header.payloadRange.lowerBound..<(node.header.payloadRange.lowerBound + 2))
      let annotationB = PayloadAnnotation(
        label: "fieldB", value: "value",
        byteRange: (node.header.payloadRange.lowerBound + 2)..<(node.header.payloadRange.lowerBound
          + 4))
    ...
    func testSelectingByteUpdatesAnnotationSelection() async throws {
      ...
      let provider = HexSliceProviderStub(
        result: HexSlice(
          offset: node.header.payloadRange.lowerBound, bytes: Data(repeating: 0x11, count: 16)))
      let annotationA = PayloadAnnotation(
        label: "fieldA", value: "value",
        byteRange: node.header.payloadRange.lowerBound..<(node.header.payloadRange.lowerBound + 2))
      let annotationB = PayloadAnnotation(
