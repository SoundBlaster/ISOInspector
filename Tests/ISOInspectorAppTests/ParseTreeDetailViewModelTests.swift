    ...
    @MainActor final class ParseTreeDetailViewModelTests: XCTestCase {
        func testSelectingNodePublishesDetailAndHexSlice() async throws {
            ...
            let node = ParseTreeNode(
                header: header, metadata: metadata, payload: nil,
                validationIssues: [
                    ValidationIssue(ruleID: "VR-001", message: "problem", severity: .warning)
                ], children: [])
            let snapshot = ParseTreeSnapshot(
                nodes: [node], validationIssues: node.validationIssues,
                lastUpdatedAt: Date(timeIntervalSince1970: 123))
            let subject = PassthroughSubject<ParseTreeSnapshot, Never>()
            let provider = HexSliceProviderStub(
                result: HexSlice(
                    offset: node.header.payloadRange.lowerBound, bytes: Data([0xAA, 0xBB, 0xCC])))
            let annotation = PayloadAnnotation(
                label: "major_brand", value: "isom",
                byteRange: node.header.payloadRange
                    .lowerBound..<(node.header.payloadRange.lowerBound + 4))
            let annotations = PayloadAnnotationProviderStub(result: [annotation])
            let viewModel = ParseTreeDetailViewModel(
                hexSliceProvider: provider, annotationProvider: annotations, windowSize: 16)
        ...
        func testAnnotationsDeriveFromPayloadWhenAvailable() async throws {
            ...
            let payload = ParsedBoxPayload(fields: [
                ParsedBoxPayload.Field(
                    name: "major_brand", value: "isom", description: "Primary brand",
                    byteRange: header.payloadRange.lowerBound..<(header.payloadRange.lowerBound + 4)
                )
            ])
            let node = ParseTreeNode(
                header: header, metadata: nil, payload: payload, validationIssues: [], children: [])
            let snapshot = ParseTreeSnapshot(
                nodes: [node], validationIssues: [], lastUpdatedAt: Date())
            let subject = PassthroughSubject<ParseTreeSnapshot, Never>()
            let provider = HexSliceProviderStub(
                result: HexSlice(offset: header.payloadRange.lowerBound, bytes: Data()))
            let viewModel = ParseTreeDetailViewModel(
                hexSliceProvider: provider, annotationProvider: nil, windowSize: 16)
        ...
        func testHexSliceRequestsPayloadWindow() async throws {
            let node = ParseTreeNode(
                header: makeHeader(identifier: 1, type: "mdat", payloadLength: 512), metadata: nil,
                payload: nil, validationIssues: [], children: [])
            let snapshot = ParseTreeSnapshot(
                nodes: [node], validationIssues: [], lastUpdatedAt: Date())
            let subject = PassthroughSubject<ParseTreeSnapshot, Never>()
            let provider = HexSliceProviderStub(
                result: HexSlice(offset: node.header.payloadRange.lowerBound, bytes: Data([0x00])))
            let viewModel = ParseTreeDetailViewModel(
                hexSliceProvider: provider, annotationProvider: nil, windowSize: 64)
            ...
            XCTAssertEqual(
                provider.requests.first?.window.offset, node.header.payloadRange.lowerBound)
            XCTAssertEqual(provider.requests.first?.window.length, 64)
        }

        func testClearingSelectionResetsDetail() async throws {
            let node = ParseTreeNode(
                header: makeHeader(identifier: 1, type: "trak", payloadLength: 40), metadata: nil,
                payload: nil, validationIssues: [], children: [])
            ...
            let provider = HexSliceProviderStub(
                result: HexSlice(offset: node.header.payloadRange.lowerBound, bytes: Data([0x00])))
            let viewModel = ParseTreeDetailViewModel(
                hexSliceProvider: provider, annotationProvider: nil, windowSize: 32)
        ...
        func testSelectingAnnotationUpdatesHighlight() async throws {
            let node = ParseTreeNode(
                header: makeHeader(identifier: 1, type: "ftyp", payloadLength: 16), metadata: nil,
                payload: nil, validationIssues: [], children: [])
            let snapshot = ParseTreeSnapshot(
                nodes: [node], validationIssues: [], lastUpdatedAt: Date())
            let subject = PassthroughSubject<ParseTreeSnapshot, Never>()
            let provider = HexSliceProviderStub(
                result: HexSlice(
                    offset: node.header.payloadRange.lowerBound,
                    bytes: Data(repeating: 0x00, count: 16)))
            let annotationA = PayloadAnnotation(
                label: "fieldA", value: "value",
                byteRange: node.header.payloadRange
                    .lowerBound..<(node.header.payloadRange.lowerBound + 2))
            let annotationB = PayloadAnnotation(
                label: "fieldB", value: "value",
                byteRange: (node.header.payloadRange.lowerBound + 2)..<(node.header.payloadRange
                    .lowerBound + 4))
            let annotations = PayloadAnnotationProviderStub(result: [annotationA, annotationB])
            let viewModel = ParseTreeDetailViewModel(
                hexSliceProvider: provider, annotationProvider: annotations, windowSize: 32)
            ...
            XCTAssertEqual(
                viewModel.annotations.count, 2, "Annotations should be loaded before selecting")

            // Verify annotationB exists in the loaded annotations
            let loadedAnnotationB = viewModel.annotations.first {
                $0.byteRange == annotationB.byteRange
            }
        ...
        func testHandlerPayloadDerivesAnnotationsForDetailView() async throws {
            ...
            let payload = ParsedBoxPayload(fields: [
                ParsedBoxPayload.Field(
                    name: "handler_type", value: "vide", description: "Handler type",
                    byteRange: (header.payloadRange.lowerBound + 8)..<(header.payloadRange
                        .lowerBound + 12)),
                ParsedBoxPayload.Field(
                    name: "handler_category", value: "Video", description: "Handler category"),
                ParsedBoxPayload.Field(
                    name: "handler_name", value: "Video Handler",
                    description: "Handler display name",
                    byteRange: (header.payloadRange.lowerBound + 20)..<(header.payloadRange
                        .lowerBound + 32)),
            ])
            ...
            XCTAssertEqual(
                viewModel.annotations.first { $0.label == "handler_name" }?.value, "Video Handler")
        }
        ...
        func testFocusingIssueHighlightsRangeAndRequestsSlice() async throws {
            ...
            let issueRange =
                (header.payloadRange.lowerBound + 120)..<(header.payloadRange.lowerBound + 144)
            let issue = ParseIssue(
                severity: .error, code: "TP001", message: "Corrupt atom", byteRange: issueRange)
            let node = ParseTreeNode(
                header: header, metadata: nil, payload: nil, validationIssues: [], issues: [issue],
                children: [])
            let snapshot = ParseTreeSnapshot(
                nodes: [node], validationIssues: [], lastUpdatedAt: Date())
            let subject = PassthroughSubject<ParseTreeSnapshot, Never>()
            let provider = HexSliceProviderStub(
                result: HexSlice(
                    offset: header.payloadRange.lowerBound, bytes: Data(repeating: 0, count: 256)))
            let viewModel = ParseTreeDetailViewModel(
                hexSliceProvider: provider, annotationProvider: nil, windowSize: 64)
        ...
        private func makeHeader(identifier: Int64, type: String, payloadLength: Int64) -> BoxHeader
        {
            ...
            return BoxHeader(
                type: try! FourCharCode(type), totalSize: headerSize + payloadLength,
                headerSize: headerSize,
                payloadRange: (start + headerSize)..<(start + headerSize + payloadLength),
                range: start..<(start + headerSize + payloadLength), uuid: nil)
        }
    }

    private func findNode(withFourCC fourCC: String, in nodes: [ParseTreeNode]) -> ParseTreeNode? {
        for node in nodes {
            if node.header.type.description == fourCC { return node }
            if let match = findNode(withFourCC: fourCC, in: node.children) { return match }
        }
        return nil
    }
    ...
    private final class HexSliceProviderStub: HexSliceProvider {
        ...
        init(result: HexSlice) { self.result = result }
    ...
    private final class PayloadAnnotationProviderStub: PayloadAnnotationProvider {
        ...
        init(result: [PayloadAnnotation]) { self.result = result }

        func annotations(for header: BoxHeader) async throws -> [PayloadAnnotation] { result }
    }
#endif
