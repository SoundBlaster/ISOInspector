...
final class AnnotationBookmarkSessionTests: XCTestCase {
    func testLoadsAnnotationsAndBookmarksForSelectedNode() {
        ...
        store.annotationsStorage[file] = [
            AnnotationRecord(id: UUID(), nodeID: 1, note: "Root note", createdAt: createdAt, updatedAt: createdAt),
            AnnotationRecord(id: UUID(), nodeID: 2, note: "Other node", createdAt: createdAt, updatedAt: createdAt),
            AnnotationRecord(id: UUID(), nodeID: 1, note: "Follow up", createdAt: later, updatedAt: later)
        ]
...
private final class AnnotationBookmarkStoreStub: AnnotationBookmarkStoring {
    ...
    func bookmarks(for file: URL) throws -> [BookmarkRecord] {
        if let errorToThrow { throw errorToThrow }
        return bookmarksStorage[file, default: []].map { BookmarkRecord(nodeID: $0, createdAt: currentDate) }
    }
    ...
    func updateAnnotation(for file: URL, annotationID: UUID, note: String) throws -> AnnotationRecord {
        ...
        guard var annotations = annotationsStorage[file],
              let index = annotations.firstIndex(where: { $0.id == annotationID }) else {
