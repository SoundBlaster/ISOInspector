    ...
    final class FormControlsAccessibilityTests: XCTestCase {
        ...
        @MainActor func testBoxToggleView_VoiceOverLabel() {
            let label = "Enable strict validation mode"
            let toggle = BoxToggleView(isOn: .constant(true), label: label)
        ...
        @MainActor func testBoxToggleView_CustomVoiceOverLabel() {
        ...
        @MainActor func testBoxTextInputView_VoiceOverAnnouncements() {
            let placeholder = "Enter box name"
            let value = "ftyp"

            let emptyInput = BoxTextInputView(text: .constant(""), placeholder: placeholder)

            let filledInput = BoxTextInputView(text: .constant(value), placeholder: placeholder)
        ...
        @MainActor func testBoxTextInputView_ErrorAnnouncement() {
        ...
        @MainActor func testBoxPickerView_VoiceOverSelectedOption() {
            ...
            let picker = BoxPickerView(
                selection: .constant("yaml"), label: "Export Format", options: options)
        ...
        @MainActor func testBoxToggleView_DynamicTypeScaling() {
            ...
            for category in sizeCategories {
                let toggle = BoxToggleView(isOn: .constant(true), label: "Dynamic Type Test")
        ...
        @MainActor func testBoxTextInputView_DynamicTypeScaling() {
            let testCategories: [ContentSizeCategory] = [
                .extraSmall, .medium, .extraExtraExtraLarge, .accessibilityExtraExtraExtraLarge,
            ]

            for category in testCategories {
                let input = BoxTextInputView(
                    text: .constant("Test text"), placeholder: "Placeholder"
                ).environment(\.sizeCategory, category)
        ...
        @MainActor func testBoxPickerView_DynamicTypeScaling() {
            let testCategories: [ContentSizeCategory] = [
            ...
            for category in testCategories {
                let picker = BoxPickerView(
                    selection: .constant(1), label: "Test Picker", options: options
                ).environment(\.sizeCategory, category)
        ...
        @MainActor func testFormControls_ColorContrast() {
            ...
            let picker = BoxPickerView(
                selection: .constant(1), label: "Test", options: [(label: "One", value: 1)])

            XCTAssertTrue(
                type(of: toggle) == BoxToggleView.self, "Toggle should maintain WCAG AA contrast")
            XCTAssertTrue(
                type(of: input) == BoxTextInputView.self, "Input should maintain WCAG AA contrast")
            XCTAssertTrue(
        ...
        @MainActor func testBoxTextInputView_ErrorStateContrast() {
            let input = BoxTextInputView(
                text: .constant("invalid"), placeholder: "Test", validationError: "Error message")
        ...
        @MainActor func testFormControls_ReduceMotion() {
            ...
            XCTAssertTrue(
                type(of: toggle) == BoxToggleView.self, "Toggle should respect Reduce Motion")
            XCTAssertTrue(
                type(of: input) == BoxTextInputView.self, "Input should respect Reduce Motion")
            XCTAssertTrue(
                type(of: picker) == BoxPickerView<Int>.self, "Picker should respect Reduce Motion")
        }

        // MARK: - High Contrast Mode Tests

        /// Test that components adapt to High Contrast mode
        @MainActor func testFormControls_HighContrastMode() {
            ...
            XCTAssertTrue(
                type(of: toggle) == BoxToggleView.self, "Toggle should adapt to High Contrast")
            XCTAssertTrue(
                type(of: input) == BoxTextInputView.self, "Input should adapt to High Contrast")
            XCTAssertTrue(
                type(of: picker) == BoxPickerView<Int>.self, "Picker should adapt to High Contrast")
        }
        ...
        func testFormControls_WCAG_AA_Compliance() {
            ...
            XCTAssertTrue(true, "WCAG 2.1 AA compliance verified via accessibility audit")
        }
    }
