    ...
    final class FormControlsTests: XCTestCase {
        ...
        @MainActor func testBoxToggleView_InitializationWithLabel() {
            ...
            XCTAssertTrue(
                type(of: toggle) == BoxToggleView.self, "Should create BoxToggleView instance")
        }

        /// Test that BoxToggleView uses custom accessibility label when provided
        @MainActor func testBoxToggleView_CustomAccessibilityLabel() {
            let visualLabel = "Strict mode"
            let a11yLabel = "Enable strict validation for all boxes"

            let toggle = BoxToggleView(
                isOn: .constant(false), label: visualLabel, accessibilityLabel: a11yLabel)
        ...
        @MainActor func testBoxToggleView_DefaultAccessibilityLabel() {
            let label = "Enable warnings"

            let toggle = BoxToggleView(
                isOn: .constant(false), label: label, accessibilityLabel: nil)
        ...
        @MainActor func testBoxToggleView_DisabledState() {
            let toggleDisabled = BoxToggleView(
                isOn: .constant(false), label: "Disabled", disabled: true)

            let toggleEnabled = BoxToggleView(
                isOn: .constant(false), label: "Enabled", disabled: false)
        ...
        @MainActor func testBoxTextInputView_InitializationWithPlaceholder() {
            let placeholder = "Enter box name..."
            let input = BoxTextInputView(text: .constant(""), placeholder: placeholder)

            XCTAssertTrue(
                type(of: input) == BoxTextInputView.self, "Should create BoxTextInputView instance")
        }

        /// Test that BoxTextInputView handles validation error display
        @MainActor func testBoxTextInputView_ValidationError() {
            let errorMessage = "Box name must be exactly 4 characters"

            let inputWithError = BoxTextInputView(
                text: .constant("invalid"), placeholder: "Box name", validationError: errorMessage)

            let inputWithoutError = BoxTextInputView(
                text: .constant("ftyp"), placeholder: "Box name", validationError: nil)
        ...
        @MainActor func testBoxTextInputView_KeyboardTypes() {
            let keyboardTypes: [BoxTextInputView.KeyboardType] = [
                .default, .numberPad, .decimalPad, .URL, .emailAddress,
            ]

            for keyboardType in keyboardTypes {
                let input = BoxTextInputView(
                    text: .constant(""), placeholder: "Test", keyboardType: keyboardType)
        ...
        @MainActor func testBoxTextInputView_EditingChangedCallback() {
            var callbackFired = false
            var editingState: Bool?

            let input = BoxTextInputView(
                text: .constant("test"), placeholder: "Test",
            ...
            XCTAssertTrue(
                type(of: input) == BoxTextInputView.self, "Input should be created with callback")
        ...
        @MainActor func testBoxPickerView_InitializationWithOptions() {
            ...
            let picker = BoxPickerView(
                selection: .constant("json"), label: "Export Format", options: options)
        ...
        @MainActor func testBoxPickerView_EmptyOptions() {
            let emptyOptions: [(label: String, value: String)] = []

            let picker = BoxPickerView(
                selection: .constant(""), label: "Empty Picker", options: emptyOptions)
        ...
        @MainActor func testFormControls_Composition() {
            // Simulate a settings form with all three controls
            let toggle = BoxToggleView(isOn: .constant(true), label: "Enable feature")

            let textInput = BoxTextInputView(text: .constant("value"), placeholder: "Enter value")

            let picker = BoxPickerView(
                selection: .constant("a"), label: "Select option",
                options: [(label: "A", value: "a"), (label: "B", value: "b")])
        ...
        // MARK: - Keyboard Type Conversion (iOS-specific)
            ...
            func testBoxTextInputView_UIKeyboardTypeConversion() {
                ...
                XCTAssertEqual(BoxTextInputView.KeyboardType.default.uiKeyboardType, .default)
                XCTAssertEqual(BoxTextInputView.KeyboardType.numberPad.uiKeyboardType, .numberPad)
                XCTAssertEqual(BoxTextInputView.KeyboardType.decimalPad.uiKeyboardType, .decimalPad)
                XCTAssertEqual(BoxTextInputView.KeyboardType.URL.uiKeyboardType, .URL)
                XCTAssertEqual(
                    BoxTextInputView.KeyboardType.emailAddress.uiKeyboardType, .emailAddress)
            }
        #endif
    }
