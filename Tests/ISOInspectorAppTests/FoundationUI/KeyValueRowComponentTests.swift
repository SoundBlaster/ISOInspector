    ...
    final class KeyValueRowComponentTests: XCTestCase {
        ...
        func testKeyValueRowInitializationWithAllParameters() {
            let row = KeyValueRow(
                key: "Full Path", value: "/path/to/file", layout: .vertical, copyable: true)
        ...
        func testKeyValueRowAsBoxDetails() {
            ...
            let descRow = KeyValueRow(
                key: "Description",
                value: "File Type Box - defines brand and version compatibility", layout: .vertical)
        ...
        func testKeyValueRowInList() {
            ...
            rows.forEach { row in XCTAssertNotNil(row.body) }
        }
