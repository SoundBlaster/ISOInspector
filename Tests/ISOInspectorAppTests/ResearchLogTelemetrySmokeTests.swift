...
final class ResearchLogTelemetrySmokeTests: XCTestCase {
    func testSmokeTelemetryCapturesResearchLogEntriesFromParsePipeline() async throws {
        ...
        var iterator = pipeline.events(for: reader, context: .init(source: source))
            .makeAsyncIterator()
        ...
        default: XCTFail("Expected ready snapshot, received \(snapshot.state)")
        }
    }
    ...
    func testSmokeTelemetryFlagsMissingResearchLog() {
        ...
        case .missingLog(let audit):
            ...
            XCTAssertTrue(
                snapshot.diagnostics.contains(where: { $0.lowercased().contains("missing") }))
        default: XCTFail("Expected missing log snapshot, received \(snapshot.state)")
        }
    }
    ...
    func testSmokeTelemetryFlagsEmptyResearchLog() throws {
        ...
        case .emptyLog(let audit):
            ...
            XCTAssertTrue(
                snapshot.diagnostics.contains(where: { $0.lowercased().contains("empty") }))
        default: XCTFail("Expected empty log snapshot, received \(snapshot.state)")
        }
    }
    ...
    func testSmokeTelemetrySurfacesSchemaMismatches() throws {
        ...
        case .schemaMismatch(let expected, let actual):
            ...
            XCTAssertTrue(
                snapshot.diagnostics.contains(where: { $0.lowercased().contains("schema") }))
        default: XCTFail("Expected schema mismatch snapshot, received \(snapshot.state)")
        }
    }
}
...
private struct InMemoryReader: RandomAccessReader {
    ...
    func read(at offset: Int64, count: Int) throws -> Data {
        ...
        guard lower >= 0, count >= 0 else { throw ReaderError.outOfBounds }
        guard lower <= data.count else { throw ReaderError.outOfBounds }
        let upper = min(data.count, lower + count)
        return data.subdata(in: lower..<upper)
    }

    enum ReaderError: Swift.Error { case outOfBounds }
}
