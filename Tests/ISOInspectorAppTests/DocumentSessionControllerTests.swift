    ...
    @MainActor final class DocumentSessionControllerTests: XCTestCase {
        ...
        func testInitializerMigratesBookmarkDataToPersistenceStore() throws {
            ...
            let recent = DocumentRecent(
                url: URL(fileURLWithPath: "/tmp/migrate.mp4"), bookmarkData: data,
                displayName: "Migrate", lastOpened: Date(timeIntervalSince1970: 1))
            let recentsStore = DocumentRecentsStoreStub(initialRecents: [recent])

            let controller = makeController(
                store: recentsStore, bookmarkStore: bookmarkStore, resetBookmarkTracking: false)

            XCTAssertEqual(
                bookmarkStore.upsertedURLs.map { $0.standardizedFileURL },
            ...
            XCTAssertEqual(
                recentsStore.savedRecents?.first?.bookmarkIdentifier,
                controller.recents.first?.bookmarkIdentifier)
        ...
        func testOpenDocumentUpsertsBookmarkRecord() throws {
            ...
            let controller = makeController(
                store: recentsStore, bookmarkStore: bookmarkStore,
            ...
            XCTAssertEqual(
                bookmarkStore.upsertedURLs.map { $0.standardizedFileURL }, [url.standardizedFileURL]
            )
        ...
        func testOpeningDocumentStartsParsingAndUpdatesRecents() throws {
            ...
            let controller = makeController(
                store: store, filesystemAccess: filesystemStub.makeAccess())
            let url = URL(fileURLWithPath: "/tmp/example.mp4")
            var cancellables: Set<AnyCancellable> = []
            let finished = expectation(description: "Parsing finished")

            controller.parseTreeStore.$state.dropFirst().sink { state in
            ...
            XCTAssertEqual(
                controller.recents.first?.url.standardizedFileURL, url.standardizedFileURL)
            XCTAssertEqual(
                store.savedRecents?.first?.url.standardizedFileURL, url.standardizedFileURL)
            XCTAssertEqual(
                controller.currentDocument?.url.standardizedFileURL, url.standardizedFileURL)
            XCTAssertEqual(
                controller.parseTreeStore.fileURL?.standardizedFileURL, url.standardizedFileURL)
        ...
        func testOpeningSecondDocumentRevokesPreviousScope() throws {
            ...
            XCTAssertEqual(filesystemStub.manager.stoppedURLs, [firstURL.standardizedFileURL])
        }
        ...
        func testInitializerRestoresSessionSnapshotAndPersistsUpdates() throws {
            ...
            let focusRecent = DocumentRecent(
                url: focusURL, bookmarkIdentifier: focusBookmarkID, bookmarkData: Data([0x01]),
                displayName: "Focus", lastOpened: Date(timeIntervalSince1970: 10))
            let otherBookmarkID = UUID(uuidString: "00000000-0000-0000-0000-000000000098")!
            let otherRecent = DocumentRecent(
                url: otherURL, bookmarkIdentifier: otherBookmarkID, bookmarkData: nil,
                displayName: "Other", lastOpened: Date(timeIntervalSince1970: 5))

            let focusDiff = WorkspaceSessionBookmarkDiff(
                id: UUID(uuidString: "00000000-0000-0000-0000-0000000000D1")!,
                bookmarkID: focusBookmarkID, isRemoved: false, noteDelta: "retain")
            let otherDiff = WorkspaceSessionBookmarkDiff(
                id: UUID(uuidString: "00000000-0000-0000-0000-0000000000D2")!,
                bookmarkID: otherBookmarkID, isRemoved: true, noteDelta: "")

            sessionStore.loadedSnapshot = WorkspaceSessionSnapshot(
                id: UUID(uuidString: "00000000-0000-0000-0000-000000000001")!,
                createdAt: Date(timeIntervalSince1970: 1),
                updatedAt: Date(timeIntervalSince1970: 2), appVersion: "1.0",
                files: [
                    WorkspaceSessionFileSnapshot(
                        id: UUID(uuidString: "00000000-0000-0000-0000-000000000010")!,
                        recent: focusRecent, orderIndex: 0, lastSelectionNodeID: 99,
                        isPinned: false, scrollOffset: nil,
                        bookmarkIdentifier: focusRecent.bookmarkIdentifier,
                        bookmarkDiffs: [focusDiff], validationConfiguration: nil),
                    WorkspaceSessionFileSnapshot(
                        id: UUID(uuidString: "00000000-0000-0000-0000-000000000011")!,
                        recent: otherRecent, orderIndex: 1, lastSelectionNodeID: nil,
                        isPinned: false, scrollOffset: nil,
                        bookmarkIdentifier: otherRecent.bookmarkIdentifier,
                        bookmarkDiffs: [otherDiff], validationConfiguration: nil),
                ], focusedFileURL: focusURL, lastSceneIdentifier: nil, windowLayouts: [])

            let filesystemStub = FilesystemAccessStub()
            filesystemStub.resolutions[Data([0x01])] = BookmarkResolution(
                url: focusURL, isStale: false)

            let controller = makeController(
                store: recentsStore, sessionStore: sessionStore, annotationsStore: annotationStore,
                bookmarkStore: bookmarkStore, filesystemAccess: filesystemStub.makeAccess())

            XCTAssertEqual(controller.recents.count, 2)
            XCTAssertEqual(
                controller.recents.first?.url.standardizedFileURL, focusURL.standardizedFileURL)
            XCTAssertEqual(
                controller.currentDocument?.url.standardizedFileURL, focusURL.standardizedFileURL)
        ...
        func testPersistSessionIncludesBookmarkIdentifiers() throws {
            ...
            let bookmarkStore = BookmarkPersistenceStoreStub()
            let controller = makeController(
                store: recentsStore, sessionStore: sessionStore, bookmarkStore: bookmarkStore,
                bookmarkDataProvider: { _ in Data("bookmark".utf8) })
        ...
        func testOpenDocumentFailurePublishesLoadFailureUntilDismissed() throws {
            ...
            let controller = makeController(
                ...
                }, workQueue: ImmediateWorkQueue(), filesystemAccess: filesystemStub.makeAccess())
        ...
        func testOpenRecentResolvesBookmarkAndActivatesSecurityScope() throws {
            ...
            let recent = DocumentRecent(
                url: recentURL, bookmarkData: bookmarkData, displayName: "Recent",
                lastOpened: Date())
            let recentsStore = DocumentRecentsStoreStub(initialRecents: [recent])
            let filesystemStub = FilesystemAccessStub()
            ...
            XCTAssertEqual(
                resolvedReaderURL.value?.standardizedFileURL, recentURL.standardizedFileURL)
        ...
        func testOpenRecentRemovesBookmarkRecordWhenResolutionFails() throws {
            ...
            let recent = DocumentRecent(
                url: recentURL, bookmarkIdentifier: try bookmarkStore.record(for: recentURL)?.id,
                bookmarkData: nil, displayName: "Failed", lastOpened: Date())
            let recentsStore = DocumentRecentsStoreStub(initialRecents: [recent])
            let filesystemStub = FilesystemAccessStub()
            ...
            let controller = makeController(
                store: recentsStore, workQueue: ImmediateWorkQueue(), bookmarkStore: bookmarkStore,
            ...
            XCTAssertEqual(bookmarkStore.markedResolutions.map { $0.1 }, [.failed])
        }
        ...
        func testOpenRecentRefreshesStaleBookmarkRecord() throws {
            ...
            let recent = DocumentRecent(
                url: recentURL, bookmarkIdentifier: record.id, bookmarkData: nil,
                displayName: "Stale", lastOpened: Date())
            let recentsStore = DocumentRecentsStoreStub(initialRecents: [recent])
            let filesystemStub = FilesystemAccessStub()
            filesystemStub.resolutions[staleData] = BookmarkResolution(
                url: recentURL, isStale: true)
        ...
        func testExportJSONWritesFullDocument() async throws {
            ...
            let destination = FileManager.default.temporaryDirectory.appendingPathComponent(
                "export-full-\(UUID().uuidString).json")
            defer { try? FileManager.default.removeItem(at: destination) }
            filesystemStub.saveHandler = { _ in destination }

            let controller = makeController(
                store: DocumentRecentsStoreStub(initialRecents: []),
                filesystemAccess: filesystemStub.makeAccess(), parseTreeStore: parseTreeStore)

            await controller.exportJSON(scope: DocumentSessionController.ExportScope.document)

            XCTAssertEqual(
                filesystemStub.lastSaveConfiguration?.allowedContentTypes, [UTType.json.identifier])
            XCTAssertEqual(
                filesystemStub.lastSavedURL?.standardizedFileURL, destination.standardizedFileURL)
            let data = try Data(contentsOf: destination)
            let expected = try JSONParseTreeExporter().export(
                tree: ParseTree(nodes: [root], validationIssues: [issue]))
            XCTAssertEqual(data, expected)
            let status: DocumentSessionController.ExportStatus = try XCTUnwrap(
                controller.exportStatus)
            XCTAssertTrue(status.isSuccess)
            XCTAssertEqual(
                status.destinationURL?.standardizedFileURL, destination.standardizedFileURL)
        ...
        func testExportJSONSelectionWritesSubtree() async throws {
            ...
            let parentIssue = ValidationIssue(
                ruleID: "parent", message: "parent", severity: .warning)
            let parent = makeNode(
                identifier: 0, type: "moov", issues: [parentIssue], children: [grandChild])
            let snapshot = ParseTreeSnapshot(
                nodes: [parent], validationIssues: [parentIssue, childIssue])
            let parseTreeStore = ParseTreeStore(initialSnapshot: snapshot, initialState: .finished)
            let filesystemStub = FilesystemAccessStub()
            let destination = FileManager.default.temporaryDirectory.appendingPathComponent(
                "export-selection-\(UUID().uuidString).json")
            defer { try? FileManager.default.removeItem(at: destination) }
            filesystemStub.saveHandler = { _ in destination }
        ...
        func testExportJSONSelectionMissingNodeEmitsFailureStatus() async throws {
            ...
            let controller = makeController(
                store: DocumentRecentsStoreStub(initialRecents: []), diagnostics: diagnostics,
        ...
        func testExportIssueSummaryWritesFullDocument() async throws {
            let parseIssue = ParseIssue(
                severity: .error, code: "VR-001", message: "Corrupt node", byteRange: 24..<32,
                affectedNodeIDs: [0])
            let node = makeNode(identifier: 0, type: "moov", parseIssues: [parseIssue])
            let snapshot = ParseTreeSnapshot(nodes: [node], validationIssues: [])
            let parseTreeStore = ParseTreeStore(initialSnapshot: snapshot, initialState: .finished)
            await MainActor.run { parseTreeStore.issueStore.replaceAll(with: [parseIssue]) }

            let filesystemStub = FilesystemAccessStub()
            let destination = FileManager.default.temporaryDirectory.appendingPathComponent(
                "issue-summary-\(UUID().uuidString).txt")
            defer { try? FileManager.default.removeItem(at: destination) }
            filesystemStub.saveHandler = { _ in destination }
        ...
        func testExportIssueSummarySelectionLimitsScope() async throws {
            let selectedIssue = ParseIssue(
                severity: .error, code: "VR-100", message: "Selected issue", byteRange: 8..<16,
                affectedNodeIDs: [8])
            let otherIssue = ParseIssue(
                severity: .warning, code: "VR-200", message: "Other issue", byteRange: 16..<24,
                affectedNodeIDs: [16])
            let selectedNode = makeNode(identifier: 8, type: "trak", parseIssues: [selectedIssue])
            let otherNode = makeNode(identifier: 16, type: "mdia", parseIssues: [otherIssue])
            let root = makeNode(identifier: 0, type: "moov", children: [selectedNode, otherNode])
            let snapshot = ParseTreeSnapshot(nodes: [root], validationIssues: [])
            let parseTreeStore = ParseTreeStore(initialSnapshot: snapshot, initialState: .finished)
            await MainActor.run {
                parseTreeStore.issueStore.replaceAll(with: [selectedIssue, otherIssue])
            }

            let filesystemStub = FilesystemAccessStub()
            let destination = FileManager.default.temporaryDirectory.appendingPathComponent(
                "issue-summary-selection-\(UUID().uuidString).txt")
            defer { try? FileManager.default.removeItem(at: destination) }
            filesystemStub.saveHandler = { _ in destination }
            ...
            await controller.exportIssueSummary(scope: .selection(selectedNode.id))

            let savedURL = try XCTUnwrap(filesystemStub.lastSavedURL)
            XCTAssertEqual(savedURL.standardizedFileURL, destination.standardizedFileURL)
            let contents = try String(contentsOf: destination, encoding: .utf8)
            XCTAssertTrue(contents.contains("VR-100"))
            XCTAssertTrue(contents.contains("Selected issue"))
            XCTAssertTrue(contents.contains("Total Issues: 1"))
            XCTAssertTrue(contents.contains("Errors: 1"))
            XCTAssertFalse(contents.contains("VR-200"))
            XCTAssertFalse(contents.contains("Other issue"))
        }
        ...
        func testCanExportSelectionReflectsSnapshotMembership() throws {
            ...
            let parseTreeStore = ParseTreeStore(initialSnapshot: snapshot, initialState: .finished)
            let controller = makeController(
                store: DocumentRecentsStoreStub(initialRecents: []), parseTreeStore: parseTreeStore)
        ...
        func testPersistRecentsFailureEmitsDiagnostics() throws {
            ...
            let diagnostics = DiagnosticsLoggerStub()
            let controller = makeController(store: recentsStore, diagnostics: diagnostics)
        ...
        func testPersistSessionFailureEmitsDiagnostics() throws {
            ...
            let diagnostics = DiagnosticsLoggerStub()
            let controller = makeController(
                store: recentsStore, sessionStore: sessionStore, diagnostics: diagnostics)
        ...
        func testSelectingWorkspacePresetFiltersIssuesAndPersistsOverride() throws {
            ...
            let presets = [
                ...
                ValidationPreset(
                    id: "structural", name: "Structural", summary: "",
                    rules: [
                        ValidationPreset.RuleState(ruleID: .researchLogRecording, isEnabled: false)
                    ]),
            ]
            ...
            let controller = makeController(
                store: recentsStore, sessionStore: sessionStore, pipeline: pipeline,
                filesystemAccess: FilesystemAccessStub().makeAccess(),
                parseTreeStore: parseTreeStore, validationConfigStore: configStore,
            ...
            XCTAssertEqual(
                controller.parseTreeStore.snapshot.validationIssues.filter { $0.ruleID == "VR-006" }
                    .count, 2)

            controller.selectValidationPreset(
                "structural",
                scope: DocumentSessionController.ValidationConfigurationScope.workspace)
            ...
            let savedConfiguration = try XCTUnwrap(
                savedSnapshot.files.first?.validationConfiguration)
        ...
        func testUpdatingGlobalConfigurationAppliesWhenNoOverride() throws {
            ...
            controller.setValidationRule(
                ValidationRuleIdentifier.researchLogRecording, isEnabled: false,
                scope: DocumentSessionController.ValidationConfigurationScope.global)

            XCTAssertEqual(
                controller.globalValidationConfiguration.ruleOverrides[
                    ValidationRuleIdentifier.researchLogRecording], false)
            XCTAssertEqual(
                configStore.savedConfigurations.last?.ruleOverrides[
                    ValidationRuleIdentifier.researchLogRecording], false)
            XCTAssertEqual(
                controller.parseTreeStore.snapshot.validationIssues.filter { $0.ruleID == "VR-006" }
                    .count, 0)
        ...
        private func makeController(
            ...
            validationPresets: [ValidationPreset]? = nil, resetBookmarkTracking: Bool = true
        ) -> DocumentSessionController {
            let resolvedPipeline =
                pipeline ?? ParsePipeline(buildStream: { _, _ in .finishedStream })
            ...
            let controller = DocumentSessionController(
                ...
                recentsStore: store, sessionStore: sessionStore,
                pipelineFactory: { resolvedPipeline },
                readerFactory: readerFactory ?? { @Sendable _ in StubRandomAccessReader() },
                workQueue: workQueue, diagnostics: resolvedDiagnostics,
                bookmarkStore: bookmarkStore, filesystemAccess: access,
            ...
            if resetBookmarkTracking, let stub = bookmarkStore as? BookmarkPersistenceStoreStub {
                stub.resetTracking()
            }
            return controller
        }
        ...
        private func makeValidationEventStream() -> ParsePipeline.EventStream {
            let parentHeader = BoxHeader(
                type: try! FourCharCode("root"), totalSize: 32, headerSize: 8, payloadRange: 8..<32,
                range: 0..<32, uuid: nil)
            let childHeader = BoxHeader(
                type: try! FourCharCode("test"), totalSize: 16, headerSize: 8, payloadRange: 8..<16,
                range: 8..<24, uuid: nil)
            let issues = [
            ...
            let events = [
                ParseEvent(
                    kind: .willStartBox(header: parentHeader, depth: 0), offset: 0, metadata: nil,
                    validationIssues: []),
                ParseEvent(
                    kind: .willStartBox(header: childHeader, depth: 1), offset: 8, metadata: nil,
                    validationIssues: issues),
                ParseEvent(
                    kind: .didFinishBox(header: childHeader, depth: 1), offset: 24, metadata: nil,
                    validationIssues: issues),
                ParseEvent(
                    kind: .didFinishBox(header: parentHeader, depth: 0), offset: 32, metadata: nil,
                    validationIssues: []),
            ]
        ...
        private func sampleRecent(index: Int) -> DocumentRecent {
            DocumentRecent(
                url: URL(fileURLWithPath: "/tmp/sample-\(index).mp4"), bookmarkData: nil,
                displayName: "Sample \(index)",
                lastOpened: Date(timeIntervalSince1970: TimeInterval(index)))
        }

        private func makeNode(
            identifier: Int64, type: String, issues: [ValidationIssue] = [],
            children: [ParseTreeNode] = [], parseIssues: [ParseIssue] = []
        ) -> ParseTreeNode {
            let totalSize: Int64 = 32
            let headerSize: Int64 = 8
            let header = BoxHeader(
                type: try! FourCharCode(type), totalSize: totalSize, headerSize: headerSize,
                payloadRange: (identifier + headerSize)..<(identifier + totalSize),
                range: identifier..<(identifier + totalSize), uuid: nil)
            return ParseTreeNode(
                header: header, metadata: nil, payload: nil, validationIssues: issues,
