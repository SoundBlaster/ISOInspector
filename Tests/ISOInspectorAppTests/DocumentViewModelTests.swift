    ...
    @MainActor final class DocumentViewModelTests: XCTestCase {
        ...
        func testFilterUpdateKeepsSelectionVisible() throws {
            ...
            viewModel.outlineViewModel.filter = ParseTreeOutlineFilter(focusedSeverities: [.warning]
            )
            RunLoop.main.run(until: Date(timeIntervalSinceNow: 0.05))
        ...
        private static func makeSampleSnapshot(secondSeverity: ValidationIssue.Severity = .warning)
            throws -> (ParseTreeSnapshot, ParseTreeNode.ID)
        {
            ...
            let payload = ParsedBoxPayload(fields: [
                ParsedBoxPayload.Field(
                    name: "major_brand", value: "isom", description: nil, byteRange: 0..<4)
            ])
            ...
            let root = ParseTreeNode(
                header: headerA, payload: payload, validationIssues: [issueA], children: [child])
            let snapshot = ParseTreeSnapshot(
                nodes: [root, child], validationIssues: [issueA, issueB], lastUpdatedAt: Date())
