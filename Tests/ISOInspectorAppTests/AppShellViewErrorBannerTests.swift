    ...
    @MainActor final class AppShellViewErrorBannerTests: XCTestCase {
        ...
        func skip_testBannerAppearsForLoadFailureAndClearsAfterRetry() throws {
            ...
            let controller = DocumentSessionController(
                ...
                annotations: AnnotationBookmarkSession(store: nil), recentsStore: recentsStore,
                sessionStore: nil,
            ...
            DispatchQueue.main.async { failureExpectation.fulfill() }
            ...
            DispatchQueue.main.async { retryExpectation.fulfill() }
        ...
        func skip_testCorruptionWarningRibbonAppearsForIssueMetrics() throws {
            ...
            let controller = DocumentSessionController(
                ...
                sessionStore: nil,
                pipelineFactory: { ParsePipeline(buildStream: { _, _ in .finishedStream }) },
                readerFactory: { _ in StubRandomAccessReader() }, workQueue: ImmediateWorkQueue(),
                filesystemAccess: FilesystemAccessStub().makeAccess())
            ...
            controller.parseTreeStore.issueStore.record(
                ParseIssue(
                    severity: .warning, code: "VR-100", message: "Simulated warning",
                    byteRange: 0..<8, affectedNodeIDs: [1]), depth: 1)
    ...
    extension NSView {
        fileprivate func containsText(_ substring: String) -> Bool {
            ...
            for subview in subviews where subview.containsText(substring) { return true }
            return false
        }
    }
#endif
