    ...
    final class ParseTreeAccessibilityFormatterTests: XCTestCase {
        func testOutlineRowDescriptorIncludesTypeSeverityAndBookmarkState() throws {
            let header = BoxHeader(
                type: try FourCharCode("trak"), totalSize: 128, headerSize: 8,
                payloadRange: 8..<120, range: 0..<128, uuid: nil)
            ...
            let node = ParseTreeNode(
                ...
                validationIssues: [
                    ValidationIssue(
                        ruleID: "VR-002", message: "Track missing language", severity: .warning)
                ],
                children: [
                    ParseTreeNode(
                        header: BoxHeader(
                            type: try FourCharCode("mdia"), totalSize: 64, headerSize: 8,
                            payloadRange: 8..<56, range: 200..<264, uuid: nil), metadata: nil,
                        payload: nil, validationIssues: [], children: [])
                ])
            let row = ParseTreeOutlineRow(
                node: node, depth: 1, isExpanded: false, isSearchMatch: false,
                hasMatchingDescendant: false, hasValidationIssues: true, corruptionSummary: nil,
            ...
            XCTAssertEqual(
                descriptor.hint,
                "Press Return to expand or collapse. Press Space to toggle bookmark.")
        }

        func testDetailSummaryEnumeratesKeyFields() throws {
            let header = BoxHeader(
                type: try FourCharCode("mvhd"), totalSize: 200, headerSize: 12,
                payloadRange: 12..<200, range: 100..<300, uuid: nil)
            let metadata = try XCTUnwrap(BoxCatalog.shared.descriptor(for: header))
            let detail = ParseTreeNodeDetail(
                header: header, metadata: metadata, payload: nil,
                validationIssues: [
                    ValidationIssue(ruleID: "VR-101", message: "Duration missing", severity: .error)
                ], issues: [], status: .valid, snapshotTimestamp: Date(timeIntervalSince1970: 0),
                hexSlice: nil)
        ...
        func testDetailSummaryMentionsSampleEncryptionMetadata() throws {
            let header = BoxHeader(
                type: try FourCharCode("senc"), totalSize: 64, headerSize: 8, payloadRange: 8..<64,
                range: 0..<64, uuid: nil)
            let payload = ParsedBoxPayload(
                detail: .sampleEncryption(
            ...
            let detail = ParseTreeNodeDetail(
                header: header, metadata: nil, payload: payload, validationIssues: [], issues: [],
                status: .valid, snapshotTimestamp: Date(timeIntervalSince1970: 0), hexSlice: nil)
        ...
        func testDetailSummaryMentionsSampleAuxInfoOffsets() throws {
            let header = BoxHeader(
                type: try FourCharCode("saio"), totalSize: 56, headerSize: 8, payloadRange: 8..<56,
                range: 0..<56, uuid: nil)
            let payload = ParsedBoxPayload(
                detail: .sampleAuxInfoOffsets(
                    ParsedBoxPayload.SampleAuxInfoOffsetsBox(
                        version: 0, flags: 0x000001, entryCount: 3,
                        auxInfoType: try FourCharCode("cenc"), auxInfoTypeParameter: 2,
                        entrySize: .eightBytes, entriesRange: 32..<56, entriesByteLength: 24)))
            let detail = ParseTreeNodeDetail(
                header: header, metadata: nil, payload: payload, validationIssues: [], issues: [],
                status: .valid, snapshotTimestamp: Date(timeIntervalSince1970: 0), hexSlice: nil)
        ...
        func testDetailSummaryMentionsSampleAuxInfoSizes() throws {
            let header = BoxHeader(
                type: try FourCharCode("saiz"), totalSize: 48, headerSize: 8, payloadRange: 8..<48,
                range: 0..<48, uuid: nil)
            let payload = ParsedBoxPayload(
                detail: .sampleAuxInfoSizes(
                    ParsedBoxPayload.SampleAuxInfoSizesBox(
                        version: 0, flags: 0x000001, defaultSampleInfoSize: 0, entryCount: 3,
                        auxInfoType: try FourCharCode("cenc"), auxInfoTypeParameter: 2,
                        variableEntriesRange: 40..<43, variableEntriesByteLength: 3)))
            let detail = ParseTreeNodeDetail(
                header: header, metadata: nil, payload: payload, validationIssues: [], issues: [],
                status: .valid, snapshotTimestamp: Date(timeIntervalSince1970: 0), hexSlice: nil)
