    ...
    @MainActor final class WindowSessionControllerTests: XCTestCase {
        ...
        func testMultipleWindowsHaveIndependentState() throws {
            ...
            let appController = DocumentSessionController(
                recentsStore: recentsStore, sessionStore: nil,
            ...
            let recent1 = DocumentRecent(
                url: testURL1, bookmarkIdentifier: nil, bookmarkData: nil,
                displayName: "Document 1", lastOpened: Date())

            let recent2 = DocumentRecent(
                url: testURL2, bookmarkIdentifier: nil, bookmarkData: nil,
                displayName: "Document 2", lastOpened: Date())
        ...
        func testWindowsHaveIndependentParseTreeStores() {
            ...
            XCTAssertNotEqual(
                ObjectIdentifier(window1.parseTreeStore), ObjectIdentifier(window2.parseTreeStore))
        }
        ...
        func testWindowsHaveIndependentDocumentViewModels() {
            ...
            XCTAssertNotEqual(
                ObjectIdentifier(window1.documentViewModel),
                ObjectIdentifier(window2.documentViewModel))
        }
        ...
        func testWindowsHaveIndependentAnnotationSessions() {
            ...
            XCTAssertNotEqual(
                ObjectIdentifier(window1.annotations), ObjectIdentifier(window2.annotations))
        }
        ...
        func testDocumentViewModelPublishesParseTreeStoreChanges() async {
            ...
            windowController.$issueMetrics.dropFirst()  // Skip initial value
                .sink { metrics in if metrics.totalCount > 0 { expectation.fulfill() } }.store(
                    in: &cancellables)

            // Simulate parse tree store update with issues
            let mockIssue = ParseIssue(
                severity: .error, code: "TEST-001", message: "Test issue message",
                byteRange: 0..<10, affectedNodeIDs: [])
        ...
        func testDocumentViewModelAccessibilityAndConsistency() {
            ...
            XCTAssertEqual(
                ObjectIdentifier(viewModel.store), ObjectIdentifier(windowController.parseTreeStore)
            )
        ...
        func testDocumentViewModelBindingIntegrity() async throws {
            ...
            documentViewModel.$snapshot.dropFirst()  // Skip initial value
                .sink { updatedSnapshot in
                    if !updatedSnapshot.nodes.isEmpty { expectation.fulfill() }
        ...
        private static func makeHeader(offset: Int64, type: String) throws -> BoxHeader {
            ...
            return BoxHeader(
                type: fourcc, totalSize: 48, headerSize: 8, payloadRange: offset + 8..<offset + 16,
                range: offset..<offset + 48, uuid: nil)
        }
    }
    ...
    private class InMemoryDocumentRecentsStore: DocumentRecentsStoring {
        ...
        func load() throws -> [DocumentRecent] { recents }

        func save(_ recents: [DocumentRecent]) throws { self.recents = recents }
    }
#endif
