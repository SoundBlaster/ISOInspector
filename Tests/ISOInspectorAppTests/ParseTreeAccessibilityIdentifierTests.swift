    ...
    @MainActor final class ParseTreeAccessibilityIdentifierTests: XCTestCase {
        func testExplorerAppliesRootIdentifier() {
            ...
            let view = ParseTreeExplorerView(viewModel: documentViewModel).frame(
        ...
        func testExplorerSearchFieldReceivesNestedIdentifier() {
            ...
            let identifier = ParseTreeAccessibilityID.path(
                ParseTreeAccessibilityID.root, ParseTreeAccessibilityID.Outline.root,
        ...
        func testInspectorToggleButtonReceivesNestedIdentifier() {
            ...
            let identifier = ParseTreeAccessibilityID.path(
                ParseTreeAccessibilityID.root, ParseTreeAccessibilityID.Header.root,
        ...
        func testAnnotationNoteControlsExposeNestedIdentifiers() throws {
            ...
            let record = AnnotationRecord(
                id: recordID, nodeID: 0, note: "Existing note", createdAt: now, updatedAt: now)
            ...
            let header = BoxHeader(
                type: try FourCharCode("test"), totalSize: 32, headerSize: 8, payloadRange: 8..<32,
                range: 0..<32, uuid: nil)
            ...
            ).a11yRoot(ParseTreeAccessibilityID.root).nestedAccessibilityIdentifier(
            ...
            let editIdentifier = ParseTreeAccessibilityID.path(
                ParseTreeAccessibilityID.root, ParseTreeAccessibilityID.Detail.root,
        ...
        private func findView(in root: UIView, withIdentifier identifier: String) -> UIView? {
            ...
            for subview in root.subviews {
                if let match = findView(in: subview, withIdentifier: identifier) { return match }
            }
            return nil
        }
    }
    ...
    private struct DetailHostView: View {
        ...
        var body: some View {
            ParseTreeDetailView(
                viewModel: viewModel, annotationSession: session, selectedNodeID: $selectedNodeID,
                focusTarget: $focus)
        }
    }
    ...
    private final class MockAnnotationStore: AnnotationBookmarkStoring {
        ...
        func annotations(for file: URL) throws -> [AnnotationRecord] { annotations }

        func bookmarks(for file: URL) throws -> [BookmarkRecord] { bookmarks }

        func createAnnotation(for file: URL, nodeID: Int64, note: String) throws -> AnnotationRecord
        {
            let record = AnnotationRecord(
                nodeID: nodeID, note: note, createdAt: Date(), updatedAt: Date())
