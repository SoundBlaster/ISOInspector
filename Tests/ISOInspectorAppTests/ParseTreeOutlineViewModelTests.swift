    ...
    @MainActor final class ParseTreeOutlineViewModelTests: XCTestCase {
        func testApplyingSnapshotExpandsRootNodes() throws {
            let snapshot = ParseTreeSnapshot(
                nodes: [
                    makeNode(
                        identifier: 1, type: "moov",
                        children: [makeNode(identifier: 2, type: "trak")])
                ], validationIssues: [])
                    ...
                    makeNode(
                        ...
                        children: [
                            makeNode(
                                ...
                                children: [makeNode(identifier: 3, type: "mdia")])
                        ])
                ], validationIssues: [])
            let viewModel = ParseTreeOutlineViewModel()
            viewModel.apply(snapshot: snapshot)
        ...
        func testSeverityFilterShowsMatchingIssuesAndContext() throws {
            ...
            let warningIssue = ValidationIssue(
                ruleID: "VR-002", message: "warning", severity: .warning)
            let snapshot = ParseTreeSnapshot(
                nodes: [
                    makeNode(
        ...
        @MainActor func testBindingToSnapshotPublisherStreamsUpdates() async throws {
            let viewModel = ParseTreeOutlineViewModel()
        ...
        func testIssueOnlyFilterHidesHealthyBranchesButKeepsAncestors() throws {
            let corruptLeaf = makeNode(
                identifier: 3, type: "leaf",
                parseIssues: [
                    ParseIssue(
                        severity: .error, code: "VR-999", message: "Corrupt", byteRange: 0..<8,
                        affectedNodeIDs: [3])
                ])
            ...
            let root = makeNode(
                identifier: 1, type: "moov", children: [intermediate, healthyBranch])
            let snapshot = ParseTreeSnapshot(nodes: [root], validationIssues: [])
            let viewModel = ParseTreeOutlineViewModel()
            viewModel.apply(snapshot: snapshot)
        ...
        func testAvailableCategoriesReflectsSnapshotContents() throws {
            ...
            let root = makeNode(
                identifier: 1, type: "moov",
                children: [metadataNode, mediaNode, indexNode, otherNode])
            let snapshot = ParseTreeSnapshot(nodes: [root], validationIssues: [])
            let viewModel = ParseTreeOutlineViewModel()
        ...
        func testRowsExposeCorruptionSummaryFromParseIssues() throws {
            let parseIssues = [
                ParseIssue(
                    severity: .error, code: "VR-900", message: "Stub error", byteRange: 0..<8,
                    affectedNodeIDs: [1]),
                ParseIssue(
                    severity: .warning, code: "VR-901", message: "Stub warning", byteRange: 8..<16,
                    affectedNodeIDs: [1]),
            ]
        ...
        func testContainsNodeReflectsLatestSnapshot() throws {
            let rootWithChild = makeNode(
                identifier: 1, type: "moov", children: [makeNode(identifier: 2, type: "trak")])
        ...
        func testNavigationMovesBetweenVisibleRows() throws {
            let root = makeNode(
                identifier: 1, type: "root",
                children: [
                    makeNode(identifier: 2, type: "trak"), makeNode(identifier: 3, type: "mdia"),
        ...
        func testNavigationMovesToParentOrChildRows() throws {
            let child = makeNode(
                identifier: 2, type: "trak", children: [makeNode(identifier: 3, type: "mdia")])
        ...
        func testIssueNavigationSkipsHealthyNodes() throws {
            let corruptA = makeNode(
                ...
                parseIssues: [
                    ParseIssue(
                        severity: .warning, code: "VR-100", message: "Warning", byteRange: 0..<4,
                        affectedNodeIDs: [2])
                ])
            let healthy = makeNode(identifier: 3, type: "mdia")
            let corruptB = makeNode(
                identifier: 4, type: "minf",
                issues: [ValidationIssue(ruleID: "VR-002", message: "Error", severity: .error)])
            let root = makeNode(
                identifier: 1, type: "root", children: [corruptA, healthy, corruptB])
            let snapshot = ParseTreeSnapshot(nodes: [root], validationIssues: [])
            let viewModel = ParseTreeOutlineViewModel()
        ...
        private func makeNode(
            ...
            parseIssues: [ParseIssue] = [], status: ParseTreeNode.Status = .valid,
            ...
            let header = BoxHeader(
                ...
                range: start..<(start + totalSize), uuid: nil)
            return ParseTreeNode(
                header: header, metadata: nil, payload: nil, validationIssues: issues,
                issues: parseIssues, status: status, children: children)
        }
    }
#endif
