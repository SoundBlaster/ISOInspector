    ...
    final class AnnotationBookmarkStoreTests: XCTestCase {
        ...
        func testCreateAnnotationPersistsRecord() throws {
            ...
            let annotation = try store.createAnnotation(
                for: file, nodeID: 42, note: "Check major brand")
        ...
        func testUpdateAnnotationChangesNoteAndTimestamp() throws {
            ...
            let updated = try store.updateAnnotation(
                for: file, annotationID: annotation.id, note: "Revised note")
        ...
        func testUpdateMissingAnnotationThrows() throws {
            ...
            ) { error in XCTAssertEqual(error as? AnnotationBookmarkStoreError, .annotationNotFound)
            }
        }
        ...
        func testInitializingWithExplicitModelVersionUsesSchema() throws {
            ...
            let annotation = try store.createAnnotation(
                for: file, nodeID: 99, note: "Ensure schema loads")
        ...
        func testSessionPersistenceRoundTrip() throws {
            ...
            let focusRecent = DocumentRecent(
                url: focusURL, bookmarkData: Data([0x01]), displayName: "Focus",
                lastOpened: referenceDate)
            let otherRecent = DocumentRecent(
                url: otherURL, bookmarkData: nil, displayName: "Other",
                lastOpened: referenceDate.addingTimeInterval(-60))

            let focusConfiguration = ValidationConfiguration(
                activePresetID: "structural", ruleOverrides: [.researchLogRecording: false])

            let snapshot = WorkspaceSessionSnapshot(
                id: UUID(uuidString: "00000000-0000-0000-0000-000000000021")!,
                createdAt: referenceDate, updatedAt: referenceDate.addingTimeInterval(120),
                appVersion: "1.0",
                files: [
                    WorkspaceSessionFileSnapshot(
                        id: UUID(uuidString: "00000000-0000-0000-0000-000000000022")!,
                        recent: focusRecent, orderIndex: 0, lastSelectionNodeID: 77,
                        isPinned: false, scrollOffset: WorkspaceSessionScrollOffset(x: 1, y: 2),
                        bookmarkIdentifier: focusRecent.bookmarkIdentifier, bookmarkDiffs: [],
                    ...
                    WorkspaceSessionFileSnapshot(
                        ...
                        bookmarkIdentifier: otherRecent.bookmarkIdentifier, bookmarkDiffs: [],
            ...
            XCTAssertEqual(
                loaded?.files.first?.recent.url.standardizedFileURL, focusURL.standardizedFileURL)
        ...
        func testClearingCurrentSessionRemovesSnapshot() throws {
            ...
            let snapshot = WorkspaceSessionSnapshot(
                id: UUID(), createdAt: referenceDate, updatedAt: referenceDate, appVersion: nil,
                files: [], focusedFileURL: nil, lastSceneIdentifier: nil, windowLayouts: [])
        ...
        func testSavingSessionLinksBookmarkDiffsToBookmarks() throws {
            ...
            let recent = DocumentRecent(
                ...
                bookmarkData: Data([0xAB]), displayName: "Diff", lastOpened: referenceDate)

            let diff = WorkspaceSessionBookmarkDiff(
                id: UUID(uuidString: "00000000-0000-0000-0000-0000B000000C")!,
                bookmarkID: bookmarkID, isRemoved: false, noteDelta: "sync")

            let snapshot = WorkspaceSessionSnapshot(
                id: UUID(uuidString: "00000000-0000-0000-0000-00000000D155")!,
                createdAt: referenceDate, updatedAt: referenceDate, appVersion: "1.0",
                files: [
                    WorkspaceSessionFileSnapshot(
                        id: UUID(uuidString: "00000000-0000-0000-0000-00000000D156")!,
                        recent: recent, orderIndex: 0, lastSelectionNodeID: nil, isPinned: false,
                        scrollOffset: nil, bookmarkIdentifier: recent.bookmarkIdentifier,
                        bookmarkDiffs: [diff], validationConfiguration: nil)
                ], focusedFileURL: fileURL, lastSceneIdentifier: nil, windowLayouts: [])
        ...
        private func performSync<T>(
            ...
            context.performAndWait {
                do { result = .success(try block(context)) } catch { result = .failure(error) }
            }
            switch result {
            case .success(let value): return value
            case .failure(let error): throw error
            case .none: throw CocoaError(.coderReadCorrupt)
            }
        }

        private func makeStore(
            directory: URL, modelVersion: CoreDataAnnotationBookmarkStore.ModelVersion = .latest,
            now: @escaping @Sendable () -> Date
        ) throws -> CoreDataAnnotationBookmarkStore {
            try CoreDataAnnotationBookmarkStore(
                directory: directory, modelVersion: modelVersion, makeDate: now)
        }

        private func makeStore(
            directory: URL, modelVersion: CoreDataAnnotationBookmarkStore.ModelVersion
        ) throws -> CoreDataAnnotationBookmarkStore {
