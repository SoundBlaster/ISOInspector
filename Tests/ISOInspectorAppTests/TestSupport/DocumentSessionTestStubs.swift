    ...
    final class DocumentRecentsStoreStub: DocumentRecentsStoring {
        ...
        init(initialRecents: [DocumentRecent]) { self.initialRecents = initialRecents }

        func load() throws -> [DocumentRecent] { initialRecents }

        func save(_ recents: [DocumentRecent]) throws {
            savedRecents = recents
            if let saveHandler { try saveHandler(recents) }
        }
    }
    ...
    final class BookmarkPersistenceStoreStub: BookmarkPersistenceManaging, @unchecked Sendable {
        private struct StoredRecord { var record: BookmarkPersistenceStore.Record }
            ...
            -> BookmarkPersistenceStore.Record
        {
            ...
            let record = BookmarkPersistenceStore.Record(
                id: UUID(), canonicalURL: canonical, bookmarkData: bookmarkData,
                createdAt: dateProvider(), updatedAt: dateProvider(), lastResolvedAt: nil,
                lastResolutionState: .unknown)
            storage[canonical] = StoredRecord(record: record)
            return record
        }

        @discardableResult func markResolution(for file: URL, state: BookmarkResolutionState) throws
            -> BookmarkPersistenceStore.Record?
        {
            let canonical = canonicalize(file)
            markedResolutions.append((file, state))
            guard var stored = storage[canonical]?.record else { return nil }
    ...
    final class WorkspaceSessionStoreStub: WorkspaceSessionStoring, @unchecked Sendable {
        ...
        func loadCurrentSession() throws -> WorkspaceSessionSnapshot? { loadedSnapshot }

        func saveCurrentSession(_ snapshot: WorkspaceSessionSnapshot) throws {
            savedSnapshots.append(snapshot)
            if let saveHandler { try saveHandler(snapshot) }
        }

        func clearCurrentSession() throws {
            clearCallCount += 1
            if let clearHandler { try clearHandler() }
        }
    }
    ...
    struct ImmediateWorkQueue: DocumentSessionWorkQueue {
        func execute(_ work: @Sendable @escaping () -> Void) {
            work()
            if Thread.isMainThread { RunLoop.main.run(until: Date(timeIntervalSinceNow: 0.05)) }
        }
    }
        ...
        func makeAccess() -> FilesystemAccess {
            FilesystemAccess(
                ...
                bookmarkCreator: { [weak self] url in
                    if let creator = self?.bookmarkCreator { return try creator(url) }
                    return Data(url.path.utf8)
                },
                bookmarkResolver: { [weak self] data in
                    if let error = self?.resolutionError { throw error }
                    if let resolution = self?.resolutions[data] { return resolution }
                    let path = String(decoding: data, as: UTF8.self)
                    return BookmarkResolution(url: URL(fileURLWithPath: path), isStale: false)
                }, securityScopeManager: manager, logger: .disabled)
        }
    }
#endif
