    ...
    final class ParseTreeStoreTests: XCTestCase {
        @MainActor func testFilteringValidationIssuesUpdatesSnapshot() {
            let filteredIssue = ValidationIssue(
                ruleID: "VR-006", message: "research", severity: .info)
            let retainedIssue = ValidationIssue(
                ruleID: "VR-001", message: "structure", severity: .error)
            ...
            store.$state.dropFirst().sink { state in if state == .finished { finished.fulfill() } }
            ...
            XCTAssertEqual(
                store.snapshot.validationIssues.filter { $0.ruleID == "VR-006" }.count, 2)
            XCTAssertEqual(
                store.snapshot.validationIssues.filter { $0.ruleID == "VR-001" }.count, 2)
        ...
        @MainActor func testTreeStoreBuildsHierarchyAndAggregatesIssues() {
            let events = makeSampleEvents(childIssues: [
                ValidationIssue(ruleID: "VR-999", message: "Stub issue", severity: .warning)
            ])
            let stream = AsyncThrowingStream<ParseEvent, Error> { continuation in
                for event in events { continuation.yield(event) }
                continuation.finish()
            ...
            XCTAssertGreaterThan(
                store.snapshot.lastUpdatedAt.timeIntervalSince1970,
        ...
        @MainActor func testTreeStoreSurfacesFailures() {
        ...
        @MainActor func testIssueMetricsPublishesAggregatedCounts() {
            let store = ParseTreeStore()
            ...
            store.$issueMetrics.dropFirst().sink { metrics in
            ...
            let issue = ParseIssue(
                severity: .error, code: "VR-999", message: "Stub issue", byteRange: 0..<8,
                affectedNodeIDs: [1, 2])
            let warning = ParseIssue(
                severity: .warning, code: "VR-998", message: "Stub warning", byteRange: 8..<16,
                affectedNodeIDs: [1])
        ...
        @MainActor func testSnapshotIncludesParseIssuesFromEvents() {
            let parseIssue = ParseIssue(
                severity: .error, code: "VR-900", message: "Stub parse issue", byteRange: 8..<16,
                affectedNodeIDs: [1, 2])
        ...
        @MainActor func testFinishingBoxDoesNotDuplicateParseIssues() {
            let initialIssue = ParseIssue(
                severity: .warning, code: "VR-901", message: "Initial issue", byteRange: 0..<4,
                affectedNodeIDs: [2])
            let additionalIssue = ParseIssue(
                severity: .error, code: "VR-902", message: "Additional issue", byteRange: 4..<8,
                affectedNodeIDs: [2])
            let events = makeSampleEvents(
            ...
            XCTAssertEqual(
                Set(childIssues.map(\.code)), Set([initialIssue.code, additionalIssue.code]))
        }

        @MainActor func testPlaceholderNodesRecordedForMissingRequiredChildren() throws {
            let minfType = try FourCharCode("minf")
            let smhdType = try FourCharCode("smhd")

            let minfHeader = BoxHeader(
                type: minfType, totalSize: 64, headerSize: 8, payloadRange: 8..<64, range: 0..<64,
                uuid: nil)
            let smhdHeader = BoxHeader(
                type: smhdType, totalSize: 16, headerSize: 8, payloadRange: 8..<16, range: 8..<24,
                uuid: nil)

            let events: [ParseEvent] = [
                ParseEvent(
                    kind: .willStartBox(header: minfHeader, depth: 0),
                    offset: minfHeader.startOffset),
                ParseEvent(
                    kind: .willStartBox(header: smhdHeader, depth: 1),
                    offset: smhdHeader.startOffset),
                ParseEvent(
                    kind: .didFinishBox(header: smhdHeader, depth: 1), offset: smhdHeader.endOffset),
                ParseEvent(
                    kind: .didFinishBox(header: minfHeader, depth: 0), offset: minfHeader.endOffset),
            ]
        ...
        private func makeSampleEvents(
            childIssues: [ValidationIssue] = [], childParseIssues: [ParseIssue] = [],
            ...
            return [
                ...
                ParseEvent(
                    ...
                    validationIssues: childIssues, issues: childParseIssues),
                ParseEvent(
