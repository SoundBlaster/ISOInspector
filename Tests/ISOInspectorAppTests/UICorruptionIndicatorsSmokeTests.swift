    ...
    @MainActor final class UICorruptionIndicatorsSmokeTests: XCTestCase {
        ...
        private lazy var manifestURL: URL = {
            ...
            return root.appendingPathComponent("Documentation").appendingPathComponent(
                "FixtureCatalog"
        ...
        private lazy var fixturesRoot: URL = {
            let root = URL(fileURLWithPath: #filePath).deletingLastPathComponent()
                .deletingLastPathComponent().deletingLastPathComponent()
            return root.appendingPathComponent("Fixtures").appendingPathComponent("Corrupt")
        }()
        ...
        func testCorruptionBadgeDisplaysForCorruptNode() throws {
            ...
            let header = BoxHeader(
                type: try FourCharCode("moov"), totalSize: 32, headerSize: 8, payloadRange: 8..<24,
                range: 0..<32, uuid: nil)
        ...
        func testPartialBadgeDisplaysForPartialNode() throws {
            ...
            let header = BoxHeader(
                type: try FourCharCode("trak"), totalSize: 24, headerSize: 8, payloadRange: 8..<16,
                range: 0..<24, uuid: nil)
            let node = ParseTreeNode(
                header: header, metadata: nil, payload: nil, validationIssues: [], issues: [],
        ...
        func testNoBadgeForValidNode() throws {
            ...
            let header = BoxHeader(
                type: try FourCharCode("ftyp"), totalSize: 24, headerSize: 8, payloadRange: 8..<24,
                range: 0..<24, uuid: nil)
            let node = ParseTreeNode(
                header: header, metadata: nil, payload: nil, validationIssues: [], issues: [],
        ...
        func testIntegritySummaryDisplaysCorruptionIssues() async {
            ...
            store.issueStore.record(
                ParseIssue(
                    severity: .error, code: "payload.truncated",
                    message: "Moov box declares more bytes than reader provides",
                    byteRange: 100..<200, affectedNodeIDs: [1]), depth: 1)
            store.issueStore.record(
                ParseIssue(
                    severity: .warning, code: "guard.zero_size_loop",
                    message: "Zero-length loop detected", byteRange: 300..<400, affectedNodeIDs: [2]
                ), depth: 2)
        ...
        func testIntegritySummaryOffsetSortingForCorruptionIssues() async {
            ...
            store.issueStore.record(
                ParseIssue(
                    severity: .error, code: "header.truncated_field",
                    message: "Size field truncated", byteRange: 300..<304, affectedNodeIDs: [3]),
                depth: 1)
            store.issueStore.record(
                ParseIssue(
                    severity: .error, code: "payload.truncated", message: "Payload truncated",
                    byteRange: 100..<200, affectedNodeIDs: [1]), depth: 1)
            store.issueStore.record(
                ParseIssue(
                    severity: .error, code: "header.invalid_fourcc", message: "Invalid fourcc",
                    byteRange: 200..<204, affectedNodeIDs: [2]), depth: 1)
        ...
        func testCorruptionDetailSectionPopulatedWithIssueDetails() async throws {
            ...
            let header = BoxHeader(
                type: try FourCharCode("moov"), totalSize: 100, headerSize: 8, payloadRange: 8..<50,
                range: 0..<100, uuid: nil)
            let issues = [
                ParseIssue(
                    severity: .error, code: "payload.truncated",
                    message: "Moov payload truncated at reader boundary", byteRange: 8..<50,
                    affectedNodeIDs: [0])
            ]
            let node = ParseTreeNode(
                header: header, metadata: nil, payload: nil, validationIssues: [], issues: issues,
                status: .corrupt, children: [])
        ...
        func testCorruptionWarningRibbonAccessibilityLabel() {
            ...
            let metrics = ParseIssueStore.IssueMetrics(
                errorCount: 2, warningCount: 5, infoCount: 1, deepestAffectedDepth: 3)
        ...
        private func fixtureURL(for filename: String) throws -> URL {
            ...
            if FileManager.default.fileExists(atPath: binaryURL.path) { return binaryURL }
