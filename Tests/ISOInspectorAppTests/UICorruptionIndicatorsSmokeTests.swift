...
final class UICorruptionIndicatorsSmokeTests: XCTestCase {
    ...
    func testCorruptionDetailSectionPopulatedWithIssueDetails() async throws {
        ...
        let hexProvider = HexSliceProviderStub(result: HexSlice(offset: header.payloadRange.lowerBound, bytes: Data()))
        let viewModel = ParseTreeDetailViewModel(hexSliceProvider: hexProvider, annotationProvider: nil)
    ...
    func testTolerantParsingProducesCorruptionIndicatorsForSmokeFixtures() async throws {
        ...
        for fixture in smokeFixtures {
            ...
            XCTAssertFalse(events.isEmpty, "Pipeline should emit events for fixture \(fixture.id)")
            ...
            XCTAssertGreaterThan(metrics.totalCount, 0, "Fixture \(fixture.id) should have detected issues")
        }
    }
    ...
    private func fixtureURL(for filename: String) throws -> URL {
        ...
        guard FileManager.default.fileExists(atPath: base64URL.path) else {
            throw NSError(
                domain: "FixtureError",
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: "Missing base64 source for fixture \(filename)"]
            )
        }
        ...
        guard let data = Data(base64Encoded: encoded, options: .ignoreUnknownCharacters) else {
            throw NSError(
                domain: "FixtureError",
                code: 2,
                userInfo: [NSLocalizedDescriptionKey: "Fixture \(filename) contains invalid base64 payload"]
            )
        }
