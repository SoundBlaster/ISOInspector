...
final class BoxValidatorTests: XCTestCase {
  ...
  func testContainerRuleReportsChildOffsetMismatch() throws {
    ...
    let annotatedChildStart = validator.annotate(
      event: ParseEvent(
        kind: .willStartBox(header: misalignedChild, depth: 1), offset: misalignedChild.startOffset),
      reader: reader
    )
  ...
  func testContainerRuleReportsChildOverflowBeyondParentPayload() throws {
    ...
    let annotatedChildStart = validator.annotate(
      event: ParseEvent(
        kind: .willStartBox(header: oversizedChild, depth: 1), offset: oversizedChild.startOffset),
      reader: reader
    )
  ...
  func testContainerRuleReportsChildOverlapWithSibling() throws {
    ...
    let annotatedChildStart = validator.annotate(
      event: ParseEvent(
        ...
        offset: overlappingChild.startOffset),
      reader: reader
    )
