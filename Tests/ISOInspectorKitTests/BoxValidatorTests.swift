...
final class BoxValidatorTests: XCTestCase {
    ...
    func testStructuralRuleReportsHeaderExtendingBeyondReaderLength() throws {
        let header = try makeHeader(
            type: ContainerTypes.movie, totalSize: 32, headerSize: 8, offset: 0)
        let event = ParseEvent(
            kind: .willStartBox(header: header, depth: 0), offset: header.startOffset, metadata: nil
        )
    ...
    func testContainerRuleReportsUnderflowWhenClosingContainerEarly() throws {
        ...
        let moov = try makeHeader(
            type: ContainerTypes.movie, totalSize: 28, headerSize: 8, offset: 0)
        let trak = try makeHeader(
            type: ContainerTypes.track, totalSize: 12, headerSize: 8, offset: 8)

        let validator = BoxValidator()

        _ = validator.annotate(
            event: ParseEvent(
                kind: .willStartBox(header: moov, depth: 0), offset: moov.startOffset),
            reader: reader)
        _ = validator.annotate(
            event: ParseEvent(
                kind: .willStartBox(header: trak, depth: 1), offset: trak.startOffset),
            reader: reader)
        _ = validator.annotate(
    ...
    func testContainerRuleReportsChildOffsetMismatch() throws {
        ...
        let moov = try makeHeader(
            type: ContainerTypes.movie, totalSize: 32, headerSize: 8, offset: 0)
        let misalignedChild = try makeHeader(
            type: ContainerTypes.track, totalSize: 12, headerSize: 8, offset: 10)
        ...
        let annotatedChildStart = validator.annotate(
            event: ParseEvent(
                kind: .willStartBox(header: misalignedChild, depth: 1),
                offset: misalignedChild.startOffset), reader: reader)

        let vr002Issues = annotatedChildStart.validationIssues.filter { $0.ruleID == "VR-002" }
        XCTAssertEqual(vr002Issues.count, 1)
        XCTAssertTrue(
            vr002Issues.first?.message.contains("expected child to start at offset") ?? false)
        XCTAssertEqual(vr002Issues.first?.severity, .error)
    }
    ...
    func testContainerRuleAllowsPerfectlyConsumedContainer() throws {
        ...
        let moov = try makeHeader(
            type: ContainerTypes.movie, totalSize: 28, headerSize: 8, offset: 0)
        let trak = try makeHeader(
            type: ContainerTypes.track, totalSize: 20, headerSize: 8, offset: 8)
        ...
        _ = validator.annotate(
            event: ParseEvent(
                kind: .willStartBox(header: tkhd, depth: 2), offset: tkhd.startOffset),
            reader: reader)
        _ = validator.annotate(
    ...
    func testContainerRuleReportsChildOverflowBeyondParentPayload() throws {
        ...
        let moov = try makeHeader(
            type: ContainerTypes.movie, totalSize: 28, headerSize: 8, offset: 0)
        let oversizedChild = try makeHeader(
            type: ContainerTypes.track, totalSize: 24, headerSize: 8, offset: 8)
        ...
        let annotatedChildStart = validator.annotate(
            event: ParseEvent(
                kind: .willStartBox(header: oversizedChild, depth: 1),
                offset: oversizedChild.startOffset), reader: reader)
    ...
    func testContainerRuleReportsChildOverlapWithSibling() throws {
        let reader = InMemoryRandomAccessReader(data: Data(count: 64))
        let moov = try makeHeader(
            type: ContainerTypes.movie, totalSize: 40, headerSize: 8, offset: 0)
        let firstChild = try makeHeader(
            type: ContainerTypes.track, totalSize: 16, headerSize: 8, offset: 8)
        let overlappingChild = try makeHeader(
            type: ContainerTypes.track, totalSize: 16, headerSize: 8, offset: 20)
        ...
        _ = validator.annotate(
            event: ParseEvent(
                kind: .willStartBox(header: firstChild, depth: 1), offset: firstChild.startOffset),
            reader: reader)
        _ = validator.annotate(
            event: ParseEvent(
                kind: .didFinishBox(header: firstChild, depth: 1), offset: firstChild.endOffset),
            reader: reader)

        let annotatedChildStart = validator.annotate(
            event: ParseEvent(
                kind: .willStartBox(header: overlappingChild, depth: 1),
                offset: overlappingChild.startOffset), reader: reader)
    ...
    func testFragmentSequenceRuleWarnsOnNonMonotonicSequenceNumbers() throws {
        ...
        _ = validator.annotate(
            event: try makeMfhdEvent(sequenceNumber: 10, offset: 0), reader: reader)
        let second = validator.annotate(
            event: try makeMfhdEvent(sequenceNumber: 9, offset: 16), reader: reader)
    ...
    func testFragmentSequenceRuleWarnsWhenSequenceNumberIsZero() throws {
        ...
        let event = validator.annotate(
            event: try makeMfhdEvent(sequenceNumber: 0, offset: 0), reader: reader)
    ...
    func testFragmentSequenceRulePassesWhenSequenceNumbersIncrease() throws {
        ...
        let first = validator.annotate(
            event: try makeMfhdEvent(sequenceNumber: 1, offset: 0), reader: reader)
        XCTAssertTrue(first.validationIssues.filter { $0.ruleID == "VR-016" }.isEmpty)

        let second = validator.annotate(
            event: try makeMfhdEvent(sequenceNumber: 2, offset: 16), reader: reader)
    ...
    func testFragmentRunRuleFlagsZeroSampleCount() throws {
        ...
        let run = ParsedBoxPayload.TrackRunBox(
            version: 0, flags: 0, sampleCount: 0, dataOffset: nil, firstSampleFlags: nil,
            entries: [], totalSampleDuration: 0, totalSampleSize: 0, startDecodeTime: nil,
            endDecodeTime: nil, startPresentationTime: nil, endPresentationTime: nil,
            startDataOffset: nil, endDataOffset: nil, trackID: 1, sampleDescriptionIndex: nil,
            runIndex: 0, firstSampleGlobalIndex: 1)
    ...
    func testFragmentRunRuleFlagsMissingDurations() throws {
        ...
        let entry = ParsedBoxPayload.TrackRunBox.Entry(
            index: 1, decodeTime: nil, presentationTime: nil, sampleDuration: nil, sampleSize: 1024,
            sampleFlags: 0x0102_0304, sampleCompositionTimeOffset: nil, dataOffset: 4096,
            byteRange: nil)
        let run = ParsedBoxPayload.TrackRunBox(
            version: 0, flags: 0, sampleCount: 1, dataOffset: nil, firstSampleFlags: nil,
            entries: [entry], totalSampleDuration: nil, totalSampleSize: 1024, startDecodeTime: nil,
            endDecodeTime: nil, startPresentationTime: nil, endPresentationTime: nil,
            startDataOffset: 4096, endDataOffset: 4096 + 1024, trackID: 3,
            sampleDescriptionIndex: 4, runIndex: 0, firstSampleGlobalIndex: 1)
        let payload = ParsedBoxPayload(detail: .trackRun(run))
        let event = ParseEvent(
            kind: .willStartBox(header: header, depth: 0), offset: header.startOffset,
            payload: payload)
    ...
    func testFragmentRunRulePassesForWellFormedRun() throws {
        let header = try makeHeader(type: "trun", payloadSize: 0)
        let entry = ParsedBoxPayload.TrackRunBox.Entry(
            index: 1, decodeTime: 10, presentationTime: 10, sampleDuration: 100, sampleSize: 512,
            sampleFlags: 0x0, sampleCompositionTimeOffset: nil, dataOffset: 2048, byteRange: nil)
        let run = ParsedBoxPayload.TrackRunBox(
            version: 0, flags: 0, sampleCount: 1, dataOffset: nil, firstSampleFlags: nil,
            entries: [entry], totalSampleDuration: 100, totalSampleSize: 512, startDecodeTime: 10,
            endDecodeTime: 110, startPresentationTime: 10, endPresentationTime: 110,
            startDataOffset: 2048, endDataOffset: 2560, trackID: 5, sampleDescriptionIndex: 6,
            runIndex: 0, firstSampleGlobalIndex: 1)
        let payload = ParsedBoxPayload(detail: .trackRun(run))
        let event = ParseEvent(
            kind: .willStartBox(header: header, depth: 0), offset: header.startOffset,
    ...
    func testCodecRuleAcceptsHevcArraysWithArrayCompletenessZero() throws {
        ...
        let stsd = makeStsdBox(entries: [
            makeVisualSampleEntry(format: "hvc1", additionalBoxes: [hevcC])
        ])
        let header = try makeHeader(type: "stsd", payloadSize: stsd.count - 8)
        let reader = InMemoryRandomAccessReader(data: stsd)
        let payload = try XCTUnwrap(BoxParserRegistry.shared.parse(header: header, reader: reader))
    ...
    func testCodecRuleFlagsHevcMissingLengthSize() throws {
        ...
        let header = try makeHeader(type: "stsd", payloadSize: stsd.count - 8)
        let reader = InMemoryRandomAccessReader(data: stsd)
        let payloadDetail = try XCTUnwrap(
    ...
    private func makeMfhdEvent(sequenceNumber: UInt32, offset: Int64) throws -> ParseEvent {
        ...
        let sequenceRange =
            (header.payloadRange.lowerBound + 4)..<(header.payloadRange.lowerBound + 8)
        let payload = ParsedBoxPayload(
            fields: [
                ParsedBoxPayload.Field(
                    name: "version", value: "0", description: "Structure version",
                    byteRange: versionRange),
                ParsedBoxPayload.Field(
                    name: "flags", value: "0x000000", description: "Bit flags",
    ...
    private func makeVisualSampleEntry(
        format: String = "avc1", width: UInt16 = 1920, height: UInt16 = 1080,
        dataReferenceIndex: UInt16 = 1, additionalBoxes: [Data]
    ) -> Data {
        var entry = Data()
        ...
        entry.append(contentsOf: UInt32(0x0048_0000).bigEndianBytes)
        entry.append(contentsOf: UInt32(0x0048_0000).bigEndianBytes)
        entry.append(contentsOf: UInt32(0).bigEndianBytes)
    ...
    private func makeValidHevcConfigurationBox(arrayCompleteness: Bool = true) -> Data {
        ...
        let arrays: [(UInt8, [UInt8])] = [
            (32, [0x40, 0x01]), (33, [0x42, 0x01, 0x01]), (34, [0x44, 0x01]),
        ]
        payload.append(0x03)
