...
final class StcoChunkOffsetParserTests: XCTestCase {
    func testParses32BitChunkOffsets() throws {
        ...
        let parsed = try parseChunkOffsets(
            fourCC: "stco",
            offsets: offsets.map(UInt64.init)
        )
        ...
        XCTAssertEqual(fields["entries[0].chunk_offset"]?.value, "128")
        XCTAssertEqual(fields["entries[1].chunk_offset"]?.value, "2048")
        XCTAssertEqual(fields["entries[2].chunk_offset"]?.value, "1048576")
    ...
    func testParses64BitChunkOffsets() throws {
        let offsets: [UInt64] = [65_536, 4_294_967_295, 1099511627776]
        let parsed = try parseChunkOffsets(
            fourCC: "co64",
            offsets: offsets
        )
        ...
        XCTAssertEqual(fields["flags"]?.value, "0x000000")
        XCTAssertEqual(fields["entry_count"]?.value, "3")
        XCTAssertEqual(fields["entries[0].chunk_offset"]?.value, "65536")
        XCTAssertEqual(fields["entries[1].chunk_offset"]?.value, "4294967295")
        XCTAssertEqual(fields["entries[2].chunk_offset"]?.value, "1099511627776")
    ...
    func testReportsTruncatedChunkOffsetTable() throws {
        ...
        XCTAssertEqual(fields["entries[1].status"]?.description, "Entry truncated before chunk offset completed")
