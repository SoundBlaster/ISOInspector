...
final class TolerantParsingDocTests: XCTestCase {
    ...
    func testBasicTolerantParsingWorkflow() async throws {
        ...
        for try await event in pipeline.events(for: reader, context: context) {
            eventCount += 1
            switch event.kind {
            case .willStartBox: break
            case .didFinishBox: break
            }
        }
    ...
    func testStrictVsTolerantComparison() async throws {
        ...
        for try await event in strictPipeline.events(for: strictReader, context: strictContext) {
            if case .willStartBox = event.kind { strictBoxCount += 1 }
        }
        ...
        for try await event in tolerantPipeline.events(
            for: tolerantReader, context: tolerantContext)
        { if case .willStartBox = event.kind { tolerantBoxCount += 1 } }
    ...
    func testHandlingIncompletePayload() async throws {
        ...
        for try await event in pipeline.events(for: reader, context: context) {
            ...
            if event.payload != nil { boxesWithPayload += 1 } else { boxesWithoutPayload += 1 }
...
extension FixedWidthInteger {
    fileprivate var bytes: [UInt8] { withUnsafeBytes(of: self) { Array($0) } }
}
