...
final class MediaAndIndexBoxCodeTests: XCTestCase {
    func testAllCasesMatchRawValues() {
        let expectedRawValues: Set<String> = [
            MediaAndIndexBoxCode.mediaData.rawValue, MediaAndIndexBoxCode.segmentIndex.rawValue,
    ...
    func testMediaPayloadClassification() throws {
        ...
        XCTAssertFalse(
            MediaAndIndexBoxCode.isMediaPayload(MediaAndIndexBoxCode.segmentIndex.rawValue))
        XCTAssertFalse(
            MediaAndIndexBoxCode.isMediaPayload(MediaAndIndexBoxCode.segmentType.rawValue))
    }
    ...
    private func makeHeader(type: String) throws -> BoxHeader {
        BoxHeader(
            type: try FourCharCode(type), totalSize: 16, headerSize: 8, payloadRange: 8..<16,
            range: 0..<16, uuid: nil)
    }
}
