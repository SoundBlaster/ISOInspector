...
final class FilesystemAccessTests: XCTestCase {
    func testOpenFileStartsSecurityScopeAndLogs() async throws {
        ...
        let accessLogger = FilesystemAccessLogger(
            diagnosticsLogger, auditTrail: auditTrail,
            makeDate: { Date(timeIntervalSinceReferenceDate: 1) })

        let access = FilesystemAccess(
            openFileHandler: openDialog.open, saveFileHandler: { _ in expectedURL },
            ...
            securityScopeManager: scopeManager, logger: accessLogger)
    ...
    func testCreateBookmarkLogsSuccess() throws {
        ...
        let access = FilesystemAccess(
            openFileHandler: { _ in expectedURL }, saveFileHandler: { _ in expectedURL },
            bookmarkCreator: { url in try bookmarkManager.createBookmark(for: url) },
            bookmarkResolver: { data in try bookmarkManager.resolveBookmark(data: data) },
            securityScopeManager: scopeManager, logger: accessLogger)
        ...
        XCTAssertTrue(
            diagnosticsLogger.infoMessages.contains { $0.contains("bookmark.create.success") })
    ...
    func testResolveBookmarkDataReturnsStaleState() throws {
        ...
        XCTAssertEqual(scopeManager.startedURLs, [expectedURL])
        XCTAssertTrue(
            diagnosticsLogger.errorMessages.contains { $0.contains("bookmark.resolve.stale") })
    ...
    func testResolveBookmarkDataLogsBookmarkIdentifierWhenProvided() throws {
        ...
        let resolved = try access.resolveBookmarkData(
            Data("bookmark".utf8), bookmarkIdentifier: recordID)
    ...
    func testResolveBookmarkDataPropagatesErrors() {
        ...
        XCTAssertTrue(
            diagnosticsLogger.errorMessages.contains { $0.contains("bookmark.resolve.failure") })
    ...
    func testOpenFileAuthorizationFailureThrows() async {
        ...
        XCTAssertTrue(
            diagnosticsLogger.errorMessages.contains { $0.contains("authorization_denied") })
    ...
    func testAdoptSecurityScopeLogsAndManagesLifecycle() throws {
        ...
        XCTAssertEqual(scopeManager.startedURLs, [expectedURL])
        XCTAssertTrue(
            diagnosticsLogger.infoMessages.contains {
                $0.contains("scope.external_grant.access_granted")
            })
        ...
        func testLiveUsesDocumentPickerPresenterWhenAppKitUnavailable() async throws {
            ...
            let access = FilesystemAccess.live(
                bookmarkManager: bookmarkManager, securityScopeManager: scopeManager,
                logger: logger, documentPickerPresenter: presenter)

            let openConfiguration = FilesystemOpenConfiguration(
                allowedContentTypes: ["public.movie"], allowsMultipleSelection: true)
            let openURL = try await access.openFile(configuration: openConfiguration)
            let saveConfiguration = FilesystemSaveConfiguration(
                allowedContentTypes: ["public.movie"], suggestedFilename: "clip.mp4")
            ...
            XCTAssertEqual(
                scopeManager.startedURLs,
                [expectedOpenURL.standardizedFileURL, expectedSaveURL.standardizedFileURL])
            XCTAssertTrue(diagnosticsLogger.infoMessages.contains { $0.contains("access_granted") })
            let events = auditTrail.snapshot()
            XCTAssertTrue(
                events.contains { $0.category == .openFile && $0.outcome == .accessGranted })
            XCTAssertTrue(
                events.contains { $0.category == .saveFile && $0.outcome == .accessGranted })
        }
    #endif
...
private final class FilesystemAccessTestSecurityScope: SecurityScopedAccessManaging {
    ...
    func stopAccessing(_ url: URL) { lock.withLock { stoppedURLsStorage.append(url) } }
}
...
private final class FilesystemAccessTestBookmarkManager: BookmarkDataManaging {
    ...
    func resolveBookmark(data: Data) throws -> BookmarkResolution {
        ...
        if let next = nextResolution { return next }
...
private final class FilesystemAccessTestLogger: DiagnosticsLogging {
    ...
    func info(_ message: String) { lock.withLock { infoStorage.append(message) } }

    func error(_ message: String) { lock.withLock { errorStorage.append(message) } }
}
...
private final class FilesystemDocumentPickerPresenterRecorder {
    ...
    var openConfigurations: [FilesystemOpenConfiguration] {
        lock.withLock { openConfigurationsStorage }
    }
    var saveConfigurations: [FilesystemSaveConfiguration] {
        lock.withLock { saveConfigurationsStorage }
    }
