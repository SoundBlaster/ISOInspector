...
final class BoxParserRegistryTests: XCTestCase {
    func testDefaultRegistryParsesFileTypeBox() throws {
        ...
        let header = BoxHeader(
            type: try FourCharCode("ftyp"), totalSize: Int64(totalSize), headerSize: 8,
    ...
    func testDefaultRegistryParsesMovieExtendsContainer() throws {
        let totalSize = 8
        let header = BoxHeader(
            type: try FourCharCode("mvex"), totalSize: Int64(totalSize), headerSize: 8,
            payloadRange: 8..<Int64(totalSize), range: 0..<Int64(totalSize), uuid: nil)
    ...
    func testDefaultRegistryParsesTrackExtendsDefaultsBox() throws {
        ...
        let totalSize = 8 + payload.count
        let header = BoxHeader(
            type: try FourCharCode("trex"), totalSize: Int64(totalSize), headerSize: 8,
            payloadRange: 8..<Int64(totalSize), range: 0..<Int64(totalSize), uuid: nil)
    ...
    func testDefaultFallbackProvidesPlaceholderPayloadForUnknownBox() throws {
        ...
        let totalSize = 8 + payload.count
        let header = BoxHeader(
            type: try FourCharCode("zzzz"), totalSize: Int64(totalSize), headerSize: 8,
            payloadRange: 8..<Int64(totalSize), range: 0..<Int64(totalSize), uuid: nil)
        ...
        XCTAssertEqual(
            value(named: "payload_range", in: parsed), String(describing: header.payloadRange))
    ...
    func testDefaultRegistryParsesMovieHeaderBox() throws {
        ...
        let totalSize = 8 + payload.count
        let header = BoxHeader(
            type: try FourCharCode("mvhd"), totalSize: Int64(totalSize), headerSize: 8,
            payloadRange: 8..<Int64(totalSize), range: 0..<Int64(totalSize), uuid: nil)
        ...
        XCTAssertEqual(
            ...
            [
                ...
                "rate", "volume", "matrix.a", "matrix.b", "matrix.u", "matrix.c", "matrix.d",
                "matrix.v", "matrix.x", "matrix.y", "matrix.w", "next_track_ID",
            ])
    ...
    func testDefaultRegistryParsesMovieHeaderVersion1Box() throws {
        ...
        payload.append(
            contentsOf: [
                UInt32(0x0002_0000), UInt32(0xFFFF_0000), UInt32(0x2000_0000), UInt32(0x0000_8000),
                UInt32(0x0001_0000), UInt32(0xE000_0000), UInt32(0x0000_0000), UInt32(0x0001_0000),
                UInt32(0x4000_0000),
            ].flatMap { $0.bigEndianBytes })
    ...
    func testDefaultRegistryParsesMediaDataBoxRecordsOffsets() throws {
        ...
        let header = BoxHeader(
            type: try FourCharCode("mdat"), totalSize: Int64(totalSize), headerSize: 8,
            payloadRange: (startOffset + 8)..<(startOffset + Int64(totalSize)),
            range: startOffset..<(startOffset + Int64(totalSize)), uuid: nil)
    ...
    func testDefaultRegistryParsesLargeMediaDataBoxRecordsOffsets() throws {
        ...
        let header = BoxHeader(
            type: try FourCharCode("mdat"), totalSize: totalSize, headerSize: headerSize,
            payloadRange: (startOffset + headerSize)..<(startOffset + totalSize),
            range: startOffset..<(startOffset + totalSize), uuid: nil)
    ...
    func testDefaultRegistryParsesFreeBoxAsPadding() throws {
        ...
        let header = BoxHeader(
            type: try FourCharCode("free"), totalSize: Int64(totalSize), headerSize: 8,
            payloadRange: (startOffset + 8)..<(startOffset + Int64(totalSize)),
            range: startOffset..<(startOffset + Int64(totalSize)), uuid: nil)
    ...
    func testDefaultRegistryParsesSkipBoxAsPadding() throws {
        ...
        let header = BoxHeader(
            type: try FourCharCode("skip"), totalSize: totalSize, headerSize: headerSize,
            payloadRange: (startOffset + headerSize)..<(startOffset + totalSize),
            range: startOffset..<(startOffset + totalSize), uuid: nil)
    ...
    func testDefaultRegistryParsesSoundMediaHeaderBox() throws {
        ...
        let totalSize = 8 + payload.count
        let header = BoxHeader(
            type: try FourCharCode("smhd"), totalSize: Int64(totalSize), headerSize: 8,
            payloadRange: 8..<Int64(totalSize), range: 0..<Int64(totalSize), uuid: nil)
    ...
    func testDefaultRegistryParsesVideoMediaHeaderBox() throws {
        ...
        let totalSize = 8 + payload.count
        let header = BoxHeader(
            type: try FourCharCode("vmhd"), totalSize: Int64(totalSize), headerSize: 8,
            payloadRange: 8..<Int64(totalSize), range: 0..<Int64(totalSize), uuid: nil)
    ...
    func testDefaultRegistryParsesMediaHeaderBoxVersion0() throws {
        ...
        let totalSize = 8 + payload.count
        let header = BoxHeader(
            type: try FourCharCode("mdhd"), totalSize: Int64(totalSize), headerSize: 8,
            payloadRange: 8..<Int64(totalSize), range: 0..<Int64(totalSize), uuid: nil)
    ...
    func testDefaultRegistryParsesHandlerBoxWithNullTerminatedName() throws {
        ...
        let totalSize = 8 + payload.count
        let header = BoxHeader(
            type: try FourCharCode("hdlr"), totalSize: Int64(totalSize), headerSize: 8,
            payloadRange: 8..<Int64(totalSize), range: 0..<Int64(totalSize), uuid: nil)
    ...
    func testRegistryAllowsOverrides() throws {
        ...
        let parser: BoxParserRegistry.Parser = { header, _ in
            ParsedBoxPayload(fields: [
                ParsedBoxPayload.Field(
                    name: "identifier", value: header.identifierString, description: nil,
                    byteRange: nil)
            ])
        }
        ...
        let header = BoxHeader(
            type: customType, totalSize: 8, headerSize: 8, payloadRange: 8..<8, range: 0..<8,
            uuid: nil)
    ...
    func testTrackHeaderParserParsesVersion0Box() throws {
        let headerAndReader = try makeTrackHeaderFixture(
            version: 0, flags: 0x000001, creationTime: 1, modificationTime: 2, trackID: 3,
            duration: 540, matrix: .identity, width: 1280, height: 720, volumeRaw: 0x0100)

        let parsed = try XCTUnwrap(
            BoxParserRegistry.shared.parse(
                header: headerAndReader.header, reader: headerAndReader.reader))
    ...
    func testTrackHeaderParserParsesVersion1Box() throws {
        let matrix = ParsedBoxPayload.TransformationMatrix(
            a: 2.0, b: -1.0, u: 0.5, c: 0.5, d: 1.0, v: -0.5, x: 0.0, y: 1.0, w: 1.0)

        let headerAndReader = try makeTrackHeaderFixture(
            version: 1, flags: 0x000007, creationTime: 10, modificationTime: 11, trackID: 12,
            duration: 900, matrix: matrix, width: 1920, height: 1080, volumeRaw: 0x0100, layer: 1,
            alternateGroup: 2)
    ...
    func testTrackHeaderParserHandlesDisabledZeroDurationTrack() throws {
        let headerAndReader = try makeTrackHeaderFixture(
            version: 0, flags: 0x000000, creationTime: 0, modificationTime: 0, trackID: 33,
            duration: 0, matrix: .identity, width: 0, height: 0, volumeRaw: 0x0000)
    ...
    func testDataReferenceParserEmitsEntries() throws {
        ...
        let parsed = try XCTUnwrap(
            BoxParserRegistry.shared.parse(header: fixture.header, reader: fixture.reader))
        ...
        XCTAssertEqual(
            value(named: "entries[1].location", in: parsed), "https://cdn.example.com/audio.aac")
    ...
    func testMetadataKeysParserEmitsEntries() throws {
        ...
        let entries: [(namespace: String, name: String)] = [
            ("mdta", "com.example.title"), ("mdta", "com.example.year"),
        ]
        ...
        let totalSize = 8 + payload.count
        let header = BoxHeader(
            type: try FourCharCode("keys"), totalSize: Int64(totalSize), headerSize: 8,
            payloadRange: 8..<Int64(totalSize), range: 0..<Int64(totalSize), uuid: nil)
    ...
    func testMetadataItemListParserDecodesStringAndIntegerValues() throws {
        let titleEntry = makeMetadataItemEntry(
            ...
            dataBoxes: [makeMetadataDataBox(type: 1, locale: 0, data: Data("Example Title".utf8))])
        ...
        let totalSize = 8 + payload.count
        let header = BoxHeader(
            type: try FourCharCode("ilst"), totalSize: Int64(totalSize), headerSize: 8,
            payloadRange: 8..<Int64(totalSize), range: 0..<Int64(totalSize), uuid: nil)
        ...
        let keyEntry = ParsedBoxPayload.MetadataKeyTableBox.Entry(
            index: 1, namespace: "mdta", name: "com.example.rating")

        let environment = BoxParserRegistry.MetadataEnvironment(
            handlerType: HandlerType(code: try FourCharCode("mdir")), keyTable: [1: keyEntry])
    ...
    func testMetadataItemListParserDecodesBooleanFloatingPointAndDataValues() throws {
        let booleanEntry = makeMetadataItemEntry(
            ...
            dataBoxes: [makeMetadataDataBox(type: 27, locale: 0, data: Data([0x01]))])
        ...
        let jpegEntry = makeMetadataItemEntry(
            ...
            dataBoxes: [makeMetadataDataBox(type: 13, locale: 0, data: jpegData)])
        ...
        let gifEntry = makeMetadataItemEntry(
            ...
            dataBoxes: [makeMetadataDataBox(type: 12, locale: 0, data: gifData)])
        ...
        let tiffEntry = makeMetadataItemEntry(
            ...
            dataBoxes: [makeMetadataDataBox(type: 16, locale: 0, data: tiffData)])
        ...
        let fixedPointEntry = makeMetadataItemEntry(
            ...
            dataBoxes: [makeMetadataDataBox(type: 66, locale: 0, data: fixedPointData)])
        ...
        let fixedPoint88Entry = makeMetadataItemEntry(
            ...
            dataBoxes: [makeMetadataDataBox(type: 65, locale: 0, data: fixedPoint88Data)])
    ...
    private func makeTrackHeaderFixture(
        ...
        trackID: UInt32, duration: UInt64, matrix: ParsedBoxPayload.TransformationMatrix,
        width: UInt32, height: UInt32, volumeRaw: UInt16, layer: Int16 = 0,
        ...
        let totalSize = 8 + payload.count
        let header = BoxHeader(
            type: try FourCharCode("tkhd"), totalSize: Int64(totalSize), headerSize: 8,
            payloadRange: 8..<Int64(totalSize), range: 0..<Int64(totalSize), uuid: nil)
        ...
        .TrackHeaderBox]
    {
        ...
        try walker.walk(
            reader: reader, cancellationCheck: {},
            onEvent: { event in
                ...
                if let payload = try? BoxParserRegistry.shared.parse(
                    header: header, reader: reader), let detail = payload.trackHeader
                {
...
extension BoxParserRegistryTests {
    fileprivate func makeDataReferenceFixture() throws -> (
        ...
        let totalSize = 8 + payload.count
        let header = BoxHeader(
            type: try FourCharCode("dref"), totalSize: Int64(totalSize), headerSize: 8,
            payloadRange: 8..<Int64(totalSize), range: 0..<Int64(totalSize), uuid: nil)
...
extension ParsedBoxPayload.TransformationMatrix {
    fileprivate func fixedPointRepresentation() -> [Int32] {
        [
            Int32((a * 65536.0).rounded()), Int32((b * 65536.0).rounded()),
            Int32((u * Double(1 << 30)).rounded()), Int32((c * 65536.0).rounded()),
            Int32((d * 65536.0).rounded()), Int32((v * Double(1 << 30)).rounded()),
            Int32((x * 65536.0).rounded()), Int32((y * 65536.0).rounded()),
