...
final class CodecValidationCoverageTests: XCTestCase {
    ...
    func testCodecInvalidConfigsFixtureEmitsAvcAndHevcWarnings() async throws {
        ...
        XCTAssertTrue(messages.contains(where: { $0.localizedCaseInsensitiveContains("avcC") && $0.localizedCaseInsensitiveContains("zero length") }))
        XCTAssertTrue(messages.contains(where: { $0.localizedCaseInsensitiveContains("hvcC") && $0.localizedCaseInsensitiveContains("zero length") }))
    }
}
