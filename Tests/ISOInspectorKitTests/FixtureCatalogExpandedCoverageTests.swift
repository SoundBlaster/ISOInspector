...
final class FixtureCatalogExpandedCoverageTests: XCTestCase {
    ...
    func testEditListEmptyFixtureReportsDurationWarnings() throws {
        ...
        XCTAssertTrue(
            warnings.contains(
                "Track 1 edit list spans 0 movie ticks but movie header duration is 1200 (short by 1200 > 1 tick)."
            ))
        XCTAssertTrue(
            warnings.contains(
                "Track 1 edit list spans 0 movie ticks but track header duration is 900 (short by 900 > 1 tick)."
            ))
        XCTAssertTrue(
            warnings.contains(
                "Track 1 edit list consumes 0 media ticks but media duration is 96000 (short by 96000 > 1 tick)."
            ))
    }
    ...
    func testEditListMultiSegmentFixtureReportsOverageWarnings() throws {
        ...
        XCTAssertTrue(
            warnings.contains(
                "Track 3 edit list spans 1200 movie ticks but movie header duration is 1000 (over by 200 > 1 tick)."
            ))
        XCTAssertTrue(
            warnings.contains(
                "Track 3 edit list spans 1200 movie ticks but track header duration is 950 (over by 250 > 1 tick)."
            ))
    }
    ...
    func testEditListRateAdjustedFixtureReportsPlaybackWarnings() throws {
        ...
        XCTAssertTrue(
            warnings.contains(
                "Track 4 edit list entry 0 uses media_rate_integer=-1; reverse playback is unsupported."
            ))
        XCTAssertTrue(
            warnings.contains(
                "Track 4 edit list entry 1 uses media_rate_integer=2; playback rate adjustments above 1x are unsupported."
            ))
        XCTAssertTrue(
            warnings.contains(
                "Track 4 edit list entry 2 sets media_rate_fraction=1; fractional playback rates are unsupported."
            ))
    }
    ...
    func testCodecInvalidConfigsFixtureDocumentsCodecWarnings() throws {
        ...
        XCTAssertTrue(
            errors.contains(
                "Track 1 sample description entry 0 (format avc1) avcC sequence parameter set #0 has zero length."
            ))
        XCTAssertTrue(
            errors.contains(
                "Track 1 sample description entry 1 (format hvc1) hvcC SPS NAL #0 has zero length.")
        )
    }
    ...
    func testSampleEncryptionFixtureDocumentsPlaceholderCoverage() throws {
        ...
        XCTAssertTrue(
            warnings.contains(
                "Sample encryption metadata surfaces for 2 samples with override defaults."))
        XCTAssertTrue(
            warnings.contains("64-bit sample auxiliary offsets recorded via range references."))
    }
