...
final class FilesystemAccessAuditTrailTests: XCTestCase {
    func testAppendsEventsAndPreservesOrder() {
        ...
        let events = (0..<3).map { index in
            FilesystemAccessAuditEvent(
                ...
                category: .bookmarkCreate, outcome: .success, pathHash: "hash-\(index)",
                bookmarkIdentifier: UUID(
                    uuidString: "00000000-0000-0000-0000-00000000000\(index)")!,
    ...
    func testRotationDropsOldestEvents() {
        ...
        let first = FilesystemAccessAuditEvent(
            timestamp: Date(timeIntervalSinceReferenceDate: 1), category: .openFile,
            outcome: .accessGranted, pathHash: "first", bookmarkIdentifier: nil,
            errorDescription: nil)
        let second = FilesystemAccessAuditEvent(
            timestamp: Date(timeIntervalSinceReferenceDate: 2), category: .saveFile,
            outcome: .accessGranted, pathHash: "second", bookmarkIdentifier: nil,
            errorDescription: nil)
        let third = FilesystemAccessAuditEvent(
            timestamp: Date(timeIntervalSinceReferenceDate: 3), category: .bookmarkResolve,
            outcome: .stale, pathHash: "third", bookmarkIdentifier: nil, errorDescription: nil)
    ...
    func testLimitZeroSkipsStorage() {
        ...
        trail.append(
            FilesystemAccessAuditEvent(
                timestamp: Date(), category: .bookmarkCreate, outcome: .success,
                pathHash: "ignored", bookmarkIdentifier: nil, errorDescription: nil))
