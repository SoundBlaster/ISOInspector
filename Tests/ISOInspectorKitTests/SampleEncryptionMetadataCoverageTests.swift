...
final class SampleEncryptionMetadataCoverageTests: XCTestCase {
    ...
    func testSampleEncryptionFixtureEmitsStructuredMetadata() async throws {
        ...
        for try await event in pipeline.events(for: reader, context: .init()) {
            ...
            switch header.type {
            case senc:
                if let detail = payload.sampleEncryption { sampleEncryptionBoxes.append(detail) }
            case saio:
                if let detail = payload.sampleAuxInfoOffsets { auxOffsetBoxes.append(detail) }
            case saiz: if let detail = payload.sampleAuxInfoSizes { auxSizeBoxes.append(detail) }
            default: break
            }
        }
        ...
        XCTAssertEqual(
            encryption.sampleInfoByteLength, sampleInfoRange.upperBound - sampleInfoRange.lowerBound
        )
