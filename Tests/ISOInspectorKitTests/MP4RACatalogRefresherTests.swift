...
final class MP4RACatalogRefresherTests: XCTestCase {
    private struct StubDataProvider: MP4RARegistryDataProvider {
        ...
        func fetchRegistryPayload() throws -> Data { payload }
    }
    ...
    func testBuildCatalogTransformsRecords() throws {
        let payload = Data(
            ...
            """.utf8)
        let provider = StubDataProvider(
            payload: payload, sourceURL: URL(string: "https://example.com/api")!)
        ...
        XCTAssertEqual(
            catalog.boxes[2].summary, "audio handler entry (handler: Audio, objectType: 0xAF)")
    ...
    func testMakeCatalogMergesBaselineEntries() throws {
        ...
        let provider = StubDataProvider(
            payload: payload, sourceURL: URL(string: "https://example.com/api")!)
        let refresher = MP4RACatalogRefresher(
            dataProvider: provider, clock: { Date(timeIntervalSince1970: 1_700_000_000) })

        let baseline = MP4RACatalogRefresher.Catalog(
            metadata: .init(source: "baseline", fetchedAt: "", recordCount: 1),
            boxes: [
                .init(
                    type: "ftyp", uuid: nil, name: "File Type Box",
                    summary: "Identifies the file type and compatibility brands for the bitstream.",
                    category: nil, specification: "ISO/IEC 14496-12", version: 0, flags: "000001"),
                .init(
                    type: "uuid", uuid: "11111111-1111-1111-1111-111111111111",
                    name: "KLV Sample Entry",
                    summary: "Carries SMPTE KLV sample metadata using a registered UUID container.",
                    category: nil, specification: "SMPTE ST 336", version: nil, flags: nil),
            ])
        ...
        XCTAssertEqual(
            ftypEntry.summary,
            "Identifies the file type and compatibility brands for the bitstream.")
        ...
        XCTAssertEqual(
            uuidEntry.summary,
            "Carries SMPTE KLV sample metadata using a registered UUID container.")
