...
final class PlaintextIssueSummaryExporterTests: XCTestCase {
    func testExporterIncludesMetadataAndGroupedIssues() throws {
        ...
        let issues = [
            ParseIssue(
                severity: .error, code: "VR-001", message: "Truncated box", byteRange: 48..<60,
                affectedNodeIDs: [grandChild.header.startOffset]),
            ParseIssue(
                severity: .warning, code: "VR-010", message: "Unexpected padding",
                byteRange: 32..<40, affectedNodeIDs: [child.header.startOffset]),
        ]

        let tree = ParseTree(nodes: [
            BoxNode(
                header: root.header, metadata: nil, payload: nil, validationIssues: [],
                issues: [issues[0], issues[1]], status: .corrupt,
                children: [
                    BoxNode(
                        header: child.header, metadata: nil, payload: nil, validationIssues: [],
                        issues: [issues[1]], status: .corrupt,
                        children: [
                            BoxNode(
                                header: grandChild.header, metadata: nil, payload: nil,
                                validationIssues: [], issues: [issues[0]], status: .corrupt,
                                children: [])
                        ])
                ])
        ])
        ...
        let metadata = PlaintextIssueSummaryExporter.Metadata(
            filePath: "/tmp/sample.mp4", fileSize: 1024, analyzedAt: Date(timeIntervalSince1970: 0),
            sha256: nil)
    ...
    func testExporterHandlesNoIssues() throws {
        ...
        let metadata = PlaintextIssueSummaryExporter.Metadata(
            filePath: "/tmp/clean.mp4", fileSize: 2048,
            analyzedAt: Date(timeIntervalSince1970: 1_000), sha256: "abc123")
        let summary = ParseIssueStore.IssueSummary(metrics: .init(), totalCount: 0)
        let exporter = PlaintextIssueSummaryExporter()
        let data = try exporter.export(tree: tree, metadata: metadata, summary: summary, issues: [])
