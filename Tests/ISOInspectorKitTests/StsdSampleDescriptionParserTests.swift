...
final class StsdSampleDescriptionParserTests: XCTestCase {
    func testParsesVisualAndAudioSampleEntries() throws {
        ...
        let audioEntry = makeAudioSampleEntry(
            channelCount: 2, sampleSize: 16, sampleRate: 48_000, dataReferenceIndex: 2)
    ...
    func testParsesAvcCodecMetadata() throws {
        let avcC = makeAvcConfigurationBox(
            profileIndication: 0x4D, profileCompatibility: 0x40, levelIndication: 0x1F,
            nalLengthSize: 4, sequenceParameterSets: [Data([0x67, 0x64, 0x00, 0x1F])],
            pictureParameterSets: [Data([0x68, 0xEB, 0xEF])])
        let entry = makeVisualSampleEntry(
            width: 1280, height: 720, dataReferenceIndex: 1, additionalBoxes: [avcC])
        ...
        XCTAssertEqual(
            fieldsByName["entries[0].codec.avc.sequence_parameter_sets.count"]?.value, "1")
        XCTAssertEqual(
            fieldsByName["entries[0].codec.avc.sequence_parameter_sets[0].length"]?.value, "4")
        XCTAssertEqual(
            fieldsByName["entries[0].codec.avc.picture_parameter_sets.count"]?.value, "1")
        XCTAssertEqual(
            fieldsByName["entries[0].codec.avc.picture_parameter_sets[0].length"]?.value, "3")
    }

    func testParsesHevcCodecMetadata() throws {
        let hvcC = makeHevcConfigurationBox(
            profileSpace: 0, tierFlag: 1, profileIDC: 1, levelIDC: 120, nalLengthSize: 4,
            vpsNalUnits: [Data([0x40, 0x01])], spsNalUnits: [Data([0x42, 0x01, 0x01])],
            ppsNalUnits: [Data([0x44, 0x01])])
        let entry = makeVisualSampleEntry(
            format: "hvc1", width: 1920, height: 1080, dataReferenceIndex: 1,
        ...
        XCTAssertEqual(
            fieldsByName["entries[0].codec.hevc.constraint_indicator_flags"]?.value,
            "0x000000000000")
    ...
    func testParsesEsdsAudioSpecificConfig() throws {
        let esds = makeEsdsBox(
            objectTypeIndication: 0x40, audioObjectType: 2, samplingFrequencyIndex: 3,
            channelConfiguration: 2)
        let entry = makeAudioSampleEntry(
            channelCount: 2, sampleSize: 16, sampleRate: 48_000, dataReferenceIndex: 1,
    ...
    func testParsesEncryptedSampleEntryAndPreservesCodecMetadata() throws {
        ...
        let sinf = makeSinfBox(
            originalFormat: "avc1", schemeType: "cenc", schemeVersion: 0x0001_0000,
            defaultIsProtected: 1, defaultPerSampleIVSize: 8,
            defaultKID: Array(repeating: 0xAB, count: 16))
        let entry = makeVisualSampleEntry(
            format: "encv", width: 1920, height: 1080, dataReferenceIndex: 1,
        ...
        XCTAssertEqual(
            fieldsByName["entries[0].encryption.default_kid"]?.value,
    ...
    func testParsesDolbyVisionConfiguration() throws {
        let dolbyVision = makeDolbyVisionConfigurationBox(
            versionMajor: 1, versionMinor: 2, profile: 4, level: 8, rpuPresent: true,
            elPresent: false, blPresent: true, compatibilityID: 5)
        let entry = makeVisualSampleEntry(
            format: "dvh1", width: 3840, height: 2160, dataReferenceIndex: 1,
    ...
    func testParsesAv1CodecConfiguration() throws {
        let av1C = makeAv1ConfigurationBox(
            version: 0, sequenceProfile: 2, sequenceLevelIndex: 10, tierFlagHigh: true,
            highBitDepth: true, twelveBit: false, monochrome: false, chromaSubsamplingX: true,
            chromaSubsamplingY: true, chromaSamplePosition: 2,
            initialPresentationDelayPresent: true, initialPresentationDelayMinusOne: 7,
            configObuBytes: Data([0x01, 0x02, 0x03]))
        let entry = makeVisualSampleEntry(
            format: "av01", width: 1920, height: 1080, dataReferenceIndex: 1,
        ...
        XCTAssertEqual(
            fieldsByName["entries[0].codec.av1.chroma_sample_position"]?.value, "colocated")
        XCTAssertEqual(
            fieldsByName["entries[0].codec.av1.initial_presentation_delay_present"]?.value, "true")
        XCTAssertEqual(
            fieldsByName["entries[0].codec.av1.initial_presentation_delay_minus_one"]?.value, "7")
    ...
    func testParsesVp9CodecConfiguration() throws {
        let vp9 = makeVp9ConfigurationBox(
            profile: 2, level: 31, bitDepthCode: 1, chromaSubsamplingCode: 1, videoFullRange: true,
            colourPrimaries: 9, transferCharacteristics: 10, matrixCoefficients: 11)
        let entry = makeVisualSampleEntry(
            format: "vp09", width: 1280, height: 720, dataReferenceIndex: 1, additionalBoxes: [vp9])
    ...
    func testParsesDolbyAC4Descriptor() throws {
        let dac4 = makeDolbyAC4ConfigurationBox(
            dsiVersion: 1, bitstreamVersion: 1, sampleRateIndex: 1, frameRateIndex: 4,
            presentationCount: 0, bitRateMode: 2, bitRate: 1_000_000, bitRatePrecision: 0)
        let entry = makeAudioSampleEntry(
            format: "ac-4", channelCount: 2, sampleSize: 16, sampleRate: 48_000,
            dataReferenceIndex: 1, additionalBoxes: [dac4])
    ...
    func testParsesMpegHConfiguration() throws {
        let mhaC = makeMpegHConfigurationBox(
            configurationVersion: 1, profileLevelIndication: 0x2A, referenceChannelLayout: 0x1234,
            compatibleSetIndication: 0x56, generalProfileCompatibilitySet: 0x789A_BCDE)
        let entry = makeAudioSampleEntry(
            format: "mha1", channelCount: 6, sampleSize: 16, sampleRate: 48_000,
            dataReferenceIndex: 1, additionalBoxes: [mhaC])

        let fieldsByName = try parseFields(entries: [entry])

        XCTAssertEqual(fieldsByName["entries[0].codec.mpeg_h.configuration_version"]?.value, "1")
        XCTAssertEqual(
            fieldsByName["entries[0].codec.mpeg_h.profile_level_indication"]?.value, "42")
        XCTAssertEqual(
            fieldsByName["entries[0].codec.mpeg_h.reference_channel_layout"]?.value, "4660")
        XCTAssertEqual(
            fieldsByName["entries[0].codec.mpeg_h.compatible_set_indication"]?.value, "86")
        XCTAssertEqual(
            fieldsByName["entries[0].codec.mpeg_h.general_profile_compatibility_set"]?.value,
            "2023406814")
    }
    ...
    private func parseFields(entries: [Data]) throws -> [String: ParsedBoxPayload.Field] {
        ...
        let header = BoxHeader(
            type: try FourCharCode("stsd"), totalSize: Int64(totalSize), headerSize: 8,
    ...
    private func makeVisualSampleEntry(
        format: String = "avc1", width: UInt16, height: UInt16, dataReferenceIndex: UInt16,
        ...
        entry.append(contentsOf: UInt32(0x0048_0000).bigEndianBytes)  // horizresolution
        entry.append(contentsOf: UInt32(0x0048_0000).bigEndianBytes)  // vertresolution
        ...
        for box in additionalBoxes { entry.append(box) }
    ...
    private func makeAudioSampleEntry(
        format: String = "mp4a", channelCount: UInt16, sampleSize: UInt16, sampleRate: UInt32,
        dataReferenceIndex: UInt16, additionalBoxes: [Data] = []
    ) -> Data {
        var entry = Data()
        entry.append(contentsOf: [0x00, 0x00, 0x00, 0x00])  // size placeholder
        entry.append(contentsOf: Data(format.utf8))
    ...
    private func makeDolbyVisionConfigurationBox(
        versionMajor: UInt8, versionMinor: UInt8, profile: UInt8, level: UInt8, rpuPresent: Bool,
        elPresent: Bool, blPresent: Bool, compatibilityID: UInt8
    ) -> Data {
        let flagsByte: UInt8 =
            (rpuPresent ? 0x80 : 0x00) | (elPresent ? 0x40 : 0x00) | (blPresent ? 0x20 : 0x00)
            | (compatibilityID & 0x0F)
        let payload: [UInt8] = [versionMajor, versionMinor, profile, level, flagsByte]
        return makeBox(type: "dvvC", payload: Data(payload))
    }

    private func makeAv1ConfigurationBox(
        version: UInt8, sequenceProfile: UInt8, sequenceLevelIndex: UInt8, tierFlagHigh: Bool,
        highBitDepth: Bool, twelveBit: Bool, monochrome: Bool, chromaSubsamplingX: Bool,
        chromaSubsamplingY: Bool, chromaSamplePosition: UInt8,
        initialPresentationDelayPresent: Bool, initialPresentationDelayMinusOne: UInt8,
        configObuBytes: Data
    ) -> Data {
        ...
        let byte2: UInt8 =
            (tierFlagHigh ? 0x80 : 0x00) | (highBitDepth ? 0x40 : 0x00) | (twelveBit ? 0x20 : 0x00)
            | (monochrome ? 0x10 : 0x00) | (chromaSubsamplingX ? 0x08 : 0x00)
            | (chromaSubsamplingY ? 0x04 : 0x00) | ((chromaSamplePosition & 0x03) << 0)
        let byte3: UInt8 =
    ...
    private func makeVp9ConfigurationBox(
        profile: UInt8, level: UInt8, bitDepthCode: UInt8, chromaSubsamplingCode: UInt8,
        videoFullRange: Bool, colourPrimaries: UInt8, transferCharacteristics: UInt8,
        matrixCoefficients: UInt8
    ) -> Data {
        let version: UInt8 = 1
        let flags: [UInt8] = [0x00, 0x00, 0x00]
        let colourRange: UInt8 =
            ((bitDepthCode & 0x0F) << 4) | ((chromaSubsamplingCode & 0x07) << 1)
    ...
    private func makeDolbyAC4ConfigurationBox(
        dsiVersion: UInt8, bitstreamVersion: UInt8, sampleRateIndex: UInt8, frameRateIndex: UInt8,
        presentationCount: UInt16, bitRateMode: UInt8, bitRate: UInt32, bitRatePrecision: UInt32
    ) -> Data {
    ...
    private func makeMpegHConfigurationBox(
        configurationVersion: UInt8, profileLevelIndication: UInt8, referenceChannelLayout: UInt16,
        compatibleSetIndication: UInt8, generalProfileCompatibilitySet: UInt32
    ) -> Data {
        var payload = Data()
    ...
    private func makeAvcConfigurationBox(
        configurationVersion: UInt8 = 1, profileIndication: UInt8, profileCompatibility: UInt8,
        levelIndication: UInt8, nalLengthSize: UInt8, sequenceParameterSets: [Data],
    ...
    private func makeHevcConfigurationBox(
        profileSpace: UInt8, tierFlag: UInt8, profileIDC: UInt8, levelIDC: UInt8,
        nalLengthSize: UInt8, vpsNalUnits: [Data], spsNalUnits: [Data], ppsNalUnits: [Data]
    ) -> Data {
        var payload = Data()
        payload.append(1)  // configurationVersion
        let profileByte =
            (profileSpace & 0x03) << 6 | ((tierFlag & 0x01) << 5) | (profileIDC & 0x1F)
        ...
        let arrays: [(UInt8, [Data])] = [(32, vpsNalUnits), (33, spsNalUnits), (34, ppsNalUnits)]
    ...
    private func makeEsdsBox(
        objectTypeIndication: UInt8, audioObjectType: Int, samplingFrequencyIndex: Int,
        ...
        let audioSpecific = makeAudioSpecificConfig(
            audioObjectType: audioObjectType, samplingFrequencyIndex: samplingFrequencyIndex,
    ...
    private func makeSinfBox(
        originalFormat: String, schemeType: String, schemeVersion: UInt32,
        defaultIsProtected: UInt8, defaultPerSampleIVSize: UInt8, defaultKID: [UInt8]
    ) -> Data {
    ...
    private func makeAudioSpecificConfig(
        audioObjectType: Int, samplingFrequencyIndex: Int, channelConfiguration: Int
    ) -> Data {
        var bits: [UInt8] = []
        ...
        if objectType >= 32 { objectType = 32 }
    ...
    private func encodeDescriptorLength(_ length: Int) -> [UInt8] {
        ...
        for index in stack.indices.reversed() {
            ...
            if index != stack.startIndex { byte |= 0x80 }
            bytes.append(byte)
        }
        return bytes
    }
