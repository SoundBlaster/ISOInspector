...
final class JSONExportSnapshotTests: XCTestCase {
    ...
    private let regenerateSnapshots =
        ProcessInfo.processInfo.environment["ISOINSPECTOR_REGENERATE_SNAPSHOTS"] != nil

    override func setUpWithError() throws { catalog = try FixtureCatalog.load() }
    ...
    func testEditListSingleOffsetSnapshotMatchesFixture() async throws {
        try XCTSkipIf(
            true,
            "Muted due to known failure: JSONExportSnapshotTests.testEditListSingleOffsetSnapshotMatchesFixture"
        )
    ...
    func testEditListMultiSegmentSnapshotMatchesFixture() async throws {
        try XCTSkipIf(
            true,
            "Muted due to known failure: JSONExportSnapshotTests.testEditListMultiSegmentSnapshotMatchesFixture"
        )
    ...
    func testFragmentedNoTfdtIssueMetricsMatchCounts() async throws {
        ...
        let metrics = try XCTUnwrap(
            object["issue_metrics"] as? [String: Any], "Issue metrics summary missing from export")
    ...
    func testIssueMetricsDeduplicatesIssuesRecordedAcrossNodes() throws {
        let issue = ParseIssue(
            severity: .error, code: "guard.recursion_depth_exceeded",
            message: "Depth budget exceeded", byteRange: 0..<4, affectedNodeIDs: [0, 8])
    ...
    func testBaselineSampleNodesExposeCompatibilityAliases() async throws {
        ...
        let nodes = try XCTUnwrap(
            object["nodes"] as? [[String: Any]], "nodes array missing from export")
        ...
        for node in flattened {
            ...
            XCTAssertEqual(
                name, fourcc,
                "Compatibility alias name should mirror fourcc for node \(String(describing: fourcc))"
            )
            ...
            XCTAssertEqual(
                headerAlias, header,
                "header_size alias should equal sizes.header for node \(String(describing: fourcc))"
            )
            XCTAssertEqual(
                sizeAlias, total,
                "size alias should equal sizes.total for node \(String(describing: fourcc))")
        }
    }
    ...
    func testBaselineSampleFormatSummaryIncludesBrandsAndDuration() async throws {
        ...
        let fileType = try XCTUnwrap(
            findFileType(in: tree), "File type detail missing from parse tree")
        XCTAssertEqual(format["major_brand"] as? String, fileType.majorBrand.rawValue)
        XCTAssertEqual(
            Self.intValue(forKey: "minor_version", in: format), Int(fileType.minorVersion))
        ...
        let movieHeader = try XCTUnwrap(
            findMovieHeader(in: tree), "Movie header detail missing from parse tree")
    ...
    private func makeParseTree(forFixtureID id: String) async throws -> ParseTree {
        let fixture = try XCTUnwrap(
            catalog.fixture(withID: id), "Fixture \(id) not found in catalog")
    ...
    private func assertSnapshot(named name: String, matches json: String) throws {
        ...
        if regenerateSnapshots || bundleURL == nil {
            ...
            print(json)
            XCTFail(
                "Snapshot baseline for \(name) was regenerated. Re-run tests without ISOINSPECTOR_REGENERATE_SNAPSHOTS to validate the output."
            )
            return
        }

        let baselineURL = try XCTUnwrap(
            bundleURL, "Snapshot baseline for \(name) missing from bundle")
        let baseline = try String(contentsOf: baselineURL, encoding: .utf8)
        XCTAssertEqual(
            json, baseline,
            "Snapshot mismatch for \(name). Set ISOINSPECTOR_REGENERATE_SNAPSHOTS=1 swift test --filter JSONExportSnapshotTests to refresh baselines after intentional schema updates."
        )
    }

    private func snapshotBundleURL(named name: String) -> URL? {
        if let url = Bundle.module.url(
            forResource: name, withExtension: "json", subdirectory: snapshotsSubdirectory)
        {
            return url
        }
    ...
    private func snapshotSourceURL(named name: String) -> URL {
        ...
        ).appendingPathComponent(snapshotsSubdirectory, isDirectory: true).appendingPathComponent(
    ...
    private func findFileType(in tree: ParseTree) -> ParsedBoxPayload.FileTypeBox? {
        for node in flatten(nodes: tree.nodes) {
            if let fileType = node.payload?.fileType { return fileType }
        }
        return nil
    }
    ...
    private func findMovieHeader(in tree: ParseTree) -> ParsedBoxPayload.MovieHeaderBox? {
        for node in flatten(nodes: tree.nodes) {
            if let movieHeader = node.payload?.movieHeader { return movieHeader }
        }
        return nil
    }
    ...
    private func accumulateIssues(
        in nodes: [ParseTreeNode], depth: Int, metrics: inout IssueMetrics
    ) {
        for node in nodes {
    ...
    private func makeIssuesParseTree() throws -> ParseTree {
        ...
        let rootIssues = [
            ...
            ParseIssue(
                severity: .warning, code: "structure.missing_child",
                message: "trak is missing required child boxes.", byteRange: nil,
                affectedNodeIDs: [rootHeader.startOffset, childHeader.startOffset]),
        ]

        let childIssues = [
            ParseIssue(
                severity: .info, code: "payload.placeholder_field",
                message: "Field was synthesized as placeholder.",
                byteRange: childHeader.payloadRange
                    .lowerBound..<(childHeader.payloadRange.lowerBound + 8),
                affectedNodeIDs: [childHeader.startOffset])
        ]

        let childNode = ParseTreeNode(
            header: childHeader, metadata: nil, payload: nil, validationIssues: [],
            issues: childIssues, status: .partial, children: [])

        let siblingNode = ParseTreeNode(
            header: siblingHeader, metadata: nil, payload: nil, validationIssues: [], issues: [],
            status: .valid, children: [])

        let rootNode = ParseTreeNode(
            header: rootHeader, metadata: nil, payload: nil, validationIssues: [],
            issues: rootIssues, status: .partial, children: [childNode, siblingNode])
    ...
    private func makeHeader(type: String, start: Int64, size: Int64) throws -> BoxHeader {
        ...
        let end = start + size
        return BoxHeader(
            type: fourcc, totalSize: size, headerSize: headerSize,
            payloadRange: (start + headerSize)..<end, range: start..<end, uuid: nil)
    }
}
