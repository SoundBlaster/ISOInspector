...
final class ParseIssueTests: XCTestCase {
    func testInitializerStoresAllProperties() {
        ...
        let issue = ParseIssue(
            severity: .warning, code: "MISSING_REQUIRED", message: "Sample description missing",
            byteRange: range, affectedNodeIDs: [1, 42])
    ...
    func testCodableRoundTripPreservesData() throws {
        let issue = ParseIssue(
            severity: .error, code: "TRUNCATED_PAYLOAD",
            message: "Payload ended before expected size", byteRange: 4096..<8192,
            affectedNodeIDs: [100])
