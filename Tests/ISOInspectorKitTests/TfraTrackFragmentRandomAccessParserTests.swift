...
final class TfraTrackFragmentRandomAccessParserTests: XCTestCase {
    func testParsesEntriesAndResolvesFragmentContext() throws {
        ...
        let environment = makeEnvironment(
            sequenceNumber: sequenceNumber, trackID: trackID, baseDecodeTime: baseDecodeTime,
            baseDataOffset: baseDataOffset, runDataOffset: runDataOffset,
        ...
        XCTAssertEqual(
            ...
            baseDataOffset + UInt64(Int64(runDataOffset)) + UInt64(sampleSizes[0]))
    ...
    private func makeEnvironment(
        sequenceNumber: UInt32, trackID: UInt32, baseDecodeTime: UInt64, baseDataOffset: UInt64,
        runDataOffset: Int32, sampleDurations: [UInt32], sampleSizes: [UInt32]
    ) -> BoxParserRegistry.RandomAccessEnvironment {
        let entries: [ParsedBoxPayload.TrackRunBox.Entry] = sampleDurations.enumerated().map {
            index, duration in
            ParsedBoxPayload.TrackRunBox.Entry(
                ...
                sampleDuration: duration, sampleSize: sampleSizes[index], sampleFlags: nil,
                sampleCompositionTimeOffset: nil,
                dataOffset: baseDataOffset + UInt64(Int64(runDataOffset))
                    + UInt64(sampleSizes.prefix(index).reduce(0, +)), byteRange: nil)
        }

        let run = ParsedBoxPayload.TrackRunBox(
            version: 0, flags: 0, sampleCount: UInt32(entries.count), dataOffset: runDataOffset,
            firstSampleFlags: nil, entries: entries,
            totalSampleDuration: UInt64(sampleDurations.reduce(0, +)),
            totalSampleSize: UInt64(sampleSizes.reduce(0, +)), startDecodeTime: baseDecodeTime,
            ...
            endDataOffset: baseDataOffset + UInt64(Int64(runDataOffset))
                + UInt64(sampleSizes.reduce(0, +)), trackID: trackID, sampleDescriptionIndex: 1,
            runIndex: 0, firstSampleGlobalIndex: 1)

        let fragment = ParsedBoxPayload.TrackFragmentBox(
            trackID: trackID, sampleDescriptionIndex: 1, baseDataOffset: baseDataOffset,
            defaultSampleDuration: sampleDurations.first, defaultSampleSize: sampleSizes.first,
            defaultSampleFlags: nil, durationIsEmpty: false, defaultBaseIsMoof: false,
            baseDecodeTime: baseDecodeTime, baseDecodeTimeIs64Bit: true, runs: [run],
        ...
        let trackFragment = BoxParserRegistry.RandomAccessEnvironment.TrackFragment(
            order: 1, detail: fragment)
        let fragmentSummary = BoxParserRegistry.RandomAccessEnvironment.Fragment(
            sequenceNumber: sequenceNumber, trackFragments: [trackFragment])
    ...
    private func makeHeader(for boxData: Data, type: String) throws -> BoxHeader {
        return BoxHeader(
            type: try FourCharCode(type), totalSize: Int64(boxData.count), headerSize: 8,
            payloadRange: 8..<Int64(boxData.count), range: 0..<Int64(boxData.count), uuid: nil)
    }
