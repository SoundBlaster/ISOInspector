...
final class TolerantParsingFuzzTests: XCTestCase {
    ...
    func testHeaderTruncationMutations() async throws {
        ...
        XCTAssertGreaterThanOrEqual(successRate, 0.95, "Header truncation mutations should complete ≥95%")
    }
    ...
    func testOverlappingRangeMutations() async throws {
        ...
        let successRate = Double(successCount) / 20.0
        XCTAssertGreaterThanOrEqual(successRate, 0.95, "Overlapping range mutations should complete ≥95%")
    }
    ...
    func testFuzzHarnessCompletesWithinBenchmarkBudget() async throws {
        ...
        XCTAssertLessThan(estimatedTotal, 300.0, "Estimated total runtime \(estimatedTotal)s exceeds 5min budget")
    }
...
struct FuzzStatistics {
    ...
    mutating func recordSuccess(seed: UInt64, mutation: MutationDescription, eventCount: Int, issueCount: Int) {
        successCount += 1
    }
