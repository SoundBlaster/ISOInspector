...
final class TolerantParsingFuzzTests: XCTestCase {
        ...
        for iteration in 0..<fuzzIterations {
            ...
            do {
                ...
                statistics.recordSuccess(
                    seed: mutationSeed, mutation: corruptPayload.mutation, eventCount: events.count,
                    issueCount: store.issuesSnapshot().count)

            } catch {
                // Record failure with reproduction artifact
                statistics.recordFailure(
                    seed: mutationSeed, mutation: corruptPayload.mutation, error: error)

                // Capture reproduction artifact for analysis
                let artifact = ReproductionArtifact(
                    seed: mutationSeed, mutation: corruptPayload.mutation,
                    payload: corruptPayload.data, error: error)
    ...
    func testHeaderTruncationMutations() async throws {
        ...
        for i in 0..<20 {
            ...
            if result.completed { successCount += 1 }
        }
    ...
    private func parsePayload(_ data: Data, pipeline: ParsePipeline) async throws -> ParseResult {
        ...
        return ParseResult(
            completed: true, eventCount: events.count, issueCount: store.issuesSnapshot().count)
    }

    private func saveReproductionArtifact(_ artifact: ReproductionArtifact) throws {
        let root = URL(fileURLWithPath: #filePath).deletingLastPathComponent()
            .deletingLastPathComponent().deletingLastPathComponent()
        let artifactsDir = root.appendingPathComponent(reproArtifactsPath)
...
struct CorruptPayloadGenerator {
    ...
    init(seed: UInt64) { self.baseSeed = seed }
    ...
    func generateCorruptPayload(mutationSeed: UInt64) -> CorruptPayload {
        ...
        switch mutationType {
        case .headerTruncation: return generateHeaderTruncation(mutationSeed: mutationSeed)
        case .overlappingRanges: return generateOverlappingRanges(mutationSeed: mutationSeed)
        case .bogusSize: return generateBogusSize(mutationSeed: mutationSeed)
        case .invalidFourCC: return generateInvalidFourCC(mutationSeed: mutationSeed)
        case .zeroSizeLoop: return generateZeroSizeLoop(mutationSeed: mutationSeed)
        case .payloadTruncation: return generatePayloadTruncation(mutationSeed: mutationSeed)
        }
    }
    ...
    func generateHeaderTruncation(mutationSeed: UInt64) -> CorruptPayload {
        ...
        for _ in 0..<truncateAt { data.append(UInt8.random(in: 0...255, using: &rng)) }

        return CorruptPayload(
            data: data,
            mutation: MutationDescription(
                type: .headerTruncation, description: "Header truncated at byte \(truncateAt)"))
    }
    ...
    func generateOverlappingRanges(mutationSeed: UInt64) -> CorruptPayload {
        ...
        while data.count < Int(parentSize) { data.append(0) }
    ...
    func generateInvalidFourCC(mutationSeed: UInt64) -> CorruptPayload {
        ...
        for _ in 0..<4 { data.append(UInt8.random(in: 0x00...0x1F, using: &rng)) }

        // Pad payload
        while data.count < Int(size) { data.append(0) }

        return CorruptPayload(
            data: data,
            mutation: MutationDescription(
                type: .invalidFourCC, description: "FourCC contains non-ASCII bytes"))
    }
    ...
    func generateZeroSizeLoop(mutationSeed: UInt64) -> CorruptPayload {
        ...
        return CorruptPayload(
            data: data,
            mutation: MutationDescription(
                type: .zeroSizeLoop, description: "Parent contains \(zeroCount) zero-size children")
        )
    }
    ...
    func generatePayloadTruncation(mutationSeed: UInt64) -> CorruptPayload {
        ...
        for _ in 0..<actualSize { data.append(UInt8.random(in: 0...255, using: &rng)) }
