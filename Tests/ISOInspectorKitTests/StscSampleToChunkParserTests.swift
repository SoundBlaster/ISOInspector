...
final class StscSampleToChunkParserTests: XCTestCase {
    func testParsesSampleToChunkEntries() throws {
        let entries: [SampleToChunkTestEntry] = [
            SampleToChunkTestEntry(firstChunk: 1, samplesPerChunk: 5, sampleDescriptionIndex: 2),
            SampleToChunkTestEntry(firstChunk: 10, samplesPerChunk: 8, sampleDescriptionIndex: 3),
        ]
    ...
    private func parse(entries: [SampleToChunkTestEntry]) throws -> ParsedBoxPayload {
        var payload = Data()
        payload.append(0x00)  // version
        payload.append(contentsOf: [0x00, 0x00, 0x00])  // flags
    ...
    private func parse(payload: Data) throws -> ParsedBoxPayload {
        ...
        let header = BoxHeader(
            type: try FourCharCode("stsc"), totalSize: Int64(totalSize), headerSize: 8,
...
private struct SampleToChunkTestEntry {
