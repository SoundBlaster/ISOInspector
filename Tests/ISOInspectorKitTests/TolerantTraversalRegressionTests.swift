...
final class TolerantTraversalRegressionTests: XCTestCase {
    ...
    private lazy var manifestURL: URL = {
        let root = URL(fileURLWithPath: #filePath).deletingLastPathComponent()  // TolerantTraversalRegressionTests.swift
            .deletingLastPathComponent()  // ISOInspectorKitTests
            .deletingLastPathComponent()  // Tests
        return root.appendingPathComponent("Documentation").appendingPathComponent("FixtureCatalog")
    ...
    private func fixtureURL(for filename: String) throws -> URL {
        ...
        guard FileManager.default.fileExists(atPath: base64URL.path) else {
            throw NSError(
                domain: "Test", code: 1,
                userInfo: [NSLocalizedDescriptionKey: "Missing base64 source for \(filename)"])
        }
        ...
        guard let data = Data(base64Encoded: encoded, options: .ignoreUnknownCharacters) else {
            throw NSError(
                domain: "Test", code: 2,
                userInfo: [NSLocalizedDescriptionKey: "Invalid base64 for \(filename)"])
        }
    ...
    func testTruncatedMoovContinuesTraversalAndRecordsIssue() async throws {
        ...
        guard let fixture = manifest.fixtures.first(where: { $0.id == "corrupt-truncated-moov" })
        else {
    ...
    func testZeroLengthLoopGuardPreventsInfiniteIteration() async throws {
        let manifest = try loadManifest()
        guard let fixture = manifest.fixtures.first(where: { $0.id == "corrupt-zero-length-loop" })
        else {
    ...
    func testDeepRecursionGuardLimitsTraversalDepth() async throws {
        let manifest = try loadManifest()
        guard let fixture = manifest.fixtures.first(where: { $0.id == "corrupt-deep-recursion" })
        else {
        ...
        XCTAssertLessThanOrEqual(
            metrics.deepestAffectedDepth, maxDepth, "Depth should respect configured limit")
    }
    ...
    func testInvalidFourCCRecordsHeaderIssue() async throws {
        let manifest = try loadManifest()
        guard let fixture = manifest.fixtures.first(where: { $0.id == "corrupt-invalid-fourcc" })
        else {
    ...
    func testTruncatedSizeFieldRecordsHeaderIssue() async throws {
        let manifest = try loadManifest()
        guard
            let fixture = manifest.fixtures.first(where: { $0.id == "corrupt-truncated-size-field" }
            )
        else {
    ...
    func testChildExceedingParentRangeRecordsPayloadIssue() async throws {
        let manifest = try loadManifest()
        guard
            let fixture = manifest.fixtures.first(where: {
                $0.id == "corrupt-parent-truncated-child"
            })
        else {
        ...
        let truncatedIssues = issues.filter { $0.code == "payload.truncated" }
        XCTAssertFalse(
            truncatedIssues.isEmpty, "Expected payload.truncated for child exceeding parent")
    }
    ...
    func testAllFixturesProcessInTolerantModeWithoutCrashing() async throws {
        ...
        for fixture in manifest.fixtures {
            ...
            print(
                "✓ Fixture \(fixture.id): \(events.count) events, \(issues.count) issues, codes: \(codes)"
            )

            // Verify at least that processing completed without crash
            XCTAssertTrue(
                hasExpectedOrRelated || fixture.filename.contains("empty"),
