...
final class Stz2CompactSampleSizeParserTests: XCTestCase {
    ...
    func testReportsTruncatedEntries() throws {
        ...
        XCTAssertEqual(fields["entries[2].status"]?.description, "Entry truncated before size field completed")
    ...
    private func parse(fieldSize: UInt8, entrySizes: [UInt8]) throws -> ParsedBoxPayload {
        ...
        if bitCount > 0 {
            data.append(packed << (8 - bitCount))
        }
        return try parse(fieldSize: fieldSize, rawEntryBytes: data, sampleCount: UInt32(entrySizes.count))
    }
    ...
    private func parse(fieldSize: UInt8, rawEntryBytes: Data, sampleCount: UInt32) throws -> ParsedBoxPayload {
        var payload = Data()
        payload.append(0x00)
