...
final class StreamingBoxWalkerTests: XCTestCase {
    func testWalkerEmitsEventsForNestedBoxes() throws {
        ...
        var finished = false
        try walker.walk(
            reader: reader, cancellationCheck: {}, onEvent: { event in events.append(event) },
        ...
        try assertEvent(
            events[2], kind: .willStart, type: moovType, depth: 0, offset: Int64(ftyp.count))
        try assertEvent(
            events[3], kind: .willStart, type: trakType, depth: 1, offset: Int64(ftyp.count + 8))
        try assertEvent(
        ...
        try assertEvent(
            events[6], kind: .didFinish, type: trakType, depth: 1,
            offset: Int64(ftyp.count + moov.count))
        try assertEvent(
            events[7], kind: .didFinish, type: moovType, depth: 0,
            offset: Int64(ftyp.count + moov.count))
    }
    ...
    func testWalkerPropagatesHeaderErrors() {
        ...
        XCTAssertThrowsError(
            try walker.walk(
                reader: reader, cancellationCheck: {}, options: .strict, onEvent: { _ in },
                onFinish: {})
        ) { error in XCTAssertTrue(error is BoxHeaderDecodingError) }
    }
    ...
    func testWalkerRecoversFromHeaderErrorsInTolerantMode() throws {
        ...
        try walker.walk(
            reader: reader, cancellationCheck: {}, options: .tolerant,
            onEvent: { event in events.append(event) },
            onIssue: { issue, _ in recordedIssues.append(issue) }, onFinish: { finished = true })
    ...
    func testWalkerClampsTruncatedChildPayloadInTolerantMode() throws {
        ...
        try assertEvent(
            events[1], kind: .didFinish, type: moovType, depth: 0, offset: Int64(moov.count))
    ...
    func testWalkerEmitsZeroLengthGuardAfterLimit() throws {
        ...
        try walker.walk(
            reader: reader, cancellationCheck: {}, options: options, onEvent: { events.append($0) },
            onIssue: { issue, _ in issues.append(issue) }, onFinish: { finished = true })
        ...
        try assertEvent(
            events[3], kind: .didFinish, type: containerType, depth: 0,
            offset: Int64(container.count))
    ...
    func testWalkerStopsWhenCancellationCheckThrows() {
        ...
        XCTAssertThrowsError(
            try walker.walk(
                reader: reader,
                cancellationCheck: { if shouldCancel { throw TestCancellationError() } },
                onEvent: { event in
                    events.append(event)
        ...
        ) { error in XCTAssertTrue(error is TestCancellationError) }
