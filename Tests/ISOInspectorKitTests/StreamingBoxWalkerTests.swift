...
final class StreamingBoxWalkerTests: XCTestCase {
  func testWalkerEmitsEventsForNestedBoxes() throws {
    ...
    try assertEvent(
      events[6], kind: .didFinish, type: trakType, depth: 1, offset: Int64(ftyp.count + moov.count))
    try assertEvent(
      events[7], kind: .didFinish, type: moovType, depth: 0, offset: Int64(ftyp.count + moov.count))
  }
  ...
  func testWalkerEmitsZeroLengthGuardAfterLimit() throws {
    ...
    try assertEvent(
      events[3], kind: .didFinish, type: containerType, depth: 0, offset: Int64(container.count))
