...
final class ParsePipelineLiveTests: XCTestCase {
    func testLivePipelineEmitsEventsForNestedBoxes() async throws {
        ...
        try assertEvent(
            events[1], kind: .didFinish, type: "ftyp", depth: 0, offset: Int64(ftyp.count))
        try assertEvent(
            events[2], kind: .willStart, type: ContainerTypes.movie, depth: 0,
            offset: Int64(ftyp.count))
        try assertEvent(
            events[3], kind: .willStart, type: ContainerTypes.track, depth: 1,
            offset: Int64(ftyp.count + 8))
        try assertEvent(
            events[4], kind: .willStart, type: "tkhd", depth: 2, offset: Int64(ftyp.count + 16))
        try assertEvent(
            events[5], kind: .didFinish, type: "tkhd", depth: 2,
            offset: Int64(ftyp.count + moov.count))
        try assertEvent(
            events[6], kind: .didFinish, type: ContainerTypes.track, depth: 1,
            offset: Int64(ftyp.count + moov.count))
        try assertEvent(
            events[7], kind: .didFinish, type: ContainerTypes.movie, depth: 0,
            offset: Int64(ftyp.count + moov.count))
    }
    ...
    func testRandomAccessTableCorrelatesWithFragments() async throws {
        ...
        let tfhd = makeTrackFragmentHeaderBox(
            trackID: trackID, baseDataOffset: baseDataOffset,
            defaultSampleDuration: sampleDurations[0], defaultSampleSize: sampleSizes[0],
            defaultSampleFlags: 0)
        let tfdt = makeTrackFragmentDecodeTimeBox(baseDecodeTime: baseDecodeTime)
        let trun = makeTrackRunBox(
            sampleCount: UInt32(sampleDurations.count), dataOffset: dataOffset,
            sampleDurations: sampleDurations, sampleSizes: sampleSizes)
        ...
        let tfra = makeTrackFragmentRandomAccessBox(
            version: 1, trackID: trackID,
            entries: [
                .init(
                    time: baseDecodeTime + UInt64(sampleDurations[0]), moofOffset: 0, trafNumber: 1,
                    trunNumber: 1, sampleNumber: 2)
            ])
        ...
        let tfraEvent = try XCTUnwrap(
            events.first { event in
                guard case .willStartBox(let header, _) = event.kind else { return false }
        ...
        XCTAssertEqual(
            entry.resolvedPresentationTime, Int64(baseDecodeTime + UInt64(sampleDurations[0])))
        XCTAssertEqual(
            entry.resolvedDataOffset,
            baseDataOffset + UInt64(Int64(dataOffset)) + UInt64(sampleSizes[0]))
        XCTAssertEqual(entry.resolvedSampleSize, sampleSizes[1])

        let mfraEvent = try XCTUnwrap(
            events.first { event in
                guard case .didFinishBox(let header, _) = event.kind else { return false }
    ...
    func testLivePipelineHandlesLargeSizeBoxes() async throws {
        ...
        try assertEvent(
            events[1], kind: .didFinish, type: mediaType, depth: 0, offset: Int64(largeBox.count))
    }
    ...
    func testLivePipelineEmitsPaddingDetailsForFreeAndSkip() async throws {
        ...
        for event in events {
            guard case .willStartBox(let header, _) = event.kind else { continue }
    ...
    func testLivePipelinePropagatesHeaderErrors() async {
        ...
        } catch let error as BoxHeaderDecodingError { XCTAssertNotNil(error) } catch {
    ...
    func testLivePipelineAttachesParsedPayloadFromRegistry() async throws {
        ...
        let events = try await collectEvents(from: pipeline.events(for: reader))
        let ftypEvent = try XCTUnwrap(
            events.first(where: { event in
                if case .willStartBox(let header, _) = event.kind {
    ...
    func testLivePipelineNormalizesEditListUsingHeaderTimescales() async throws {
        ...
        let events = try await collectEvents(from: pipeline.events(for: reader))
        let editListEvent = try XCTUnwrap(
            events.first { event in
                if case .willStartBox(let header, _) = event.kind {
    ...
    func testEditListValidationDefersMediaDurationUntilMediaHeader() async throws {
        ...
        let mediaHeaderEvent = try XCTUnwrap(
            events.first { event in
                if case .willStartBox(let header, _) = event.kind {
    ...
    func testLivePipelineStreamsParseIssueMetadata() async throws {
        ...
        let events = try await collectEvents(from: pipeline.events(for: reader))
        let finishEvent = try XCTUnwrap(
            events.first { event in
                if case .didFinishBox(let header, _) = event.kind {
    ...
    func testSampleTableValidationPassesForMatchingTables() async throws {
        ...
        let stsc = makeSampleToChunkBox(entries: [
            SampleToChunkEntryParameters(
                firstChunk: 1, samplesPerChunk: 2, sampleDescriptionIndex: 1),
            SampleToChunkEntryParameters(
                firstChunk: 3, samplesPerChunk: 4, sampleDescriptionIndex: 1),
        ])
        ...
        let stbl = makeContainer(
            type: FourCharContainerCode.stbl.rawValue, children: [stts, ctts, stsc, stsz, stco])
    ...
    func testSampleTableValidationReportsSampleCountMismatch() async throws {
        ...
        let stbl = makeContainer(
            type: FourCharContainerCode.stbl.rawValue, children: [stsc, stsz, stco])
        let minf = makeContainer(type: FourCharContainerCode.minf.rawValue, children: [stbl])
        let mdia = makeContainer(type: ContainerTypes.media, children: [minf])
        let trak = makeContainer(type: ContainerTypes.track, children: [tkhd, mdia])
        let moov = makeContainer(type: ContainerTypes.movie, children: [trak])
    ...
    func testSampleTableValidationReportsNonMonotonicChunkOffsets() async throws {
        ...
        let events = try await collectEvents(from: pipeline.events(for: reader))
        let chunkOffsetEvent = try XCTUnwrap(
            events.first { event in
                if case .willStartBox(let header, _) = event.kind {
    ...
    func testSampleTableValidationReportsTimeToSampleMismatch() async throws {
        ...
        let stbl = makeContainer(
            type: FourCharContainerCode.stbl.rawValue, children: [stts, stsc, stsz, stco])
        let minf = makeContainer(type: FourCharContainerCode.minf.rawValue, children: [stbl])
        let mdia = makeContainer(type: ContainerTypes.media, children: [minf])
        let trak = makeContainer(type: ContainerTypes.track, children: [tkhd, mdia])
        let moov = makeContainer(type: ContainerTypes.movie, children: [trak])
        ...
        let events = try await collectEvents(from: pipeline.events(for: reader))
        let sampleSizeEvent = try XCTUnwrap(
            events.first { event in
                if case .willStartBox(let header, _) = event.kind {
        ...
        XCTAssertFalse(issues.isEmpty)
        XCTAssertTrue(issues.contains { issue in issue.message.contains("time-to-sample table") })
    }
    ...
    func testSampleTableValidationReportsCompositionOffsetMismatch() async throws {
        ...
        XCTAssertTrue(
            stszIssues.contains { issue in issue.message.contains("composition offset table") })

        let cttsEvent = try XCTUnwrap(
            events.first { event in
                if case .willStartBox(let header, _) = event.kind {
        ...
        XCTAssertTrue(
            cttsIssues.contains { issue in issue.message.contains("composition offset table") })
    }
    ...
    func testLivePipelineReportsVersionAndFlagMismatchWarnings() async throws {
        let mismatchTkhd = makeBox(
            type: "tkhd", payload: Data([0x01, 0x00, 0x00, 0x00]) + Data(repeating: 0, count: 28))
    ...
    func testLivePipelineLeavesMatchingVersionAndFlagsUnchanged() async throws {
        let matchingTkhd = makeBox(
            type: "tkhd", payload: Data([0x00, 0x00, 0x00, 0x07]) + Data(repeating: 0, count: 28))
    ...
    func testLivePipelineEmitsResearchIssueForUnknownBoxes() async throws {
        ...
        let events = try await collectEvents(from: pipeline.events(for: reader))
        let unknownEvent = try XCTUnwrap(
            events.first(where: { event in
                if case .willStartBox(let header, _) = event.kind {
    ...
    func testLivePipelineAppendsUnknownBoxesToResearchLog() async throws {
        ...
        _ = try await collectEvents(
            from: pipeline.events(
                for: reader, context: .init(source: URL(fileURLWithPath: "/tmp/sample.mp4"))))
    ...
    func testLivePipelineReportsErrorWhenMediaAppearsBeforeFileType() async throws {
        ...
        let offendingEvent = try XCTUnwrap(
            events.first(where: { event in
                if case .willStartBox(let header, _) = event.kind {
    ...
    func testLivePipelineDoesNotWarnForCommonTopLevelOrdering() async throws {
        ...
        let ftypEvent = try XCTUnwrap(
            events.first(where: { event in
                if case .willStartBox(let header, let depth) = event.kind {
                    return depth == 0 && header.type.rawValue == "ftyp"
                }
                return false
            }))
        let moovEvent = try XCTUnwrap(
            events.first(where: { event in
                if case .willStartBox(let header, let depth) = event.kind {
    ...
    func testLivePipelineAdvisesWhenMovieFollowsStreamingMediaPayload() async throws {
        ...
        XCTAssertTrue(
            advisoryIssues.first?.message.contains(MediaAndIndexBoxCode.mediaData.rawValue) ?? false
        )
    }
    ...
    func testLivePipelineAggregatesTrackFragmentSummary() async throws {
        let tfhd = makeTrackFragmentHeaderBox(
            trackID: 1, baseDataOffset: 4096, defaultSampleDuration: 100, defaultSampleSize: 512,
            defaultSampleFlags: 0)
        let tfdt = makeTrackFragmentDecodeTimeBox(baseDecodeTime: 1200)
        let trun = makeTrackRunBox(
            sampleCount: 2, dataOffset: 256, sampleDurations: [110, 120], sampleSizes: [500, 600])
    ...
    func testLivePipelineParsesHandlerBoxPayload() async throws {
        ...
        let events = try await collectEvents(from: pipeline.events(for: reader))
        let handlerEvent = try XCTUnwrap(
            events.first { event in
                if case .willStartBox(let header, _) = event.kind {
    ...
    private func makeTrackHeaderBox(
        trackID: UInt32, duration: UInt32, width: UInt32, height: UInt32
    ) -> Data {
        var payload = Data()
        payload.append(0x00)  // version
    ...
    private func makeEditListBox(version: UInt8 = 0, entries: [EditListEntryParameters]) -> Data {
        var payload = Data()
    ...
    private final class PayloadSpyRandomAccessReader: RandomAccessReader {
        ...
        var didReadPayload: Bool { readRanges.contains { $0.overlaps(payloadRange) } }
    ...
    private func makeContainer(type: String, children: [Data]) -> Data {
        let payload = children.reduce(into: Data()) { result, child in result.append(child) }
    ...
    private func assertEvent(
        _ event: ParseEvent, kind expectedKind: EventKind, type expectedType: String,
        depth expectedDepth: Int, offset expectedOffset: Int64
    ) throws {
        let type = try FourCharCode(expectedType)
        switch (event.kind, expectedKind) {
        case (.willStartBox(let header, let depth), .willStart):
            XCTAssertEqual(header.type, type)
            XCTAssertEqual(depth, expectedDepth)
            XCTAssertEqual(event.offset, expectedOffset)
        case (.didFinishBox(let header, let depth), .didFinish):
            XCTAssertEqual(header.type, type)
            XCTAssertEqual(depth, expectedDepth)
            XCTAssertEqual(event.offset, expectedOffset)
        default: XCTFail("Unexpected event kind: \(event.kind)")
        }
    }
    ...
    private func makeMetadataContainer() -> Data {
        ...
        let keys = makeMetadataKeysBox(entries: [(namespace: "mdta", name: "com.example.rating")])
        let ilst = makeMetadataItemListBox(entries: [
            MetadataItemBoxEntry(
                identifier: [0xA9, 0x6E, 0x61, 0x6D], type: 1, locale: 0,
                data: Data("Example Title".utf8)),
            MetadataItemBoxEntry(
                identifier: [0x00, 0x00, 0x00, 0x01], type: 21, locale: 0,
    ...
    private func makeMetadataItemListBox(entries: [MetadataItemBoxEntry]) -> Data {
        ...
        for entry in entries {
            payload.append(
                makeMetadataItemEntry(
                    identifierBytes: entry.identifier,
                    dataBox: makeMetadataDataBox(
                        type: entry.type, locale: entry.locale, data: entry.data)))
        }
    ...
    private func makeTrackFragmentHeaderBox(
        trackID: UInt32, baseDataOffset: UInt64, defaultSampleDuration: UInt32,
        defaultSampleSize: UInt32, defaultSampleFlags: UInt32
    ) -> Data {
    ...
    private func makeTrackRunBox(
        sampleCount: UInt32, dataOffset: Int32, sampleDurations: [UInt32], sampleSizes: [UInt32]
    ) -> Data {
    ...
    private func makeCompositionOffsetBox(
        version: UInt8 = 0, entries: [CompositionOffsetEntryParameters]
    ) -> Data {
        var payload = Data()
        payload.append(version)
        payload.append(contentsOf: [0x00, 0x00, 0x00])  // flags
    ...
    private func makeSampleSizeBox(
        defaultSampleSize: UInt32, sampleCount: UInt32, explicitSizes: [UInt32] = []
    ) -> Data {
        if defaultSampleSize == 0 {
            precondition(
                explicitSizes.count == Int(sampleCount),
                "Explicit sample sizes must match sampleCount when default size is zero")
        } else {
            precondition(
                explicitSizes.isEmpty,
                "Explicit sample sizes are only supported when default size is zero")
        }
    ...
    private func makeTrackFragmentRandomAccessBox(
        version: UInt8, trackID: UInt32, entries: [TrackFragmentRandomAccessEntryParameters]
    ) -> Data {
        ...
        for entry in entries {
            if version == 1 {
                ...
                payload.append(
                    contentsOf: UInt32(truncatingIfNeeded: entry.moofOffset).bigEndianBytes)
            }
            ...
            payload.append(
                contentsOf: UInt32(truncatingIfNeeded: entry.sampleNumber).bigEndianBytes)
        }
    ...
    private func makeMovieFragmentRandomAccessBox(trackEntries: [Data]) -> Data {
        var payload = Data()
        for table in trackEntries { payload.append(table) }
...
extension FixedWidthInteger {
    fileprivate var bigEndianBytes: [UInt8] { withUnsafeBytes(of: self.bigEndian, Array.init) }
}
