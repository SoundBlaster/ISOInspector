...
final class ParsePipelineLiveTests: XCTestCase {
  func testLivePipelineEmitsEventsForNestedBoxes() async throws {
    ...
    try assertEvent(
      events[2], kind: .willStart, type: ContainerTypes.movie, depth: 0, offset: Int64(ftyp.count))
    try assertEvent(
    ...
    try assertEvent(
      events[5], kind: .didFinish, type: "tkhd", depth: 2, offset: Int64(ftyp.count + moov.count))
    try assertEvent(
  ...
  func testRandomAccessTableCorrelatesWithFragments() async throws {
    ...
    XCTAssertEqual(
      entry.resolvedDataOffset, baseDataOffset + UInt64(Int64(dataOffset)) + UInt64(sampleSizes[0]))
  ...
  private func makeTrackHeaderBox(trackID: UInt32, duration: UInt32, width: UInt32, height: UInt32)
    -> Data
  {
    var payload = Data()
    payload.append(0x00)  // version
  ...
  private func makeSampleSizeBox(
    ...
    if defaultSampleSize == 0 {
      ...
      precondition(
        explicitSizes.isEmpty, "Explicit sample sizes are only supported when default size is zero")
    }
