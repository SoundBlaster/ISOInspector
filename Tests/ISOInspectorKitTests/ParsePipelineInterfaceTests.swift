...
final class ParsePipelineInterfaceTests: XCTestCase {
    func testEventStreamDeliversEventsInDeclaredOrder() async throws {
        ...
        let headers: [BoxHeader] = try [
            BoxHeader(
                type: FourCharCode("ftyp"), totalSize: Int64(24), headerSize: Int64(16),
                payloadRange: Int64(16)..<Int64(24), range: Int64(0)..<Int64(24), uuid: nil),
            BoxHeader(
                type: FourCharContainerCode.moov.fourCharCode, totalSize: Int64(104),
                headerSize: Int64(8), payloadRange: Int64(8)..<Int64(104),
                range: Int64(24)..<Int64(128), uuid: nil),
        ]

        let expectedEvents: [ParseEvent] = headers.enumerated().map { index, header in
            ParseEvent(
                kind: .willStartBox(header: header, depth: index), offset: header.startOffset)
        }

        let pipeline = ParsePipeline(buildStream: { _ in
            AsyncThrowingStream { continuation in
                for event in expectedEvents { continuation.yield(event) }
                continuation.finish()
            }
        })

        var collected: [ParseEvent] = []
        for try await event in pipeline.events(for: reader) { collected.append(event) }
    ...
    func testEventStreamPropagatesErrors() async {
        ...
        } catch let error as StubError { XCTAssertEqual(error, StubError()) } catch {
