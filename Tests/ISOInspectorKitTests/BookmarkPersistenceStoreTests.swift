...
final class BookmarkPersistenceStoreTests: XCTestCase {
    ...
    func testUpsertingNewBookmarkCreatesRecord() throws {
        ...
        XCTAssertEqual(record.canonicalURL, fileURL.standardizedFileURL.resolvingSymlinksInPath().absoluteString)
    ...
    func testRecordsPersistToDisk() throws {
        ...
        do {
            ...
            _ = try store.upsertBookmark(for: fileURL.appendingPathComponent("other"), bookmarkData: Data([0xBB]))
            makeDate.advance(by: 10)
            _ = try store.markResolution(for: fileURL, state: .succeeded)
        }

        let reloaded = BookmarkPersistenceStore(directory: directory, makeDate: makeDate.sendableNext)
...
private final class DateGenerator: @unchecked Sendable {
    ...
    var sendableNext: @Sendable () -> Date {
        { [unowned self] in self.next() }
    }
