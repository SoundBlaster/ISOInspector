...
final class LargeFileBenchmarkTests: XCTestCase {
    ...
    func testCLIValidationCompletesWithinPerformanceBudget() throws {
        ...
        let strictResults = try measureCLIValidationBenchmark(
            fixture: fixture, configuration: configuration, options: .strict)

        XCTAssertLessThanOrEqual(
            strictResults.averageDuration, configuration.cliDurationBudgetSeconds(),
            "Strict-mode CLI validation exceeded budget for payload of \(configuration.payloadBytes) bytes"
        )

        XCTAssertEqual(
            strictResults.eventCount, fixture.expectedEventCount,
    ...
    func testCLIValidationLenientModePerformanceStaysWithinToleranceBudget() throws {
        ...
        let tolerantResults = try measureCLIValidationBenchmark(
            fixture: fixture, configuration: configuration, options: .tolerant)
        ...
        XCTAssertLessThanOrEqual(
            ratio, configuration.tolerantDurationOverheadBudget(),
            ...
            let tolerantPeak = tolerantResults.peakMemoryBytes
        {
            ...
            XCTAssertLessThanOrEqual(
                overhead, configuration.tolerantAdditionalMemoryBudgetBytes(),
                "Lenient mode CLI validation exceeded allowed memory overhead")
        }

        XCTAssertEqual(
            tolerantResults.eventCount, fixture.expectedEventCount,
            "Lenient mode CLI validation produced unexpected event count")
    }

    #if canImport(Combine)
        @MainActor func testAppStreamingPipelineDeliversUpdatesWithinLatencyBudget() throws {
            let fixture = try LargeFileBenchmarkFixture.make(configuration: configuration)
            ...
            performBenchmark(iterations: configuration.iterationCount) {
                ...
                store.$snapshot.dropFirst().sink { snapshot in
                    if firstEventLatency == nil, snapshot.lastUpdatedAt > .distantPast {
                        firstEventLatency = Date().timeIntervalSince(start)
                    }
                }.store(in: &cancellables)

                store.$state.dropFirst().sink { state in
                    if state == .finished { expectation.fulfill() }
                }.store(in: &cancellables)

                store.start(pipeline: pipeline, reader: reader, context: .init(source: fixture.url))
                ...
                if let latency = firstEventLatency {
                    XCTAssertLessThanOrEqual(
                        latency, configuration.uiLatencyBudgetSeconds(),
    ...
    func testMappedReaderRandomSliceBenchmarkMeetsPerformanceExpectations() throws {
        ...
        try runRandomSliceBenchmark(
            readerName: "MappedReader", fixture: fixture,
    ...
    func testChunkedFileReaderRandomSliceBenchmarkMeetsPerformanceExpectations() throws {
        let fixture = try LargeFileBenchmarkFixture.make(configuration: configuration)
        try runRandomSliceBenchmark(
            readerName: "ChunkedFileReader", fixture: fixture,
...
extension LargeFileBenchmarkTests {
    fileprivate func performBenchmark(iterations: Int, block: () throws -> Void) {
            ...
            if #available(macOS 13.0, iOS 16.0, *) {
                ...
                measure(metrics: metrics, options: options) {
                    do { try block() } catch { XCTFail("Benchmark iteration failed: \(error)") }
                }
                return
            }
        #endif
    ...
    fileprivate func runRandomSliceBenchmark(
        readerName: String, fixture: LargeFileBenchmarkFixture,
        ...
        let preparedScenarios: [PreparedScenario] = scenarios.map { scenario in
            ...
            return PreparedScenario(
                scenario: scenario, requests: requests, expectedBytes: expectedBytes)
        }
        ...
        performBenchmark(iterations: configuration.iterationCount) {
            for prepared in preparedScenarios {
                ...
                let bytesRead = try executeRandomSliceRequests(
                    prepared.requests, using: reader, readerName: readerName,
                    scenarioName: prepared.scenario.name)
                XCTAssertEqual(
                    bytesRead, prepared.expectedBytes,
        ...
        for prepared in preparedScenarios {
            let summary = prepared.scenario.makeSummary(
                readerName: readerName, totalBytesPerIteration: prepared.expectedBytes,
                requestCount: prepared.requests.count, readerLength: fileLength)
            print(summary)
        }
    }

    fileprivate func measureCLIValidationBenchmark(
        fixture: LargeFileBenchmarkFixture, configuration: PerformanceBenchmarkConfiguration,
        ...
        for _ in 0..<configuration.iterationCount {
            ...
            if let usage = currentMemoryUsageBytes() { peakMemory = max(peakMemory, usage) }
            eventCount = try drainEvents(using: environment.parsePipeline, fileURL: fixture.url)
        }

        return CLIBenchmarkResult(
            durations: durations, peakMemoryBytes: peakMemory == 0 ? nil : peakMemory,
            eventCount: eventCount)
    }

    fileprivate func executeRandomSliceRequests(
        _ requests: [RandomSliceRequest], using reader: any RandomAccessReader, readerName: String,
        scenarioName: String
    ) throws -> Int {
        var totalBytes = 0
    ...
    fileprivate func runValidateCommand(on fileURL: URL, environment: ISOInspectorCLIEnvironment)
        throws
    {
        ...
        Task {
            ...
            } catch { capturedError.withValue { $0 = error } }
            await MainActor.run {
                ISOInspectorCommandContextStore.reset()
        ...
        wait(for: [expectation], timeout: max(configuration.cliDurationBudgetSeconds() * 4, 1))

        if let error = capturedError.withValue({ $0 }) { throw error }
    }
}

private func makeCLIEnvironment(options: ParsePipeline.Options) -> ISOInspectorCLIEnvironment {
    ISOInspectorCLIEnvironment(
        refreshCatalog: { _, _ in }, makeReader: { url in try ChunkedFileReader(fileURL: url) },
        parsePipeline: .live(options: options), formatter: EventConsoleFormatter(), print: { _ in },
        printError: { _ in })
}

private func drainEvents(using pipeline: ParsePipeline, fileURL: URL) throws -> Int {
    ...
    Task {
        do { for try await _ in stream { count.withValue { $0 += 1 } } } catch {
...
private func currentMemoryUsageBytes() -> UInt64? {
        ...
        for line in status.split(whereSeparator: \.isNewline) where line.hasPrefix("VmRSS:") {
            ...
            if components.count >= 2, let value = UInt64(components[1]) { return value * 1024 }
        }
        return nil
    #else
...
private struct LargeFileBenchmarkFixture {
    ...
    static func make(
        configuration: PerformanceBenchmarkConfiguration, fileManager: FileManager = .default
    ) throws -> LargeFileBenchmarkFixture {
        let payloadBytes = configuration.payloadBytes
        let directory = fileManager.temporaryDirectory.appendingPathComponent(
            "isoinspector-benchmarks", isDirectory: true)
...
private struct RandomSliceBenchmarkScenario: Sendable {
    ...
    static func makeDefaultScenarios(payloadBytes: Int) -> [RandomSliceBenchmarkScenario] {
        ...
        return [
            RandomSliceBenchmarkScenario(
                name: "small", minimumBytes: 32, maximumBytes: 512, sampleCount: 256,
                seed: 0xA5A5_0001),
            RandomSliceBenchmarkScenario(
                name: "medium", minimumBytes: 4_096, maximumBytes: mediumUpper, sampleCount: 128,
                seed: 0xA5A5_0002),
            RandomSliceBenchmarkScenario(
                name: "large", minimumBytes: 65_536, maximumBytes: largeUpper, sampleCount: 64,
                seed: 0xA5A5_0003),
        ]
    }
    ...
    func makeSummary(
        readerName: String, totalBytesPerIteration: Int, requestCount: Int, readerLength: Int64
    ) -> String {
