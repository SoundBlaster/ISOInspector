    ...
    final class ColorSchemeAdapterTests: XCTestCase {
        ...
        func testLightColorSchemeDetection() throws {
            ...
            XCTAssertEqual(adapter.colorScheme, .light, "Adapter should detect light color scheme")
            XCTAssertFalse(adapter.isDarkMode, "isDarkMode should be false in light mode")
        }
        ...
        func testDarkColorSchemeDetection() throws {
            ...
            XCTAssertEqual(adapter.colorScheme, .dark, "Adapter should detect dark color scheme")
            XCTAssertTrue(adapter.isDarkMode, "isDarkMode should be true in dark mode")
        }
        ...
        func testAdaptiveColorSchemeModifier() throws {
            struct TestView: View { var body: some View { Text("Test").adaptiveColorScheme() } }
        ...
        func testEquatableConformance() throws {
            ...
            XCTAssertEqual(
                adapter1.colorScheme, adapter2.colorScheme,
                "Adapters with same color scheme should be equal")
            XCTAssertNotEqual(
                adapter1.colorScheme, adapter3.colorScheme,
        ...
        func testAdapterCreationPerformance() throws {
            measure { for _ in 0..<1000 { _ = ColorSchemeAdapter(colorScheme: .light) } }
        }
    ...
    final class ColorSchemeAdapterIntegrationTests: XCTestCase {
        ...
        func testCardWithAdaptiveColors() throws {
            struct AdaptiveCard: View {
                ...
                var body: some View {
                    let adapter = ColorSchemeAdapter(colorScheme: colorScheme)
                    return VStack { Text("Card Title").foregroundColor(adapter.adaptiveTextColor) }
                        .padding(DS.Spacing.m).background(adapter.adaptiveElevatedSurface)
                        .cornerRadius(DS.Radius.card).overlay(
        ...
        func testBadgeWithAdaptiveColors() throws {
            struct AdaptiveBadge: View {
                ...
                var body: some View {
                    let adapter = ColorSchemeAdapter(colorScheme: colorScheme)
                    return Text(level.rawValue.uppercased()).font(DS.Typography.caption)
                        .foregroundColor(adapter.adaptiveTextColor).padding(
                            .horizontal, DS.Spacing.m
                        ).padding(.vertical, DS.Spacing.s).background(level.backgroundColor)
            ...
            struct AdaptiveInspector: View {
                ...
                var body: some View {
                    ...
                    return HStack(spacing: 0) {
                        // Main content
                        VStack { Text("Content") }.frame(maxWidth: .infinity).background(
                            adapter.adaptiveBackground)

                        // Inspector panel
                        VStack { Text("Inspector") }.frame(width: 250).background(
                            adapter.adaptiveSecondaryBackground
                        ).overlay(
                            Rectangle().frame(width: 1).foregroundColor(
                                adapter.adaptiveDividerColor), alignment: .leading)
                    }
                }
            }
            ...
            struct SwitchableView: View {
                ...
                var body: some View {
                    ...
                    return VStack(spacing: DS.Spacing.m) {
                        ...
                        Text("Background adapts automatically").foregroundColor(
                            adapter.adaptiveSecondaryTextColor)
                    }.padding(DS.Spacing.l).background(adapter.adaptiveBackground)
                }
            }
        ...
        func testCompatibilityWithDesignTokens() throws {
            ...
            let colors: [Color] = [
                DS.Color.infoBG, DS.Color.warnBG, DS.Color.errorBG, DS.Color.successBG,
            ]

            for color in colors { XCTAssertNotNil(color, "DS.Color token should be valid") }
