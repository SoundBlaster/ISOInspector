...
final class ISOInspectorCLIScaffoldTests: XCTestCase {
    ...
    func testRunExportJSONWritesExpectedFile() throws {
        ...
        let outputDirectory = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent(
            UUID().uuidString)
        ...
        ISOInspectorCLIRunner.run(
            arguments: ["isoinspect", "export-json", input.path, "--output", output.path],
    ...
    func testRunExportCaptureWritesBinaryCapture() throws {
        ...
        ISOInspectorCLIRunner.run(
            arguments: ["isoinspect", "export-capture", input.path, "--output", output.path],
            environment: environment)
    ...
    func testExportCommandsDefaultOutputPathWhenNotProvided() throws {
        ...
        let input = outputDirectory.appendingPathComponent("sample.mp4")
        XCTAssertTrue(FileManager.default.createFile(atPath: input.path, contents: Data()))
        let expectedJSON = input.appendingPathExtension("isoinspector").appendingPathExtension(
            "json")
        let expectedCapture = input.appendingPathExtension("capture")
        ...
        let environment = ISOInspectorCLIEnvironment(
            ...
            }), formatter: EventConsoleFormatter(), print: { _ in }, printError: { _ in })

        ISOInspectorCLIRunner.run(
            arguments: ["isoinspect", "export-json", input.path], environment: environment)

        ISOInspectorCLIRunner.run(
            arguments: ["isoinspect", "export-capture", input.path], environment: environment)
    ...
    func testInspectDefaultsToStrictMode() throws {
        ...
        ISOInspectorCLIRunner.run(
            arguments: ["isoinspect", "inspect", "/tmp/sample.mp4"], environment: environment)
    ...
    func testInspectTolerantFlagOverridesDefaultMode() throws {
        ...
        ISOInspectorCLIRunner.run(
            arguments: ["isoinspect", "inspect", "--tolerant", "/tmp/sample.mp4"],
            environment: environment)
    ...
    func testInspectRejectsConflictingParseModeFlags() throws {
        ...
        let environment = ISOInspectorCLIEnvironment(
            ...
            }), formatter: EventConsoleFormatter(), print: { _ in },
            printError: { printedErrors.value.append($0) },
            makeResearchLogWriter: { _ in NullResearchLog() },
            defaultResearchLogURL: { URL(fileURLWithPath: "/tmp/research-log.json") })

        ISOInspectorCLIRunner.run(
            arguments: ["isoinspect", "inspect", "--tolerant", "--strict", "/tmp/sample.mp4"],
            environment: environment)

        XCTAssertTrue(
            printedErrors.value.contains(where: {
                $0.contains("Cannot combine --tolerant and --strict")
            }))
    }
        ...
        let environment = ISOInspectorCLIEnvironment(
            ...
            parsePipeline: ParsePipeline(buildStream: { _, context in
                context.issueStore?.record(
                    ParseIssue(severity: .error, code: "VR-001", message: "Box exceeds parent"),
                    depth: 4)
                return AsyncThrowingStream { continuation in continuation.finish() }
            }), formatter: EventConsoleFormatter(), print: { printed.value.append($0) },
    ...
    func testInspectStrictRunOmitsCorruptionSummaryEvenWhenIssuesRecorded() throws {
        ...
        let environment = ISOInspectorCLIEnvironment(
            ...
            parsePipeline: ParsePipeline(buildStream: { _, context in
                context.issueStore?.record(
                    ParseIssue(severity: .warning, code: "VR-002", message: "Warning issued"),
                    depth: 2)
                return AsyncThrowingStream { continuation in continuation.finish() }
            }), formatter: EventConsoleFormatter(), print: { printed.value.append($0) },
            printError: { _ in }, makeResearchLogWriter: { _ in NullResearchLog() },
        ...
        ISOInspectorCLIRunner.run(
            arguments: ["isoinspect", "inspect", "--strict", "/tmp/sample.mp4"],
            environment: environment)
    ...
    func testExportJSONHonorsTolerantFlag() throws {
        ...
        ISOInspectorCLIRunner.run(
            arguments: [
                "isoinspect", "export-json", "--tolerant", input.path, "--output", output.path,
            ], environment: environment)
    ...
    func testRunExecutesMP4RARefreshWithParsedArguments() throws {
        ...
        ISOInspectorCLIRunner.run(
            arguments: [
                "isoinspect", "mp4ra", "refresh", "--source", "https://example.com/custom.json",
    ...
    func testRunInspectPrintsFormattedEvents() throws {
        ...
        let issue = ValidationIssue(
            ruleID: "VR-006", message: "Unknown box encountered", severity: .info)
        let event = ParseEvent(
            kind: .willStartBox(header: header, depth: 0), offset: 0, metadata: descriptor,
    ...
    func testRunInspectAppendsResearchLogEntries() throws {
        ...
        let event = ParseEvent(
            kind: .willStartBox(header: header, depth: 0), offset: 0, metadata: nil,
            validationIssues: [issue])
    ...
    func testRunInspectRequiresFilePath() {
        ...
        ISOInspectorCLIRunner.run(arguments: ["isoinspect", "inspect"], environment: environment)
    ...
    func testExportJSONIncludesHandlerMetadata() throws {
        ...
        let output = input.appendingPathExtension("isoinspector").appendingPathExtension("json")
        ...
        let environment = ISOInspectorCLIEnvironment(
            ...
            parsePipeline: .live(), formatter: EventConsoleFormatter(), print: { _ in },
            printError: { printedErrors.value.append($0) })
        ...
        let category = try XCTUnwrap(
            payload.first { ($0["name"] as? String) == "handler_category" })
    ...
    func testExportTextProducesIssueSummary() throws {
        ...
        let input = directory.appendingPathComponent("sample.mp4")
        let output = input.appendingPathExtension("isoinspector").appendingPathExtension("txt")
        try makeHandlerFixture().write(to: input)
        ...
        ISOInspectorCLIRunner.run(
            arguments: ["isoinspect", "export-text", input.path], environment: environment)
    ...
    private func findNode(named fourcc: String, in nodes: [[String: Any]]) -> [String: Any]? {
        for node in nodes {
            if node["fourcc"] as? String == fourcc { return node }
