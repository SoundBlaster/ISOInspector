...
final class ISOInspectorCommandTests: XCTestCase {
    func testRunWithoutArgumentsShowsHelpWithPlannedSubcommands() async throws {
        ...
        } catch let exit as CleanExit { XCTAssertEqual(exit.description, "--help") } catch {
    ...
    func testRunBootstrapsSharedContextWithEnvironmentFromFactory() async throws {
        ...
        let customEnvironment = ISOInspectorCLIEnvironment(
            refreshCatalog: { _, _ in }, parsePipeline: .live(), formatter: EventConsoleFormatter(),
            print: { printed.value.append($0) }, printError: { _ in })
    ...
    func testBootstrapDisablesTelemetryWhenRequested() async throws {
        let environment = ISOInspectorCLIEnvironment(refreshCatalog: { _, _ in })
    ...
    func testGlobalOptionsResolvesValidationConfigurationFromPresetAliasAndOverrides() throws {
        let options = try ISOInspectorCommand.GlobalOptions.parse([
            "--structural-only", "--disable-rule", "VR-006", "--enable-rule", "VR-001",
        ])
        ...
        XCTAssertEqual(
            result.configuration.ruleOverrides[ValidationRuleIdentifier.researchLogRecording], false
        )
        XCTAssertEqual(
            result.configuration.ruleOverrides[ValidationRuleIdentifier.structuralSize], true)
        XCTAssertTrue(result.wasCustomized)
    }

    func testGlobalOptionsRejectsUnknownRuleIdentifiers() {
        XCTAssertThrowsError(
            try ISOInspectorCommand.GlobalOptions.parse(["--disable-rule", "VR-999"])
        ) { error in XCTAssertTrue(String(describing: error).contains("VR-999")) }
    }

    func testGlobalOptionsRejectsConflictingPresetSelections() {
        XCTAssertThrowsError(
            try ISOInspectorCommand.GlobalOptions.parse([
                "--preset", "all-rules", "--structural-only",
            ])
        ) { error in
    ...
    func testGlobalOptionsRejectsConflictingParseModeFlags() {
        XCTAssertThrowsError(
            try ISOInspectorCommand.GlobalOptions.parse(["--tolerant", "--strict"])
        ) { error in
            let description = String(describing: error)
    ...
    func testInspectCommandStreamsEventsAndRespectsResearchLogOption() async throws {
        final class Recorder: ResearchLogRecording, @unchecked Sendable {
            ...
            func record(_ entry: ResearchLogEntry) { entries.append(entry) }
        }
        ...
        let event = ParseEvent(
            ...
            metadata: descriptor, validationIssues: [issue])
        ...
        let environment = ISOInspectorCLIEnvironment(
            ...
            parsePipeline: ParsePipeline(buildStream: { _, context in
                context.researchLog?.record(
                    ResearchLogEntry(
                        boxType: header.identifierString, filePath: context.source?.path ?? "",
                        startOffset: header.startOffset, endOffset: header.endOffset))
                return AsyncThrowingStream { continuation in
                    continuation.yield(event)
                    continuation.finish()
                }
            }), formatter: EventConsoleFormatter(), print: { printed.value.append($0) },
        ...
        var command = try ISOInspectorCommand.Commands.Inspect.parse([
            "/tmp/sample.mp4", "--research-log", "/tmp/custom-log.json",
        ])
        ...
        let environment = ISOInspectorCLIEnvironment(
            ...
            parsePipeline: ParsePipeline(buildStream: { _, context in
                ...
                context.issueStore?.record(
                    ParseIssue(severity: .error, code: "VR-001", message: "Corruption surfaced"),
                    depth: 2)
                return AsyncThrowingStream { continuation in continuation.finish() }
            }), formatter: EventConsoleFormatter(), print: { printed.value.append($0) },
            printError: { _ in }, makeResearchLogWriter: { _ in NullResearchLog() },
    ...
    func testValidateCommandFiltersDisabledRulesAndPrintsPresetMetadata() async throws {
        ...
        let disabledIssue = ValidationIssue(
            ...
            message: "Research log advisory", severity: .info)
        let errorIssue = ValidationIssue(
            ruleID: ValidationRuleIdentifier.structuralSize.rawValue, message: "Size mismatch",
            severity: .error)
        let event = ParseEvent(
            kind: .willStartBox(header: header, depth: 0), offset: header.startOffset,
            metadata: descriptor, validationIssues: [disabledIssue, errorIssue])
        ...
        let presets = [
            ...
            ValidationPreset(
                id: "structural", name: "Structural Focus", summary: "",
                rules: [
                    ValidationPreset.RuleState(ruleID: .movieDataOrdering, isEnabled: false),
        ...
        await MainActor.run {
            ISOInspectorCommand.contextFactory = { _ in
                ISOInspectorCommandContext(
                    environment: environment, validationConfiguration: configuration,
                    validationPresets: presets, validationConfigurationWasCustomized: true)
            }
            ISOInspectorCommandContextStore.reset()
        }

        var command = try ISOInspectorCommand.Commands.Validate.parse(["/tmp/sample.mp4"])

        do {
            try await command.run()
            XCTFail("Expected ExitCode to be thrown")
        } catch let exit as ExitCode { XCTAssertEqual(exit.rawValue, 2) }
    ...
    func testValidateCommandPrintsSummaryAndSetsExitCodeBasedOnIssues() async throws {
        ...
        let warning = ValidationIssue(
            ruleID: "VR-003", message: "Flags mismatch", severity: .warning)
        let error = ValidationIssue(ruleID: "VR-001", message: "Size mismatch", severity: .error)
        let event = ParseEvent(
            kind: .willStartBox(header: header, depth: 0), offset: header.startOffset,
            metadata: descriptor, validationIssues: [warning, error])
    ...
    func testValidateCommandEmitsCodecWarningsFromFixture() async throws {
        ...
        let environment = ISOInspectorCLIEnvironment(
            ...
            }, parsePipeline: .live(), formatter: EventConsoleFormatter(),
            print: { printed.value.append($0) }, printError: { errors.value.append($0) })
        ...
        XCTAssertTrue(
            printed.value.contains(where: { $0.contains("VR-018") && $0.contains("avcC") }))
        XCTAssertTrue(
            printed.value.contains(where: { $0.contains("VR-018") && $0.contains("hvcC") }))
    ...
    func testInspectCommandSurfacesSampleEncryptionMetadata() async throws {
        ...
        let environment = ISOInspectorCLIEnvironment(
            ...
            print: { printed.value.append($0) }, printError: { errors.value.append($0) },
            makeResearchLogWriter: { url in
                XCTAssertEqual(url.deletingLastPathComponent().path, temporaryDirectory.path)
                return NullResearchLog()
            },
            defaultResearchLogURL: {
                temporaryDirectory.appendingPathComponent("research-log.json")
            })
        ...
        var command = try ISOInspectorCommand.Commands.Inspect.parse([fileURL.path])

        try await command.run()

        XCTAssertTrue(errors.value.isEmpty)
        XCTAssertTrue(
            printed.value.contains(where: { $0.contains("encryption") && $0.contains("samples=2") })
        )
        XCTAssertTrue(
            printed.value.contains(where: {
                $0.contains("aux_offsets") && $0.contains("bytes_per_entry=8")
            }))
        XCTAssertTrue(
            printed.value.contains(where: {
                $0.contains("aux_sizes") && $0.contains("entry_count=2")
            }))
    ...
    func testBatchCommandAggregatesResultsAndWritesCSV() async throws {
        ...
        _ = FileManager.default.createFile(
            atPath: firstFile.path, contents: Data(repeating: 0, count: 16))
        _ = FileManager.default.createFile(
            atPath: secondFile.path, contents: Data(repeating: 1, count: 24))

        let header = try makeHeader(type: "ftyp", size: 24)
        let descriptor = try XCTUnwrap(BoxCatalog.shared.descriptor(for: header))
        let infoIssue = ValidationIssue(
            ruleID: "VR-006", message: "Research detail", severity: .info)
        let warningIssue = ValidationIssue(
            ruleID: "VR-003", message: "Flags mismatch", severity: .warning)
        let errorIssue = ValidationIssue(
            ruleID: "VR-001", message: "Size mismatch", severity: .error)

        let eventsByFile: [URL: [ParseEvent]] = [
            firstFile: [
                ParseEvent(
                    kind: .willStartBox(header: header, depth: 0), offset: header.startOffset,
                    metadata: descriptor, validationIssues: [infoIssue])
            ],
            secondFile: [
                ParseEvent(
                    kind: .willStartBox(header: header, depth: 0), offset: header.startOffset,
                    metadata: descriptor, validationIssues: [warningIssue, errorIssue])
            ],
        ]
        ...
        let environment = ISOInspectorCLIEnvironment(
            ...
            parsePipeline: ParsePipeline(buildStream: { _, context in
                AsyncThrowingStream { continuation in
                    ...
                    for event in eventsByFile[source, default: []] { continuation.yield(event) }
                    continuation.finish()
                }
            }), formatter: EventConsoleFormatter(), print: { printed.value.append($0) },
            printError: { errors.value.append($0) })
    ...
    func testExportJSONCommandStreamsEventsAndWritesDefaultOutput() async throws {
        ...
        let expectedOutputURL = fileURL.appendingPathExtension("isoinspector")
    ...
    func testExportCaptureCommandStreamsEventsAndRespectsOutputOption() async throws {
        ...
        var command = try ISOInspectorCommand.Commands.Export.Capture.parse([
            fileURL.path, "--output", outputURL.path,
        ])
    ...
    func testExportCommandThrowsExitCodeWhenOutputDirectoryMissing() async throws {
        ...
        let fileURL = temporaryDirectory.appendingPathComponent("sample.mp4")
        let invalidOutput = temporaryDirectory.appendingPathComponent("missing")
        ...
        var command = try ISOInspectorCommand.Commands.Export.JSON.parse([
            fileURL.path, "--output", invalidOutput.path,
        ])

        do {
            try await command.run()
            XCTFail("Expected ExitCode to be thrown")
        } catch let exit as ExitCode { XCTAssertEqual(exit.rawValue, 3) }

        XCTAssertTrue(printed.value.isEmpty)
        XCTAssertEqual(
            errors.value.last,
            "Destination is not writable: \(invalidOutput.deletingLastPathComponent().path)")
    ...
    func testFilteredEventPreservesParseIssueMetadata() throws {
        ...
        let parseIssue = ParseIssue(
            severity: .warning, code: "guard.zero_size_loop", message: "Repeated zero-sized box",
            byteRange: 16..<24, affectedNodeIDs: [header.startOffset])
        let validationIssue = ValidationIssue(
            ruleID: ValidationRuleIdentifier.researchLogRecording.rawValue, message: "Filtered",
            severity: .info)
        let event = ParseEvent(
            kind: .willStartBox(header: header, depth: 0), offset: header.startOffset,
            validationIssues: [validationIssue], issues: [parseIssue])

        let filtered = ISOInspectorCommand.filteredEvent(event, removing: [validationIssue.ruleID])
...
private func loadFixtureData(named name: String) throws -> Data {
    let baseDirectory = URL(fileURLWithPath: #filePath).deletingLastPathComponent()
...
private struct StubReader: RandomAccessReader {
    ...
    init(length: Int64 = 0) { self.length = length }
...
private func makeHeader(type: String, size: Int64) throws -> BoxHeader {
    ...
    return BoxHeader(
        type: fourCC, totalSize: size, headerSize: 8, payloadRange: payloadRange, range: range,
        uuid: nil)
}
