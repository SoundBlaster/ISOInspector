        ...
        ScrollView {
            VStack(alignment: .leading, spacing: DS.Spacing.xl) {
                ...
                VStack(alignment: .leading, spacing: DS.Spacing.m) {
                    Text("Badge Component").font(DS.Typography.title)

                    Text(
                        "Displays status indicators with semantic color coding and optional icons. Fully accessible with VoiceOver labels."
                ...
                // All Badge Levels
                VStack(alignment: .leading, spacing: DS.Spacing.m) {
                    Text("All Badge Levels").font(DS.Typography.subheadline)
                ...
                VStack(alignment: .leading, spacing: DS.Spacing.m) {
                    Text("Text Length Variations").font(DS.Typography.subheadline)
                ...
                // Use Cases
                VStack(alignment: .leading, spacing: DS.Spacing.m) {
                    Text("Common Use Cases").font(DS.Typography.subheadline)

                    // File Status
                    HStack(spacing: DS.Spacing.m) {
                        Text("File:").font(DS.Typography.body)
                        Badge(text: "Valid", level: .success, showIcon: true)
                    }

                    // Box Type
                    HStack(spacing: DS.Spacing.m) {
                        Text("Box Type:").font(DS.Typography.body)
                    ...
                    // Error Indicator
                    HStack(spacing: DS.Spacing.m) {
                        Text("Validation:").font(DS.Typography.body)
                ...
                // Accessibility Features
                VStack(alignment: .leading, spacing: DS.Spacing.m) {
                    Text("Accessibility").font(DS.Typography.subheadline)

                    VStack(alignment: .leading, spacing: DS.Spacing.s) {
                        Text("• VoiceOver announces level (e.g., 'Info: File Valid')").font(
                            DS.Typography.caption)
                        Text("• Contrast ratios ≥4.5:1 (WCAG 2.1 AA)").font(DS.Typography.caption)
                        Text("• Touch targets meet 44×44pt minimum").font(DS.Typography.caption)
                        Text("• Dynamic Type support (scales with system settings)").font(
                            DS.Typography.caption)
...
#Preview("Badge Screen") { NavigationStack { BadgeScreen() } }

#Preview("Dark Mode") { NavigationStack { BadgeScreen() }.preferredColorScheme(.dark) }
