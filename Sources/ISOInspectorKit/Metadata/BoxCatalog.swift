...
public struct BoxDescriptor: Equatable, Sendable {
    ...
    init(
        identifier: Identifier, name: String, summary: String, category: String?,
        specification: String?, version: Int?, flags: UInt32?
    ) {
...
public struct BoxCatalog: Sendable {
    ...
    public func descriptor(for header: BoxHeader) -> BoxDescriptor? {
        if let uuid = header.uuid, let descriptor = byExtendedType[uuid] { return descriptor }
...
extension BoxCatalog {
    static func loadBundledCatalog(
        logger: DiagnosticsLogger = DiagnosticsLogger(
            subsystem: "ISOInspectorKit", category: "BoxCatalog")
    ) throws -> BoxCatalog {
    ...
    public static let shared: BoxCatalog = {
        do { return try BoxCatalog.loadBundledCatalog() } catch {
...
private struct CatalogLoader {
    ...
    func loadEntries() throws -> [BoxDescriptor] {
        ...
        return registry.boxes.compactMap { entry in
            do { return try makeDescriptor(from: entry) } catch {
                logger?.error(
                    "Failed to decode MP4RA entry for type \(entry.type): \(String(describing: error))"
                )
                return nil
            }
        }
    ...
    private func makeDescriptor(from entry: Registry.Entry) throws -> BoxDescriptor {
        ...
        return BoxDescriptor(
            identifier: identifier, name: entry.name, summary: entry.summary, category: category,
            specification: entry.specification, version: entry.version, flags: flags)
    }
    ...
    private func extractCategory(from summary: String) -> String? {
        ...
        if let commaIndex = value.firstIndex(of: ",") { value = value[..<commaIndex] }
