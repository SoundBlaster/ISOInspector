...
public enum BoxClassifier {
    ...
    private static let metadataRawValues: Set<String> = ["meta", "ilst", "udta", "mdta", "cprt"]

    public static func category(for header: BoxHeader, metadata: BoxDescriptor?) -> BoxCategory {
        if MediaAndIndexBoxCode.isStreamingIndicator(header) { return .index }
        if MediaAndIndexBoxCode.isMediaPayload(header) { return .media }
        if isMetadata(header: header, descriptor: metadata) { return .metadata }
        if FourCharContainerCode.isContainer(header) { return .container }
        return .other
    }
    ...
    private static func isMetadata(header: BoxHeader, descriptor: BoxDescriptor?) -> Bool {
        if metadataRawValues.contains(header.type.rawValue) { return true }
        if let descriptor {
            ...
            if name.contains("metadata") || name.contains("meta-data") { return true }
        }
        return false
    }
}
