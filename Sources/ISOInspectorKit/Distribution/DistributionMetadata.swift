...
public enum DistributionMetadataLoader {
    ...
    public static func defaultMetadata() throws -> DistributionMetadata {
        ...
        let resourceURL =
            bundle.url(
                forResource: resourceFileName, withExtension: resourceExtension,
                subdirectory: "Distribution")
            ?? bundle.url(forResource: resourceFileName, withExtension: resourceExtension)

        guard let url = resourceURL else { throw DistributionMetadataError.resourceNotFound }
        let data = try Data(contentsOf: url)
        return try JSONDecoder().decode(DistributionMetadata.self, from: data)
    }
}

public enum DistributionMetadataError: Error { case resourceNotFound }
