...
public struct PerformanceBenchmarkConfiguration: Sendable {
    ...
    public init(environment: [String: String] = ProcessInfo.processInfo.environment) {
        let intensity =
            Intensity(rawValue: environment[EnvironmentKey.intensity]?.lowercased() ?? "") ?? .ci
        ...
        if let override = environment[EnvironmentKey.payloadBytes], let value = Int(override),
            value > 0
        {
            payload = value
        }

        if let override = environment[EnvironmentKey.iterationCount], let value = Int(override),
            value > 0
        {
            iterations = value
        }

        if let override = environment[EnvironmentKey.slackMultiplier], let value = Double(override),
            value >= 1
        {
            slack = value
        }
    ...
    public func uiLatencyBudgetSeconds() -> TimeInterval { Self.uiLatencySeconds * slackMultiplier }

    public func tolerantDurationOverheadBudget() -> Double { Self.tolerantDurationOverhead }
