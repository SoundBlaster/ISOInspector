...
public struct PlaintextIssueSummaryExporter {
  ...
  public func export(
    tree: ParseTree,
    metadata: Metadata,
    summary: ParseIssueStore.IssueSummary,
    issues: [ParseIssue]
  ) throws -> Data {
    var lines: [String] = []
    ...
    let fileName = fileURL.lastPathComponent.isEmpty ? metadata.filePath : fileURL.lastPathComponent
  ...
  private var severityOrder: [ParseIssue.Severity] {
    [.error, .warning, .info]
  }
    ...
    func traverse(_ current: [String], nodes: [ParseTreeNode]) {
      for node in nodes {
        ...
        if !node.children.isEmpty {
          traverse(path, nodes: node.children)
        }
      }
    }
