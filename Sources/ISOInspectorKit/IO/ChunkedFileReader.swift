...
public final class ChunkedFileReader: RandomAccessReader {
    ...
    public enum Error: Swift.Error { case ioError(underlying: Swift.Error) }
    ...
    public convenience init(fileURL: URL, chunkSize: Int = ChunkedFileReader.defaultChunkSize)
        throws
    {
        ...
        self.init(
            length: fileSize.int64Value, chunkSize: chunkSize,
            file: FileHandleAdapter(handle: handle), closeHandler: { try? handle.close() })
    }

    init(
        length: Int64, chunkSize: Int, file: ChunkedFileReaderRandomAccessFile,
    ...
    public func read(at offset: Int64, count: Int) throws -> Data {
        ...
        guard upperResult.overflow == false else { throw RandomAccessReaderError.overflowError }
    ...
    private func loadChunk(at offset: Int64) throws -> Data {
        if cachedChunkOffset == offset, !cachedChunkData.isEmpty { return cachedChunkData }
        ...
        } catch { throw RandomAccessReaderError.ioError(underlying: error) }
    }
}
...
private final class FileHandleAdapter: ChunkedFileReaderRandomAccessFile {
    ...
    func seek(toOffset offset: UInt64) throws { try handle.seek(toOffset: offset) }
