...
public enum RandomAccessReaderError: Swift.Error, Equatable, Sendable {
    ...
    public static func == (lhs: RandomAccessReaderError, rhs: RandomAccessReaderError) -> Bool {
        switch (lhs, rhs) {
        case (.overflowError, .overflowError): return true
        case (.boundsError(let lhsError), .boundsError(let rhsError)): return lhsError == rhsError
...
extension RandomAccessReader {
    public func readUInt32(at offset: Int64) throws -> UInt32 { try readInteger(at: offset) }

    public func readUInt64(at offset: Int64) throws -> UInt64 { try readInteger(at: offset) }

    public func readFourCC(at offset: Int64) throws -> FourCharCode {
        let byteCount = 4
        let data = try read(at: offset, count: byteCount)
        guard data.count == byteCount else {
            throw RandomAccessReaderValueDecodingError.truncatedRead(
                expected: byteCount, actual: data.count)
        }
        do { return try FourCharCode(data: data) } catch {
    ...
    private func readInteger<T: FixedWidthInteger>(at offset: Int64) throws -> T {
        ...
        return data.reduce(T(0)) { partialResult, byte in (partialResult << 8) | T(byte) }
    }
}
