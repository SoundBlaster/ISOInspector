...
public struct BoxParserRegistry: Sendable {
    public typealias Parser =
        @Sendable (_ header: BoxHeader, _ reader: RandomAccessReader) throws -> ParsedBoxPayload?
    ...
    public init(
        typeParsers: [FourCharCode: Parser] = [:], extendedTypeParsers: [UUID: Parser] = [:],
        fallback: Parser? = nil
    ) {
    ...
    public func parser(for header: BoxHeader) -> Parser {
        if let uuid = header.uuid, let parser = extendedTypeParsers[uuid] { return parser }
        if let parser = typeParsers[header.type] { return parser }
        return fallback
    }
    ...
    private static let placeholderFallback: Parser = { header, _ in
        ...
        var fields: [ParsedBoxPayload.Field] = [
            ParsedBoxPayload.Field(
                name: "parser", value: "placeholder",
                description:
                    "No parser registered for this box type; returning placeholder metadata."),
            ParsedBoxPayload.Field(
                name: "payload_length", value: String(payloadLength),
        ...
        if payloadLength > 0 {
            fields.append(
                ParsedBoxPayload.Field(
                    name: "payload_range", value: String(describing: payloadRange),
    ...
    public struct FragmentEnvironment: Sendable {
        ...
        public init(
            trackID: UInt32? = nil, sampleDescriptionIndex: UInt32? = nil,
            defaultSampleDuration: UInt32? = nil, defaultSampleSize: UInt32? = nil,
            defaultSampleFlags: UInt32? = nil, baseDataOffset: UInt64? = nil,
            dataCursor: UInt64? = nil, nextDecodeTime: UInt64? = nil, baseDecodeTime: UInt64? = nil,
            ...
            runIndex: UInt32 = 0, nextSampleNumber: UInt64 = 1
        ) {
    ...
    private static let defaultEditListEnvironmentProvider:
        @Sendable (_ header: BoxHeader, _ reader: RandomAccessReader) -> EditListEnvironment = {
            _, _ in EditListEnvironment()
        }

    @TaskLocal private static var editListEnvironmentProviderOverride:
        (@Sendable (BoxHeader, RandomAccessReader) -> EditListEnvironment)?

    static func resolveEditListEnvironment(header: BoxHeader, reader: RandomAccessReader)
        -> EditListEnvironment
    {
        if let override = editListEnvironmentProviderOverride { return override(header, reader) }
    ...
    private static let defaultMetadataEnvironmentProvider:
        @Sendable (_ header: BoxHeader, _ reader: RandomAccessReader) -> MetadataEnvironment = {
            _, _ in MetadataEnvironment()
        }

    @TaskLocal private static var metadataEnvironmentProviderOverride:
        (@Sendable (BoxHeader, RandomAccessReader) -> MetadataEnvironment)?

    static func resolveMetadataEnvironment(header: BoxHeader, reader: RandomAccessReader)
        -> MetadataEnvironment
    {
        if let override = metadataEnvironmentProviderOverride { return override(header, reader) }
    ...
    private static let defaultFragmentEnvironmentProvider:
        @Sendable (_ header: BoxHeader, _ reader: RandomAccessReader) -> FragmentEnvironment = {
            _, _ in FragmentEnvironment()
        }

    @TaskLocal private static var fragmentEnvironmentProviderOverride:
        (@Sendable (BoxHeader, RandomAccessReader) -> FragmentEnvironment)?

    static func resolveFragmentEnvironment(header: BoxHeader, reader: RandomAccessReader)
        -> FragmentEnvironment
    {
        if let override = fragmentEnvironmentProviderOverride { return override(header, reader) }
    ...
    private static let defaultRandomAccessEnvironmentProvider:
        @Sendable (_ header: BoxHeader, _ reader: RandomAccessReader) -> RandomAccessEnvironment = {
            _, _ in RandomAccessEnvironment()
        }

    @TaskLocal private static var randomAccessEnvironmentProviderOverride:
        (@Sendable (BoxHeader, RandomAccessReader) -> RandomAccessEnvironment)?

    static func resolveRandomAccessEnvironment(header: BoxHeader, reader: RandomAccessReader)
    ...
    public func registering(parser: @escaping Parser, forExtendedType uuid: UUID)
        -> BoxParserRegistry
    {
        var copy = self
