...
public struct BoxParserRegistry: Sendable {
    ...
    private static func resolveEditListEnvironment(
        header: BoxHeader,
    ...
    private enum DefaultParsers {
        ...
        static func sampleSize(header: BoxHeader, reader: RandomAccessReader) throws -> ParsedBoxPayload? {
            guard let fullHeader = try FullBoxReader.read(header: header, reader: reader) else { return nil }
        ...
        static func compactSampleSize(header: BoxHeader, reader: RandomAccessReader) throws -> ParsedBoxPayload? {
            guard let fullHeader = try FullBoxReader.read(header: header, reader: reader) else { return nil }
        ...
        static func dataReference(header: BoxHeader, reader: RandomAccessReader) throws -> ParsedBoxPayload? {
            ...
            fields.append(ParsedBoxPayload.Field(
                name: "entry_count",
                value: String(entryCount),
                description: "Number of data reference entries",
                byteRange: countRange
            ))
            cursor = countEnd
            ...
            while index < entryCount {
                ...
                if declaredSize == 1 {
                    ...
                    guard largeSizeOffset + 8 <= end,
                          let largeSize = try readUInt64(reader, at: largeSizeOffset, end: end) else {
                ...
                fields.append(ParsedBoxPayload.Field(
                    ...
                    description: "Data reference entry type",
                    byteRange: typeRange
                ))
                ...
                fields.append(ParsedBoxPayload.Field(
                    name: "entries[\(index)].version",
                    value: String(entryVersion),
                    description: "Entry version",
                    byteRange: versionOffset..<(versionOffset + 1)
                ))

                fields.append(ParsedBoxPayload.Field(
                    name: "entries[\(index)].flags",
                    value: String(format: "0x%06X", entryFlags),
                    description: "Entry flags",
                    byteRange: (versionOffset + 1)..<(versionOffset + 4)
                ))
                ...
                fields.append(ParsedBoxPayload.Field(
                    ...
                    description: "Indicates media data is stored in the same file",
                    byteRange: nil
                ))

                let payloadStart = versionOffset + 4
                let payloadLength = max(Int64(0), entryEnd - payloadStart)
                let payloadRange = payloadLength > 0 ? payloadStart..<entryEnd : payloadStart..<payloadStart
                if payloadLength > 0 {
                    fields.append(ParsedBoxPayload.Field(
                        name: "entries[\(index)].payload_length",
                        value: String(payloadLength),
                        description: "Bytes in location payload",
                ...
                if payloadLength == 0 {
                    payloadData = Data()
                          ...
                          let data = try readData(reader, at: payloadStart, count: Int(payloadLength), end: entryEnd) {
                    payloadData = data
                } else {
                ...
                entries.append(ParsedBoxPayload.DataReferenceBox.Entry(
                    index: index,
                    type: entryType,
                    version: entryVersion,
                    flags: entryFlags,
                    location: locationResult.location,
                    byteRange: entryFieldRange,
            ...
            let detail: ParsedBoxPayload.Detail?
            if UInt32(entries.count) == entryCount {
                detail = .dataReference(ParsedBoxPayload.DataReferenceBox(
                    version: fullHeader.version,
                    flags: fullHeader.flags,
                    entryCount: entryCount,
                    entries: entries
        ...
        private static func normalizedSeconds(for value: UInt64, timescale: UInt32?) -> Double? {
            guard let timescale, timescale != 0 else { return nil }
            return Double(value) / Double(timescale)
        }

        private static func normalizedSeconds(forSigned value: Int64, timescale: UInt32?) -> Double? {
            guard let timescale, timescale != 0 else { return nil }
            return Double(value) / Double(timescale)
        }
        ...
        private static func formatMediaRate(_ value: Double) -> String {
