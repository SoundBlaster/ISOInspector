...
public struct StreamingBoxWalker: Sendable {
  ...
  public func walk(
    ...
    while let index = stack.indices.last {
      ...
      switch headerResult {
      case .success(let decodedHeader):
...
extension StreamingBoxWalker {
  ...
  fileprivate struct Frame {
    ...
    init(
      header: BoxHeader?, range: Range<Int64>, cursor: Int64, depth: Int, shouldParseChildren: Bool
    ) {
      self.header = header
      self.range = range
      self.cursor = cursor
  ...
  fileprivate static func initialCursor(for header: BoxHeader, payloadRange: Range<Int64>) -> Int64
  {
    guard payloadRange.lowerBound < payloadRange.upperBound else {
  ...
  fileprivate static func hex(_ value: Int64) -> String {
