...
extension BoxParserRegistry.DefaultParsers {
        ...
        -> ParsedBoxPayload?
    {
        ...
        if fullHeader.version == 1 {
            ...
            fields.append(
                ParsedBoxPayload.Field(
                    ...
                    description: "Media creation timestamp", byteRange: cursor..<(cursor + 8)))
            cursor += 8
            ...
            fields.append(
                ParsedBoxPayload.Field(
                    name: "timescale", value: String(timescale),
                    description: "Media time units per second", byteRange: cursor..<(cursor + 4)))
            cursor += 4

            guard let duration = try readUInt64(reader, at: cursor, end: end) else { return nil }
            fields.append(
                ParsedBoxPayload.Field(
                    name: "duration", value: String(duration), description: "Media duration",
            ...
            fields.append(
                ParsedBoxPayload.Field(
                    name: "creation_time", value: String(creation),
                    description: "Media creation timestamp", byteRange: cursor..<(cursor + 4)))
            cursor += 4
        ...
        let languageValue =
            decodeISO639Language(languageRaw) ?? String(format: "0x%04X", languageRaw)
        fields.append(
            ParsedBoxPayload.Field(
                name: "language", value: languageValue, description: "ISO-639-2/T language code",
        ...
        -> ParsedBoxPayload?
    {
        ...
        fields.append(
            ParsedBoxPayload.Field(
                name: "balance", value: String(format: "%.2f", balance),
                description: "Audio channel balance (center = 0)", byteRange: balanceRange))
        fields.append(
            ParsedBoxPayload.Field(
                name: "balance_raw", value: String(Int(balanceRaw)),
                description: "Raw signed 8.8 balance value", byteRange: balanceRange))
        cursor += 2

        guard let reserved = try readUInt16(reader, at: cursor, end: end) else { return nil }
        fields.append(
            ParsedBoxPayload.Field(
                name: "reserved", value: String(reserved), description: "Reserved field",
                byteRange: cursor..<(cursor + 2)))

        let detail = ParsedBoxPayload.SoundMediaHeaderBox(
            version: fullHeader.version, flags: fullHeader.flags, balance: balance,
        ...
        -> ParsedBoxPayload?
    {
        ...
        fields.append(
            ParsedBoxPayload.Field(
                name: "graphics_mode", value: graphicsValue,
                description: "Video transfer/compositing mode", byteRange: graphicsRange))
        fields.append(
            ParsedBoxPayload.Field(
                name: "graphics_mode_raw", value: String(graphicsMode),
                description: "Raw graphics mode value", byteRange: graphicsRange))
        cursor += 2
        ...
        for entry in entries {
            ...
            fields.append(
                ParsedBoxPayload.Field(
                    name: entry.name, value: String(format: "%.4f", normalized),
                    description: entry.description, byteRange: range))
            fields.append(
                ParsedBoxPayload.Field(
                    name: "\(entry.name)_raw", value: String(value),
                    description: "Raw \(entry.name) value", byteRange: range))
            components.append(
                ParsedBoxPayload.VideoMediaHeaderBox.OpcolorComponent(
                    raw: value, normalized: normalized))
            cursor += 2
        }
        ...
        let detail = ParsedBoxPayload.VideoMediaHeaderBox(
            version: fullHeader.version, flags: fullHeader.flags, graphicsMode: graphicsMode,
            graphicsModeDescription: graphicsDescription, opcolor: opcolor)
