...
extension BoxParserRegistry.DefaultParsers {
        ...
        -> ParsedBoxPayload?
    {
        ...
        func appendTruncationStatus(_ missingField: String) {
            ...
            if cursor < end { range = cursor..<end } else { range = nil }
            fields.append(
                ParsedBoxPayload.Field(
                    name: "entries[\(index)].status", value: "truncated",
                    description: "Entry truncated before \(missingField) field", byteRange: range))
        }
        ...
        while index < entryCount, cursor < end {
            ...
            fields.append(
                ParsedBoxPayload.Field(
                    name: "entries[\(index)].first_chunk", value: String(firstChunk),
            ...
            fields.append(
                ParsedBoxPayload.Field(
                    name: "entries[\(index)].samples_per_chunk", value: String(samplesPerChunk),
                    description: "Number of samples in each chunk", byteRange: samplesRange))
            ...
            entries.append(
                ParsedBoxPayload.SampleToChunkBox.Entry(
                    firstChunk: firstChunk, samplesPerChunk: samplesPerChunk,
                    sampleDescriptionIndex: sampleDescriptionIndex, byteRange: entryRange))
        ...
        -> ParsedBoxPayload?
    {
        ...
        fields.append(
            ParsedBoxPayload.Field(
                name: "entry_count", value: String(entryCount),
                description: "Number of decoding time entries", byteRange: cursor..<countEnd))
        cursor = countEnd
        ...
        while index < entryCount, cursor < end {
            ...
            fields.append(
                ParsedBoxPayload.Field(
                    name: "entries[\(index)].sample_count", value: String(sampleCount),
            ...
            fields.append(
                ParsedBoxPayload.Field(
                    name: "entries[\(index)].sample_delta", value: String(sampleDelta),
            ...
            entries.append(
                ParsedBoxPayload.DecodingTimeToSampleBox.Entry(
                    index: index, sampleCount: sampleCount, sampleDelta: sampleDelta,
        ...
        -> ParsedBoxPayload?
    {
        ...
        fields.append(
            ParsedBoxPayload.Field(
                name: "entry_count", value: String(entryCount),
                description: "Number of composition offset entries", byteRange: cursor..<countEnd))
        cursor = countEnd
        ...
        while index < entryCount, cursor < end {
            ...
            if fullHeader.version == 0 {
                ...
                fields.append(
                    ParsedBoxPayload.Field(
                        name: "entries[\(index)].sample_offset", value: String(raw),
                        description: "Composition time offset", byteRange: offsetRange))
                ...
                fields.append(
                    ParsedBoxPayload.Field(
                        name: "entries[\(index)].sample_offset", value: String(signed),
                        description: "Composition time offset", byteRange: offsetRange))
                offsetValue = signed
            }
            cursor = offsetEnd
            ...
            entries.append(
                ParsedBoxPayload.CompositionOffsetBox.Entry(
                    index: index, sampleCount: sampleCount, sampleOffset: sampleOffset,
                    byteRange: entryRange))
    ...
    @Sendable private static func parseChunkOffsets(
        header: BoxHeader, reader: RandomAccessReader, width: ParsedBoxPayload.ChunkOffsetBox.Width
        ...
        fields.append(
            ParsedBoxPayload.Field(
                name: "entry_count", value: String(entryCount),
                description: "Number of chunk offsets", byteRange: cursor..<countEnd))
        cursor = countEnd
        ...
        func appendTruncationStatus(_ index: UInt32) {
            fields.append(
                ParsedBoxPayload.Field(
                    name: "entries[\(index)].status", value: "truncated",
                    description: "Entry truncated before chunk offset completed", byteRange: nil))
        }
        ...
        while index < entryCount {
            ...
            case .bits64: offsetValue = try readUInt64(reader, at: cursor, end: end)
            }
            ...
            fields.append(
                ParsedBoxPayload.Field(
                    name: "entries[\(index)].chunk_offset", value: formattedValue,
                    description: width == .bits32
                        ? "Chunk offset (32-bit)" : "Chunk offset (64-bit)", byteRange: entryRange))

            entries.append(
                ParsedBoxPayload.ChunkOffsetBox.Entry(
                    index: index, offset: offset, byteRange: entryRange))
        ...
        -> ParsedBoxPayload?
    {
        ...
        fields.append(
            ParsedBoxPayload.Field(
                name: "entry_count", value: String(entryCount),
                description: "Number of sync sample entries", byteRange: countRange))
        cursor = countEnd
        ...
        while index < entryCount {
            ...
            let entryEnd = entryEndResult.partialValue
            guard entryEnd <= end, let sampleNumber = try readUInt32(reader, at: cursor, end: end)
            else {
                appendTruncationStatus(index)
                break
            }
            ...
            fields.append(
                ParsedBoxPayload.Field(
                    ...
                    description: "Sync sample number (1-based)", byteRange: byteRange))
            entries.append(
                ParsedBoxPayload.SyncSampleTableBox.Entry(
                    index: index, sampleNumber: sampleNumber, byteRange: byteRange))
            cursor = entryEnd
            if cursor <= entryStart { break }
            index += 1
        }
