...
extension BoxParserRegistry.DefaultParsers {
    @Sendable static func movieExtends(header: BoxHeader, reader _: RandomAccessReader) throws
        -> ParsedBoxPayload?
    {
    ...
    @Sendable static func trackExtends(header: BoxHeader, reader: RandomAccessReader) throws
        -> ParsedBoxPayload?
    {
        ...
        guard
            let defaultSampleDescriptionIndex = try readUInt32(reader, at: cursor, end: payloadEnd)
        else { return nil }
        fields.append(
            ParsedBoxPayload.Field(
        ...
        guard let defaultSampleDuration = try readUInt32(reader, at: cursor, end: payloadEnd) else {
            return nil
        }
        fields.append(
            ParsedBoxPayload.Field(
                name: "default_sample_duration", value: String(defaultSampleDuration),
                description: "Default sample duration in timescale units",
                byteRange: cursor..<(cursor + 4)))
        cursor += 4

        guard let defaultSampleSize = try readUInt32(reader, at: cursor, end: payloadEnd) else {
            return nil
        }
        fields.append(
            ParsedBoxPayload.Field(
                name: "default_sample_size", value: String(defaultSampleSize),
                description: "Default sample size in bytes", byteRange: cursor..<(cursor + 4)))
        cursor += 4

        guard let defaultSampleFlags = try readUInt32(reader, at: cursor, end: payloadEnd) else {
            return nil
        }
        fields.append(
            ParsedBoxPayload.Field(
                name: "default_sample_flags", value: String(format: "0x%08X", defaultSampleFlags),
