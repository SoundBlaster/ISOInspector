...
extension BoxParserRegistry.DefaultParsers {
    ...
    -> ParsedBoxPayload?
  {
    ...
    for index in 0..<entryCount {
      ...
      guard let namespaceData = try readData(reader, at: cursor, count: 4, end: payloadEnd) else {
        break
      }
    ...
    -> ParsedBoxPayload?
  {
    ...
    while cursor + 8 <= payloadEnd {
      ...
      guard let identifierData = try readData(reader, at: typeOffset, count: 4, end: payloadEnd)
      else { break }
      ...
      while valueCursor + 8 <= entryEnd {
        guard let childSize = try readUInt32(reader, at: valueCursor, end: entryEnd), childSize >= 8
        else { break }
        ...
        if childType.rawValue == "data" {
          ...
          guard let locale = try readUInt32(reader, at: contentStart + 4, end: childEnd) else {
            break
          }
