...
extension BoxParserRegistry.DefaultParsers {
    @Sendable static func userData(header: BoxHeader, reader _: RandomAccessReader) throws
        -> ParsedBoxPayload?
    {
    ...
    @Sendable static func metadata(header: BoxHeader, reader: RandomAccessReader) throws
        -> ParsedBoxPayload?
    {
        ...
        guard reservedStart + 4 <= payloadEnd,
            let reserved = try readUInt32(reader, at: reservedStart, end: payloadEnd)
        else { return nil }
        let reservedRange = reservedStart..<(reservedStart + 4)
        fields.append(
            ParsedBoxPayload.Field(
                name: "reserved", value: String(format: "0x%08X", reserved),
                description: "Reserved value", byteRange: reservedRange))

        let detail = ParsedBoxPayload.Detail.metadata(
            ParsedBoxPayload.MetadataBox(
                version: fullHeader.version, flags: fullHeader.flags, reserved: reserved))

        return ParsedBoxPayload(fields: fields, detail: detail)
    }

    @Sendable static func metadataKeys(header: BoxHeader, reader: RandomAccessReader) throws
        -> ParsedBoxPayload?
    {
        guard let fullHeader = try FullBoxReader.read(header: header, reader: reader) else {
            return nil
        ...
        guard cursor + 4 <= payloadEnd,
            ...
            let entryCount = Int(exactly: entryCountRaw)
        else { return nil }
        let countRange = cursor..<(cursor + 4)
        fields.append(
            ParsedBoxPayload.Field(
                name: "entry_count", value: String(entryCountRaw),
                description: "Number of metadata key entries", byteRange: countRange))
        cursor += 4
        ...
        for index in 0..<entryCount {
            guard cursor + 4 <= payloadEnd,
                let keySize = try readUInt32(reader, at: cursor, end: payloadEnd)
            else { break }
            ...
            guard entrySize >= 4 else {
                fields.append(
                    ParsedBoxPayload.Field(
                        name: "entries[\(index)].status", value: "invalid",
                        description: "Entry shorter than namespace field", byteRange: nil))
                break
            }
            ...
            guard entryEnd <= payloadEnd else {
                fields.append(
                    ParsedBoxPayload.Field(
                        name: "entries[\(index)].status", value: "truncated",
                        description: "Entry exceeds available payload", byteRange: nil))
                break
            }

            guard let namespaceData = try readData(reader, at: cursor, count: 4, end: payloadEnd)
            else { break }
            let namespaceRange = cursor..<(cursor + 4)
            cursor += 4
            let nameLength = Int(entrySize) - 4
            guard nameLength >= 0,
                let nameData = try readData(reader, at: cursor, count: nameLength, end: payloadEnd)
            else { break }
            let nameRange = cursor..<(cursor + Int64(nameLength))
            cursor += Int64(nameLength)

            let namespace =
                String(data: namespaceData, encoding: .ascii) ?? bytesToHex(namespaceData)
            let name = String(data: nameData, encoding: .utf8) ?? bytesToHex(nameData)

            fields.append(
                ParsedBoxPayload.Field(
                    name: "entries[\(index)].size", value: String(keySize),
                    description: "Entry size", byteRange: sizeRange))
            fields.append(
                ParsedBoxPayload.Field(
                    name: "entries[\(index)].namespace", value: namespace,
                    description: "Metadata key namespace", byteRange: namespaceRange))
            fields.append(
                ParsedBoxPayload.Field(
                    name: "entries[\(index)].name", value: name, description: "Metadata key name",
                    byteRange: nameRange))

            entries.append(
                ParsedBoxPayload.MetadataKeyTableBox.Entry(
                    index: UInt32(index + 1), namespace: namespace, name: name))
        }

        let detail = ParsedBoxPayload.Detail.metadataKeyTable(
            ParsedBoxPayload.MetadataKeyTableBox(
                version: fullHeader.version, flags: fullHeader.flags, entries: entries))

        return ParsedBoxPayload(fields: fields, detail: detail)
    }

    @Sendable static func metadataItemList(header: BoxHeader, reader: RandomAccessReader) throws
        -> ParsedBoxPayload?
    {
        let payloadStart = header.payloadRange.lowerBound
        let payloadEnd = header.payloadRange.upperBound
        ...
        var fields: [ParsedBoxPayload.Field] = []
        let environment = BoxParserRegistry.resolveMetadataEnvironment(
            header: header, reader: reader)

        if let handler = environment.handlerType {
            fields.append(
                ParsedBoxPayload.Field(
                    name: "handler_type", value: handler.rawValue,
                    description: "Owning metadata handler"))
            if let category = handler.category?.displayName {
                fields.append(
                    ParsedBoxPayload.Field(
                        name: "handler_category", value: category, description: "Handler category"))
            }
        }
        ...
        while cursor + 8 <= payloadEnd {
            ...
            guard
                let identifierData = try readData(reader, at: typeOffset, count: 4, end: payloadEnd)
            else { break }
            ...
            fields.append(
                ParsedBoxPayload.Field(
                    name: "entries[\(entryIndex)].identifier", value: displayIdentifier(identifier),
                    description: "Metadata entry identifier", byteRange: identifierRange))

            if let namespace {
                fields.append(
                    ParsedBoxPayload.Field(
                        name: "entries[\(entryIndex)].namespace", value: namespace,
            ...
            if let name {
                fields.append(
                    ParsedBoxPayload.Field(
                        name: "entries[\(entryIndex)].name", value: name, description: "Entry name")
                )
            }
            ...
            while valueCursor + 8 <= entryEnd {
                guard let childSize = try readUInt32(reader, at: valueCursor, end: entryEnd),
                    childSize >= 8
                else { break }
                let childEnd = valueCursor + Int64(childSize)
                guard childEnd <= entryEnd,
                    let childType = try readFourCC(reader, at: valueCursor + 4, end: entryEnd)
                else { break }
                ...
                if childType.rawValue == "data" {
                    ...
                    guard let locale = try readUInt32(reader, at: contentStart + 4, end: childEnd)
                    else { break }
                    ...
                    fields.append(
                        ParsedBoxPayload.Field(
                            ...
                            value: kindAndValue.typeDescription, description: "Value type"))
                    fields.append(
                        ParsedBoxPayload.Field(
                            name: "entries[\(entryIndex)].values[\(valueIndex)].raw_type",
                            value: String(format: "0x%06X", rawType), description: "Raw data type"))
                    fields.append(
                        ParsedBoxPayload.Field(
                            name: "entries[\(entryIndex)].values[\(valueIndex)].value",
                            value: valueString, description: "Decoded value",
                    ...
                    values.append(
                        ParsedBoxPayload.MetadataItemListBox.Entry.Value(
                            kind: valueKind, rawType: rawType, locale: locale))
                    valueIndex += 1
                }
            ...
            entries.append(
                ParsedBoxPayload.MetadataItemListBox.Entry(
                    identifier: identifier, namespace: namespace, name: name, values: values))
        ...
        fields.append(
            ParsedBoxPayload.Field(
                name: "entry_count", value: String(entries.count),
        ...
        let detail = ParsedBoxPayload.Detail.metadataItemList(
            ParsedBoxPayload.MetadataItemListBox(
                handlerType: environment.handlerType, entries: entries))
...
extension BoxParserRegistry.DefaultParsers {
    fileprivate static func decodeMetadataIdentifier(_ raw: UInt32) -> String? {
    ...
    fileprivate static func displayIdentifier(
        _ identifier: ParsedBoxPayload.MetadataItemListBox.Entry.Identifier
    ) -> String {
        switch identifier {
        case .fourCC(let raw, let display):
            if display.isEmpty { return String(format: "0x%08X", raw) }
            return display
        case .keyIndex(let index): return "key[\(index)]"
        case .raw(let value): return String(format: "0x%08X", value)
        }
    }
    ...
    private static func decodeMetadataValue(type: UInt32, data: Data) -> MetadataValueDescription {
        ...
        case 1:
            if let string = String(data: data, encoding: .utf8) {
                return MetadataValueDescription(
                    kind: .utf8(string), display: string, typeDescription: "UTF-8")
            }
        case 2:
            if let string = String(data: data, encoding: .utf16BigEndian) {
                return MetadataValueDescription(
                    kind: .utf16(string), display: string, typeDescription: "UTF-16")
            }
        case 21:
            if let value = parseSignedInteger(from: data) {
                return MetadataValueDescription(
                    kind: .integer(value), display: String(value), typeDescription: "Integer")
            }
        case 22:
            if let value = parseUnsignedInteger(from: data) {
                return MetadataValueDescription(
                    kind: .unsignedInteger(value), display: String(value),
                    typeDescription: "Unsigned Integer")
            }
        case 23:
            if let value = parseFloat32(from: data) {
                return MetadataValueDescription(
                    kind: .float32(value), display: formatFloatingPoint(value),
                    typeDescription: "Float32")
            }
        case 24:
            if let value = parseFloat64(from: data) {
                return MetadataValueDescription(
                    kind: .float64(value), display: formatFloatingPoint(value),
                    typeDescription: "Float64")
            }
        case 27:
            if let value = parseBoolean(from: data) {
                return MetadataValueDescription(
                    kind: .boolean(value), display: value ? "true" : "false",
                    typeDescription: "Boolean")
            }
        case 12: if isGIFData(data) { return describeDataValue(format: .gif, data: data) }
        case 13: return describeDataValue(format: .jpeg, data: data)
        case 14: return describeDataValue(format: .png, data: data)
        case 15: return describeDataValue(format: .bmp, data: data)
        case 16: if isTIFFData(data) { return describeDataValue(format: .tiff, data: data) }
        case 65:
            if let fixed = parseSignedFixedPoint8_8(from: data) {
                return MetadataValueDescription(
                    kind: .signedFixedPoint(fixed), display: formatFixedPoint(fixed),
        ...
        return MetadataValueDescription(
            kind: .bytes(data), display: hex, typeDescription: String(format: "type=0x%06X", type))
    }

    fileprivate static func bytesToHex(_ data: Data) -> String {
        if data.isEmpty { return "" }
        return data.map { String(format: "%02X", $0) }.joined(separator: " ")
    }

    fileprivate static func parseSignedInteger(from data: Data) -> Int64? {
        guard !data.isEmpty, data.count <= MemoryLayout<Int64>.size else { return nil }
        var value: Int64 = 0
        for byte in data { value = (value << 8) | Int64(UInt8(byte)) }
    ...
    fileprivate static func parseUnsignedInteger(from data: Data) -> UInt64? {
    ...
    private static func describeDataValue(
        format: ParsedBoxPayload.MetadataItemListBox.Entry.Value.DataFormat, data: Data
    ) -> MetadataValueDescription {
        let display = "\(format.valuePrefix) (\(data.count) bytes)"
        return MetadataValueDescription(
            kind: .data(format: format, data: data), display: display,
            typeDescription: format.displayName)
    }

    private static func inferImageFormat(from data: Data) -> ParsedBoxPayload.MetadataItemListBox
        .Entry.Value.DataFormat?
    {
        if isGIFData(data) { return .gif }
        if isJPEGData(data) { return .jpeg }
        if isPNGData(data) { return .png }
        if isBMPData(data) { return .bmp }
    ...
    private static func isBMPData(_ data: Data) -> Bool { data.starts(with: Data([0x42, 0x4D])) }
    ...
    private static func parseSignedFixedPoint8_8(from data: Data) -> ParsedBoxPayload
        .MetadataItemListBox.Entry.Value.SignedFixedPoint?
    {
        ...
        return ParsedBoxPayload.MetadataItemListBox.Entry.Value.SignedFixedPoint(
            value: value, rawValue: Int32(raw), format: .s8_8)
    }

    private static func parseSignedFixedPoint16_16(from data: Data) -> ParsedBoxPayload
        .MetadataItemListBox.Entry.Value.SignedFixedPoint?
    {
