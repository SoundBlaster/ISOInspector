...
public struct BoxNode: Equatable, Swift.Sendable {
    ...
    public init(
        header: BoxHeader, metadata: BoxDescriptor? = nil, payload: ParsedBoxPayload? = nil,
        validationIssues: [ValidationIssue] = [], issues: [ParseIssue] = [],
        status: Status = .valid, children: [BoxNode] = []
    ) {
        self.header = header
...
extension BoxNode: Identifiable {
    ...
    public var category: BoxCategory { BoxClassifier.category(for: header, metadata: metadata) }

    public var isStreamingIndicator: Bool { BoxClassifier.isStreamingIndicator(header: header) }
}
