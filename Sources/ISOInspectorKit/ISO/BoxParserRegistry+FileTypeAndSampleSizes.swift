...
extension BoxParserRegistry.DefaultParsers {
    @Sendable static func fileType(header: BoxHeader, reader: RandomAccessReader) throws
        -> ParsedBoxPayload?
    {
        ...
        if let major {
            fields.append(
                ParsedBoxPayload.Field(
                    name: "major_brand", value: major.rawValue,
        ...
        if let minor = minorVersion {
            fields.append(
                ParsedBoxPayload.Field(
                    name: "minor_version", value: String(minor),
                    description: "File type minor version", byteRange: (start + 4)..<(start + 8)))
        }

        var offset = start + 8
        var index = 0
        while offset + 4 <= payloadRange.upperBound {
            guard let brand = try readFourCC(reader, at: offset, end: payloadRange.upperBound)
            else { break }
            let range = offset..<(offset + 4)
            fields.append(
                ParsedBoxPayload.Field(
                    name: "compatible_brand[\(index)]", value: brand.rawValue,
                    description: "Brand compatibility entry", byteRange: range))
        ...
        if let major, let minor = minorVersion {
            detail = .fileType(
                ParsedBoxPayload.FileTypeBox(
                    majorBrand: major, minorVersion: minor, compatibleBrands: compatibleBrands))
        } else {
            detail = nil
        }
    ...
    @Sendable static func sampleSize(header: BoxHeader, reader: RandomAccessReader) throws
        -> ParsedBoxPayload?
    {
        ...
        guard defaultEnd <= end,
            let defaultSampleSize = try readUInt32(reader, at: cursor, end: end)
        else { return nil }
        let defaultRange = cursor..<defaultEnd
        fields.append(
            ParsedBoxPayload.Field(
                name: "sample_size", value: String(defaultSampleSize),
                description: "Default sample size (0 = variable)", byteRange: defaultRange))
        cursor = defaultEnd
        ...
        guard countEnd <= end, let sampleCount = try readUInt32(reader, at: cursor, end: end) else {
            return nil
        }
        ...
        fields.append(
            ParsedBoxPayload.Field(
                ...
                description: "Number of samples in table", byteRange: countRange))
        cursor = countEnd
        ...
        func appendTruncationStatus(_ entryIndex: UInt32) {
            fields.append(
                ParsedBoxPayload.Field(
                    name: "entries[\(entryIndex)].status", value: "truncated",
                    description: "Entry truncated before size field completed", byteRange: nil))
        }
        ...
        if defaultSampleSize == 0 {
            ...
            while index < sampleCount {
                ...
                guard entryEnd <= end, let entrySize = try readUInt32(reader, at: cursor, end: end)
                else {
                    appendTruncationStatus(index)
                    break
                }
                ...
                fields.append(
                    ParsedBoxPayload.Field(
                        name: "entries[\(index)].size", value: String(entrySize),
                        description: "Sample size entry", byteRange: byteRange))
                entries.append(
                    ParsedBoxPayload.SampleSizeBox.Entry(
                        index: index, size: entrySize, byteRange: byteRange))
                cursor = entryEnd
            ...
            if UInt32(entries.count) == sampleCount {
                detail = .sampleSize(
                    ParsedBoxPayload.SampleSizeBox(
                        ...
                        defaultSampleSize: defaultSampleSize, sampleCount: sampleCount,
                        entries: entries))
            } else {
                detail = nil
            }
            ...
            let detail = ParsedBoxPayload.SampleSizeBox(
                version: fullHeader.version, flags: fullHeader.flags,
                defaultSampleSize: defaultSampleSize, sampleCount: sampleCount, entries: [])
            return ParsedBoxPayload(fields: fields, detail: .sampleSize(detail))
        }
    }

    @Sendable static func compactSampleSize(header: BoxHeader, reader: RandomAccessReader) throws
        -> ParsedBoxPayload?
    {
        guard let fullHeader = try FullBoxReader.read(header: header, reader: reader) else {
            return nil
        ...
        guard reservedEnd <= end,
            let reservedData = try readData(reader, at: cursor, count: 3, end: end)
        else { return nil }
        let reservedValue = reservedData.reduce(UInt32(0)) { ($0 << 8) | UInt32($1) }
        let reservedRange = cursor..<reservedEnd
        fields.append(
            ParsedBoxPayload.Field(
                name: "reserved", value: String(format: "0x%06X", reservedValue),
                description: "Reserved value (must be zero)", byteRange: reservedRange))
        cursor = reservedEnd
        ...
        fields.append(
            ParsedBoxPayload.Field(
                name: "field_size", value: String(fieldSize),
                description: "Bit width of each entry", byteRange: fieldSizeRange))
        cursor = fieldSizeEnd
        ...
        if fieldSize > 0, sampleCount > 0 {
            ...
            while index < sampleCount {
                ...
                fields.append(
                    ParsedBoxPayload.Field(
                        name: "entries[\(index)].size", value: String(result.value),
                        description: "Sample size entry", byteRange: byteStart..<byteEnd))
                entries.append(
                    ParsedBoxPayload.CompactSampleSizeBox.Entry(
                        index: index, size: UInt32(result.value), byteRange: byteStart..<byteEnd))
                index += 1
            }
            ...
            if actualBytes < expectedBytes { return ParsedBoxPayload(fields: fields, detail: nil) }
        }

        let detail = ParsedBoxPayload.CompactSampleSizeBox(
            version: fullHeader.version, flags: fullHeader.flags, fieldSize: fieldSize,
            sampleCount: sampleCount, entries: entries)
