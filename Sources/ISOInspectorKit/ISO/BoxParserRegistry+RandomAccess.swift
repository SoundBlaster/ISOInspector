...
extension BoxParserRegistry.DefaultParsers {
    @Sendable static func movieFragmentRandomAccess(header: BoxHeader, reader _: RandomAccessReader)
    ...
    @Sendable static func movieFragmentRandomAccessOffset(
        header: BoxHeader, reader: RandomAccessReader
    ) throws -> ParsedBoxPayload? {
        ...
        fields.append(
            ParsedBoxPayload.Field(
                name: "mfra_size", value: String(mfraSize),
        ...
        throws -> ParsedBoxPayload?
    {
        ...
        fields.append(
            ParsedBoxPayload.Field(
                name: "track_ID", value: String(trackID),
                description: "Track identifier owning the random access table",
                byteRange: cursor..<(cursor + 4)))
        cursor += 4

        guard let lengthSizes = try readUInt32(reader, at: cursor, end: payloadEnd) else {
            return nil
        }
        ...
        fields.append(
            ParsedBoxPayload.Field(
                name: "length_size_of_traf_num", value: String(trafLength),
                description: "Number of bytes used for traf_number values",
                byteRange: cursor..<(cursor + 4)))
        fields.append(
            ParsedBoxPayload.Field(
                name: "length_size_of_trun_num", value: String(trunLength),
                description: "Number of bytes used for trun_number values",
                byteRange: cursor..<(cursor + 4)))
        fields.append(
            ParsedBoxPayload.Field(
                name: "length_size_of_sample_num", value: String(sampleLength),
        ...
        fields.append(
            ParsedBoxPayload.Field(
                name: "number_of_entries", value: String(entryCount),
                description: "Number of random access entries", byteRange: cursor..<(cursor + 4)))
        cursor += 4
        ...
        func readVariableLengthUInt(
            _ reader: RandomAccessReader, at offset: Int64, length: Int, end: Int64
        ) throws -> UInt64? {
        ...
        for index in 0..<Int(entryCount) {
            ...
            if fullHeader.version == 1 {
                ...
                fields.append(
                    ParsedBoxPayload.Field(
                        name: "entries[\(index)].time", value: String(value),
                        description: "Presentation time for the entry",
                        byteRange: cursor..<(cursor + 8)))
                cursor += 8
            } else {
                guard let value32 = try readUInt32(reader, at: cursor, end: payloadEnd) else {
                    return nil
                }
                time = UInt64(value32)
                fields.append(
                    ParsedBoxPayload.Field(
                        name: "entries[\(index)].time", value: String(value32),
                        description: "Presentation time for the entry",
                        byteRange: cursor..<(cursor + 4)))
                cursor += 4
            }
            ...
            if fullHeader.version == 1 {
                ...
                fields.append(
                    ParsedBoxPayload.Field(
                        name: "entries[\(index)].moof_offset", value: String(value),
                ...
                fields.append(
                    ParsedBoxPayload.Field(
                        name: "entries[\(index)].moof_offset", value: String(value32),
                        description: "Byte offset of the referenced movie fragment",
                        byteRange: cursor..<(cursor + 4)))
                cursor += 4
            }
            ...
            guard
                let trafNumber = try readVariableLengthUInt(
                    reader, at: cursor, length: Int(trafLength), end: payloadEnd)
            else { return nil }
            fields.append(
                ParsedBoxPayload.Field(
                    name: "entries[\(index)].traf_number", value: String(trafNumber),
            ...
            guard
                let trunNumber = try readVariableLengthUInt(
                    reader, at: cursor, length: Int(trunLength), end: payloadEnd)
            else { return nil }
            fields.append(
                ParsedBoxPayload.Field(
                    name: "entries[\(index)].trun_number", value: String(trunNumber),
            ...
            guard
                let sampleNumber = try readVariableLengthUInt(
                    reader, at: cursor, length: Int(sampleLength), end: payloadEnd)
            else { return nil }
            fields.append(
                ParsedBoxPayload.Field(
                    name: "entries[\(index)].sample_number", value: String(sampleNumber),
            ...
            rawEntries.append(
                RawEntry(
                    time: time, moofOffset: moofOffset, trafNumber: trafNumber,
                    trunNumber: trunNumber, sampleNumber: sampleNumber))

            if cursor > payloadEnd { return nil }
        }

        let environment = BoxParserRegistry.resolveRandomAccessEnvironment(
            header: header, reader: reader)

        func findTrackFragment(
            in fragment: BoxParserRegistry.RandomAccessEnvironment.Fragment, matching order: UInt64
        ) -> ParsedBoxPayload.TrackFragmentBox? {
            for candidate in fragment.trackFragments {
                let candidateOrder = UInt64(candidate.order)
                if candidateOrder == order { return candidate.detail }
            }
            return nil
        }

        func findRun(in trackFragment: ParsedBoxPayload.TrackFragmentBox, matching index: UInt64)
        ...
        let entries: [ParsedBoxPayload.TrackFragmentRandomAccessBox.Entry] = rawEntries.enumerated()
            .map { offset, raw in
                let fragment = environment.fragmentsByMoofOffset[raw.moofOffset]
                let trackFragment = fragment.flatMap {
                    findTrackFragment(in: $0, matching: raw.trafNumber)
                }
                ...
                let resolvedDescriptionIndex =
                    trackFragment?.sampleDescriptionIndex ?? run?.sampleDescriptionIndex
                ...
                return ParsedBoxPayload.TrackFragmentRandomAccessBox.Entry(
                    index: UInt32(offset + 1), time: raw.time, moofOffset: raw.moofOffset,
                    trafNumber: raw.trafNumber, trunNumber: raw.trunNumber,
                    sampleNumber: raw.sampleNumber,
                    fragmentSequenceNumber: fragment?.sequenceNumber, trackID: resolvedTrackID,
                    sampleDescriptionIndex: resolvedDescriptionIndex, runIndex: resolvedRunIndex,
                    ...
                    resolvedDataOffset: sample?.dataOffset, resolvedSampleSize: sample?.sampleSize,
        ...
        let detail = ParsedBoxPayload.TrackFragmentRandomAccessBox(
            ...
            trafNumberLength: trafLength, trunNumberLength: trunLength,
            sampleNumberLength: sampleLength, entryCount: entryCount, entries: entries)
