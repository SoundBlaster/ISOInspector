...
extension BoxParserRegistry.DefaultParsers {
    static let visualSampleEntryTypes: Set<FourCharCode> = {
        let rawValues = [
            "avc1", "avc2", "avc3", "avc4", "hvc1", "hev1", "dvh1", "dvhe", "dvav", "dvvc", "av01",
            "vp08", "vp09", "encv",
        ]
    ...
    @Sendable static func sampleDescription(header: BoxHeader, reader: RandomAccessReader) throws
        -> ParsedBoxPayload?
    {
        ...
        fields.append(
            ParsedBoxPayload.Field(
                ...
                description: "Number of sample entries", byteRange: cursor..<(cursor + 4)))
        cursor += 4

        var index: UInt32 = 0
        while index < entryCount, cursor < end {
            guard
                let result = try parseSampleEntry(
                    reader: reader, startOffset: cursor, end: end, index: Int(index))
            else { break }
            fields.append(contentsOf: result.fields)
            cursor = result.nextOffset
            if cursor <= result.startOffset { break }
            index += 1
        }
    ...
    private static func parseSampleEntry(
        reader: RandomAccessReader, startOffset: Int64, end: Int64, index: Int
    ) throws -> SampleEntryParseResult? {
        guard startOffset + 8 <= end else { return nil }
        guard let declaredSize = try readUInt32(reader, at: startOffset, end: end) else {
            return nil
        }
        ...
        fields.append(
            ParsedBoxPayload.Field(
                name: "entries[\(index)].byte_length", value: String(recordedLength),
                description: "Sample entry byte length", byteRange: startOffset..<(startOffset + 4))
        )

        fields.append(
            ParsedBoxPayload.Field(
                name: "entries[\(index)].format", value: format.rawValue,
                description: "Sample entry format", byteRange: (startOffset + 4)..<(startOffset + 8)
            ))
        ...
        guard
            let dataReferenceIndex = try readUInt16(reader, at: dataReferenceOffset, end: entryEnd)
        else { return nil }
        fields.append(
            ParsedBoxPayload.Field(
                name: "entries[\(index)].data_reference_index", value: String(dataReferenceIndex),
        ...
        if visualSampleEntryTypes.contains(format) {
            fields.append(
                contentsOf: parseVisualSampleEntry(
                    reader: reader, contentStart: contentStart, entryEnd: entryEnd, index: index))
        } else if audioSampleEntryTypes.contains(format) {
            fields.append(
                contentsOf: parseAudioSampleEntry(
                    reader: reader, contentStart: contentStart, entryEnd: entryEnd, index: index))
        }

        guard let headerLength = baseHeaderLength else {
            return SampleEntryParseResult(
                fields: fields, nextOffset: entryEnd, startOffset: startOffset)
        }
        ...
        if protectedSampleEntryTypes.contains(format) {
            let protection = parseProtectedSampleEntry(
                reader: reader, boxes: childBoxes, entryIndex: index)
            effectiveFormat = protection.originalFormat ?? format
            fields.append(contentsOf: protection.fields)
        }

        fields.append(
            contentsOf: parseCodecSpecificFields(
                format: effectiveFormat, boxes: childBoxes, reader: reader, entryIndex: index))
