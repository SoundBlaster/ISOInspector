...
public struct BookmarkPersistenceStore: Sendable {
    public struct Record: Codable, Equatable, Identifiable, Sendable {
        ...
        public init(
            id: UUID, canonicalURL: String, bookmarkData: Data, createdAt: Date, updatedAt: Date,
            lastResolvedAt: Date? = nil, lastResolutionState: ResolutionState = .unknown
        ) {
            self.id = id
        ...
        public var fileURL: URL? { URL(string: canonicalURL) }
    }
    ...
    public init(
        directory: URL, fileName: String = "Bookmarks.json",
    ...
    @discardableResult public func upsertBookmark(for file: URL, bookmarkData: Data) throws
        -> Record
    {
        ...
        if var existing = index[canonical] {
            ...
            let record = Record(
                id: UUID(), canonicalURL: canonical, bookmarkData: bookmarkData, createdAt: now,
                updatedAt: now, lastResolvedAt: nil, lastResolutionState: .unknown)
    ...
    @discardableResult public func markResolution(for file: URL, state: Record.ResolutionState)
        throws -> Record?
    {
        var index = try loadIndex()
        let canonical = canonicalIdentifier(for: file)
        guard var existing = index[canonical] else { return nil }
        let now = makeDate()
    ...
    private func persist(_ index: [String: Record]) throws {
        let records = index.values.sorted { lhs, rhs in
            if lhs.canonicalURL == rhs.canonicalURL { return lhs.id.uuidString < rhs.id.uuidString }
