...
public struct FilesystemAccess: Sendable {
    ...
    public init(
        openFileHandler: @escaping OpenFileHandler, saveFileHandler: @escaping SaveFileHandler,
        bookmarkCreator: @escaping BookmarkCreator, bookmarkResolver: @escaping BookmarkResolver,
    ...
    public func openFile(configuration: FilesystemOpenConfiguration = FilesystemOpenConfiguration())
    ...
    public func saveFile(configuration: FilesystemSaveConfiguration) async throws
        -> SecurityScopedURL
    {
    ...
    public func createBookmark(for scopedURL: SecurityScopedURL) throws -> Data {
        do {
            ...
            logger.log(category: .bookmarkCreate, outcome: .success, pathHash: pathHash)
            return data
        } catch {
            logger.log(
                category: .bookmarkCreate, outcome: .failure,
    ...
    public func resolveBookmarkData(_ data: Data, bookmarkIdentifier: UUID? = nil) throws
        -> ResolvedSecurityScopedURL
    {
        do {
            ...
            if resolution.isStale {
                logger.log(
                    category: .bookmarkResolve, outcome: .stale, pathHash: pathHash,
                    bookmarkIdentifier: bookmarkIdentifier)
            } else {
                logger.log(
                    category: .bookmarkResolve, outcome: .success, pathHash: pathHash,
                    bookmarkIdentifier: bookmarkIdentifier)
            }
            return ResolvedSecurityScopedURL(url: scoped, isStale: resolution.isStale)
        } catch {
            logger.log(
                category: .bookmarkResolve, outcome: .failure,
                bookmarkIdentifier: bookmarkIdentifier,
                errorDescription: sanitisedDescription(for: error))
    ...
    public func adoptSecurityScope(
        for url: URL, category: FilesystemAccessAuditEvent.Category = .externalGrant
    ) throws -> SecurityScopedURL { try activateSecurityScope(for: url, category: category) }

    private func activateSecurityScope(for url: URL, category: FilesystemAccessAuditEvent.Category)
        throws -> SecurityScopedURL
    {
        ...
        guard securityScopeManager.startAccessing(standardized) else {
            logger.log(category: category, outcome: .authorizationDenied, pathHash: pathHash)
            throw FilesystemAccessError.authorizationDenied(url: standardized)
        }
        logger.log(category: category, outcome: .accessGranted, pathHash: pathHash)
    ...
    private func sanitisedDescription(for error: Error) -> String {
        if let cocoa = error as? CocoaError { return "CocoaError.\(cocoa.code.rawValue)" }
