...
public struct FilesystemDocumentPickerPresenter: Sendable {
    ...
    #if canImport(UIKit)
        public typealias PresentingViewControllerProvider =
            @MainActor @Sendable () -> UIViewController?
    #endif

    private let openHandler: OpenHandler
    private let saveHandler: SaveHandler

    public init(openHandler: @escaping OpenHandler, saveHandler: @escaping SaveHandler) {
    ...
    extension FilesystemDocumentPickerPresenter {
        ...
        public static func uikit(
            ...
            let provider =
                presentingViewControllerProvider ?? defaultPresentingViewControllerProvider()
            return FilesystemDocumentPickerPresenter(
                openHandler: { configuration in
                    try await presentOpenPicker(
                        configuration: configuration, presentingViewControllerProvider: provider)
                },
    ...
    @MainActor private func presentPicker(
        _ picker: UIDocumentPickerViewController, dialog: FilesystemAccessError.Dialog,
        presentingViewControllerProvider: FilesystemDocumentPickerPresenter
            .PresentingViewControllerProvider
    ) async throws -> URL {
        let presenter = presentingViewControllerProvider()?.iso_topMostPresented()
        guard let presenter else { throw FilesystemAccessError.dialogUnavailable(dialog: dialog) }
    ...
    @MainActor private func presentOpenPicker(
    ...
    @MainActor private func presentSavePicker(
        ...
        defer {
            if let placeholder = payload.placeholderURL { cleanupPlaceholder(at: placeholder) }
        }
        return try await presentPicker(
            payload.picker, dialog: .save,
            presentingViewControllerProvider: presentingViewControllerProvider)
    }

    @MainActor private func makeOpenPicker(configuration: FilesystemOpenConfiguration)
        -> UIDocumentPickerViewController
    {
        let picker: UIDocumentPickerViewController
        if #available(iOS 14.0, macCatalyst 14.0, *) {
            let contentTypes =
                makeUniformTypes(from: configuration.allowedContentTypes) ?? [UTType.item]
            picker = UIDocumentPickerViewController(
                forOpeningContentTypes: contentTypes, asCopy: false)
        } else {
            let identifiers = configuration.allowedContentTypes
            picker = UIDocumentPickerViewController(
                documentTypes: identifiers.isEmpty ? ["public.item"] : identifiers, in: .open)
        }
    ...
    @MainActor private func makeSavePicker(configuration: FilesystemSaveConfiguration) throws
        -> SavePickerPayload
    {
        if #available(iOS 14.0, macCatalyst 14.0, *) {
            let contentType =
                configuration.allowedContentTypes.compactMap { UTType($0) }.first ?? UTType.data
            let filename = resolveFilename(
                suggested: configuration.suggestedFilename, contentType: contentType)
            let placeholder = try createPlaceholderFile(named: filename)
            let picker = UIDocumentPickerViewController(forExporting: [placeholder], asCopy: true)
            picker.modalPresentationStyle = UIModalPresentationStyle.formSheet
    ...
    private func resolveFilename(suggested: String?, contentType: UTType) -> String {
        ...
        guard let preferredExtension = contentType.preferredFilenameExtension else { return base }
        let existingExtension = (base as NSString).pathExtension
        if !existingExtension.isEmpty
            || base.lowercased().hasSuffix(".\(preferredExtension.lowercased())")
        {
            return base
        }
    ...
    private func createPlaceholderFile(named filename: String) throws -> URL {
        ...
        let root = manager.temporaryDirectory.appendingPathComponent(
            "FilesystemSavePlaceholders", isDirectory: true)
        ...
        try manager.createDirectory(
            at: workingDirectory, withIntermediateDirectories: true, attributes: nil)
        ...
        guard created else { throw FilesystemAccessError.dialogUnavailable(dialog: .save) }
    ...
    @MainActor private func defaultPresentingViewController() -> UIViewController? {
        UIApplication.shared.connectedScenes.compactMap { $0 as? UIWindowScene }.flatMap {
            $0.windows
        }.first(where: { $0.isKeyWindow })?.rootViewController?.iso_topMostPresented()
    }
    ...
    private final class UIDocumentPickerCoordinator: NSObject, UIDocumentPickerDelegate {
        ...
        init(dialog: FilesystemAccessError.Dialog, continuation: CheckedContinuation<URL, Error>) {
            self.dialog = dialog
        ...
        func install(on picker: UIDocumentPickerViewController) {
            ...
            objc_setAssociatedObject(
                picker, &AssociatedKeys.coordinator, self, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
        ...
        private func resume(
            with result: Result<URL, Error>, controller: UIDocumentPickerViewController
        ) {
            objc_setAssociatedObject(
                controller, &AssociatedKeys.coordinator, nil, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
    ...
    private enum AssociatedKeys { static var coordinator = 0 }

    extension UIViewController {
        fileprivate func iso_topMostPresented() -> UIViewController {
