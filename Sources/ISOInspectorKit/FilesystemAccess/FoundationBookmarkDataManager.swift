...
public struct FoundationBookmarkDataManager: BookmarkDataManaging {
    ...
    extension FoundationBookmarkDataManager {
        @usableFromInline static func defaultCreationOptions() -> URL.BookmarkCreationOptions {
            #if os(macOS)
        ...
        @usableFromInline static func defaultResolutionOptions() -> URL.BookmarkResolutionOptions {
            #if os(macOS)
                return [.withSecurityScope]
            #else
                return []
