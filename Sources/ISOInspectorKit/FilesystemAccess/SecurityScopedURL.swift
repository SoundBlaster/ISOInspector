...
public struct SecurityScopedURL: Sendable, Equatable {
    ...
    public func revoke() { handle.revoke() }

    /// Executes work while access to the underlying resource remains active.
    @discardableResult public func withAccess<T>(_ work: (URL) throws -> T) rethrows -> T {
        try work(url)
    }
}
...
extension SecurityScopedURL {
    fileprivate final class Handle: @unchecked Sendable {
        private let url: URL
