...
public struct ValidationPreset: Codable, Equatable, Sendable {
    ...
    public static func loadBundledPresets(
        bundle: Bundle? = nil,
        logger: DiagnosticsLogger? = DiagnosticsLogger(subsystem: "ISOInspectorKit", category: "ValidationPreset")
    ) throws -> [ValidationPreset] {
        let resolvedBundle = bundle ?? .module
        guard let url = resolvedBundle.url(forResource: "ValidationPresets", withExtension: "json") else {
            logger?.error("Validation preset manifest ValidationPresets.json is missing from bundle \(resolvedBundle)")
...
public extension ValidationPreset {
