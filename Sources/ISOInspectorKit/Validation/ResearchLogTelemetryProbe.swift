...
public struct ResearchLogTelemetrySnapshot: Equatable {
    public enum State: Equatable, CustomStringConvertible {
        ...
        public var description: String {
            switch self {
            case .ready(let audit): return "ready(entries: \(audit.entryCount))"
            case .missingLog: return "missingLog"
            case .emptyLog: return "emptyLog"
...
public enum ResearchLogTelemetryProbe {
    public static func capture(logURL: URL, fileManager: FileManager = .default)
        ...
        do {
            ...
            if !audit.logExists {
                return ResearchLogTelemetrySnapshot(
                    logURL: logURL, state: .missingLog(audit),
                    diagnostics: missingDiagnostics(for: logURL, audit: audit))
            }
            if audit.entryCount == 0 {
                return ResearchLogTelemetrySnapshot(
                    logURL: logURL, state: .emptyLog(audit),
                    diagnostics: emptyDiagnostics(for: logURL, audit: audit))
            }
            return ResearchLogTelemetrySnapshot(
                logURL: logURL, state: .ready(audit),
                diagnostics: readyDiagnostics(for: logURL, audit: audit))
        } catch let error as ResearchLogMonitor.Error {
            switch error {
            case .schemaMismatch(let expected, let actual):
                return ResearchLogTelemetrySnapshot(
                    logURL: logURL, state: .schemaMismatch(expected: expected, actual: actual),
                    diagnostics: schemaMismatchDiagnostics(
                        for: logURL, expected: expected, actual: actual))
            }
        } catch {
            return ResearchLogTelemetrySnapshot(
                logURL: logURL, state: .loadFailure(message: error.localizedDescription),
                diagnostics: loadFailureDiagnostics(
                    for: logURL, message: error.localizedDescription))
        }
    }
    ...
    private static func schemaMismatchDiagnostics(
        for logURL: URL, expected: [String], actual: [String]
    ) -> [String] {
        [
