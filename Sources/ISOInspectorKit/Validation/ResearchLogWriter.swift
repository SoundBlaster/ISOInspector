...
public protocol ResearchLogRecording: Sendable { func record(_ entry: ResearchLogEntry) }
...
public final class ResearchLogWriter: ResearchLogRecording, @unchecked Sendable {
    ...
    public func loadEntries() -> [ResearchLogEntry] { queue.sync { sortedEntriesLocked() } }
    ...
    private func sortedEntriesLocked() -> [ResearchLogEntry] {
        entries.sorted(by: { lhs, rhs in
            if lhs.boxType == rhs.boxType {
                if lhs.filePath == rhs.filePath { return lhs.startOffset < rhs.startOffset }
