...
final class TopLevelOrderingAdvisoryRule: BoxValidationRule, @unchecked Sendable {
    ...
    private static let allowedBetweenTypes: Set<String> = allowedPreFileTypeTypes.union(
        streamingIndicatorTypes)
    ...
    private func fileTypeIssuesIfNeeded() -> [ValidationIssue] {
        ...
        let unexpected = Set(
            typesBeforeFileType.filter { !Self.allowedPreFileTypeTypes.contains($0) })
    ...
    private func movieIssuesIfNeeded() -> [ValidationIssue] {
        ...
        let unexpected = Set(
            typesBetweenFileTypeAndMovie.filter { type in
                !Self.allowedBetweenTypes.contains(type)
                    && !MediaAndIndexBoxCode.isMediaPayload(type)
            })
    ...
    private static func describe(_ codes: Set<String>) -> String { describe(Array(codes)) }
    ...
    private static func describe(_ codes: [String]) -> String {
        ...
        if unique.count == 1 { return "\"\(first)\"" }
        if unique.count == 2 { return "\"\(first)\" and \"\(unique[1])\"" }
