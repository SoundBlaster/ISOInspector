...
final class SampleTableCorrelationRule: BoxValidationRule, @unchecked Sendable {  // swiftlint:disable:this type_body_length
        ...
        case .willStartBox(let header, let depth):
            ...
            case BoxType.sampleToChunk:
                ...
                trackStack[trackIndex].sampleToChunk = SampleToChunkState(
                    identifier: header.identifierString, box: box)
            ...
            case BoxType.sampleSize:
                ...
                trackStack[trackIndex].sampleSize = SampleSizeState(
                    identifier: header.identifierString, sampleCount: box.sampleCount)
            ...
            case BoxType.chunkOffset32, BoxType.chunkOffset64:
                ...
                trackStack[trackIndex].chunkOffsets = ChunkOffsetState(
                    identifier: header.identifierString, entries: box.entries)
                var issues = chunkOffsetOrderingIssues(
                    entries: box.entries, trackIndex: trackIndex, header: header)
                issues.append(
                    contentsOf: evaluateTrack(for: trackIndex, triggeredKind: .chunkOffsets))
                return issues
    ...
    private func chunkCorrelationIssues(for trackIndex: Int) -> [ValidationIssue] {
        ...
        guard let sampleToChunk = context.sampleToChunk, let sampleSize = context.sampleSize,
        ...
        for (index, entry) in sortedEntries.enumerated() {
            ...
            let nextStart =
                ...
                ? Int(sortedEntries[index + 1].firstChunk) : chunkCount + 1
        ...
        -> [ValidationIssue]
    {
        ...
        if triggeredKind == .sampleSize || triggeredKind == .timeToSample,
            let sampleSize = context.sampleSize, let timeToSample = context.timeToSample
        {
        ...
        if triggeredKind == .sampleSize || triggeredKind == .compositionOffset,
            let sampleSize = context.sampleSize, let composition = context.compositionOffset
        {
            let declared = UInt64(sampleSize.sampleCount)
        ...
        if triggeredKind == .timeToSample || triggeredKind == .compositionOffset,
            let timeToSample = context.timeToSample, let composition = context.compositionOffset,
    ...
    private func chunkOffsetOrderingIssues(
        entries: [ParsedBoxPayload.ChunkOffsetBox.Entry], trackIndex: Int, header: BoxHeader
    ) -> [ValidationIssue] {
