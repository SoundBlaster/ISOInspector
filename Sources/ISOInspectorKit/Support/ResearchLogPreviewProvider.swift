...
public struct ResearchLogPreviewSnapshot: Equatable {
    public enum State: Equatable, CustomStringConvertible {
        ...
        public var description: String {
            ...
            case .missingFixture: return "missingFixture"
            case .schemaMismatch: return "schemaMismatch"
            case .loadFailure(let message): return "loadFailure(\(message))"
            }
        }
    }
...
public enum ResearchLogPreviewProvider {
    private static func resolveBundle(_ bundle: Bundle?) -> Bundle { bundle ?? .module }

    private static func snapshot(resourceName: String, bundle: Bundle?, fileManager: FileManager)
        -> ResearchLogPreviewSnapshot
    {
        let resolvedBundle = resolveBundle(bundle)
        let fixtureName = "\(resourceName).json"

        guard let fixtureURL = resolvedBundle.url(forResource: resourceName, withExtension: "json")
        else {
            let audit = ResearchLogAudit(
                schemaVersion: ResearchLogSchema.version, fieldNames: ResearchLogSchema.fieldNames,
                entryCount: 0, logExists: false)

            return ResearchLogPreviewSnapshot(
                fixtureName: fixtureName, state: .missingFixture(audit),
                diagnostics: [
        ...
        do {
            ...
            return ResearchLogPreviewSnapshot(
                fixtureName: fixtureName, state: .ready(audit),
                diagnostics: [
            ...
            return ResearchLogPreviewSnapshot(
                fixtureName: fixtureName, state: .loadFailure(message: error.localizedDescription),
                diagnostics: [
    ...
    public static func validFixture(bundle: Bundle? = nil, fileManager: FileManager = .default)
        -> ResearchLogPreviewSnapshot
    { snapshot(resourceName: "VR006PreviewLog", bundle: bundle, fileManager: fileManager) }

    public static func missingFixture(bundle: Bundle? = nil, fileManager: FileManager = .default)
        -> ResearchLogPreviewSnapshot
    ...
    public static func schemaMismatchFixture(
        bundle: Bundle? = nil, fileManager: FileManager = .default
    ) -> ResearchLogPreviewSnapshot {
