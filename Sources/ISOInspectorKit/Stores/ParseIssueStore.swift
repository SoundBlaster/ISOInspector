...
public final class ParseIssueStore: ObservableObject {
    ...
    public struct IssueMetrics: Equatable, Sendable {
        ...
        public init(
            countsBySeverity: [ParseIssue.Severity: Int] = [:], deepestAffectedDepth: Int = 0
        ) {
        ...
        public init(
            errorCount: Int = 0, warningCount: Int = 0, infoCount: Int = 0,
            deepestAffectedDepth: Int = 0
        ) {
            self.init(
                countsBySeverity: [.error: errorCount, .warning: warningCount, .info: infoCount],
                deepestAffectedDepth: deepestAffectedDepth)
        }

        public var countsBySeverity: [ParseIssue.Severity: Int] { counts }

        public func count(for severity: ParseIssue.Severity) -> Int { counts[severity, default: 0] }
    ...
    public struct IssueSummary: Equatable, Sendable {
        ...
        public init(metrics: IssueMetrics = IssueMetrics(), totalCount: Int = 0) {
            self.metrics = metrics
            self.totalCount = totalCount
        }

        public var countsBySeverity: [ParseIssue.Severity: Int] { metrics.countsBySeverity }

        public var deepestAffectedDepth: Int { metrics.deepestAffectedDepth }

        public func count(for severity: ParseIssue.Severity) -> Int { metrics.count(for: severity) }
    }
    ...
    public init(issues: [ParseIssue] = [], queue: DispatchQueue = .main) {
        self.queue = queue
        ...
        self.infoCount = 0
        self.deepestDepth = 0
        if !issues.isEmpty { replaceAll(with: issues) }
    }

    public func record(_ issue: ParseIssue, depth: Int? = nil) {
        performOnQueue { [self] in self.append(issue, depth: depth) }
    }

    public func record(
        _ issues: [ParseIssue], depthResolver: (@Sendable (ParseIssue) -> Int?)? = nil
    ) {
    ...
    public func replaceAll(
        with issues: [ParseIssue], depthResolver: (@Sendable (ParseIssue) -> Int?)? = nil
    ) {
        performOnQueue { [self] in
            self.resetStorage()
            for issue in issues {
    ...
    public func metricsSnapshot() -> IssueMetrics { readOnQueue { metrics } }

    public func makeIssueSummary() -> IssueSummary {
        readOnQueue { IssueSummary(metrics: metrics, totalCount: issues.count) }
    }

    public func issuesSnapshot() -> [ParseIssue] { readOnQueue { issues } }

    public func reset() { performOnQueue { [self] in self.resetStorage() } }

    private func append(_ issue: ParseIssue, depth: Int?) {
        issues.append(issue)
        for nodeID in issue.affectedNodeIDs { issuesByNodeID[nodeID, default: []].append(issue) }
