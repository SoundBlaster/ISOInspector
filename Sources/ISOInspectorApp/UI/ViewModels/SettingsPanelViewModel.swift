    ...
    @MainActor final class SettingsPanelViewModel: ObservableObject {
        enum Section {
            case permanent
            case session
            case advanced
            ...
            do {
                ...
                if let sessionController = sessionController {
                    sessionSettings = SessionSettings(
                        ...
                        globalValidationConfiguration: sessionController
        ...
        func resetSessionSettings() async {
            ...
            guard let sessionController = sessionController else {
                logger.warning(
                    "Cannot reset session settings: DocumentSessionController not available")
                isLoading = false
                return
            }
        ...
        func selectValidationPreset(
            _ presetID: String, scope: DocumentSessionController.ValidationConfigurationScope
        ) {
            guard let sessionController = sessionController else {
                logger.warning(
                    "Cannot update validation preset: DocumentSessionController not available")
                return
            }
        ...
        func setValidationRule(
            _ rule: ValidationRuleIdentifier, isEnabled: Bool,
            scope: DocumentSessionController.ValidationConfigurationScope
        ) {
            guard let sessionController = sessionController else {
                logger.warning(
                    "Cannot update validation rule: DocumentSessionController not available")
                return
            }
    ...
    struct SessionSettings: Equatable {
        ...
        var hasOverrides: Bool { validationConfiguration != globalValidationConfiguration }
    }
#endif
