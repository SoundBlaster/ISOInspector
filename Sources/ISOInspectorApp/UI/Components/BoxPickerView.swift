...
@MainActor
public struct BoxPickerView<T: Hashable>: View {
    // MARK: - Properties
    ...
    private var resolvedPickerStyle: some PickerStyle {
        if let override = useSegmentedStyle {
            return override ? AnyPickerStyle(.segmented) : AnyPickerStyle(.menu)
        }

        // Automatic style selection based on platform and option count
        #if os(macOS)
        // macOS: Segmented for ≤4 options, menu for >4
        return options.count <= 4 ? AnyPickerStyle(.segmented) : AnyPickerStyle(.menu)
        #else
        // iOS/iPadOS: Segmented for ≤3 options, menu for >3
        return options.count <= 3 ? AnyPickerStyle(.segmented) : AnyPickerStyle(.menu)
        #endif
    }
...
// MARK: - Type-Erased Picker Style

private struct AnyPickerStyle: PickerStyle {
    private let _makeBody: (PickerStyleConfiguration) -> AnyView
...
extension PickerStyle where Self == SegmentedPickerStyle {
    fileprivate static var segmented: SegmentedPickerStyle {
        SegmentedPickerStyle()
    }
}

extension PickerStyle where Self == MenuPickerStyle {
    fileprivate static var menu: MenuPickerStyle {
        MenuPickerStyle()
    }
}
