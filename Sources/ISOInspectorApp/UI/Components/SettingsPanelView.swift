    ...
    struct SettingsPanelView: View {
        ...
        var body: some View {
            SidebarPattern(sections: sidebarSections, selection: $selectedSectionID) { sectionID in
                detailContent(for: sectionID)
            }.task { await viewModel.loadSettings() }.accessibilityIdentifier("SettingsPanelView")
        }
        ...
        private var sidebarSections: [SidebarPattern<String, AnyView>.Section] {
            [
                SidebarPattern.Section(
                    ...
                    items: [
                        SidebarPattern.Item(
                            id: "permanent", title: "Permanent", iconSystemName: "gearshape",
                            accessibilityLabel: "Permanent Settings"),
                        SidebarPattern.Item(
                            id: "session", title: "Session", iconSystemName: "doc",
        ...
        private func detailContent(for sectionID: String?) -> AnyView {
            guard let sectionID = sectionID else {
                return AnyView(Text("Select a section").foregroundColor(.secondary))
            }

            switch sectionID {
            case "permanent": return AnyView(permanentSettingsContent)
            case "session": return AnyView(sessionSettingsContent)
            case "advanced": return AnyView(advancedSettingsContent)
            default: return AnyView(Text("Unknown section").foregroundColor(.secondary))
            }
        }
                ...
                VStack(spacing: DS.Spacing.l) {
                    Card {
                        VStack(alignment: .leading, spacing: DS.Spacing.m) {
                            Text("Permanent Settings").font(DS.Typography.title)
                            Text("These settings persist across all sessions and app restarts.")
                                .font(DS.Typography.caption).foregroundColor(.secondary)

                            Divider()

                            // @todo #222 Add validation preset controls
                            // @todo #222 Add telemetry/logging verbosity controls
                            Text("Settings controls will appear here").foregroundColor(.secondary)

                            Divider()

                            Button("Reset to Defaults") { showResetPermanentConfirmation = true }
                                .buttonStyle(.bordered).accessibilityIdentifier(
                                    "ResetPermanentSettingsButton")
                        }.padding(DS.Spacing.l)
                    }
                }.padding(DS.Spacing.l)
            }.accessibilityIdentifier("PermanentSettingsContent").alert(
                "Reset Permanent Settings?", isPresented: $showResetPermanentConfirmation
            ) {
                Button("Cancel", role: .cancel) {}
                Button("Reset", role: .destructive) {
                    Task { await viewModel.resetPermanentSettings() }
                }
            } message: {
                Text(
        ...
        private var sessionSettingsContent: some View {
            ScrollView {
                VStack(spacing: DS.Spacing.l) {
                    Card {
                        VStack(alignment: .leading, spacing: DS.Spacing.m) {
                            Text("Session Settings").font(DS.Typography.title)
                            Text("These settings only affect the current document session.").font(
                            ...
                            Text("Session controls will appear here").foregroundColor(.secondary)

                            if viewModel.hasSessionOverrides {
                                HStack {
                                    Image(systemName: "exclamationmark.circle").foregroundColor(
                                        .orange)
                                    Text("Session has custom overrides").font(DS.Typography.caption)
                                        .foregroundColor(.secondary)
                                }
                            }
                            ...
                            Button("Reset to Global") { showResetSessionConfirmation = true }
                                .buttonStyle(.bordered).accessibilityIdentifier(
                                    "ResetSessionSettingsButton")
                        }.padding(DS.Spacing.l)
                    }
                }.padding(DS.Spacing.l)
            }.accessibilityIdentifier("SessionSettingsContent").alert(
                "Reset Session Settings?", isPresented: $showResetSessionConfirmation
            ) {
                Button("Cancel", role: .cancel) {}
                Button("Reset", role: .destructive) {
                    Task { await viewModel.resetSessionSettings() }
                }
            } message: {
                Text(
        ...
        private var advancedSettingsContent: some View {
            ScrollView {
                VStack(spacing: DS.Spacing.l) {
                    Card {
                        VStack(alignment: .leading, spacing: DS.Spacing.m) {
                            Text("Advanced Settings").font(DS.Typography.title)
                            Text("Advanced configuration options.").font(DS.Typography.caption)
                                .foregroundColor(.secondary)

                            Divider()

                            // @todo #222 Add advanced configuration controls
                            Text("Advanced controls will appear here").foregroundColor(.secondary)
                        }.padding(DS.Spacing.l)
                    }
                }.padding(DS.Spacing.l)
