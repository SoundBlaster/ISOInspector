    ...
    @MainActor final class NodeSelectionViewModel: ObservableObject {
        ...
        init(
            outlineViewModel: ParseTreeOutlineViewModel, detailViewModel: ParseTreeDetailViewModel,
            ...
            outlineViewModel.$rows.sink { [weak self] rows in self?.ensureSelectionVisible(in: rows)
            }.store(in: &cancellables)

            outlineViewModel.$filter.dropFirst().sink { [weak self] newFilter in
        ...
        func handleSnapshotUpdate(snapshot: ParseTreeSnapshot) {
            ...
            if let current = selectedNodeID, snapshot.containsNode(with: current) { return }

            let fallback = outlineViewModel.firstVisibleNodeID() ?? snapshot.nodes.first?.id
            if selectedNodeID != fallback { select(nodeID: fallback) }
        }

        private func ensureSelectionVisible(in rows: [ParseTreeOutlineRow]) {
            guard !rows.isEmpty else {
                if selectedNodeID != nil { select(nodeID: nil) }
                return
            }

            if let current = selectedNodeID, rows.contains(where: { $0.id == current }),
        ...
        private func rowMatchesActiveFilter(
            _ row: ParseTreeOutlineRow, filter: ParseTreeOutlineFilter
        ) -> Bool {
            if !filter.focusedSeverities.isEmpty {
                ...
                let parseMatch = row.node.issues.contains { issue in
                    ...
                    case .error: return filter.focusedSeverities.contains(.error)
                    case .warning: return filter.focusedSeverities.contains(.warning)
                    case .info: return filter.focusedSeverities.contains(.info)
                    }
                }
                if !(validationMatch || parseMatch) { return false }
            }
            ...
            if !filter.showsStreamingIndicators && row.node.isStreamingIndicator { return false }
