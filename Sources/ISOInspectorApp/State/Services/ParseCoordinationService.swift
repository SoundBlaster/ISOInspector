    ...
    @MainActor final class ParseCoordinationService {
        // MARK: - Properties
        ...
        func openDocument(
            accessContext: AccessContext, failureRecent: DocumentRecent?,
            onSuccess:
            ...
            restoredSelection: Int64? = nil, preResolvedScope: SecurityScopedURL? = nil
        ) {
            ...
            workQueue.execute {
                do {
                    ...
                    Task { @MainActor in
                        onSuccess(
                            accessContext.scopedURL, accessContext.bookmarkData,
                            accessContext.bookmarkRecord, reader, pipeline, accessContext.recent,
                            restoredSelection)
                    }
                    ...
                    Task { @MainActor in onFailure(targetRecent, accessError) }
                } catch {
                    preResolvedScope?.revoke()
                    accessContext.scopedURL.revoke()
                    let targetRecent = failureRecent ?? accessContext.recent
                    Task { @MainActor in onFailure(targetRecent, error) }
                }
            }
        }
    }
