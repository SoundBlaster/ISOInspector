    ...
    @MainActor final class RecentsService: ObservableObject {
        ...
        init(
            recentsStore: DocumentRecentsStoring, recentLimit: Int = 10,
        ...
        func insertRecent(_ recent: DocumentRecent) {
            ...
            if recents.count > recentLimit { recents = Array(recents.prefix(recentLimit)) }
            persistRecents()
        }
        ...
        func removeRecent(at offsets: IndexSet) -> Bool {
            ...
            for url in urls { removeRecent(with: url) }
        ...
        func setLastFailedRecent(_ recent: DocumentRecent?) { lastFailedRecent = recent }
        ...
        private func persistRecents() {
            do {
                ...
                let allRecents = recents.map { $0.url.standardizedFileURL.path }.joined(
                    separator: ", ")
                diagnostics.error(
