    ...
    @MainActor final class DocumentOpeningCoordinator {  // swiftlint:disable:this type_body_length
        ...
        func setLatestSelectionNodeID(_ nodeID: Int64?) { latestSelectionNodeID = nodeID }

        func openDocument(
            recent: DocumentRecent, restoredSelection: Int64? = nil,
            preResolvedScope: SecurityScopedURL? = nil, failureRecent: DocumentRecent? = nil
        ) {
            do {
                let accessContext = try bookmarkService.prepareAccess(
                    for: recent, preResolvedScope: preResolvedScope)

                openDocument(
                    accessContext: accessContext, recent: recent,
                    restoredSelection: restoredSelection, preResolvedScope: preResolvedScope,
        ...
        private func openDocument(
            accessContext: AccessContext, recent: DocumentRecent, restoredSelection: Int64?,
            preResolvedScope: SecurityScopedURL?, failureRecent: DocumentRecent?
        ) {
            parseCoordinationService.openDocument(
                accessContext: accessContext, failureRecent: failureRecent,
                onSuccess: {
                    [weak self]
                    scopedURL, bookmark, bookmarkRecord, reader, pipeline, recent, restoredSelection
                    in
                    self?.startSession(
                        scopedURL: scopedURL, bookmark: bookmark, bookmarkRecord: bookmarkRecord,
                        reader: reader, pipeline: pipeline, recent: recent,
        ...
        func applySessionSnapshot(_ snapshot: WorkspaceSessionSnapshot) -> (
            ...
            guard !sortedFiles.isEmpty else { return ([], nil) }
        ...
        func restoreSession(_ snapshot: WorkspaceSessionSnapshot) {
            ...
            openDocument(
                recent: focused.recent, restoredSelection: focused.lastSelectionNodeID,
                preResolvedScope: nil, failureRecent: focused.recent)
        }

        // MARK: - Private Methods

        private func startSession(
            scopedURL: SecurityScopedURL, bookmark: Data?,
            bookmarkRecord: BookmarkPersistenceStore.Record?, reader: RandomAccessReader,
            pipeline: ParsePipeline, recent: DocumentRecent, restoredSelection: Int64?
        ) {
            ...
            parseTreeStore.start(
                pipeline: pipeline, reader: reader, context: .init(source: standardizedURL))
            ...
            if let bookmarkRecord {
                updatedRecent = bookmarkService.applyBookmarkRecord(
                    bookmarkRecord, to: updatedRecent)
            } else {
        ...
        private func emitLoadFailure(
            ...
            let content = loadFailureContent(
                displayName: displayName, defaultSuggestion: defaultSuggestion, error: error)
            ...
            let loadFailure = DocumentLoadFailure(
                fileURL: standardizedRecent.url, fileDisplayName: displayName,
                message: content.message, recoverySuggestion: content.suggestion,
        ...
        private func loadFailureContent(
            displayName: String, defaultSuggestion: String, error: Error?
        ) -> LoadFailureContent {
            ...
            if let localizedError = error as? LocalizedError {
                ...
                if !localized.isEmpty { message = localized }
            }
        ...
        struct DocumentLoadFailure: Identifiable, Equatable {
            ...
            init(
                id: UUID = UUID(), fileURL: URL, fileDisplayName: String, message: String,
                recoverySuggestion: String, details: String?
            ) {
                self.id = id
    ...
    enum DocumentAccessError: LocalizedError {
        ...
        var failureReason: String? {
            ...
            case .unresolvedBookmark: return "The security-scoped bookmark is no longer valid."
            }
        }
