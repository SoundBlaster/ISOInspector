    ...
    @MainActor final class BookmarkService {  // swiftlint:disable:this type_body_length
        ...
        init(
            bookmarkStore: BookmarkPersistenceManaging?, filesystemAccess: FilesystemAccess,
        ...
        deinit { activeSecurityScopedURL?.revoke() }
        ...
        func prepareAccess(for recent: DocumentRecent, preResolvedScope: SecurityScopedURL?) throws
            -> AccessContext
        {
        ...
        func sanitizeRecentsForPersistence(_ recents: [DocumentRecent]) -> [DocumentRecent] {
            ...
            return recents.map { recent in
                ...
                if sanitized.bookmarkIdentifier != nil { sanitized.bookmarkData = nil }
                return sanitized
            }
        }
        ...
        private func prepareAccessUsingPreResolvedScope(
            _ scope: SecurityScopedURL, for recent: DocumentRecent
        ) -> AccessContext {
            ...
            if let bookmarkStore {
                ...
                if let record { preparedRecent = applyBookmarkRecord(record, to: preparedRecent) }
            } else {
            ...
            return AccessContext(
                scopedURL: scope, recent: preparedRecent, bookmarkRecord: record,
                bookmarkData: bookmarkData)
        }

        // swiftlint:disable:next function_body_length cyclomatic_complexity
        private func prepareAccessResolvingBookmark(for recent: DocumentRecent) throws
            -> AccessContext
        {
            var preparedRecent = recent
            let standardized = recent.url.standardizedFileURL
            ...
            if let blob = bookmarkBlob {
                    ...
                    if let bookmarkStore {
                        if resolution.isStale,
                            ...
                            let updated = try? bookmarkStore.upsertBookmark(
                                for: resolution.url.url, bookmarkData: refreshed)
                        {
                            record = updated
                        }
                        ...
                        if let updated = try? bookmarkStore.markResolution(
                            for: resolution.url.url, state: state)
                        {
                            record = updated
                        }
                ...
                if let bookmarkStore, let data = makeBookmarkData(for: resolvedScope) {
                    record = try? bookmarkStore.upsertBookmark(
                        for: resolvedScope.url, bookmarkData: data)
                } else {
            ...
            return AccessContext(
                scopedURL: resolvedScope, recent: preparedRecent, bookmarkRecord: record,
                bookmarkData: bookmarkData)
        }
    }
