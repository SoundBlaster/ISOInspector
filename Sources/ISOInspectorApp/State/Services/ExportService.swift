  ...
  final class ExportService {
    ...
    func setCurrentDocument(_ document: DocumentRecent?) {
    ...
    private func runExport(
      ...
      do {
        ...
        let status = makeSuccessStatus(operation: operation, scope: scope, destination: destination)
        onSuccess(status)
        ...
        let status = makeFailureStatus(
          error: .destinationUnavailable(underlying: error),
          operation: operation
        )
        onFailure(status)
      }
    ...
    private func makeFailureStatus(error: ExportError, operation: ExportOperation) -> ExportStatus {
      exportLogger.error(
