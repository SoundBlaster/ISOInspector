    ...
    @MainActor final class SessionPersistenceService {
        ...
        init(
            sessionStore: WorkspaceSessionStoring?, diagnostics: any DiagnosticsLogging,
        ...
        func setIsRestoringSession(_ value: Bool) { isRestoringSession = value }

        /// Persists the current session with the provided recents and annotations.
        func persistSession(
            recents: [DocumentRecent], currentDocument: DocumentRecent?, annotationsFileURL: URL?,
            ...
            let (snapshots, nextDiffs) = buildSessionSnapshots(
                recents: recents, annotationsFileURL: annotationsFileURL,
            ...
            let snapshot = WorkspaceSessionSnapshot(
                id: sessionID, createdAt: currentSessionCreatedAt ?? now, updatedAt: now,
                appVersion: Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String,
                files: snapshots, focusedFileURL: currentDocument?.url, lastSceneIdentifier: nil,
                windowLayouts: [])
        ...
        private func buildSessionSnapshots(
            recents: [DocumentRecent], annotationsFileURL: URL?, latestSelectionNodeID: Int64?,
            sessionValidationConfigurations: [String: ValidationConfiguration]
            ...
            for (index, recent) in recents.enumerated() {
                ...
                let persistedRecent =
                    bookmarkService.sanitizeRecentsForPersistence([recent]).first ?? recent
                ...
                snapshots.append(
                    WorkspaceSessionFileSnapshot(
                        id: fileID, recent: persistedRecent, orderIndex: index,
                        lastSelectionNodeID: selection, isPinned: false, scrollOffset: nil,
                        bookmarkIdentifier: recent.bookmarkIdentifier, bookmarkDiffs: diffs,
        ...
        private func ensureSessionIdentifiers(now: Date) -> UUID {
            if currentSessionCreatedAt == nil { currentSessionCreatedAt = now }
        ...
        private func save(
            snapshot: WorkspaceSessionSnapshot, using sessionStore: WorkspaceSessionStoring
        ) {
            do { try sessionStore.saveCurrentSession(snapshot) } catch {
        ...
        private func selectionNodeID(
            for recentURL: URL, annotationsFileURL: URL?, latestSelectionNodeID: Int64?
        ) -> Int64? {
            guard let currentURL = annotationsFileURL,
