    ...
    protocol BookmarkPersistenceManaging: Sendable {
        ...
        @discardableResult func upsertBookmark(for file: URL, bookmarkData: Data) throws
            -> BookmarkPersistenceStore.Record
        @discardableResult func markResolution(
            for file: URL, state: BookmarkPersistenceStore.Record.ResolutionState
        ) throws -> BookmarkPersistenceStore.Record?
        func removeBookmark(for file: URL) throws
    }

    extension BookmarkPersistenceStore: BookmarkPersistenceManaging {}

    @MainActor final class DocumentSessionController: ObservableObject {
        ...
        var validationPresets: [ValidationPreset] {
            validationConfigurationService.validationPresets
        }
        ...
        init(
            parseTreeStore: ParseTreeStore? = nil, annotations: AnnotationBookmarkSession? = nil,
            recentsStore: DocumentRecentsStoring, sessionStore: WorkspaceSessionStoring? = nil,
            ...
            diagnostics: (any DiagnosticsLogging)? = nil, recentLimit: Int = 10,
            ...
            let dependencies = Self.makeDependencies(
                parseTreeStore: parseTreeStore, annotations: annotations,
                recentsStore: recentsStore, sessionStore: sessionStore,
                pipelineFactory: pipelineFactory, readerFactory: readerFactory,
                workQueue: workQueue, diagnostics: diagnostics, recentLimit: recentLimit,
                bookmarkStore: bookmarkStore, filesystemAccess: filesystemAccess,
        ...
        private func setupObservers(
            ...
            annotationsSelectionCancellable = resolvedAnnotations.$currentSelectedNodeID.dropFirst()
                .sink { [weak self] value in
                    ...
                    guard !self.recents.isEmpty, !self.sessionPersistenceService.isRestoringSession
                    else { return }
                    self.persistSession()
                }
        ...
        private func restoreSessionIfNeeded() {
            ...
            let (migratedRecents, document) = documentOpeningCoordinator.applySessionSnapshot(
                snapshot)
    ...
    extension DocumentSessionController {
        ...
        func openDocument(at url: URL) {
            ...
            let baseRecent = DocumentRecent(
                url: standardized, bookmarkData: nil, displayName: url.lastPathComponent,
                lastOpened: Date())
        ...
        func dismissExportStatus() { exportStatus = nil }
        ...
        func selectValidationPreset(_ presetID: String, scope: ValidationConfigurationScope) {
            ...
            applyValidationConfigurationFilter()
            if validationConfigurationService.didUpdateWorkspaceConfiguration() { persistSession() }
        }

        func setValidationRule(
            _ rule: ValidationRuleIdentifier, isEnabled: Bool, scope: ValidationConfigurationScope
        ) {
            validationConfigurationService.setValidationRule(
                rule, isEnabled: isEnabled, scope: scope)
            applyValidationConfigurationFilter()
            if validationConfigurationService.didUpdateWorkspaceConfiguration() { persistSession() }
        }
        ...
        func exportJSON(scope: ExportScope) async {
            await exportService.exportJSON(
                scope: scope, onSuccess: { [weak self] status in self?.exportStatus = status },
        ...
        private func persistSession() {
            sessionPersistenceService.persistSession(
                recents: recents, currentDocument: currentDocument,
        ...
        private static func makeDependencies(
            parseTreeStore: ParseTreeStore?, annotations: AnnotationBookmarkSession?,
            recentsStore: DocumentRecentsStoring, sessionStore: WorkspaceSessionStoring?,
            pipelineFactory: @escaping @Sendable () -> ParsePipeline,
            readerFactory: @escaping @Sendable (URL) throws -> RandomAccessReader,
            workQueue: DocumentSessionWorkQueue, diagnostics: (any DiagnosticsLogging)?,
            recentLimit: Int, bookmarkStore: BookmarkPersistenceManaging?,
            ...
            let context = resolveContext(
                parseTreeStore: parseTreeStore, annotations: annotations, diagnostics: diagnostics)

            let bookmarkService = makeBookmarkService(
                context: context, bookmarkStore: bookmarkStore, filesystemAccess: filesystemAccess,
                bookmarkDataProvider: bookmarkDataProvider)

            let recentsService = makeRecentsService(
                context: context, bookmarkService: bookmarkService, recentsStore: recentsStore,
                recentLimit: recentLimit)

            let parseCoordinationService = makeParseCoordinationService(
                pipelineFactory: pipelineFactory, readerFactory: readerFactory, workQueue: workQueue
            )

            let sessionPersistenceService = makeSessionPersistenceService(
                context: context, bookmarkService: bookmarkService, sessionStore: sessionStore)

            let validationConfigurationService = makeValidationConfigurationService(
                context: context, validationConfigurationStore: validationConfigurationStore,
                validationPresetLoader: validationPresetLoader)
            ...
            let documentOpeningCoordinator = makeDocumentOpeningCoordinator(
                ...
                validationConfigurationService: validationConfigurationService,
                recentsService: recentsService, parseTreeStore: context.parseTreeStore,
            ...
            return SessionDependencies(
                parseTreeStore: context.parseTreeStore, annotations: context.annotations,
                documentViewModel: documentViewModel, recentsService: recentsService,
                sessionPersistenceService: sessionPersistenceService,
                validationConfigurationService: validationConfigurationService,
        ...
        private static func resolveContext(
            ...
            let resolvedDiagnostics =
                diagnostics
                ?? DiagnosticsLogger(
                    subsystem: "ISOInspectorApp", category: "DocumentSessionPersistence")

            return ResolvedContext(
                parseTreeStore: resolvedParseTreeStore, annotations: resolvedAnnotations,
                diagnostics: resolvedDiagnostics)
        }

        private static func makeBookmarkService(
            context: ResolvedContext, bookmarkStore: BookmarkPersistenceManaging?,
            filesystemAccess: FilesystemAccess,
            bookmarkDataProvider: ((SecurityScopedURL) -> Data?)?
        ) -> BookmarkService {
            BookmarkService(
        ...
        private static func makeRecentsService(
            context: ResolvedContext, bookmarkService: BookmarkService,
            recentsStore: DocumentRecentsStoring, recentLimit: Int
        ) -> RecentsService {
            RecentsService(
                recentsStore: recentsStore, recentLimit: recentLimit,
                diagnostics: context.diagnostics, bookmarkService: bookmarkService)
        }
        ...
        private static func makeSessionPersistenceService(
            ...
            SessionPersistenceService(
                sessionStore: sessionStore, diagnostics: context.diagnostics,
                bookmarkService: bookmarkService)
        }
        ...
        private static func makeValidationConfigurationService(
            ...
            ValidationConfigurationService(
                validationConfigurationStore: validationConfigurationStore,
                validationPresetLoader: validationPresetLoader, diagnostics: context.diagnostics)
        }
        ...
        private static func makeExportService(
            ...
            ExportService(
                parseTreeStore: context.parseTreeStore, bookmarkService: bookmarkService,
                validationConfigurationService: validationConfigurationService,
                diagnostics: context.diagnostics)
        }

        private static func makeDocumentOpeningCoordinator(
            bookmarkService: BookmarkService, parseCoordinationService: ParseCoordinationService,
            sessionPersistenceService: SessionPersistenceService,
            validationConfigurationService: ValidationConfigurationService,
            recentsService: RecentsService, parseTreeStore: ParseTreeStore,
        ...
        typealias ValidationConfigurationScope = ValidationConfigurationService
            .ValidationConfigurationScope
    }

    // MARK: - Supporting Types

    protocol DocumentSessionWorkQueue { func execute(_ work: @Sendable @escaping () -> Void) }
    ...
    struct DocumentSessionBackgroundQueue: DocumentSessionWorkQueue {
        ...
        func execute(_ work: @Sendable @escaping () -> Void) { queue.async(execute: work) }
    }
#endif
