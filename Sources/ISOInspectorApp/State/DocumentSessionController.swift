    ...
    final class DocumentSessionController: ObservableObject {
        ...
        init(
            ...
            self.presetByID = Dictionary(uniqueKeysWithValues: loadedPresets.map { ($0.id, $0) })
            self.defaultValidationPresetID = Self.defaultPresetID(from: loadedPresets)
            ...
            let normalizedGlobal = normalizedConfiguration(loadedGlobal)
        ...
        private func normalizedConfiguration(_ configuration: ValidationConfiguration) -> ValidationConfiguration {
            var normalized = configuration
            if presetByID[normalized.activePresetID] == nil {
                normalized.activePresetID = defaultValidationPresetID
