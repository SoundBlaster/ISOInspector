    ...
    struct IntegritySummaryView: View {
        ...
        var body: some View {
            ScrollView {
                ...
                filterBar
                if viewModel.displayedIssues.isEmpty { emptyState } else { issueList }
            }
        }
        ...
        private var header: some View {
            HStack {
                VStack(alignment: .leading, spacing: DS.Spacing.xxs) {
                    Text("Integrity Summary").font(.title3).bold()
                    Text("Tolerant parsing issues detected").font(.subheadline).foregroundColor(
                        .secondary)
                }
        ...
        private var filterBar: some View {
            HStack(spacing: 8) {
                ...
                }.buttonStyle(.plain).disabled(viewModel.severityFilter.isEmpty)
        ...
        private var issueList: some View {
            LazyVStack(alignment: .leading, spacing: 0) {
                ForEach(Array(viewModel.displayedIssues.enumerated()), id: \.offset) { _, issue in
                    IssueRow(issue: issue) { onIssueSelected?(issue) }
                }
            }
        }
        ...
        private var emptyState: some View {
            VStack(spacing: 12) {
                Image(systemName: "checkmark.circle").font(.system(size: 44)).foregroundColor(
                    .green)
                Text("No integrity issues").font(.headline)
                Text("The file parsed successfully without any issues.").font(.subheadline)
        ...
        private var exportToolbar: some ToolbarContent {
            buildIntegrityExportToolbar(
                shouldShowToolbar: shouldShowExportToolbar, onExportJSON: onExportJSON,
                onExportIssueSummary: onExportIssueSummary)
        }
    }

    @ToolbarContentBuilder private func buildIntegrityExportToolbar(
        shouldShowToolbar: Bool, onExportJSON: (@MainActor () -> Void)?,
    ...
    enum IntegritySummaryToolbarPolicy {
        ...
        static func shouldShowToolbar(
            platform: Platform, hasJSONExport: Bool, hasIssueSummaryExport: Bool
        ) -> Bool {
            guard hasJSONExport && hasIssueSummaryExport else { return false }

            switch platform {
            case .macOS: return false
            case .iOSLike: return true
            }
        }
    }
    ...
    private struct IssueRow: View {
        ...
        var body: some View {
            HStack(alignment: .top, spacing: DS.Spacing.m) {
                Image(systemName: issue.severity.iconName).font(.title3).foregroundColor(
                    issue.severity.color
                ).frame(width: 24)

                VStack(alignment: .leading, spacing: DS.Spacing.xxs) {
                    HStack {
                        Text(issue.code).font(.body).fontWeight(.semibold)
                        Text("•").foregroundColor(.secondary)
                        Text(issue.severity.label.uppercased()).font(.caption).fontWeight(.semibold)
                            .foregroundColor(issue.severity.color)
                    }

                    Text(issue.message).font(.subheadline).foregroundColor(.primary)
                ...
                Button(action: onSelect) {
                    Image(systemName: "arrow.right.circle").foregroundColor(.accentColor)
                }.buttonStyle(.plain).help("Focus this issue in the tree view")
            }.padding(.vertical, DS.Spacing.s).padding(.horizontal, DS.Spacing.m).background(
    ...
    #Preview("Integrity Summary") { IntegritySummaryPreview() }
    ...
    private struct IntegritySummaryPreview: View {
        ...
        init() {
            ...
            store.issueStore.record(
                ParseIssue(
                    severity: .error, code: "ERR-001", message: "Critical error in ftyp box",
                    byteRange: 100..<200, affectedNodeIDs: [1]), depth: 1)
            store.issueStore.record(
                ParseIssue(
                    severity: .warning, code: "WARN-001",
                    message: "Non-standard box order detected", byteRange: 300..<400,
                    affectedNodeIDs: [2]), depth: 2)
            store.issueStore.record(
                ParseIssue(
                    severity: .info, code: "INFO-001", message: "Optional box missing",
                    byteRange: 500..<600, affectedNodeIDs: [3]), depth: 1)
        ...
        var body: some View {
            IntegritySummaryView(viewModel: viewModel).frame(width: 700, height: 500)
        }
    }
#endif
