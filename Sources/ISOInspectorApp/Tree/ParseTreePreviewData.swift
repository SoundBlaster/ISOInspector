...
enum ParseTreePreviewData {
    ...
    static let sampleSnapshot: ParseTreeSnapshot = {
        ...
        let moov = makeNode(type: "moov", start: 200, issues: [
            ...
            makeNode(type: "trak", start: 400, issues: [
                ValidationIssue(ruleID: "VR-TRAK", message: "Track missing language", severity: .warning)
            ], children: [
                makeNode(type: "mdia", start: 600, children: [
                    makeNode(type: "minf", start: 800, children: [
                        makeNode(type: "stbl", start: 1000, issues: [
                            ValidationIssue(ruleID: "VR-STBL", message: "Index error", severity: .error)
                        ])
                    ])
                ])
            ]),
