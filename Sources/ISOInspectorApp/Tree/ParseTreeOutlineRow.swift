    ...
    struct ParseTreeOutlineRow: Identifiable, Equatable {
        ...
        var dominantSeverity: ValidationIssue.Severity? {
            node.validationIssues.map(\.severity).max(by: { priority(for: $0) < priority(for: $1) })
        }
        ...
        struct CorruptionSummary: Equatable {
            ...
            init?(issues: [ParseIssue]) {
                ...
                for severity in ParseIssue.Severity.allCases { resolvedCounts[severity] = 0 }
                for issue in issues { resolvedCounts[issue.severity, default: 0] += 1 }

                let sortedIssues = issues.sorted { lhs, rhs in
                    ParseTreeOutlineRow.CorruptionSummary.priority(for: lhs.severity)
                        > ParseTreeOutlineRow.CorruptionSummary.priority(for: rhs.severity)
                }
            ...
            func count(for severity: ParseIssue.Severity) -> Int { counts[severity, default: 0] }

            var badgeText: String {
                if totalCount == 1 { return "1 issue" }
            ...
            var accessibilityHint: String? {
                ...
                if primaryIssue.code.isEmpty { return nil }
            ...
            var tooltipText: String? {
                ...
                if !primaryIssue.message.isEmpty { return primaryIssue.message }
                return code
            }
