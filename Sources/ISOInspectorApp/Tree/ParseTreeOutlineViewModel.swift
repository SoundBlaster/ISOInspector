    ...
    @MainActor final class ParseTreeOutlineViewModel: ObservableObject {
        ...
        @Published var searchText: String = "" { didSet { rebuildRows() } }
        @Published var filter: ParseTreeOutlineFilter = .all { didSet { rebuildRows() } }
        ...
        init(snapshot: ParseTreeSnapshot = .empty) { apply(snapshot: snapshot) }
        ...
        where P.Output == ParseTreeSnapshot, P.Failure == Never {
            ...
            publisher.sink { [weak self] snapshot in
                ...
                Task { @MainActor [weak self] in self?.apply(snapshot: snapshot) }
        ...
        func containsNode(with id: ParseTreeNode.ID) -> Bool { allIdentifiers.contains(id) }
        ...
        func rowID(after current: ParseTreeNode.ID?, direction: NavigationDirection)
            -> ParseTreeNode.ID?
        {
            ...
            case .up:
                if index == rows.startIndex { return rows.first?.id }
            ...
            case .parent:
                ...
                while searchIndex > rows.startIndex {
                    ...
                    if rows[searchIndex].depth < currentDepth { return rows[searchIndex].id }
                }
                return nil
            }
        }
    ...
    extension ParseTreeOutlineViewModel {
        ...
        private func collectRows(for node: ParseTreeNode, depth: Int, matcher: SearchMatcher)
            -> FlattenResult
        {
            ...
            for child in node.children {
                ...
                if result.matchesSearch { anyChildMatchesSearch = true }
            }
            ...
            guard isVisible else {
                return FlattenResult(
                    rows: [], identifiers: identifiers, isVisible: false,
            ...
            let row = ParseTreeOutlineRow(
                node: node, depth: depth, isExpanded: isExpanded, isSearchMatch: matchesSearch,
            ...
            if isExpanded { rows.append(contentsOf: childRows) }

            return FlattenResult(
                rows: rows, identifiers: identifiers, isVisible: true,
                matchesSearch: subtreeMatchesSearch)
        }
        ...
        private func rebuildParentLookup() {
            ...
            while let element = stack.popLast() {
                if let parent = element.parent { parentLookup[element.node.id] = parent }
                for child in element.node.children { stack.append((element.node.id, child)) }
            }
        }

        private func rebuildIssueBranchIdentifiers() {
            issueBranchIdentifiers = []
            for node in snapshot.nodes { _ = markIssueBranches(for: node) }
        }

        @discardableResult private func markIssueBranches(for node: ParseTreeNode) -> Bool {
            ...
            if subtreeHasIssue { issueBranchIdentifiers.insert(node.id) }
        ...
        private func refreshFilterMetadata() {
            ...
            while let node = stack.popLast() {
                ...
                if node.isStreamingIndicator { hasStreamingIndicators = true }
                stack.append(contentsOf: node.children)
            }
        ...
        private struct SearchMatcher {
            ...
            private func makeHaystacks(from node: ParseTreeNode) -> [String] {
                ...
                if let metadata = node.metadata {
                    ...
                    if !summary.isEmpty { values.append(summary.lowercased()) }
                }
                return values
            }
        }
