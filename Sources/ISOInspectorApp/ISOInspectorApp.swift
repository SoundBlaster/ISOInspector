    ...
    @main struct ISOInspectorApp: App {
        @StateObject private var controller = ISOInspectorApp.makeDocumentSessionController()

        var body: some Scene {
            WindowGroup {
                AppShellView(appController: controller).isoInspectorAppTheme().a11yRoot(
                ...
                CommandMenu("Export") {
                    ...
                    Button("Export Selected JSON…") {
                        guard let nodeID = controller.documentViewModel.nodeViewModel.selectedNodeID
                        else { return }
            ...
            #if os(macOS)
                Settings { ValidationSettingsView(controller: controller).isoInspectorAppTheme() }
            #endif
        }

        @MainActor private static func makeDocumentSessionController() -> DocumentSessionController
        {
            ...
            return DocumentSessionController(
                ...
                bookmarkStore: bookmarkStore, filesystemAccess: FilesystemAccess.live(),
        ...
        private static func recentsDirectory() -> URL {
        ...
        #if canImport(CoreData)
            @MainActor private static func makeAnnotationSession() -> (
            ...
            private static func annotationStoreDirectory() -> URL {
                ...
                let directory = base.appendingPathComponent(
                    "AnnotationBookmarks", isDirectory: true)
                try? FileManager.default.createDirectory(
                    at: directory, withIntermediateDirectories: true)
                return directory
            }
        ...
        private static func validationConfigurationDirectory() -> URL {
            let base =
                ...
                .first ?? FileManager.default.temporaryDirectory
            let directory = base.appendingPathComponent(
                "ValidationConfiguration", isDirectory: true)
            try? FileManager.default.createDirectory(
                at: directory, withIntermediateDirectories: true)
            return directory
        }
            ...
            CommandMenu("Focus") {
                if let focusTarget {
                    ForEach(catalog.shortcuts) { descriptor in
                        Button(descriptor.title) { focusTarget.wrappedValue = descriptor.target }
                            .keyboardShortcut(
    ...
    @main struct ISOInspectorAppFallback {
        static func main() {
