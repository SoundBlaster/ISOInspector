    ...
    @MainActor final class AnnotationBookmarkSession: ObservableObject {
        ...
        func setFileURL(_ file: URL?) {
            ...
            do {
                ...
                annotationsByNode = Dictionary(grouping: annotations, by: { $0.nodeID }).mapValues {
                    records in records.sorted(by: sortAnnotationsByCreation)
                }
        ...
        func toggleBookmark() {
            ...
            do {
                ...
                if shouldBookmark { bookmarks.insert(nodeID) } else { bookmarks.remove(nodeID) }
                lastErrorMessage = nil
                refreshActiveState()
            } catch { lastErrorMessage = error.localizedDescription }
        }

        func isBookmarked(nodeID: Int64) -> Bool { bookmarks.contains(nodeID) }
