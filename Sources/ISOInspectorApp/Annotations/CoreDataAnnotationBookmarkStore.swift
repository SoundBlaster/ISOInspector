    ...
    public final class CoreDataAnnotationBookmarkStore: @unchecked Sendable {
        ...
        public init(
            directory: URL, modelVersion: ModelVersion = .latest,
    ...
    extension CoreDataAnnotationBookmarkStore {
            ...
            try perform { context in
                guard
                    let fileEntity = try self.fetchFile(
                        for: file, createIfMissing: false, in: context)
                else { return [] }
        ...
        @discardableResult public func createAnnotation(for file: URL, nodeID: Int64, note: String)
        ...
        @discardableResult public func updateAnnotation(
            for file: URL, annotationID: UUID, note: String
        ) throws -> AnnotationRecord {
            try perform { context in
                guard
                    let fileEntity = try self.fetchFile(
                        for: file, createIfMissing: false, in: context),
                    let annotation = try self.fetchAnnotation(
                        id: annotationID, for: fileEntity, in: context)
                else { throw AnnotationBookmarkStoreError.annotationNotFound }
                let now = self.makeDate()
                annotation.note = note
                annotation.updatedAt = now
                fileEntity.touch(at: now)
        ...
        public func setBookmark(for file: URL, nodeID: Int64, isBookmarked: Bool) throws {
            try perform { context in
                guard
                    let fileEntity = try self.fetchFile(
                        for: file, createIfMissing: isBookmarked, in: context)
                else { return }
    ...
    extension CoreDataAnnotationBookmarkStore {
        ...
        public func loadCurrentSession() throws -> WorkspaceSessionSnapshot? {
            try perform { context in
                guard let session = try self.fetchCurrentSession(in: context) else { return nil }
        ...
        public func saveCurrentSession(_ snapshot: WorkspaceSessionSnapshot) throws {
            try perform { context in
                ...
                if let appVersion = snapshot.appVersion { workspace.appVersion = appVersion }
                workspace.lastOpened = snapshot.updatedAt
                workspace.schemaVersion = 3

                let session = try self.fetchOrCreateSession(
                    id: snapshot.id, in: context, workspace: workspace)
                ...
                self.replaceWindowLayouts(for: session, with: snapshot.windowLayouts, in: context)
            ...
            try perform { context in
                ...
                for session in sessions {
                    ...
                    if let layouts = session.layouts as? Set<WindowLayoutEntity> {
                        for layout in layouts { context.delete(layout) }
                    }
                }
                try context.saveIfNeeded()
            }
    ...
    extension CoreDataAnnotationBookmarkStore {
        ...
        private func perform<T>(_ block: @escaping (NSManagedObjectContext) throws -> T) throws -> T
        {
            ...
            context.performAndWait {
                do { value = try block(context) } catch { capturedError = error }
            }
        ...
        private func fetchFile(
            for file: URL, createIfMissing: Bool, in context: NSManagedObjectContext
        ) throws -> FileEntity? {
        ...
        private func fetchAnnotation(
            id: UUID, for file: FileEntity, in context: NSManagedObjectContext
        ...
        private func fetchBookmark(
            nodeID: Int64, for file: FileEntity, in context: NSManagedObjectContext
            ...
            -> SessionEntity?
        {
            ...
            request.relationshipKeyPathsForPrefetching = [
                "workspace", "files", "files.file", "files.bookmarkDiffs", "layouts",
            ]
            return try context.fetch(request).first
        }
        ...
        private func markOtherSessionsInactive(
            excluding activeSession: SessionEntity, in context: NSManagedObjectContext
        ) throws {
            let request = NSFetchRequest<SessionEntity>(entityName: EntityName.session)
            request.predicate = NSCompoundPredicate(andPredicateWithSubpredicates: [
            ...
            let sessions = try context.fetch(request)
            for session in sessions { session.isCurrent = false }
        }

        private func replaceSessionFiles(
            for session: SessionEntity, with snapshots: [WorkspaceSessionFileSnapshot],
            in context: NSManagedObjectContext
        ) throws {
            if let existing = session.files as? Set<SessionFileEntity> {
                for file in existing { context.delete(file) }
            }

            let orderedSnapshots = snapshots.sorted { lhs, rhs in
                if lhs.orderIndex == rhs.orderIndex { return lhs.id.uuidString < rhs.id.uuidString }
            ...
            for snapshot in orderedSnapshots {
                guard
                    let fileEntity = try fetchFile(
                        for: snapshot.recent.url, createIfMissing: true, in: context)
                else { continue }
                ...
                sessionFile.bookmarkIdentifier =
                    snapshot.bookmarkIdentifier ?? snapshot.recent.bookmarkIdentifier
        ...
        private func replaceWindowLayouts(
            for session: SessionEntity, with layouts: [WorkspaceWindowLayoutSnapshot],
            in context: NSManagedObjectContext
        ) {
            if let existing = session.layouts as? Set<WindowLayoutEntity> {
                for layout in existing { context.delete(layout) }
            }
    ...
    extension NSManagedObjectContext {
        fileprivate func makeManagedObject<T: NSManagedObject>() -> T {
            guard let entityName = T.entity().name else {
                preconditionFailure("Missing entity name for \(T.self)")
            }
            guard
                let object = NSEntityDescription.insertNewObject(
                    forEntityName: entityName, into: self) as? T
            else { preconditionFailure("Failed to insert \(entityName) as \(T.self)") }
            return object
        }
    }
    ...
    extension CoreDataAnnotationBookmarkStore {
        // Cache models to prevent "Multiple NSEntityDescriptions" warnings when running tests
        ...
        private static func makeModelUncached(for version: ModelVersion) -> NSManagedObjectModel {
            ...
            case .v2: return makeV2Model()
            case .v3: return makeV3Model()
            }
        }
        ...
        private final class ModelCache: @unchecked Sendable {
            ...
            func model(for version: ModelVersion) -> NSManagedObjectModel {
                lock.lock()
                defer { lock.unlock() }
                if let cached = storage[version] { return cached }
        ...
        fileprivate struct BaseEntities {
        ...
        fileprivate static func makeBaseEntities() -> BaseEntities {
            ...
            fileEntity.properties = [
                fileID, fileURL, fileCreated, fileUpdated, fileToAnnotations, fileToBookmarks,
            ]
            annotationEntity.properties = [
                annotationID, annotationNodeID, annotationNote, annotationCreated,
                annotationUpdated, annotationsToFile,
            ]
            bookmarkEntity.properties = [
                bookmarkID, bookmarkNodeID, bookmarkCreated, bookmarksToFile,
            ]

            return BaseEntities(
                file: fileEntity, annotation: annotationEntity, bookmark: bookmarkEntity)
        }

        fileprivate static func makeV1Model() -> NSManagedObjectModel {
        ...
        fileprivate static func makeV2Model() -> NSManagedObjectModel {
            makeSessionModel(includeValidationConfigurationData: false)
        }

        fileprivate static func makeV3Model() -> NSManagedObjectModel {
            ...
            -> NSManagedObjectModel
        {
            ...
            let sessionLastSceneIdentifier = stringAttribute(
                named: "lastSceneIdentifier", isOptional: true)
            ...
            let sessionFileLastSelectionNodeID = int64Attribute(
                named: "lastSelectionNodeID", isOptional: true)
            ...
            let sessionFileBookmarkIdentifier = uuidAttribute(
                named: "bookmarkIdentifier", isOptional: true)
            let sessionFileValidationConfigurationData: NSAttributeDescription?
            if includeValidationConfigurationData {
                sessionFileValidationConfigurationData = binaryAttribute(
                    named: "validationConfigurationData", isOptional: true)
            } else {
            ...
            workspaceEntity.properties = [
                workspaceID, workspaceAppVersion, workspaceLastOpened, workspaceSchemaVersion,
                workspaceToSessions,
            ]

            sessionEntity.properties = [
                sessionID, sessionCreatedAt, sessionUpdatedAt, sessionLastSceneIdentifier,
                sessionIsCurrent, sessionFocusedFileURL, sessionsToWorkspace, sessionToFiles,
                sessionToLayouts,
            ]
            ...
            var sessionFileProperties: [NSPropertyDescription] = [
                sessionFileID, sessionFileOrderIndex, sessionFileLastSelectionNodeID,
                sessionFileScrollOffsetX, sessionFileScrollOffsetY, sessionFileIsPinned,
                sessionFileDisplayName, sessionFileLastOpened, sessionFileBookmarkData,
            ...
            sessionFileProperties.append(contentsOf: [
                sessionFileToSession, sessionFileToFile, sessionFileToBookmarkDiffs,
            ])
            sessionFileEntity.properties = sessionFileProperties

            windowLayoutEntity.properties = [
                windowLayoutID, windowLayoutSceneIdentifier, windowLayoutSerializedLayout,
                windowLayoutIsFloatingInspector, windowLayoutToSession,
            ]

            sessionBookmarkDiffEntity.properties = [
                sessionBookmarkDiffID, sessionBookmarkDiffBookmarkID, sessionBookmarkDiffIsRemoved,
                sessionBookmarkDiffNoteDelta, sessionBookmarkDiffToSessionFile,
                sessionBookmarkDiffToBookmark,
            ]

            let model = NSManagedObjectModel()
            model.entities = [
                fileEntity, annotationEntity, bookmarkEntity, workspaceEntity, sessionEntity,
                sessionFileEntity, windowLayoutEntity, sessionBookmarkDiffEntity,
            ]
            return model
        }
        ...
        fileprivate static func boolAttribute(named name: String) -> NSAttributeDescription {
            let attribute = NSAttributeDescription()
            attribute.name = name
    ...
    @objc(FileEntity) private final class FileEntity: NSManagedObject {
        ...
        func touch(at date: Date) { updatedAt = date }
    }

    @objc(AnnotationEntity) private final class AnnotationEntity: NSManagedObject {
        @NSManaged var id: UUID
        ...
        var record: AnnotationRecord {
            AnnotationRecord(
                id: id, nodeID: nodeID, note: note, createdAt: createdAt, updatedAt: updatedAt)
        }
    }

    @objc(BookmarkEntity) private final class BookmarkEntity: NSManagedObject {
        @NSManaged var id: UUID
        @NSManaged var nodeID: Int64
        @NSManaged var createdAt: Date
        @NSManaged var file: FileEntity
        ...
        var record: BookmarkRecord { BookmarkRecord(nodeID: nodeID, createdAt: createdAt) }
    }

    @objc(WorkspaceEntity) private final class WorkspaceEntity: NSManagedObject {
        @NSManaged var id: UUID
    ...
    @objc(SessionEntity) private final class SessionEntity: NSManagedObject {
        @NSManaged var id: UUID
        @NSManaged var createdAt: Date
        @NSManaged var updatedAt: Date
    ...
    @objc(SessionFileEntity) private final class SessionFileEntity: NSManagedObject {
        @NSManaged var id: UUID
    ...
    @objc(WindowLayoutEntity) private final class WindowLayoutEntity: NSManagedObject {
        @NSManaged var id: UUID
    ...
    @objc(SessionBookmarkDiffEntity) private final class SessionBookmarkDiffEntity: NSManagedObject
    {
        @NSManaged var id: UUID
    ...
    extension SessionEntity {
        fileprivate func makeSnapshot() -> WorkspaceSessionSnapshot {
            let fileSnapshots =
                (files as? Set<SessionFileEntity>)?.compactMap { $0.makeSnapshot() }.sorted {
                    lhs, rhs in
            ...
            let layoutSnapshots =
                (layouts as? Set<WindowLayoutEntity>)?.map { $0.makeSnapshot() }.sorted {
                    lhs, rhs in lhs.sceneIdentifier < rhs.sceneIdentifier
                } ?? []
            ...
            return WorkspaceSessionSnapshot(
                id: id, createdAt: createdAt, updatedAt: updatedAt, appVersion: version,
                files: fileSnapshots, focusedFileURL: focusedURL,
                lastSceneIdentifier: lastSceneIdentifier, windowLayouts: layoutSnapshots)
        }
    }

    extension SessionFileEntity {
        fileprivate func makeSnapshot() -> WorkspaceSessionFileSnapshot? {
            guard let url = URL(string: file.url) else { return nil }
            let recent = DocumentRecent(
                url: url, bookmarkIdentifier: bookmarkIdentifier, bookmarkData: bookmarkData,
                displayName: displayName, lastOpened: lastOpened)
            ...
            let diffs =
                (bookmarkDiffs as? Set<SessionBookmarkDiffEntity>)?.map { $0.makeSnapshot() }.sorted
            { $0.id.uuidString < $1.id.uuidString } ?? []

            let validationConfiguration: ValidationConfiguration?
            if let data = validationConfigurationData {
                let decoder = JSONDecoder()
                validationConfiguration = try? decoder.decode(
                    ValidationConfiguration.self, from: data)
            } else {
                validationConfiguration = nil
            }

            return WorkspaceSessionFileSnapshot(
                id: id, recent: recent, orderIndex: Int(orderIndex), lastSelectionNodeID: selection,
                isPinned: isPinned, scrollOffset: scrollOffset,
                bookmarkIdentifier: bookmarkIdentifier ?? recent.bookmarkIdentifier,
                bookmarkDiffs: diffs, validationConfiguration: validationConfiguration)
        }
    }

    extension SessionBookmarkDiffEntity {
        fileprivate func makeSnapshot() -> WorkspaceSessionBookmarkDiff {
            let identifier = bookmarkID ?? bookmark?.id
            return WorkspaceSessionBookmarkDiff(
                id: id, bookmarkID: identifier, isRemoved: isRemoved, noteDelta: noteDelta)
        }
    }

    extension WindowLayoutEntity {
        fileprivate func makeSnapshot() -> WorkspaceWindowLayoutSnapshot {
            WorkspaceWindowLayoutSnapshot(
                id: id, sceneIdentifier: sceneIdentifier, serializedLayout: serializedLayout,
                isFloatingInspector: isFloatingInspector)
        }
    }

    extension NSManagedObjectContext {
        fileprivate func saveIfNeeded() throws { if hasChanges { try save() } }
    }
#else
    import Foundation
    ...
    public final class CoreDataAnnotationBookmarkStore: @unchecked Sendable {
        ...
        @discardableResult public func updateAnnotation(
            for file: URL, annotationID: UUID, note: String
        ) throws -> AnnotationRecord { throw CocoaError(.featureUnsupported) }
        ...
        public func loadCurrentSession() throws -> WorkspaceSessionSnapshot? { return nil }
        ...
        public func clearCurrentSession() throws { throw CocoaError(.featureUnsupported) }
    }
#endif
