    ...
    struct ValidationSettingsView: View {
        ...
        var body: some View {
            Form {
                ...
                presetSection
                ruleSection
                if effectiveScope == .workspace { resetSection }
            }.padding().onChangeCompat(of: controller.currentDocument) { document in
                if document == nil { selectedScope = .global }
            }
        }
        ...
        private var scopeSection: some View {
            Section("Scope") {
                Picker("Apply changes to", selection: $selectedScope) {
                    Text("Global Defaults").tag(
                        DocumentSessionController.ValidationConfigurationScope.global)
                    Text("Current Workspace").tag(
                        DocumentSessionController.ValidationConfigurationScope.workspace)
                }.pickerStyle(.segmented).disabled(!workspaceAvailable)

                Text(scopeDescription).font(.footnote).foregroundColor(.secondary)
            }
        }
        ...
        private var presetSection: some View {
            Section("Presets") {
                if controller.validationPresets.isEmpty {
                    Text("No presets available.").foregroundColor(.secondary)
                } else {
                        ...
                        Button {
                            ...
                            HStack(alignment: .top) {
                                VStack(alignment: .leading, spacing: DS.Spacing.xxs) {
                                    Text(preset.name).fontWeight(
                                        preset.id == activeConfiguration.activePresetID
                                            ? .semibold : .regular)
                                    Text(preset.summary).font(.caption).foregroundColor(.secondary)
                                }
                                Spacer()
                                if preset.id == activeConfiguration.activePresetID {
                                    if isCustomSelection {
                                        HStack(spacing: 6) {
                                            ...
                                            Text("Custom").font(.caption).foregroundColor(
                                                .accentColor)
                                        }
                                    } else {
        ...
        private var resetSection: some View {
            Section {
                Button("Reset to Global") { controller.resetWorkspaceValidationOverrides() }
                    .disabled(!controller.isUsingWorkspaceValidationOverride)
            }
        }

        private var workspaceAvailable: Bool { controller.currentDocument != nil }
        ...
        private var activeConfiguration: ValidationConfiguration {
            ...
            case .global: return controller.globalValidationConfiguration
            case .workspace: return controller.validationConfiguration
            }
        }
        ...
        private var scopeDescription: String {
            if !workspaceAvailable { return "Open a document to configure workspace overrides." }
            switch effectiveScope {
            case .global: return "Global defaults apply to new workspaces."
            case .workspace:
                return
                    "Changes apply to \(controller.currentDocument?.displayName ?? "this workspace")."
            }
        }
