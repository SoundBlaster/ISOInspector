    ...
    extension AccessibilityDescriptor {
        static func outlineRow(_ row: ParseTreeOutlineRow, isBookmarked: Bool)
            -> AccessibilityDescriptor
        {
            ...
            if let summary = row.summary, !summary.isEmpty { components.append(summary) }
    ...
    extension ParseTreeNodeDetail {
        var accessibilitySummary: String {
            ...
            if let name = metadata?.name, !name.isEmpty { components.append(name) }
            if let summary = metadata?.summary, !summary.isEmpty { components.append(summary) }
            ...
            let payloadDescription =
                "Payload \(header.payloadRange.lowerBound) – \(header.payloadRange.upperBound)"
            components.append(rangeDescription)
            components.append(payloadDescription)

            if let version = metadata?.version { components.append("Version \(version)") }
            ...
            if let payload {
                if let encryption = payload.sampleEncryption {
                    var encryptionParts: [String] = [
                        "Sample encryption entries \(encryption.sampleCount)"
                    ]
                ...
                if let offsets = payload.sampleAuxInfoOffsets {
                    var offsetParts: [String] = [
                        "Auxiliary info offsets entries \(offsets.entryCount)"
                    ]
            ...
            if !issues.isEmpty {
                ...
                for issue in issues { counts[issue.severity, default: 0] += 1 }
                let segments = ParseIssue.Severity.allCases.compactMap { severity -> String? in
                    ...
                    switch severity {
                    case .info: descriptor = "informational"
                    case .warning: descriptor = "warning"
                    case .error: descriptor = "error"
                    }
                    return count == 1 ? "1 \(descriptor) issue" : "\(count) \(descriptor) issues"
                }
                if !segments.isEmpty {
                    components.append(
                        "Corruption diagnostics: \(segments.joined(separator: ", ")).")
                }
            }
    ...
    extension ValidationIssue.Severity {
        var accessibilityDescription: String {
            switch self {
            case .info: return "informational issue"
            case .warning: return "warning issue"
            case .error: return "error issue"
            }
        }
    }
#endif
