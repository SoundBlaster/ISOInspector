    ...
    struct ParseTreeDetailView: View {
        ...
        var body: some View {
            ScrollView {
                ...
                content
            }.onChangeCompatibility(of: selectedNodeID) { _ in draftNote = "" }
        }
    }
    ...
    extension ParseTreeDetailView {
        private var header: some View {
            HStack {
                VStack(alignment: .leading, spacing: DS.Spacing.xxs) {
                    Text("Selection Details").font(.title3).bold()
        ...
        @ViewBuilder private var noSelectionView: some View {
            if #available(iOS 17, macOS 14, *) {
                ...
                VStack(spacing: DS.Spacing.m) {
                    Image(systemName: "square.dashed").font(.system(size: 44)).foregroundColor(
                        .secondary)
                    Text("No selection").font(.headline)
                    Text("Choose a box in the hierarchy to inspect its details.").font(.subheadline)
        ...
        private func metadataSection(detail: ParseTreeNodeDetail) -> some View {
            VStack(alignment: .leading, spacing: DS.Spacing.m) {
                ...
                Text("Snapshot updated ")
                    + Text(detail.snapshotTimestamp, style: .relative).font(.footnote)
                    .foregroundColor(.secondary)
            }.accessibilityElement(children: .combine).accessibilityLabel(
                detail.accessibilitySummary
        ...
        private func metadataGrid(detail: ParseTreeNodeDetail) -> some View {
            VStack(alignment: .leading, spacing: DS.Spacing.m) {
                ...
                BoxMetadataRow(
                    label: "Payload", value: byteRangeString(for: detail.header.payloadRange))
                ...
                if let summary = detail.metadata?.summary, !summary.isEmpty {
                    BoxMetadataRow(label: "Summary", value: summary, layout: .vertical)
                }
                if let specification = detail.metadata?.specification, !specification.isEmpty {
                    BoxMetadataRow(label: "Spec", value: specification, layout: .vertical)
                }
                ...
                if let flags = detail.metadata?.flags {
                    BoxMetadataRow(
                        label: "Flags", value: "0x\(String(flags, radix: 16, uppercase: true))",
                        copyable: true)
                }
            }
        }

        @ViewBuilder private func metadataStatusRow(status: ParseTreeNode.Status) -> some View {
            if let descriptor = ParseTreeStatusDescriptor(status: status) {
                HStack(spacing: DS.Spacing.m) {
                    Text("Status").font(DS.Typography.body).foregroundStyle(.secondary)
                    Spacer()
                    HStack(spacing: DS.Spacing.s) {
                        Indicator(
                            level: descriptorBadgeLevel(descriptor.level), size: .small,
                            reason: descriptor.accessibilityLabel, tooltip: .text(descriptor.text))
        ...
        @ViewBuilder private func encryptionSection(detail: ParseTreeNodeDetail) -> some View {
                ...
                VStack(alignment: .leading, spacing: DS.Spacing.m) {
                    ...
                    if !summary.sampleEncryption.isEmpty {
                        encryptionSubsection(
                            title: "Sample Encryption (senc)", rows: summary.sampleEncryption)
                    }
                    if !summary.auxInfoOffsets.isEmpty {
                        encryptionSubsection(
                            title: "Aux Info Offsets (saio)", rows: summary.auxInfoOffsets)
                    }
                    if !summary.auxInfoSizes.isEmpty {
                        encryptionSubsection(
                            title: "Aux Info Sizes (saiz)", rows: summary.auxInfoSizes)
                    }
        ...
        @ViewBuilder private func corruptionSection(detail: ParseTreeNodeDetail) -> some View {
            if detail.issues.isEmpty {
                EmptyView()
            } else {
                CorruptionIssueSection(
                    header: AnyView(
                        sectionHeader(title: "Corruption", icon: "exclamationmark.triangle")),
                    statusDescriptor: ParseTreeStatusDescriptor(status: detail.status),
                    issues: detail.issues, focusTarget: focusTarget, onCopy: copyToClipboard,
                    onViewInHex: { range in viewModel.focusIssue(on: range) },
                ...
                VStack(alignment: .leading, spacing: DS.Spacing.s) {
                    ...
                    VStack(alignment: .leading, spacing: DS.Spacing.s) {
                        ForEach(Array(issues.enumerated()), id: \.offset) { _, issue in
                            CorruptionIssueRow(
                                issue: issue, focusTarget: focusTarget, onCopy: onCopy,
                                onViewInHex: onViewInHex, rangeFormatter: rangeFormatter)
                        }
                    }
                }
            }
    ...
    extension ParseTreeDetailView {
        private struct CorruptionIssueRow: View {
            ...
            var body: some View {
                ...
                }.accessibilityAction(named: Text("Copy Message")) { onCopy(issue.message) }
            }

            private var baseRow: some View {
                VStack(alignment: .leading, spacing: DS.Spacing.xs) {
                    HStack(alignment: .firstTextBaseline, spacing: DS.Spacing.s) {
                        Image(systemName: issue.severity.iconName).foregroundColor(
                            issue.severity.color
                        ).font(.title3.weight(.semibold)).accessibilityHidden(true)
                        Text(issue.code).font(.subheadline).fontWeight(.semibold).textSelection(
                            .enabled)
                        Spacer()
                        if let range = issue.byteRange {
                            Button {
                    ...
                    Text(issue.message).font(.body).textSelection(.enabled)
                    if let range = issue.byteRange {
                        Text(rangeFormatter(range)).font(.caption).foregroundColor(.secondary)
                            .textSelection(.enabled)
                    } else {
                        Text("No byte range reported.").font(.caption).foregroundColor(.secondary)
                    }
                }.padding(DS.Spacing.s).frame(maxWidth: .infinity, alignment: .leading).background(
                    issue.severity.color.opacity(0.08),
                    in: RoundedRectangle(cornerRadius: 8, style: .continuous)
                ).overlay {
                    RoundedRectangle(cornerRadius: 8, style: .continuous).stroke(
                        issue.severity.color.opacity(0.3), lineWidth: 1)
                }.accessibilityElement(children: .combine).accessibilityLabel(accessibilityLabel)
                    .accessibilityHint(
                        issue.byteRange != nil
                            ? Text("Double-tap to view the highlighted hex range.") : Text(""))
            }

            private var accessibilityLabel: String {
                var components: [String] = [
                    "\(issue.severity.accessibilityDescription) issue", issue.code, issue.message,
                ]
                if let range = issue.byteRange {
        ...
        private func bookmarkControl() -> some View {
            Button {
                ...
                Label(
                    ...
                    systemImage: annotationSession.isSelectedNodeBookmarked
                        ? "bookmark.fill" : "bookmark")
            }.buttonStyle(.bordered).disabled(!annotationSession.isEnabled || selectedNodeID == nil)
                .accessibilityLabel(
        ...
        @ViewBuilder private func fieldAnnotationSection() -> some View {
            VStack(alignment: .leading, spacing: DS.Spacing.s) {
                ...
                if viewModel.annotations.isEmpty && viewModel.annotationError == nil {
                    Text("No field annotations available for this selection.").font(.caption)
                        .foregroundColor(.secondary)
                } else {
                    VStack(alignment: .leading, spacing: DS.Spacing.xs) {
                        ForEach(viewModel.annotations) { annotation in annotationRow(annotation) }
                    }
                }
            }.nestedAccessibilityIdentifier(ParseTreeAccessibilityID.Detail.fieldAnnotations)
        }

        @ViewBuilder private func userNotesSection() -> some View {
            VStack(alignment: .leading, spacing: DS.Spacing.s) {
                sectionHeader(title: "Notes", icon: "note.text")
                if !annotationSession.isStoreAvailable {
                    Text("Notes are unavailable on this platform.").font(.caption).foregroundColor(
                        .secondary)
                } else if !annotationSession.isEnabled || selectedNodeID == nil {
                    Text("Select a box in a parsed file to add notes and bookmarks.").font(.caption)
                        .foregroundColor(.secondary)
                } else {
                    ...
                    if annotationSession.activeAnnotations.isEmpty {
                        Text("No notes yet. Add your first note below.").font(.caption)
                            .foregroundColor(.secondary)
                    } else {
                        VStack(alignment: .leading, spacing: DS.Spacing.s) {
                            ForEach(annotationSession.activeAnnotations) { record in
                                AnnotationNoteRow(
                                    record: record,
                                    onUpdate: { updated in
                                        annotationSession.updateAnnotation(
                                            id: record.id, note: updated)
                                    },
                                    onDelete: { annotationSession.deleteAnnotation(id: record.id) })
                            }
                        }
                    }
                    noteComposer
        ...
        private var noteComposer: some View {
            VStack(alignment: .leading, spacing: DS.Spacing.xs) {
                Text("Add note").font(.caption).foregroundColor(.secondary)
                TextEditor(text: $draftNote).frame(minHeight: 72).overlay {
                    RoundedRectangle(cornerRadius: 8, style: .continuous).stroke(
                        Color.gray.opacity(0.2))
                }.focused(focusTarget, equals: .notes)
                HStack {
                    Spacer()
                    Button("Save") { submitNote() }.buttonStyle(.borderedProminent).disabled(
        ...
        private func annotationRow(_ annotation: PayloadAnnotation) -> some View {
            ...
            return Button {
                ...
                VStack(alignment: .leading, spacing: DS.Spacing.xxs) {
                    HStack(alignment: .firstTextBaseline) {
                        Text(annotation.label).font(.caption).fontWeight(.semibold)
                        Spacer()
                        Text(annotation.formattedRange).font(.caption2).foregroundColor(.secondary)
                    }
                    Text(annotation.value).font(.system(.body, design: .monospaced)).textSelection(
                        .enabled)
                    if let summary = annotation.summary, !summary.isEmpty {
                        Text(summary).font(.caption).foregroundColor(.secondary)
                    }
                }.frame(maxWidth: .infinity, alignment: .leading).padding(DS.Spacing.s)
            }.buttonStyle(.plain).background {
                RoundedRectangle(cornerRadius: 8, style: .continuous).fill(
                    isSelected ? Color.accentColor.opacity(0.15) : Color.gray.opacity(0.06))
            }.overlay {
                RoundedRectangle(cornerRadius: 8, style: .continuous).stroke(
                    isSelected ? Color.accentColor : Color.gray.opacity(0.2),
                    lineWidth: isSelected ? 1 : 0.5)
            }.accessibilityElement(children: .combine).accessibilityLabel(
                annotationAccessibilityLabel(for: annotation)
            ).accessibilityAddTraits(isSelected ? .isSelected : [])
        }

        private func annotationAccessibilityLabel(for annotation: PayloadAnnotation) -> String {
            var components: [String] = [
                annotation.label, annotation.formattedRange, annotation.value,
            ]
            if let summary = annotation.summary, !summary.isEmpty { components.append(summary) }
            return components.joined(separator: ". ")
        }
        ...
        private func validationSection(detail: ParseTreeNodeDetail) -> some View {
            VStack(alignment: .leading, spacing: DS.Spacing.s) {
                ...
                if detail.validationIssues.isEmpty {
                    Text("No validation issues detected.").font(.caption).foregroundColor(
                        .secondary)
                } else {
                    VStack(alignment: .leading, spacing: DS.Spacing.s) {
                        ForEach(Array(detail.validationIssues.enumerated()), id: \.offset) {
                            _, issue in validationRow(issue: issue)
                        }
                    }
                }
        ...
        private func validationRow(issue: ValidationIssue) -> some View {
            VStack(alignment: .leading, spacing: DS.Spacing.xxs) {
                HStack(spacing: 8) {
                    ...
                    Text(issue.ruleID).font(.caption).foregroundColor(.secondary)
                    Spacer()
                    Button {
                ...
                Text(issue.message).font(.body).textSelection(.enabled)
            }.padding(DS.Spacing.s).background(
        ...
        private func hexSection(detail: ParseTreeNodeDetail) -> some View {
            VStack(alignment: .leading, spacing: DS.Spacing.s) {
                ...
                if let slice = detail.hexSlice, !slice.isEmpty {
                    VStack(alignment: .leading, spacing: DS.Spacing.s) {
                        Text("Offsets \(slice.offset) – \(slice.endOffset)").font(.caption)
                            .foregroundColor(.secondary)
                        HexSliceView(
                            slice: slice, highlightedRange: viewModel.highlightedRange,
                            bytesPerRow: bytesPerRow, focusTarget: focusTarget,
                            onSelectOffset: { viewModel.selectByte(at: $0) })
                    }
                } else {
                    Text("No payload bytes available for this selection.").font(.caption)
                        .foregroundColor(.secondary)
                }
                ...
                ScrollViewReader { proxy in
                    ScrollView([.vertical, .horizontal]) {
                        LazyVStack(alignment: .leading, spacing: DS.Spacing.xs) {
                            ForEach(rows) { row in
                                HexSliceRowView(
                                    row: row, bytesPerRow: bytesPerRow,
                                    highlightedRange: highlightedRange, focusTarget: focusTarget,
                                    onSelectOffset: { select(offset: $0) }
                                ).id(row.index)
                            }
                        }.padding(DS.Spacing.s)
                    }.frame(minHeight: 200, idealHeight: 240, maxHeight: 320).background(
                    ...
                    }.focused(focusTarget, equals: .hex).compatibilityFocusable().onAppear {
                        ...
                        .onMoveCommand { direction in
                            ...
                            guard let nextOffset = nextOffset(from: offset, direction: direction)
                            else { return }
    ...
    extension ParseTreeDetailView.HexSliceView {
        private var rows: [HexSliceRow] {
            ...
            return stride(from: 0, to: bytes.count, by: bytesPerRow).enumerated().map {
                index, start in
        ...
        private struct EncryptionSummary {
            ...
            var accessibilityLabel: String {
                ...
                func append(_ title: String, rows: [(String, String)]) {
                    ...
                    for (label, value) in rows { segments.append("\(label) \(value)") }
                }
    ...
    extension ParseTreeNodeDetail {
        ...
        var sampleRows: [(String, String)] {
            ...
            if let encryption = payload?.sampleEncryption {
                ...
                sampleRows.append(
                    ("Overrides Defaults", encryption.overrideTrackEncryptionDefaults.yesOrNo))
                sampleRows.append(
                    ("Subsample Encryption", encryption.usesSubsampleEncryption.yesOrNo))
    ...
    extension Range where Bound == Int64 {
        fileprivate var prettyString: String { "\(lowerBound) – \(upperBound)" }
    }

    extension Bool { fileprivate var yesOrNo: String { self ? "Yes" : "No" } }
        ...
        struct HexSliceRowView: View {
            ...
            var body: some View {
                HStack(alignment: .top, spacing: DS.Spacing.m) {
                    Text(String(format: "%08llX", UInt64(max(0, row.offset)))).font(
                        ...
                        ForEach(0..<bytesPerRow, id: \.self) { index in
                            if index < row.bytes.count {
                                ...
                                let label = accessibilityFormatter.label(
                                    for: row.bytes[index], at: globalOffset,
                                    highlighted: isHighlighted(globalOffset))
                                HexByteCell(
                                    text: String(format: "%02X", row.bytes[index]),
                                    isHighlighted: isHighlighted(globalOffset), width: 28,
                        ...
                        ForEach(0..<bytesPerRow, id: \.self) { index in
                            if index < row.bytes.count {
                                ...
                                HexByteCell(
                                    ...
                                    isHighlighted: isHighlighted(globalOffset), width: 16,
                                    accessibilityLabel: accessibilityFormatter.label(
                                        for: row.bytes[index], at: globalOffset,
                                        highlighted: isHighlighted(globalOffset))
                                ) { onSelectOffset(globalOffset) }.accessibilityHidden(true)
                            } else {
    ...
    private struct HexByteCell: View {
        ...
        var body: some View {
            Button(action: onTap) {
                Text(text).font(.system(.body, design: .monospaced)).frame(width: width, height: 24)
            }.buttonStyle(.plain).background {
                RoundedRectangle(cornerRadius: 4, style: .continuous).fill(
                    isHighlighted ? Color.accentColor.opacity(0.25) : Color.clear)
            }.overlay {
            ...
            }.contentShape(Rectangle()).accessibilityAddTraits(isHighlighted ? .isSelected : [])
        ...
        struct AnnotationNoteRow: View {
            ...
            var body: some View {
                VStack(alignment: .leading, spacing: DS.Spacing.xs) {
                    HStack(alignment: .firstTextBaseline) {
                        Text(
                            "Updated " + record.updatedAt.formatted(.relative(presentation: .named))
                        ...
                        if isEditing {
                            Button("Cancel") {
                                isEditing = false
                                draft = record.note
                            }.font(.caption).nestedAccessibilityIdentifier(
                            ...
                            Button("Save") {
                                ...
                                onUpdate(trimmed)
                                isEditing = false
                            }.font(.caption).buttonStyle(.borderedProminent).disabled(
                            ...
                            }.buttonStyle(.borderless).font(.caption).accessibilityLabel(
                                "Delete note"
                            ).nestedAccessibilityIdentifier(
                    ...
                    if isEditing {
                        TextEditor(text: $draft).frame(minHeight: 60).overlay {
                            RoundedRectangle(cornerRadius: 6, style: .continuous).stroke(
                                Color.gray.opacity(0.2))
                        }
                    } else {
                        Text(record.note).font(.body).textSelection(.enabled)
                    }
                }.padding(DS.Spacing.s).background {
                    RoundedRectangle(cornerRadius: 8, style: .continuous).fill(
                ...
                ).onAppear { draft = record.note }.onChangeCompatibility(of: record.note) {
                    newValue in if !isEditing { draft = newValue }
                }
            }
        }
    }
    ...
    extension ParseTreeDetailView {
        struct SeverityBadge: View {
            ...
            var body: some View {
                Text(severity.label.uppercased()).font(.caption2).fontWeight(.semibold).padding(
                    .horizontal, DS.Spacing.xs
                ).padding(.vertical, DS.Spacing.xs / 2).background(severity.color.opacity(0.2))
                    .foregroundColor(severity.color).clipShape(
