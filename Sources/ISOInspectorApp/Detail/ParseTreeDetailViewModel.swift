    ...
    @MainActor final class ParseTreeDetailViewModel: ObservableObject {
    ...
    extension ParseTreeDetailViewModel {
        ...
        func selectByte(at offset: Int64) {
            guard let annotation = annotations.first(where: { $0.byteRange.contains(offset) })
            else { return }
    ...
    extension ParseTreeDetailViewModel {
        private func rebuildDetail() {
            ...
            let detail = ParseTreeNodeDetail(
                header: node.header, metadata: metadata, payload: node.payload,
                validationIssues: node.validationIssues, issues: node.issues, status: node.status,
                snapshotTimestamp: snapshot.lastUpdatedAt, hexSlice: nil)
            self.detail = detail
            hexError = nil
            ...
            if let payload = node.payload, let derived = annotations(from: payload),
                !derived.isEmpty
            {
                annotations = derived
    ...
    extension ParseTreeDetailViewModel {
        func focusIssue(on range: Range<Int64>) {
            ...
            guard let selectedID, let node = findNode(with: selectedID, in: snapshot.nodes),
                let detail
            else { return }
        ...
        private func loadHexSlice(
            for node: ParseTreeNode, detail: ParseTreeNodeDetail, focusRange: Range<Int64>? = nil
        ) {
            guard let provider = hexSliceProvider else { return }
        ...
        private func loadAnnotations(
            for node: ParseTreeNode, preserving preservedID: PayloadAnnotation.ID?
        ) {
        ...
        private func annotations(from payload: ParsedBoxPayload) -> [PayloadAnnotation]? {
            let mapped = payload.fields.compactMap { field -> PayloadAnnotation? in
                ...
                return PayloadAnnotation(
                    label: field.name, value: field.value, byteRange: range,
    ...
    extension ParseTreeDetailViewModel {
        private func makeWindow(for header: BoxHeader, focusRange: Range<Int64>? = nil)
