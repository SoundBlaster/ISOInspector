...
final class RandomAccessPayloadAnnotationProvider: PayloadAnnotationProvider {
    ...
    init(reader: RandomAccessReader) { self.reader = reader }
    ...
    func annotations(for header: BoxHeader) async throws -> [PayloadAnnotation] {
        ...
        case "ftyp":
            annotations.append(
                contentsOf: try parseFileTypeBox(
                    header: header, start: payloadStart, end: payloadEnd))
        case "mvhd":
            annotations.append(
                contentsOf: try parseMovieHeaderBox(
                    header: header, start: payloadStart, end: payloadEnd))
        default: break
        }

        return annotations.sorted { lhs, rhs in lhs.byteRange.lowerBound < rhs.byteRange.lowerBound
        }
    }
}

extension RandomAccessPayloadAnnotationProvider {
    fileprivate func annotations(from payload: ParsedBoxPayload) -> [PayloadAnnotation]? {
        let mapped = payload.fields.compactMap { field -> PayloadAnnotation? in
            ...
            return PayloadAnnotation(
                label: field.name, value: field.value, byteRange: range, summary: field.description)
        }
        return mapped.isEmpty ? nil : mapped
    }

    fileprivate func parseFileTypeBox(header: BoxHeader, start: Int64, end: Int64) throws
        -> [PayloadAnnotation]
    {
        ...
        results.append(
            PayloadAnnotation(
                label: "major_brand", value: majorBrand.rawValue, byteRange: start..<(start + 4),
                summary: "Primary brand identifying the file"))

        let minorVersion = try reader.readUInt32(at: start + 4)
        results.append(
            PayloadAnnotation(
                label: "minor_version", value: String(minorVersion),
                byteRange: (start + 4)..<(start + 8), summary: "File type minor version"))
        ...
        while offset + 4 <= end {
            ...
            results.append(
                PayloadAnnotation(
                    label: "compatible_brand[\(index)]", value: brand.rawValue, byteRange: range,
    ...
    fileprivate func parseMovieHeaderBox(header: BoxHeader, start: Int64, end: Int64) throws
        -> [PayloadAnnotation]
    {
        var results: [PayloadAnnotation] = []
        var cursor = start
        ...
        results.append(
            PayloadAnnotation(
                label: "version", value: String(version), byteRange: cursor..<(cursor + 1),
        ...
        if let flagsData = try safeRead(at: cursor, count: 3, end: end) {
            ...
            results.append(
                PayloadAnnotation(
                    label: "flags", value: String(format: "0x%06X", flags),
                    byteRange: cursor..<(cursor + 3), summary: "Bit flags"))
        }
        cursor += 3
        ...
        if version == 1 {
            if let creation = try readUInt64(at: cursor, end: end) {
                results.append(
                    PayloadAnnotation(
                        label: "creation_time", value: String(creation),
                        byteRange: cursor..<(cursor + 8), summary: "Movie creation timestamp"))
                cursor += 8
            }
            if let modification = try readUInt64(at: cursor, end: end) {
                results.append(
                    PayloadAnnotation(
                        label: "modification_time", value: String(modification),
                        byteRange: cursor..<(cursor + 8), summary: "Last modification timestamp"))
                cursor += 8
            }
            if let timescale = try readUInt32(at: cursor, end: end) {
                results.append(
                    PayloadAnnotation(
                        label: "timescale", value: String(timescale),
                        byteRange: cursor..<(cursor + 4), summary: "Time units per second"))
                cursor += 4
            }
            if let duration = try readUInt64(at: cursor, end: end) {
                results.append(
                    PayloadAnnotation(
                        ...
                        byteRange: cursor..<(cursor + 8), summary: "Movie duration"))
                cursor += 8
            }
        } else {
            if let creation = try readUInt32(at: cursor, end: end) {
                results.append(
                    PayloadAnnotation(
                        label: "creation_time", value: String(creation),
                        byteRange: cursor..<(cursor + 4), summary: "Movie creation timestamp"))
                cursor += 4
            }
            if let modification = try readUInt32(at: cursor, end: end) {
                results.append(
                    PayloadAnnotation(
                        label: "modification_time", value: String(modification),
                        byteRange: cursor..<(cursor + 4), summary: "Last modification timestamp"))
                cursor += 4
            }
            if let timescale = try readUInt32(at: cursor, end: end) {
                results.append(
            ...
            if let duration = try readUInt32(at: cursor, end: end) {
                results.append(
                    PayloadAnnotation(
                        label: "duration", value: String(duration),
                        byteRange: cursor..<(cursor + 4), summary: "Movie duration"))
                cursor += 4
            }
        }
        ...
        if let rate = try readUInt32(at: cursor, end: end) {
            ...
            results.append(
                PayloadAnnotation(
                    label: "rate", value: fixed, byteRange: cursor..<(cursor + 4),
        ...
        if let volumeValue = try safeRead(at: cursor, count: 2, end: end) {
            ...
            results.append(
                PayloadAnnotation(
                    label: "volume", value: String(format: "%.2f", volume),
                    byteRange: cursor..<(cursor + 2), summary: "Playback volume"))
            cursor += 2
        }
        ...
        if cursor + reservedLength <= end {
            results.append(
                PayloadAnnotation(
                    label: "reserved", value: "10 bytes",
                    byteRange: cursor..<(cursor + reservedLength), summary: "Reserved"))
        ...
        if cursor + matrixLength <= end {
            results.append(
                PayloadAnnotation(
                    label: "matrix", value: "3x3", byteRange: cursor..<(cursor + matrixLength),
        ...
        if cursor + predefinedLength <= end {
            results.append(
                PayloadAnnotation(
                    label: "pre_defined", value: "24 bytes",
                    byteRange: cursor..<(cursor + predefinedLength), summary: "Pre-defined"))
        ...
        if let nextTrackID = try readUInt32(at: cursor, end: end) {
            results.append(
                PayloadAnnotation(
                    label: "next_track_ID", value: String(nextTrackID),
                    byteRange: cursor..<(cursor + 4), summary: "Next available track identifier"))
        }

        return results
    }

    fileprivate func safeRead(at offset: Int64, count: Int, end: Int64) throws -> Data? {
    ...
    fileprivate func readUInt32(at offset: Int64, end: Int64) throws -> UInt32? {
        guard offset + 4 <= end else { return nil }
        return try reader.readUInt32(at: offset)
    }

    fileprivate func readUInt64(at offset: Int64, end: Int64) throws -> UInt64? {
        guard offset + 8 <= end else { return nil }
        return try reader.readUInt64(at: offset)
    }

    fileprivate func formatFixedPoint(_ value: UInt32, integerBits: Int) -> String {
