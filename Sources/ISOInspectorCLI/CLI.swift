...
public struct ISOInspectorCLIEnvironment: Sendable {
    ...
    public init(
        ...
        }, logVerbosity: LogVerbosity = .standard, telemetryEnabled: Bool = true,
        auditInfo: @escaping @Sendable (String) -> Void = { message in Swift.print(message) },
    ...
    public func makeFilesystemAccess() -> FilesystemAccess {
        if telemetryEnabled {
            ...
            let logger = FilesystemAccessLogger(
                info: { infoSink($0) }, error: { errorSink($0) },
                auditTrail: filesystemAccessAuditTrail, makeDate: { Date.init() })
            return makeFilesystemAccessInstance(logger)
        } else {
            let logger = FilesystemAccessLogger(
                info: { _ in }, error: { _ in }, auditTrail: FilesystemAccessAuditTrail(limit: 0),
                makeDate: { Date.init() })
            return makeFilesystemAccessInstance(logger)
        }
    }
...
public enum ISOInspectorCLIRunner {
    public static func run(
        ...
        switch first {
        case "mp4ra": handleMP4RACommand(Array(args.dropFirst()), environment: environment)
        case "inspect": handleInspectCommand(Array(args.dropFirst()), environment: environment)
        case "export-json":
            handleExportCommand(Array(args.dropFirst()), environment: environment, mode: .json)
        case "export-capture":
            handleExportCommand(Array(args.dropFirst()), environment: environment, mode: .capture)
        case "export-text":
            handleExportCommand(Array(args.dropFirst()), environment: environment, mode: .text)
        default: ISOInspectorCLIIO.printWelcome()
        }
    }
    ...
    private static func handleMP4RACommand(
        _ arguments: [String], environment: ISOInspectorCLIEnvironment
    ) {
        ...
        case "refresh":
            ...
            case .success(let options):
                ...
                } catch { environment.printError("Failed to refresh MP4RA catalog: \(error)") }
            case .failure(let error): environment.printError(error.message)
            }
        default: environment.printError("Unknown mp4ra subcommand: \(subcommand)")
        }
    }
    ...
    private enum RefreshParseError: Swift.Error {
        ...
        var message: String {
            switch self {
            case .missingOutput: return "Missing value for --output."
            case .invalidSource: return "Invalid value for --source."
            case .unknownOption(let option): return "Unknown option for mp4ra refresh: \(option)"
            }
        }
    }
        ...
        case .success(let options):
            do {
                ...
                let events = environment.parsePipeline.events(
                    for: reader,
                    context: .init(
                        source: options.fileURL, researchLog: researchLog,
                        options: options.parseOptions, issueStore: issueStore))
                ...
                Task {
                    ...
                    } catch { environment.printError("Failed to inspect file: \(error)") }
                    semaphore.signal()
                }

                semaphore.wait()
                environment.emitCorruptionSummaryIfNeeded(
                    parseOptions: options.parseOptions, issueStore: issueStore)
            } catch { environment.printError("Failed to inspect file: \(error)") }
        }
    }
    ...
    private enum InspectParseError: Swift.Error {
        ...
        var message: String {
            switch self {
            case .missingFilePath: return "inspect requires a file path."
            case .missingResearchLogPath: return "Missing value for --research-log."
            case .unknownOption(let value): return "Unknown option for inspect: \(value)"
            case .conflictingParseMode: return "Cannot combine --tolerant and --strict for inspect."
            }
        }
    }

    private static func parseInspectOptions(
        _ arguments: [String], cwd: URL, defaultResearchLogURL: URL
        ...
        while let argument = iterator.next() {
            switch argument {
            case "--research-log":
                guard let value = iterator.next() else { return .failure(.missingResearchLogPath) }
            ...
            default:
                if argument.hasPrefix("-") { return .failure(.unknownOption(argument)) }
                filePath = argument
            }
        }

        guard let filePath else { return .failure(.missingFilePath) }
    ...
    private static func parseRefreshOptions(_ arguments: [String]) -> Result<
        RefreshOptions, RefreshParseError
    > {
        var iterator = arguments.makeIterator()
        var source = MP4RARegistryEndpoints.boxes
        let cwd = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        var output = cwd.appendingPathComponent("Sources").appendingPathComponent("ISOInspectorKit")
            .appendingPathComponent("Resources").appendingPathComponent("MP4RABoxes.json")
            .standardizedFileURL

        while let argument = iterator.next() {
            switch argument {
            case "--output", "-o":
                guard let path = iterator.next() else { return .failure(.missingOutput) }
            ...
            case "--source":
                ...
                source = url
            default: return .failure(.unknownOption(argument))
            }
        }
    ...
    private enum ExportParseError: Swift.Error {
        ...
        var message: String {
            switch self {
            case .missingFilePath: return "export commands require a file path."
            case .missingOutput: return "Missing value for --output."
            case .unexpectedArgument(let argument):
                return "Unexpected argument for export command: \(argument)"
            case .unknownOption(let option): return "Unknown option for export command: \(option)"
            case .conflictingParseMode:
    ...
    private enum ExportExecutionError: Swift.Error {
        ...
        var message: String {
            switch self {
            case .unwritableDestination(let path): return "Destination is not writable: \(path)"
            }
        }
    }

    private static func handleExportCommand(
        _ arguments: [String], environment: ISOInspectorCLIEnvironment, mode: ExportMode
    ) {
        let cwd = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        ...
        case .json:
            defaultOutput = { fileURL in
                fileURL.appendingPathExtension("isoinspector").appendingPathExtension("json")
            }
        case .capture: defaultOutput = { fileURL in fileURL.appendingPathExtension("capture") }
        case .text:
            defaultOutput = { fileURL in
                fileURL.appendingPathExtension("isoinspector").appendingPathExtension("txt")
            }
        }

        switch parseExportOptions(arguments, cwd: cwd, defaultOutput: defaultOutput) {
        case .failure(let error): environment.printError(error.message)
        case .success(let options):
            do { try validateOutputPath(options.outputURL) } catch let validationError
                as ExportExecutionError
            {
            ...
            Task {
                do {
                    ...
                    let stream = environment.parsePipeline.events(
                        for: reader,
                        context: .init(
                            source: options.fileURL, options: options.parseOptions,
                    ...
                    case .text:
                        let metadata = PlaintextIssueSummaryExporter.Metadata(
                            ...
                            fileSize: reader.length, analyzedAt: Date(), sha256: nil)
                ...
                } catch { environment.printError(mode.failurePrefix + "\(error)") }
                semaphore.signal()
            }
    ...
    private static func parseExportOptions(
        _ arguments: [String], cwd: URL, defaultOutput: (URL) -> URL
        ...
        while let argument = iterator.next() {
            switch argument {
            case "--output", "-o":
                guard let value = iterator.next() else { return .failure(.missingOutput) }
                let resolved = URL(fileURLWithPath: value, relativeTo: cwd)
    ...
    private static func validateOutputPath(_ url: URL) throws {
        ...
        else { throw ExportExecutionError.unwritableDestination(parent.path) }
...
extension ISOInspectorCLIEnvironment {
    func emitCorruptionSummaryIfNeeded(
        parseOptions: ParsePipeline.Options, issueStore: ParseIssueStore? = nil
    ) {
...
enum ISOInspectorCLIIO {
    static func welcomeSummary() -> String { ISOInspectorKit.welcomeMessage() }

    static func printWelcome() { print(welcomeSummary()) }
}
