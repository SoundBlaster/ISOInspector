...
public struct ISOInspectorCommand: AsyncParsableCommand {
    ...
    public static let configuration = CommandConfiguration(
        ...
        abstract:
            "\(ISOInspectorKit.projectName) CLI for streaming ISO Base Media (MP4/QuickTime) inspection.",
        discussion:
            "Inspect, validate, and export parse events with shared streaming services. Subcommands land in subsequent tasks.",
        subcommands: [
            Commands.Inspect.self, Commands.Validate.self, Commands.Export.self,
            Commands.Batch.self,
        ], helpNames: [.short, .long])

    public static var defaultContextFactory: @Sendable (GlobalOptions) -> ISOInspectorCommandContext
    { { _ in ISOInspectorCommandContext(environment: .live) } }

    @MainActor public static var contextFactory:
        @Sendable (GlobalOptions) -> ISOInspectorCommandContext = defaultContextFactory
    ...
    static func bootstrap(with options: GlobalOptions) async {
        await MainActor.run {
            ...
            case .verbose:
                ...
                environment.print = { message in basePrint("[verbose] \(message)") }
            case .standard: environment.logVerbosity = .standard
            }

            switch options.resolvedTelemetryMode {
            case .enabled: environment.telemetryEnabled = true
            case .disabled: environment.telemetryEnabled = false
            }
    ...
    @MainActor static func resolveContext(applying options: GlobalOptions) async
    ...
    public struct GlobalOptions: ParsableArguments, Sendable {
        ...
        @Flag(name: .long, help: "Print additional diagnostics while streaming parse events.")
        public var verbose: Bool = false

        @Flag(name: .long, help: "Enable streaming telemetry metrics even if disabled by defaults.")
        public var enableTelemetry: Bool = false

        @Flag(name: .long, help: "Disable streaming telemetry metrics for this invocation.") public
            var disableTelemetry: Bool = false

        @Flag(name: .long, help: "Continue through corruption without aborting the parse stream.")
        public var tolerant: Bool = false

        @Flag(
            name: .long, help: "Abort on corruption and surface strict-mode diagnostics (default).")
        public var strict: Bool = false

        @Option(
            name: .long,
            help: ArgumentHelp(
                ISOInspectorCommand.GlobalOptions.presetOptionHelpText(), valueName: "PRESET"))
        public var preset: String?

        @Flag(name: .long, help: "Shortcut for '--preset structural'.") public var structuralOnly:
            Bool = false
        ...
        @Option(
            ...
            help: ArgumentHelp(
                ...
                valueName: "RULE_ID")) public var enableRule: [String] = []

        @Option(
            name: .long, parsing: .upToNextOption,
            help: ArgumentHelp(
                ISOInspectorCommand.GlobalOptions.ruleToggleHelpText(action: "Disable"),
                valueName: "RULE_ID")) public var disableRule: [String] = []
        ...
        public var resolvedTelemetryMode: TelemetryMode {
            if disableTelemetry { return .disabled }

            if enableTelemetry { return .enabled }
        ...
        func makeValidationConfiguration(presets: [ValidationPreset], defaultPresetID: String)
        ...
        private static func presetOptionHelpText() -> String {
            ...
            return
                "Select a validation preset identifier (aliases: \(aliases)). Available presets: \(listings.joined(separator: "; "))."
        }

        private static func ruleToggleHelpText(action: String) -> String {
            let identifiers = ValidationRuleIdentifier.allCases.map(\.rawValue).sorted()
            return
                "\(action) specific validation rule identifiers for this run. Known IDs: \(identifiers.joined(separator: ", "))."
        }

        private func resolvedPresetID(presets: [ValidationPreset], defaultPresetID: String) throws
            -> (String, Bool)
        {
        ...
        private func resolvedRuleOverrides() throws -> [ValidationRuleIdentifier: Bool] {
            let mapping = Dictionary(
                uniqueKeysWithValues: ValidationRuleIdentifier.allCases.map { ($0.rawValue, $0) })
            ...
            for id in enableRule {
                ...
                if let existing = overrides[identifier], existing == false {
                    throw ValidationError(
                        "Validation rule \(id) cannot be both enabled and disabled.")
                }
        ...
        private func selectedAliasPresetIDs() -> [String] {
            var ids: [String] = []
            if structuralOnly { ids.append("structural") }
            return ids
        }
    ...
    public enum Commands {
        public struct Inspect: AsyncParsableCommand {
            public static let configuration = CommandConfiguration(
                abstract:
                    "Stream parse events, validation summaries, and research log hooks for a media file."
            )

            @Argument(help: "Path to the media file to inspect.") public var file: String

            @Option(
                name: .long,
                help:
                    "Optional output location for the VR-006 research log (defaults to ~/.isoinspector/research-log.json)."
            ) public var researchLog: String?

            @OptionGroup public var globalOptions: ISOInspectorCommand.GlobalOptions

            public init() { self._globalOptions = .init() }
            ...
            public mutating func run() async throws {
                ...
                if let researchLog {
                    researchLogURL =
                        URL(fileURLWithPath: researchLog, relativeTo: cwd).standardizedFileURL
                } else {
                ...
                do {
                    ...
                    let events = environment.parsePipeline.events(
                        for: reader,
                        context: .init(
                            source: fileURL, researchLog: researchLog, options: parseOptions,
                            issueStore: issueStore))

                    defer {
                        environment.emitCorruptionSummaryIfNeeded(
                            parseOptions: parseOptions, issueStore: issueStore)
                    }

                    do {
                        for try await event in events {
                            let filtered = ISOInspectorCommand.filteredEvent(
                                event, removing: disabledRuleIDs)
        ...
        public struct Validate: AsyncParsableCommand {
            ...
            @Argument(help: "Path to the media file to validate.") public var file: String
            ...
            public mutating func run() async throws {
                ...
                do {
                    ...
                    let events = environment.parsePipeline.events(
                        for: reader,
                        context: .init(
                            source: fileURL, options: parseOptions, issueStore: issueStore))
                    ...
                    do {
                        for try await event in events {
                            ...
                            for issue in filtered.validationIssues {
                                ...
                                details[issue.severity, default: []].append(
                    ...
                    for severity in ValidationIssue.Severity.displayOrder {
                        ...
                        if let messages = details[severity], !messages.isEmpty {
                            for message in messages { environment.print("  • \(message)") }
                        }
                    }

                    if counts[.error, default: 0] > 0 { throw ExitCode(2) }

                    if counts[.warning, default: 0] > 0 { throw ExitCode(1) }
                } catch let exit as ExitCode { throw exit } catch {
                    environment.printError("Failed to validate file: \(error)")
                    throw ExitCode(3)
                }
            }
        ...
        public struct Export: AsyncParsableCommand {
            ...
            public mutating func run() async throws { throw CleanExit.helpRequest(Self.self) }
            ...
            public struct JSON: AsyncParsableCommand {
                ...
                @OptionGroup public var options: Export.Options
            ...
            public struct Options: ParsableArguments, Sendable {
                @Argument(help: "Path to the media file to export.") var file: String

                @Option(
                    name: [.customLong("output"), .short],
                    help: "Optional output location for the exported artifact.") var output: String?

                @OptionGroup var globalOptions: ISOInspectorCommand.GlobalOptions
            ...
            enum Mode {
                ...
                func defaultOutput(for fileURL: URL) -> URL {
                    switch self {
                    case .json:
                        return fileURL.appendingPathExtension("isoinspector")
                            .appendingPathExtension("json")
                    case .capture: return fileURL.appendingPathExtension("capture")
                    case .text:
                        return fileURL.appendingPathExtension("isoinspector")
                ...
                var successMessage: String {
                    switch self {
                    case .json: return "Exported JSON parse tree → "
                    case .capture: return "Exported parse event capture → "
                    case .text: return "Exported issue summary → "
                    }
                }

                var failurePrefix: String {
                    switch self {
                    case .json: return "Failed to export JSON parse tree: "
                    case .capture: return "Failed to export parse event capture: "
                    case .text: return "Failed to export issue summary: "
                    }
                }
            }
            ...
            static func execute(mode: Mode, with options: Options) async throws {
                let cwd = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
                let fileURL = URL(fileURLWithPath: options.file, relativeTo: cwd)
                    .standardizedFileURL
                let outputURL: URL
                ...
                let context = await ISOInspectorCommand.resolveContext(
                    applying: options.globalOptions)
                let environment = context.environment
                ...
                do {
                    ...
                    let data: Data
                    switch mode {
                    case .json: data = try JSONParseTreeExporter().export(tree: tree)
                    case .capture: data = try ParseEventCaptureEncoder().encode(events: captured)
                    case .text:
                        let metadata = PlaintextIssueSummaryExporter.Metadata(
                            filePath: fileURL.standardizedFileURL.path, fileSize: reader.length,
                            analyzedAt: Date(), sha256: nil)
                        ...
                        data = try PlaintextIssueSummaryExporter().export(
                            tree: tree, metadata: metadata, summary: summary, issues: issues)
                    }
            ...
            static func validateOutputPath(_ url: URL) throws {
                ...
                else { throw ExecutionError.unwritableDestination(parent.path) }
        ...
        public struct Batch: AsyncParsableCommand {
            ...
            @Argument(help: "File paths, directory globs, or directories to validate.") public
                var inputs: [String] = []

            @Option(name: .long, help: "Optional CSV output path for the aggregated summary table.")
            ...
            public mutating func run() async throws {
                ...
                case .success(let resolved):
                        ...
                        do {
                            ...
                            do {
                                for try await event in events {
                                    let filtered = ISOInspectorCommand.filteredEvent(
                                        event, removing: disabledRuleIDs)
                                    if case .willStartBox = filtered.kind { boxCount += 1 }
                                ...
                                environment.printError(
                                    "Failed to process \(fileURL.path): \(error)")
                                builder.appendParseFailure(
                                    fileURL: fileURL, size: reader.length,
                            ...
                            builder.append(
                                ...
                                metrics: .init(
                                    size: reader.length,
                                    counts: .init(
                                        boxes: boxCount, errors: errorCount, warnings: warningCount,
                                        info: infoCount), status: status))
                        } catch let exit as ExitCode { throw exit } catch {
                            environment.printError("Failed to open \(fileURL.path): \(error)")
                            builder.appendParseFailure(
                                fileURL: fileURL, size: fallbackSize,
                                reason: String(describing: error))
                        }
                    }
                    ...
                    for line in summary.formattedLines() { environment.print(line) }
                    ...
                    if summary.hasParseFailures { throw ExitCode(3) }

                    if summary.hasValidationFailures { throw ExitCode(2) }

                    if summary.hasWarnings { throw ExitCode(1) }
                }
            }
            ...
            struct ResolutionError: Swift.Error { let message: String }

            struct InputResolver {
                let fileManager: FileManager

                func resolve(inputs: [String], relativeTo cwd: URL) -> Result<
                    ResolvedInputs, ResolutionError
                > {
                    var collected: [URL] = []
                    ...
                    for input in inputs {
                        ...
                        let resolved = URL(fileURLWithPath: expanded, relativeTo: cwd)
                            .standardizedFileURL
                        var isDirectory: ObjCBool = false
                    ...
                    if collected.isEmpty {
                        ...
                        return .failure(
                            .init(
                                message:
                                    "No files matched the provided inputs: \(unmatched.joined(separator: ", "))"
                            ))
                    }
                ...
                private func collectFiles(in directory: URL) -> [URL] {
                    ...
                    for case let fileURL as URL in enumerator {
                        do {
                            ...
                            if values.isRegularFile == true { results.append(fileURL) }
                        } catch { continue }
                    }
                    return results
                }
                ...
                private static func expandGlob(_ pattern: String, relativeTo cwd: URL) -> [URL] {
                    ...
                    if pattern.hasPrefix("/") {
                        ...
                        return resolvedPattern.withCString { pointer in
                            ...
                            guard status == 0, let pathVector = globResult.gl_pathv else {
                                return []
                            }
...
extension ISOInspectorCommand {
    static func validationMetadata(
        configuration: ValidationConfiguration, presets: [ValidationPreset]
    ) -> ValidationMetadata {
        let disabled = ValidationRuleIdentifier.allCases.filter {
            !configuration.isRuleEnabled($0, presets: presets)
        }.map(\.rawValue).sorted()
        return ValidationMetadata(
            activePresetID: configuration.activePresetID, disabledRuleIDs: disabled)
    }

    static func filteredEvent(_ event: ParseEvent, removing disabledRuleIDs: Set<String>)
        -> ParseEvent
    {
        ...
        if filtered.count == event.validationIssues.count { return event }
        return ParseEvent(
            kind: event.kind, offset: event.offset, metadata: event.metadata,
            payload: event.payload, validationIssues: filtered, issues: event.issues)
    }

    static func printValidationMetadataIfNeeded(
        metadata: ValidationMetadata, wasCustomized: Bool,
...
extension ValidationIssue.Severity {
    fileprivate static var displayOrder: [ValidationIssue.Severity] { [.error, .warning, .info] }
