...
- [x] Implement the `hdlr` (handler) parser to close out the remaining Phase C backlog item referenced in the MVP checklists. Coordinate with the existing `BoxParserRegistry` wiring so the streaming pipeline, CLI exports, and SwiftUI detail panes surface handler metadata alongside the new `mdhd` parser coverage. **(Completed — documented in `DOCS/TASK_ARCHIVE/96_C5_hdlr_Handler_Parser/Summary_of_Work.md`.)**
