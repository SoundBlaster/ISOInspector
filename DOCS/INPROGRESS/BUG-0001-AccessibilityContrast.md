...
- Updated `baselinePrefersIncreasedContrast` to prioritise `EnvironmentValues.accessibilityContrast == .increased` behind a Swift 5.9 compiler check, ensuring "Increase Contrast" is recognised even when "Differentiate Without Color" is disabled without breaking older SDK builds.
- Added a guarded unit test (`testEnvironmentValues_RespectsAccessibilityContrast`) to exercise the scenario on Apple platforms while skipping unsupported targets and avoiding compilation on toolchains lacking the API.
