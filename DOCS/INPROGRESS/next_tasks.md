...
- ✅ **T1.2 — Extend ParseTreeNode with Issues and Status Fields** _(Completed)_ — Parse tree nodes now emit tolerant parsing metadata, JSON exports expose `status` and `issues`, and app detail views surface the status indicator. See `DOCS/INPROGRESS/Summary_of_Work.md` for recap.
