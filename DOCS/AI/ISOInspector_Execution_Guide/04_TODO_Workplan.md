...
| C19 | Surface validation presets and per-rule toggles in the UI settings workflow. | Medium | 1.5 | B7, C3 | SwiftUI | Settings pane lists presets, rule toggles, respects global defaults from user preferences, persists per-workspace overrides, and includes a “Reset to Global” affordance per `13_Validation_Rule_Toggle_Presets_PRD.md`. **(Completed — see `DOCS/INPROGRESS/Summary_of_Work.md` for delivery notes.)** |
