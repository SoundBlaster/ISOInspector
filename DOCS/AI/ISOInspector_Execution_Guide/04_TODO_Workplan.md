...
> **Completed:** Task **B7 — Validation Configuration Layer** now lives in `DOCS/TASK_ARCHIVE/145_B7_Validation_Rule_Preset_Configuration/`, establishing presets and per-rule toggles ahead of CLI (D7) and UI (C19) integrations. Follow-up wiring for ISOInspectorCLI (D7) and ISOInspectorApp settings (C19) remains prioritized in `DOCS/INPROGRESS/next_tasks.md`.
>
...
| C19 | Surface validation presets and per-rule toggles in the UI settings workflow. | Medium | 1.5 | B7, C3 | SwiftUI | Settings pane lists presets, rule toggles, respects global defaults from user preferences, persists per-workspace overrides, and includes a “Reset to Global” affordance per `13_Validation_Rule_Toggle_Presets_PRD.md`. *(In Progress — see `DOCS/INPROGRESS/C19_Validation_Preset_UI_Settings_Integration.md`.)* |
