...
| A6 | Enforce SwiftFormat-based formatting locally and in CI. | Medium | 0.5 | A2 | swift-format, GitHub Actions | `.pre-commit-config.yaml` runs `swift format --in-place` on staged Swift files; CI step executes `swift format --mode lint` and fails on diff. Documentation updated in `README.md` tooling section. |
| A7 | Reinstate SwiftLint complexity thresholds across targets. | Medium | 0.75 | A2 | SwiftLint | `.swiftlint.yml` restores `cyclomatic_complexity`, `function_body_length`, `nesting`, and `type_body_length` with agreed limits; pre-commit runs `swiftlint lint --strict`; CI publishes analyzer report artifact. |
| A8 | Gate test coverage using `coverage_analysis.py`. | Medium | 1 | A2 | Python, SwiftPM | `coverage_analysis.py --threshold 0.67` executes in pre-push and GitHub Actions after `swift test --enable-code-coverage`; pushes blocked and workflow fails when ratio drops. Coverage report archived under `Documentation/Quality/` per run. |
| A9 | Automate strict concurrency checks for core and tests. | High | 1 | A2 | SwiftPM, GitHub Actions | Pre-push hook and CI workflow run `swift build --strict-concurrency=complete` and `swift test --strict-concurrency=complete`; logs show zero warnings. Results linked to `PRD_SwiftStrictConcurrency_Store.md` rollout checklist. |

> **Current focus:** _Hardware validation follow-ups for Task A5's random slice benchmarking deliverable._ See `DOCS/TASK_ARCHIVE/65_Summary_of_Work_2025-10-15_Benchmark/Summary_of_Work.md` for the Task A5 wrap-up, reference the newly archived macOS benchmark log at `DOCS/TASK_ARCHIVE/207_Summary_of_Work_2025-11-04_macOS_Benchmark_Block/`, and consult `DOCS/TASK_ARCHIVE/200_T3_7_Integrity_Navigation_Filters/next_tasks.md` for earlier day-to-day queue history. The refreshed `DOCS/INPROGRESS/` folder now carries `Summary_of_Work.md`, `next_tasks.md`, and `blocked.md` dedicated to the macOS 1 GiB benchmark follow-up.
>
