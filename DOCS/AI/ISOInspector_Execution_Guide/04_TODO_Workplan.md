...
| G4 | Implement zero-trust logging and audit trail for file access events. | Medium | 1 | G1 | Swift Logging | Access logs omit absolute paths, include hashed identifiers, and pass diagnostics tests. *(In Progress — see `DOCS/INPROGRESS/G4_Zero_Trust_Logging.md`.)* |
