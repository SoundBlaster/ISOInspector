...
| B7 | Introduce validation configuration layer with per-rule toggles and preset registry. | Medium | 1.5 | B5 | Swift, XCTest | `ValidationConfiguration`/`ValidationPreset` types serialize settings and default to all rules enabled per the new PRD `13_Validation_Rule_Toggle_Presets_PRD.md`. |
...
| C19 | Surface validation presets and per-rule toggles in the UI settings workflow. | Medium | 1.5 | B7, C3 | SwiftUI | Settings pane lists presets, rule toggles, and persists selections per workspace per `13_Validation_Rule_Toggle_Presets_PRD.md`. |
...
| D7 | Expose validation presets and per-rule enable/disable flags on the CLI. | Medium | 1 | B7, D2 | Swift ArgumentParser | `--preset` and rule toggle flags configure `ValidationConfiguration`; help text lists available presets per `13_Validation_Rule_Toggle_Presets_PRD.md`. |

> **Newly In Progress:** Validation follow-up **E5 — Basic `stbl` Coherence Checks** now targets cross-table consistency between `stts/ctts/stsc/stsz/stz2/stco` so diagnostics surface when counts diverge. Scope and objectives tracked in `DOCS/INPROGRESS/E5_Basic_stbl_Coherence_Checks.md`.
