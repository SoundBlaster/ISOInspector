...
| F2 | Configure performance benchmarks for large files. | Medium | 1.5 | B3 | XCTest Metrics | Benchmark thresholds recorded; CI fails when regressions occur. (Completed ✅ — archived in `DOCS/TASK_ARCHIVE/44_F2_Configure_Performance_Benchmarks/Summary_of_Work.md`. Follow-up benchmark execution **In Progress** — see `DOCS/INPROGRESS/50_Combine_UI_Benchmark_macOS_Run.md`.) |
