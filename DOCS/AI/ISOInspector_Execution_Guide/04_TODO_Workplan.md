...
| D6 | Recognize `senc/saio/saiz` sample encryption placeholders and record their offsets and lengths for downstream reporting. | Medium | 1 | D3 | Swift, XCTest | Fragment parses capture stub metadata for these boxes; CLI/UI surface presence without decrypting content. *(In Progress — see `DOCS/INPROGRESS/D6_Recognize_senc_saio_saiz_Placeholders.md`.)* |
