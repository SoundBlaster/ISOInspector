...
| G5 | Provide UIDocumentPicker integration for iOS/iPadOS platform adapters. | Medium | 1 | G1 | UIKit, Swift Concurrency | FilesystemAccessKit presents `UIDocumentPickerViewController`, returns security-scoped URLs, and ships regression coverage without regressing macOS adapters. **(In Progress — implementing UIKit adapters now.)** |
