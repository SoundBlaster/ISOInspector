...
| G3 | Expose CLI flags and sandbox profile guidance for headless access. | Medium | 1 | G1 | Swift ArgumentParser, sandbox profiles | CLI accepts `--open`/`--authorize` inputs mapped to FilesystemAccessKit; documentation updated with sandbox profile steps. *(In Progress — see `DOCS/INPROGRESS/G3_Expose_CLI_Sandbox_Profile_Guidance.md`.)* |
