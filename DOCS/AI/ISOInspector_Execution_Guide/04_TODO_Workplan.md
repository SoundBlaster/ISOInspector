...
| E4a | Evaluate Apple Events automation requirement for notarized builds. | Medium | 0.5 | E4 | Xcode, Notarytool, AppleScript | Determine whether Apple Events automation is required, adjust entitlements/notarization tooling, and document the decision. *(In Progress — see `DOCS/INPROGRESS/56_Distribution_Apple_Events_Notarization_Assessment.md`.)* |
