...
> **Current focus:** _Pending assignment._ Task A2 — Implement `MappedReader` is complete (see `DOCS/INPROGRESS/Summary_of_Work.md`), so the next priority should be selected from the remaining open TODO entries.
