...
| E3 | Implement session persistence (open files, annotations, layout). | Medium | 2 | E2, C4 | CoreData/JSON | Relaunch restores previous workspace state. (Planning ✅ — migration strategy archived in `DOCS/TASK_ARCHIVE/34_E3_CoreData_Migration_Planning/`. **In Progress — tracked via `DOCS/INPROGRESS/E3_Session_Persistence.md`.**) |
