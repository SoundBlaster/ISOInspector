...
>
> **Follow-up:** Validation rule #15 remains open to correlate `stsc` chunk runs, `stsz/stz2` sample sizes, and the new `stco/co64` offsets. *(In Progress — see `DOCS/INPROGRESS/Validation_Rule_15_Sample_Table_Correlation.md`.)* The recreated `DOCS/INPROGRESS/next_tasks.md` now tracks that effort alongside the recently archived MP4RA metadata value type expansion captured in `DOCS/TASK_ARCHIVE/129_C15_Metadata_Value_Type_Expansion/`. _(See `DOCS/TASK_ARCHIVE/129_C15_Metadata_Value_Type_Expansion/C15_Metadata_Value_Type_Expansion.md` for the decoder expansion summary.)_
>
