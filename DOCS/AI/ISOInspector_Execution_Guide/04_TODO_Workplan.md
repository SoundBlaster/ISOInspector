...
| Task ID | Description | Priority | Effort (days) | Dependencies | Tools | Acceptance Criteria |
|---------|-------------|----------|---------------|--------------|-------|---------------------|
| D1 | Scaffold CLI using `swift-argument-parser` with base command. | Medium | 1 | B3 | Swift ArgumentParser | `isoinspector --help` displays subcommands. (**In Progress — executable target scaffolding to unblock Task D2.**) |
