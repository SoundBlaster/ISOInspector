...
| FR-UI-005 | ISOInspectorUI | Provide a floating or modal user settings panel that separates permanent and session-only preferences. | Panel can be invoked via keyboard shortcut, renders FoundationUI-styled sections for permanent vs. session groups, persists permanent choices to the global preferences store, queues session overrides through `DocumentSessionController`, and exposes reset actions for each scope per `14_User_Settings_Panel_PRD.md`. |
...
| NFR-MAINT-005 | Maintainability | CI blocks merges when duplicated Swift code exceeds agreed limits. | `.github/workflows/swift-duplication.yml` runs `scripts/run_swift_duplication_check.sh` (see `DOCS/AI/github-workflows/02_swift_duplication_guard/prd.md`) and fails when `jscpd` reports >1% duplicated lines or a block longer than 45 lines. |
