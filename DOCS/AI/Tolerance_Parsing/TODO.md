...
| T2.4 | Extend validation rules (VR-001 to VR-015) to produce `ParseIssue` objects instead of throwing when in lenient mode. | High | 2d | T1.1, T1.3 | Validation rules check pipeline options; generate issues for lenient, throw for strict. *(Completed — see `DOCS/INPROGRESS/Summary_of_Work.md`.)* |

> **Status (2025-10-27):** T2.4 — Validation rule dual-mode support is **Completed** (see `DOCS/INPROGRESS/Summary_of_Work.md`); tolerant runs now record VR-001…VR-015 diagnostics while strict mode remains unchanged. T2.3 remains pending design handoff for ribbon metrics.
