...
| T3.5 | Add contextual status labels (Invalid, Empty, Corrupted, Partial, Trimmed) to tree view and detail inspector. **(🚧 In Progress — see `DOCS/TASK_ARCHIVE/190_T3_4_Placeholder_Nodes_for_Missing_Children/T3_5_Contextual_Status_Labels.md`.)** | Medium | 1d | T1.2 | Labels reflect node status; color-coded for severity (error=red, warning=yellow, info=blue). |
