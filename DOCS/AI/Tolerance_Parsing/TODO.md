...
| T1.2 | Extend `ParseTreeNode` with `issues: [ParseIssue]` array and `status` enum (valid/partial/corrupt/skipped). | High | 1d | T1.1 | `ParseTreeNode` stores issues; JSON export includes status and issues. *(In Progress — see `DOCS/INPROGRESS/T1_2_Extend_ParseTreeNode_Status_and_Issues.md`.)* |
| T1.3 | Create `ParsePipeline.Options` with `abortOnStructuralError`, `maxCorruptionEvents`, `payloadValidationLevel` toggles. | High | 1d | None | Options type defined; default to lenient for app, strict for CLI (overridable via flags). |
| T1.4 | Refactor `BoxHeaderDecoder` to return `Result<BoxHeader, BoxHeaderDecodingError>` instead of throwing. | High | 2d | T1.1, T1.2 | Decoder returns `.failure` for malformed headers; caller attaches issue and continues to next sibling when safe. |
| T1.5 | Update container iteration logic to catch decoder errors, attach issues to node, and skip to next sibling using parent boundary. | High | 2d | T1.4 | Parser continues after `exceedsParent` or invalid size; generates placeholder node or skips cleanly. |
| T1.6 | Implement binary reader guards: clamp reads to parent boundaries, record `truncatedPayload` issue when size exceeds available bytes. | High | 2d | T1.1 | Reader never reads beyond parent end; truncation issues recorded with byte ranges. |
