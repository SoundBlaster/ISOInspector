...
| T3.4 | Implement placeholder nodes for missing/required children: expected fourcc, attach issues, link to hex viewer. **(✅ Completed — see `DOCS/INPROGRESS/Summary_of_Work.md`)** | Medium | 2d | T1.2 | Missing `stbl` child renders placeholder with "expected but absent" message. |
| T3.5 | Add contextual status labels (Invalid, Empty, Corrupted, Partial, Trimmed) to tree view and detail inspector. **(🚧 In Progress — see `DOCS/INPROGRESS/T3_5_Contextual_Status_Labels.md`.)** | Medium | 1d | T1.2 | Labels reflect node status; color-coded for severity (error=red, warning=yellow, info=blue). |
