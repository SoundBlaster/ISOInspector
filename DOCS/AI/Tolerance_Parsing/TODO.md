...
| T1.3 | Create `ParsePipeline.Options` with `abortOnStructuralError`, `maxCorruptionEvents`, `payloadValidationLevel` toggles. | High | 1d | None | ✅ Complete — Options type introduced with strict/tolerant presets; CLI defaults to strict while app uses tolerant caps. See `DOCS/INPROGRESS/Summary_of_Work.md`. |
