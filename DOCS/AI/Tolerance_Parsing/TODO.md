...
| T1.2 | Extend `ParseTreeNode` with `issues: [ParseIssue]` array and `status` enum (valid/partial/corrupt/skipped). | High | 1d | T1.1 | ✅ Complete — nodes now carry tolerant parsing metadata and exports include the new fields. See `DOCS/INPROGRESS/Summary_of_Work.md`. |
