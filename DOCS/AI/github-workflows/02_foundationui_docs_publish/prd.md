...
- The existing ISOInspector workflow (`.github/workflows/documentation.yml`) already builds DocC successfully; it only needs its hosting base path set to `ISOInspector` so the generated links land under `ISOInspector/documentation/isoinspectorkit/`.
...
- Retain the ISOInspector workflow for ISOInspectorKit only, adjusting its hosting base path to `ISOInspector` so the published site resolves to `ISOInspector/documentation/isoinspectorkit/`.
