...
- [ ] T2.3. Aggregate parse issue metrics for UI and CLI ribbons. _(Blocked — pending design handoff; track design dependency in `DOCS/INPROGRESS/next_tasks.md`.)_
- [x] 🛠️ T2.4. Validation rules emit `ParseIssue` diagnostics in lenient mode. _(Completed — tolerant mode now records VR-001…VR-015 issues via `ParseIssueStore`; see `DOCS/INPROGRESS/Summary_of_Work.md` for details.)_
