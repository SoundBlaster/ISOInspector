...
- [ ] J2. Persist security-scoped bookmarks and integrate with recents/session restoration flows. *(In Progress — depends on `ISOInspectorApp` session store and the FilesystemAccessKit follow-up documented in `DOCS/INPROGRESS/J2_Persist_Security_Scoped_Bookmarks.md`.)*
