...
- [x] 🔴 **P0+** C15. Metadata: `udta`, `meta` (handler), `keys`, `ilst` (basic types). **(Completed — see `DOCS/TASK_ARCHIVE/125_C15_Metadata_Box_Coverage/C15_Metadata_Box_Coverage.md` and `DOCS/TASK_ARCHIVE/125_C15_Metadata_Box_Coverage/Summary_of_Work.md`.)**
  ...
  > **In Progress:** Validation Rule #15 is now being implemented to correlate `stsc` chunk runs, `stsz/stz2` sample sizes, and the new `stco/co64` offsets. See `DOCS/INPROGRESS/Validation_Rule_15_Sample_Table_Correlation.md` for the active task outline and coordination notes with the archived edit list diagnostics in `DOCS/TASK_ARCHIVE/122_C14c_Edit_List_Duration_Validation/`.
- [x] C16. Codec configs:
