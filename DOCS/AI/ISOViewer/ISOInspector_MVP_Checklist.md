...
- [ ] C1. Stubs: `ftyp`, `mvhd`, `tkhd`, `mdhd`, `hdlr` (input/output types only) **(In Progress — `hdlr` parser active; see `DOCS/INPROGRESS/C5_hdlr_Handler_Parser.md`. `mdhd` parser landed per `DOCS/TASK_ARCHIVE/95_C4_mdhd_Media_Header_Parser/Summary_of_Work.md`.)**
