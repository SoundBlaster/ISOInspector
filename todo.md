...
- [ ] #15 Validate sample size tables against chunk mappings now that `stco/co64` chunk offsets are available to correlate `stsc`, `stsz`, and `stz2` counts. *(Follow-up — implement validation rule documented in `DOCS/TASK_ARCHIVE/113_C10_stco_co64_Chunk_Offset_Parser/C10_stco_co64_Chunk_Offset_Parser.md`.)*
