...
- [ ] PDD:30m Surface additional MP4RA metadata data types (e.g., GIF, TIFF, signed fixed-point) when fixtures land so CLI/app exports stay human-readable. *(Track scope in `DOCS/INPROGRESS/next_tasks.md` and reference `DOCS/TASK_ARCHIVE/128_C15_Metadata_Value_Decoding_Expansion/` for delivered coverage.)*
