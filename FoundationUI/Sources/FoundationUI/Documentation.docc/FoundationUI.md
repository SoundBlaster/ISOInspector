...
| Accessibility-First | WCAG 2.1 AA compliant (≥4.5:1 contrast ratios) with full VoiceOver support |
...
Foundation constants for spacing, colors, typography, radii, and animations. Access them via the `DS` namespace:
...
| Magic Numbers | None (100% DS token usage) |
...
| Efficient State | Minimal state updates with O(1) lookups |
| Conditional Rendering | Platform-specific views via `#if` directives |
| Memory Management | Weak references and no retain cycles |
