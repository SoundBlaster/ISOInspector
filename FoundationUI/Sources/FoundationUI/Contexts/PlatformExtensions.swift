...
// MARK: - macOS Keyboard Shortcuts
    ...
    extension View {
        ...
        public func platformKeyboardShortcut(_ type: PlatformKeyboardShortcutType) -> some View {
        ...
        public func platformKeyboardShortcut(
            _ type: PlatformKeyboardShortcutType, action: @escaping () -> Void
        ) -> some View {
            Button(action: action) { self }.keyboardShortcut(
                type.keyEquivalent, modifiers: type.modifiers)
        }
    }
...
// MARK: - iOS Gestures
    ...
    extension View {
        ...
        public func platformLongPressGesture(
            minimumDuration: Double = 0.5, perform action: @escaping () -> Void
        ) -> some View { onLongPressGesture(minimumDuration: minimumDuration, perform: action) }
        ...
        public func platformSwipeGesture(
            direction: PlatformSwipeDirection, perform action: @escaping () -> Void
        ) -> some View {
            gesture(
                DragGesture(minimumDistance: DS.Spacing.xl).onEnded { value in
    ...
    extension View {
            ...
            -> some View
        {
            if UIDevice.current.userInterfaceIdiom == .pad {
                ...
                case .automatic: hoverEffect(.automatic)
                }
            } else {
        ...
        @ViewBuilder public func platformHoverEffect(
            _ style: PlatformHoverEffectStyle = .lift, animation: Animation
        ) -> some View {
            if UIDevice.current.userInterfaceIdiom == .pad {
...
// MARK: - SwiftUI Previews
    ...
    #Preview("macOS Keyboard Shortcuts") {
            ...
            VStack(spacing: DS.Spacing.l) {
                Text("macOS Keyboard Shortcuts").font(DS.Typography.headline)

                Button("Copy (⌘C)") { print("Copy action") }.platformKeyboardShortcut(.copy)

                Button("Paste (⌘V)") { print("Paste action") }.platformKeyboardShortcut(.paste)

                Button("Cut (⌘X)") { print("Cut action") }.platformKeyboardShortcut(.cut)

                Button("Select All (⌘A)") { print("Select all action") }.platformKeyboardShortcut(
                    .selectAll)
            }.padding(DS.Spacing.xl)
        #else
            Text("macOS keyboard shortcuts not available on this platform").padding(DS.Spacing.xl)
        #endif
    }
    ...
    #Preview("iOS Gestures") {
        #if os(iOS)
            VStack(spacing: DS.Spacing.l) {
                Text("iOS Gestures").font(DS.Typography.headline)

                Text("Tap Me").padding(DS.Spacing.l).background(Color.blue.opacity(0.2))
                    .cornerRadius(DS.Radius.medium).platformTapGesture { print("Tapped!") }

                Text("Long Press Me").padding(DS.Spacing.l).background(Color.green.opacity(0.2))
                    .cornerRadius(DS.Radius.medium).platformLongPressGesture {
                        print("Long pressed!")
                    }

                Text("Swipe Left on Me").padding(DS.Spacing.l).background(Color.orange.opacity(0.2))
                    .cornerRadius(DS.Radius.medium).platformSwipeGesture(direction: .left) {
                        print("Swiped left!")
                    }

                Text("Double Tap Me").padding(DS.Spacing.l).background(Color.purple.opacity(0.2))
                    .cornerRadius(DS.Radius.medium).platformTapGesture(count: 2) {
                        print("Double tapped!")
                    }
            }.padding(DS.Spacing.xl)
        #else
            Text("iOS gestures not available on this platform").padding(DS.Spacing.xl)
        #endif
    }
    ...
    #Preview("iPadOS Pointer Interactions") {
        #if os(iOS)
            VStack(spacing: DS.Spacing.l) {
                Text("iPadOS Pointer Interactions").font(DS.Typography.headline)

                Text("Hover Effect (Lift)").padding(DS.Spacing.l).background(
                    Color.blue.opacity(0.2)
                ).cornerRadius(DS.Radius.medium).platformHoverEffect(.lift)

                Text("Hover Effect (Highlight)").padding(DS.Spacing.l).background(
                    Color.green.opacity(0.2)
                ).cornerRadius(DS.Radius.medium).platformHoverEffect(.highlight)

                Text("Hover Effect (Automatic)").padding(DS.Spacing.l).background(
                    Color.orange.opacity(0.2)
                ).cornerRadius(DS.Radius.medium).platformHoverEffect(.automatic)

                Text("Note: Hover effects only visible on iPad devices").font(DS.Typography.caption)
                    .foregroundColor(.secondary)
            }.padding(DS.Spacing.xl)
        #else
            Text("iPadOS hover effects not available on this platform").padding(DS.Spacing.xl)
        #endif
    }

    #Preview("Cross-Platform Integration") {
        VStack(spacing: DS.Spacing.l) {
            Text("Cross-Platform Extensions").font(DS.Typography.headline)

            #if os(macOS)
                Button("macOS: Copy (⌘C)") { print("Copy on macOS") }.platformKeyboardShortcut(
                    .copy)
            #endif

            #if os(iOS)
                Text("iOS: Tap Me").padding(DS.Spacing.l).background(Color.blue.opacity(0.2))
                    .cornerRadius(DS.Radius.medium).platformTapGesture { print("Tapped on iOS") }
                    .platformHoverEffect()  // iPad only
            #endif

            Text("Platform: \(PlatformAdapter.isMacOS ? "macOS" : "iOS")").font(
