...
public struct Indicator: View, Sendable {
    ...
    public var body: some View {
        ...
        return Circle()
            ...
            .overlay(
                Circle()
                    .stroke(level.backgroundColor, lineWidth: size.haloThickness)
            )
            .padding(size.hitPadding)
            .frame(
                minWidth: size.minimumHitTarget.width,
            ...
            .modifier(OptionalAccessibilityHintModifier(hint: accessibilityConfiguration.hint))
...
extension Indicator {
    ...
    public struct Tooltip: Equatable, Sendable {
        ...
        public static func text(_ value: String) -> Tooltip {
            Tooltip(content: .text(value))
        }
...
private struct IndicatorTooltipModifier: ViewModifier {
    ...
    @ViewBuilder
    private func applyTextTooltip(value: String, to content: Content) -> some View {
        switch style {
        case .none:
            content
        default:
