...
public struct Card<Content: View>: View {
    ...
    public init(
        ...
        material: Material? = nil, @ViewBuilder content: () -> Content
    ) {
    ...
    public var body: some View {
        Group {
            if let material {
                // Card with material background
                content.background(material).clipShape(RoundedRectangle(cornerRadius: cornerRadius))
                    .shadow(
                        color: elevation.hasShadow
                            ? Color.black.opacity(elevation.shadowOpacity) : .clear,
                        radius: elevation.shadowRadius, x: 0, y: elevation.shadowYOffset)
            } else {
                // Card with CardStyle modifier (includes background)
                content.cardStyle(
                    elevation: elevation, cornerRadius: cornerRadius, useMaterial: false)
            }
        ...
        VStack(spacing: DS.Spacing.xl) {
            Card(elevation: .none) {
                VStack {
                    ...
                    Text("Flat appearance with no shadow").font(.caption).foregroundStyle(
                        .secondary)
                }.padding()
            }
            ...
            Card(elevation: .low) {
                VStack {
                    ...
                    Text("Subtle shadow for slight depth").font(.caption).foregroundStyle(
                        .secondary)
                }.padding()
            }
            ...
            Card(elevation: .medium) {
                VStack {
                    ...
                    Text("Standard shadow for typical cards").font(.caption).foregroundStyle(
                        .secondary)
                }.padding()
            }
            ...
            Card(elevation: .high) {
                VStack {
                    ...
                    Text("Prominent shadow for emphasized content").font(.caption).foregroundStyle(
                        .secondary)
                }.padding()
            }
        }.padding()
...
#Preview("Card - Corner Radius Variants") {
    VStack(spacing: DS.Spacing.l) {
        Card(cornerRadius: DS.Radius.small) {
            VStack {
                Text("Small Radius").font(.headline)
                Text("6pt corners - compact elements").font(.caption).foregroundStyle(.secondary)
            }.padding()
        }

        Card(cornerRadius: DS.Radius.medium) {
            VStack {
                Text("Medium Radius").font(.headline)
                Text("8pt corners - balanced rounding").font(.caption).foregroundStyle(.secondary)
            }.padding()
        }

        Card(cornerRadius: DS.Radius.card) {
            VStack {
                Text("Card Radius (Default)").font(.headline)
                Text("10pt corners - standard cards").font(.caption).foregroundStyle(.secondary)
            }.padding()
        }
    }.padding()
}
    ...
    ZStack {
        ...
        VStack(spacing: DS.Spacing.l) {
            Card(material: .ultraThin) {
                VStack {
                    Text("Ultra Thin Material").font(.headline)
            ...
            Card(material: .thin) {
                VStack {
                    ...
                    Text("High translucency").font(.caption)
                }.padding()
            }
            ...
            Card(material: .regular) {
                VStack {
                    ...
                    Text("Balanced translucency").font(.caption)
                }.padding()
            }
            ...
            Card(material: .thick) {
                VStack {
                    ...
                    Text("Low translucency").font(.caption)
                }.padding()
            }
        }.padding()
    }
...
#Preview("Card - Dark Mode") {
    VStack(spacing: DS.Spacing.l) {
        Card(elevation: .low) {
            VStack {
                Text("Low Elevation").font(.headline)
                Text("Dark mode shadow rendering").font(.caption).foregroundStyle(.secondary)
            }.padding()
        }
        ...
        Card(elevation: .medium) {
            VStack {
                Text("Medium Elevation").font(.headline)
                Text("Standard dark mode appearance").font(.caption).foregroundStyle(.secondary)
            }.padding()
        }
        ...
        Card(elevation: .high) {
            VStack {
                Text("High Elevation").font(.headline)
                Text("Prominent dark mode depth").font(.caption).foregroundStyle(.secondary)
            }.padding()
        }
...
#Preview("Card - Content Examples") {
    ScrollView {
        VStack(spacing: DS.Spacing.l) {
            // Simple text card
            Card { Text("Simple Card").font(.headline).padding() }
            ...
            Card(elevation: .low) {
                VStack(spacing: DS.Spacing.m) {
                    Image(systemName: "photo.fill").font(.largeTitle).foregroundStyle(.blue)
                    Text("Image Card").font(.headline)
                    Text("Cards can contain any SwiftUI content").font(.caption).foregroundStyle(
                        .secondary
            ...
            Card(elevation: .medium) {
                VStack(alignment: .leading, spacing: DS.Spacing.s) {
                    ...
                    Text("Integration with other FoundationUI components").font(.body)
...
#Preview("Card - Nested Cards") {
    Card(elevation: .high) {
        VStack(alignment: .leading, spacing: DS.Spacing.m) {
            Text("Outer Card").font(.title2).fontWeight(.bold)

            Text("Cards can be nested for hierarchical layouts").font(.body).foregroundStyle(
                .secondary)

            Card(elevation: .low) {
                VStack(alignment: .leading, spacing: DS.Spacing.s) {
                    Text("Inner Card").font(.headline)
                    Text("This card is nested inside another card").font(.caption).foregroundStyle(
                        .secondary)
                }.padding()
            }
            ...
            Card(elevation: .low) {
                VStack(alignment: .leading, spacing: DS.Spacing.s) {
                    Text("Another Inner Card").font(.headline)
                    Text("Multiple cards can be nested").font(.caption).foregroundStyle(.secondary)
                }.padding()
            }
        }.padding()
    }.padding()
...
#Preview("Card - Platform Comparison") {
    VStack(spacing: DS.Spacing.l) {
        Card(elevation: .medium) {
            VStack(alignment: .leading, spacing: DS.Spacing.s) {
                #if os(macOS)
                    Text("macOS Card").font(.headline)
                    Text("Desktop-optimized shadows and spacing").font(.caption).foregroundStyle(
                        .secondary)
                #elseif os(iOS)
                    Text("iOS Card").font(.headline)
                    Text("Touch-optimized shadows and spacing").font(.caption).foregroundStyle(
                        .secondary)
                #else
                    Text("Platform Card").font(.headline)
                    Text("Platform-adaptive styling").font(.caption).foregroundStyle(.secondary)
                #endif
            }.padding()
        }
        ...
        Card(elevation: .high, material: .regular) {
            VStack(alignment: .leading, spacing: DS.Spacing.s) {
                Text("Material + Elevation").font(.headline)
                Text("Combining material backgrounds with elevation").font(.caption)
                    .foregroundStyle(.secondary)
            }.padding()
        }
    }.padding()
...
@available(iOS 17.0, macOS 14.0, *) @MainActor extension Card: AgentDescribable {
    public var componentType: String { "Card" }

    public var properties: [String: Any] {
        var props: [String: Any] = [
            "elevation": elevation.stringValue, "cornerRadius": cornerRadius
        ]

        if let material { props["material"] = String(describing: material) }
...
@available(iOS 17.0, macOS 14.0, *) #Preview("Card - Agent Integration") {
    VStack(alignment: .leading, spacing: DS.Spacing.l) {
        ...
        VStack(alignment: .leading, spacing: DS.Spacing.s) {
            Text("Component Type: \(card.componentType)").font(.caption).foregroundStyle(.secondary)

            Text("Elevation: \(card.properties["elevation"] as? String ?? "unknown")").font(
                .caption
            ).foregroundStyle(.secondary)

            Text("Corner Radius: \(card.properties["cornerRadius"] as? CGFloat ?? 0)pt").font(
                .caption
            ).foregroundStyle(.secondary)

            Text("Semantics: \(card.semantics)").font(.caption).foregroundStyle(.secondary)
