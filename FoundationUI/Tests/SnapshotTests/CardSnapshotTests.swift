...
@MainActor final class CardSnapshotTests: XCTestCase {
    // MARK: - Elevation Tests (Light Mode)

    func testCardElevationNone() {
        let card = Card(elevation: .none) { Text("No Elevation").padding() }
        let view = card.frame(width: 300, height: 150)

        assertSnapshot(of: view, as: .image(layout: .sizeThatFits), record: false)
    }

    func testCardElevationLow() {
        let card = Card(elevation: .low) { Text("Low Elevation").padding() }
        let view = card.frame(width: 300, height: 150)
    ...
    func testCardElevationMedium() {
        let card = Card(elevation: .medium) { Text("Medium Elevation (Default)").padding() }
        let view = card.frame(width: 300, height: 150)
    ...
    func testCardElevationHigh() {
        let card = Card(elevation: .high) { Text("High Elevation").padding() }
        let view = card.frame(width: 300, height: 150)
    ...
    func testCardElevationNoneDarkMode() {
        let card = Card(elevation: .none) { Text("No Elevation").padding() }
        let view = card.frame(width: 300, height: 150).preferredColorScheme(.dark)
    ...
    func testCardElevationMediumDarkMode() {
        let card = Card(elevation: .medium) { Text("Medium Elevation").padding() }
        let view = card.frame(width: 300, height: 150).preferredColorScheme(.dark)
    ...
    func testCardCornerRadiusSmall() {
        let card = Card(elevation: .medium, cornerRadius: DS.Radius.small) {
            Text("Small Corner Radius").padding()
        }
        let view = card.frame(width: 300, height: 150)
    ...
    func testCardCornerRadiusMedium() {
        let card = Card(elevation: .medium, cornerRadius: DS.Radius.medium) {
            Text("Medium Corner Radius").padding()
        }
        let view = card.frame(width: 300, height: 150)
    ...
    func testCardCornerRadiusCard() {
        let card = Card(elevation: .medium, cornerRadius: DS.Radius.card) {
            Text("Card Corner Radius (Default)").padding()
        }
        let view = card.frame(width: 300, height: 150)
    ...
    func testCardWithThinMaterial() {
        let card = Card(elevation: .medium, material: .thin) {
            VStack {
                ...
                Text("Translucent background").font(.caption)
            }.padding()
        }
        let view = card.frame(width: 300, height: 150)
    ...
    func testCardWithRegularMaterial() {
        let card = Card(elevation: .medium, material: .regular) {
            VStack {
                ...
                Text("Standard translucent background").font(.caption)
            }.padding()
        }
        let view = card.frame(width: 300, height: 150)
    ...
    func testCardWithThickMaterial() {
        let card = Card(elevation: .medium, material: .thick) {
            VStack {
                ...
                Text("More opaque background").font(.caption)
            }.padding()
        }
        let view = card.frame(width: 300, height: 150)
    ...
    func testCardWithUltraThinMaterial() {
        let card = Card(elevation: .low, material: .ultraThin) {
            Text("Ultra Thin Material").padding()
        }
        let view = card.frame(width: 300, height: 150)
    ...
    func testCardWithUltraThickMaterial() {
        let card = Card(elevation: .high, material: .ultraThick) {
            Text("Ultra Thick Material").padding()
        }
        let view = card.frame(width: 300, height: 150)
    ...
    func testCardDynamicTypeExtraSmall() {
        let card = Card(elevation: .medium) {
            VStack(alignment: .leading) {
                ...
                Text("Content with extra small text size").font(.body)
            }.padding()
        }
        let view = card.frame(width: 300, height: 200).environment(\.sizeCategory, .extraSmall)
    ...
    func testCardDynamicTypeMedium() {
        let card = Card(elevation: .medium) {
            VStack(alignment: .leading) {
                Text("Title").font(.headline)
                Text("Content with medium text size").font(.body)
            }.padding()
        }
        let view = card.frame(width: 300, height: 200).environment(\.sizeCategory, .medium)
    ...
    func testCardDynamicTypeExtraExtraLarge() {
        let card = Card(elevation: .medium) {
            VStack(alignment: .leading) {
                Text("Title").font(.headline)
                Text("Content with extra extra large text size").font(.body)
            }.padding()
        }
        let view = card.frame(width: 350, height: 250).environment(
    ...
    func testCardWithSimpleText() {
        let card = Card(elevation: .medium) { Text("Simple Text Content").padding() }
        let view = card.frame(width: 300, height: 150)
    ...
    func testCardWithVStackContent() {
        let card = Card(elevation: .medium) {
            VStack(alignment: .leading, spacing: DS.Spacing.m) {
                ...
                Text("This is the main content of the card with multiple lines of text.").font(
                    .body)
                Text("Additional information").font(.caption).foregroundStyle(.secondary)
            }.padding()
        }
        let view = card.frame(width: 350, height: 250)
    ...
    func testCardWithBadges() {
        ...
        let view = card.frame(width: 300, height: 200)
    ...
    func testNestedCards() {
        let card = Card(elevation: .high) {
            VStack(spacing: DS.Spacing.m) {
                Text("Outer Card").font(.headline)

                Card(elevation: .low) { Text("Inner Card").padding() }
            }.padding()
        }
        let view = card.frame(width: 350, height: 250)
    ...
    func testCardRTLLocale() {
        let card = Card(elevation: .medium) {
            VStack(alignment: .leading, spacing: DS.Spacing.m) {
                Text("عنوان").font(.headline)
                Text("هذا هو المحتوى الرئيسي للبطاقة").font(.body)
            }.padding()
        }
        let view = card.frame(width: 300, height: 200).environment(\.layoutDirection, .rightToLeft)
    ...
    func testCardAllElevationsComparison() {
        let view = VStack(spacing: DS.Spacing.l) {
            Card(elevation: .none) { Text("None").padding() }
            Card(elevation: .low) { Text("Low").padding() }
            Card(elevation: .medium) { Text("Medium").padding() }
            Card(elevation: .high) { Text("High").padding() }
        }.frame(width: 300, height: 600).padding()
    ...
    func testCardAllElevationsDarkMode() {
        let view = VStack(spacing: DS.Spacing.l) {
            ...
            Card(elevation: .medium) { Text("Medium").padding() }
            Card(elevation: .high) { Text("High").padding() }
        }.frame(width: 300, height: 600).padding().preferredColorScheme(.dark)
