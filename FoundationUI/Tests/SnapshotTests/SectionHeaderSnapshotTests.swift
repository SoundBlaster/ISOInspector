...
@MainActor final class SectionHeaderSnapshotTests: XCTestCase {
    ...
    func testSectionHeaderWithoutDividerDarkMode() {
        let header = SectionHeader(title: "File Properties")
        let view = header.frame(width: 300, height: 100).preferredColorScheme(.dark)
    ...
    func testSectionHeaderDynamicTypeExtraSmall() {
        ...
        let view = header.frame(width: 300, height: 100).environment(\.sizeCategory, .extraSmall)
    ...
    func testSectionHeaderDynamicTypeMedium() {
        let header = SectionHeader(title: "Metadata", showDivider: true)
        let view = header.frame(width: 300, height: 100).environment(\.sizeCategory, .medium)
    ...
    func testSectionHeaderDynamicTypeExtraExtraLarge() {
        let header = SectionHeader(title: "Metadata", showDivider: true)
        let view = header.frame(width: 350, height: 150).environment(
    ...
    func testSectionHeaderLongTitle() {
        let header = SectionHeader(
            title: "This is a much longer section header title", showDivider: true)
        let view = header.frame(width: 350, height: 100)
    ...
    func testSectionHeaderRTLWithoutDivider() {
        ...
        let view = header.frame(width: 300, height: 100).environment(
    ...
    func testSectionHeaderVariousTitles() {
        ...
        }.frame(width: 350, height: 300).padding()
    ...
    func testSectionHeaderWithContent() {
        let view = VStack(alignment: .leading, spacing: DS.Spacing.m) {
            ...
            VStack(alignment: .leading, spacing: DS.Spacing.s) {
                HStack {
                    Text("Name:").font(.body)
                    Text("example.mp4").font(.caption)
                }
                HStack {
                    Text("Size:").font(.body)
                    Text("42.3 MB").font(.caption)
                }
            }
        ...
        let view = ScrollView {
            VStack(alignment: .leading, spacing: DS.Spacing.xl) {
                VStack(alignment: .leading, spacing: DS.Spacing.m) {
                    SectionHeader(title: "Basic Information", showDivider: true)
                    Text("Content for basic information").font(.body)
                }

                VStack(alignment: .leading, spacing: DS.Spacing.m) {
                    SectionHeader(title: "Technical Details", showDivider: true)
                    Text("Content for technical details").font(.body)
                }

                VStack(alignment: .leading, spacing: DS.Spacing.m) {
                    SectionHeader(title: "Box Structure", showDivider: true)
                    Text("Content for box structure").font(.body)
                }
            }.padding()
    ...
    func testSectionHeaderWithAndWithoutDividerComparison() {
        let view = VStack(alignment: .leading, spacing: DS.Spacing.xl) {
            VStack(alignment: .leading) {
                Text("Without Divider:").font(.caption2).foregroundStyle(.tertiary)
                SectionHeader(title: "Section Title", showDivider: false)
            }

            VStack(alignment: .leading) {
                Text("With Divider:").font(.caption2).foregroundStyle(.tertiary)
