// swift-tools-version: 6.0
    ...
    @MainActor final class DynamicTypeSizeExtensionsTests: XCTestCase {
        ...
        func testLessThanOperator() {
            XCTAssertTrue(DynamicTypeSize.small < .large, "small should be less than large")
            XCTAssertTrue(
                DynamicTypeSize.large < .accessibilityMedium,
                "large should be less than accessibilityMedium")
            XCTAssertFalse(
                DynamicTypeSize.xLarge < .medium, "xLarge should not be less than medium")
        }

        func testGreaterThanOperator() {
            XCTAssertTrue(DynamicTypeSize.large > .small, "large should be greater than small")
            XCTAssertTrue(
                DynamicTypeSize.accessibilityLarge > .xLarge,
                "accessibilityLarge should be greater than xLarge")
            XCTAssertFalse(
                DynamicTypeSize.medium > .xxLarge, "medium should not be greater than xxLarge")
        }

        func testLessThanOrEqualOperator() {
            XCTAssertTrue(DynamicTypeSize.small <= .large, "small should be <= large")
            XCTAssertTrue(DynamicTypeSize.large <= .large, "large should be <= large (equal)")
            XCTAssertFalse(
                DynamicTypeSize.accessibilityXLarge <= .medium,
                "accessibilityXLarge should not be <= medium")
        }

        func testGreaterThanOrEqualOperator() {
            XCTAssertTrue(DynamicTypeSize.large >= .small, "large should be >= small")
            XCTAssertTrue(
                DynamicTypeSize.accessibilityMedium >= .accessibilityMedium,
                "accessibilityMedium should be >= accessibilityMedium (equal)")
            XCTAssertFalse(DynamicTypeSize.xSmall >= .xxxLarge, "xSmall should not be >= xxxLarge")
        }

        func testEqualityOperator() {
            XCTAssertTrue(DynamicTypeSize.large == .large, "large should equal large")
            XCTAssertTrue(
                DynamicTypeSize.accessibilityMedium == .accessibility1,
                "accessibilityMedium should equal accessibility1")
            XCTAssertFalse(DynamicTypeSize.small == .large, "small should not equal large")
        }
        ...
        func testSortingAllSizes() {
            let unsorted: [DynamicTypeSize] = [
                .accessibilityLarge, .small, .xxxLarge, .xSmall, .accessibilityMedium, .large,
                .medium,
            ]
        ...
        func testCompleteOrderingSequence() {
            let completeSequence: [DynamicTypeSize] = [
                ...
                .accessibilityMedium, .accessibilityLarge, .accessibilityXLarge,
                .accessibilityXxLarge, .accessibilityXxxLarge,
            ]

            // Verify each element is less than the next
            for i in 0..<(completeSequence.count - 1) {
                XCTAssertTrue(
                    completeSequence[i] < completeSequence[i + 1],
        ...
        func testAccessibilitySizeRange() {
            ...
            XCTAssertEqual(accessibilitySizes.count, 5, "Should have 5 accessibility sizes")
        ...
        func testIntegrationWithIsAccessibilitySize() {
            XCTAssertFalse(
                AccessibilityHelpers.isAccessibilitySize(.small),
            ...
            XCTAssertFalse(
                AccessibilityHelpers.isAccessibilitySize(.xxxLarge),
            ...
            XCTAssertTrue(
                AccessibilityHelpers.isAccessibilitySize(.accessibilityXxxLarge),
