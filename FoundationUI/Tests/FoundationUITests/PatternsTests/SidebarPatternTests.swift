...
final class SidebarPatternTests: XCTestCase {
    ...
    func testSectionInitializationWithDefaultID() {
        ...
        // Then
        XCTAssertEqual(section.id, "Auto ID Section", "Section should use title as ID by default")
    }
    ...
    func testItemInitializationWithCustomAccessibilityLabel() {
        ...
        // Then
        XCTAssertEqual(item.accessibilityLabel, "Settings", "Custom accessibility label should override title")
    }
    ...
    func testSidebarPatternWithEmptySections() {
        // Given
        let selection = Binding<ItemID?>.constant(nil)
        let pattern = SidebarPattern(sections: [], selection: selection) { _ in
            Text("Detail")
        }
    ...
    func testSidebarPatternWithSectionContainingNoItems() {
        // Given
        let sections = [
            SidebarPattern<ItemID, Text>.Section(title: "Empty", items: [])
        ]
        let selection = Binding<ItemID?>.constant(nil)
        let pattern = SidebarPattern(sections: sections, selection: selection) { _ in
            Text("Detail")
    ...
    func testSidebarPatternWithInitialSelection() {
        ...
        // Then
        XCTAssertEqual(pattern.selection, initialSelection, "Sidebar should respect initial selection")
    }
    ...
    func testDetailBuilderWithNilSelection() {
        ...
        // Then
        XCTAssertNil(receivedSelection, "Detail builder should receive nil when nothing is selected")
    ...
    func testSidebarPatternWithMultipleSections() {
        // Given
        let sections = [
            SidebarPattern<ItemID, Text>.Section(title: "Section 1", items: [
                .init(id: UUID(), title: "Item 1.1"),
                .init(id: UUID(), title: "Item 1.2")
            ]),
            SidebarPattern<ItemID, Text>.Section(title: "Section 2", items: [
                .init(id: UUID(), title: "Item 2.1")
            ]),
            SidebarPattern<ItemID, Text>.Section(title: "Section 3", items: [
                .init(id: UUID(), title: "Item 3.1"),
                .init(id: UUID(), title: "Item 3.2"),
    ...
    func testSidebarPatternForFileExplorer() {
        // Given
        enum FileCategory: String, Hashable {
            case recents, documents, downloads, favorites
        }

        let sections = [
            SidebarPattern<FileCategory, Text>.Section(title: "Quick Access", items: [
                .init(id: .recents, title: "Recents", iconSystemName: "clock"),
                .init(id: .favorites, title: "Favorites", iconSystemName: "star")
            ]),
            SidebarPattern<FileCategory, Text>.Section(title: "Folders", items: [
                .init(id: .documents, title: "Documents", iconSystemName: "folder"),
                .init(id: .downloads, title: "Downloads", iconSystemName: "arrow.down.circle")
            ])
        ]
        ...
        // Then
        XCTAssertEqual(pattern.sections.count, 2, "File explorer sidebar should have multiple sections")
    ...
    func testSidebarPatternForMediaLibrary() {
        ...
        enum MediaType: String, Hashable {
            case all, videos, audio, images
        }

        let sections = [
            SidebarPattern<MediaType, Text>.Section(title: "Library", items: [
        ...
        // Then
        XCTAssertEqual(pattern.sections[0].items.count, 4, "Media library should have all media types")
        XCTAssertEqual(pattern.selection, .videos)
    }

    func testSidebarPatternForISOInspector() {
        // Given
        enum InspectorView: String, Hashable {
            case overview, structure, metadata, validation
        }

        let sections = [
            SidebarPattern<InspectorView, Text>.Section(title: "Analysis", items: [
        ...
        XCTAssertTrue(pattern.sections[0].items.allSatisfy { $0.iconSystemName != nil },
    ...
    func testSidebarPatternWithManyItems() {
        ...
        let sections = [
            SidebarPattern<Int, Text>.Section(title: "Large Section", items: items)
        ]
        ...
        // Then
        XCTAssertEqual(pattern.sections[0].items.count, 100, "Sidebar should handle large number of items")
    }
    ...
    func testSidebarPatternWithSpecialCharactersInTitles() {
        // Given
        let sections = [
            SidebarPattern<String, Text>.Section(title: "Special 🎬 Characters", items: [
        ...
        // Then
        XCTAssertEqual(pattern.sections[0].items.count, 3, "Sidebar should handle special characters")
    }
    ...
    func testSidebarPatternWithVeryLongTitles() {
        ...
        let sections = [
            SidebarPattern<Int, Text>.Section(title: longTitle, items: [
                .init(id: 1, title: longTitle)
            ])
        ]
        ...
        // Then
        XCTAssertEqual(pattern.sections[0].title, longTitle, "Sidebar should handle very long titles")
    }
}
#endif
