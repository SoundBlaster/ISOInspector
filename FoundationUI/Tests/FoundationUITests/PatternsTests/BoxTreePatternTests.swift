...
@MainActor final class BoxTreePatternTests: XCTestCase {
                                ...
                                children: [
                                    TestTreeNode(
                                        ...
                                        children: [
                                            TestTreeNode(
                                                title: "Level 4",
                                                children: [TestTreeNode(title: "Level 5")])
                                        ])
                                ])
                        ])
                ])
    ...
    private func makeLargeTree() -> [TestTreeNode] {
        ...
        for i in 0..<10 {
            var children: [TestTreeNode] = []
            for j in 0..<100 { children.append(TestTreeNode(title: "Node \(i).\(j)")) }
    ...
    func testBoxTreePatternInitializesWithEmptyTree() {
        ...
        let pattern = BoxTreePattern(
            data: emptyTree, children: { $0.children }, content: { node in Text(node.title) })
    ...
    func testBoxTreePatternInitializesWithSimpleTree() {
        ...
        // When
        let pattern = BoxTreePattern(
            data: tree, children: { $0.children }, content: { node in Text(node.title) })
    ...
    func testBoxTreePatternSupportsMultiSelection() {
        ...
        if !tree[0].children.isEmpty { selection.insert(tree[0].children[0].id) }
    ...
    func testBoxTreePatternUsesDesignSystemSpacingForIndentation() {
        ...
        XCTAssertGreaterThan(
            expectedIndentPerLevel, 0, "Indentation should use positive DS.Spacing value")
        XCTAssertEqual(
            expectedIndentPerLevel, DS.Spacing.l, "Should use DS.Spacing.l (16pt) for indentation")
    }
    ...
    func testBoxTreePatternCalculatesCorrectIndentationForDeepNesting() {
        ...
        // When
        let indentations = levels.map { level in CGFloat(level) * DS.Spacing.l }
    ...
    func testBoxTreePatternSupportsAccessibilityLabels() {
        ...
        let pattern = BoxTreePattern(
            data: tree, children: { $0.children },
            content: { node in Text(node.title).accessibilityLabel(node.title) })
    ...
    func testBoxTreePatternProvidesExpandedCollapsedAnnouncements() {
        ...
        if wasExpanded { expandedNodes.remove(rootId) } else { expandedNodes.insert(rootId) }

        // Then
        let expectedState = wasExpanded ? "collapsed" : "expanded"
        XCTAssertEqual(
            expectedState, "expanded", "Should announce expanded state for accessibility")
    }
    ...
    func testBoxTreePatternHandlesLargeDataSet() {
        ...
        // When
        let pattern = BoxTreePattern(
            data: largeTree, children: { $0.children }, content: { node in Text(node.title) })
    ...
    func testBoxTreePatternUsesLazyRenderingForPerformance() {
        ...
        let pattern = BoxTreePattern(
            data: largeTree, children: { $0.children },
            expandedNodes: Binding(get: { expandedNodes }, set: { expandedNodes = $0 }),
            content: { node in Text(node.title) })
    ...
    func testBoxTreePatternAnimatesExpandCollapse() {
        ...
        _ = withAnimation(DS.Animation.medium) { expandedNodes.insert(rootId) }
