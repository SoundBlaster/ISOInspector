...
@MainActor final class InspectorPatternIntegrationTests: XCTestCase {
    func testInspectorPatternComposesWithSectionHeaderAndKeyValueRows() {
        ...
        // Then
        XCTAssertNotNil(
            pattern, "InspectorPattern should compose with SectionHeader and KeyValueRow components"
        )
    }
    ...
    func testInspectorPatternSupportsBadgeInsideContent() {
        ...
        // Then
        XCTAssertNotNil(
            pattern, "InspectorPattern should allow composing badge views inside its content")
    }
}
