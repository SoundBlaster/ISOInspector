...
@MainActor final class ComponentIntegrationTests: XCTestCase {
        ...
        for material in materials {
            let card = Card(material: material) {
                VStack {
                    ...
                    KeyValueRow(
                        key: "Level", value: material.map { String(describing: $0) } ?? "none")
                }.padding()
            }
                    ...
                    ForEach(0..<10, id: \.self) { sectionIndex in
                        VStack(alignment: .leading, spacing: DS.Spacing.m) {
                            ...
                            ForEach(0..<5, id: \.self) { rowIndex in
                                KeyValueRow(
                                    key: "Property \(rowIndex)", value: "Value \(rowIndex)",
    ...
    func testCardWithOnlySectionHeader() {
        ...
        let card = Card { SectionHeader(title: "Only Header").padding() }
