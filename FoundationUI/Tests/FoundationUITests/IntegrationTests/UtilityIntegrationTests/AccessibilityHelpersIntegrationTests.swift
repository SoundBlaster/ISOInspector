...
final class AccessibilityHelpersIntegrationTests: XCTestCase {
    ...
    func testContrastValidationOnBadgeInfoColor() {
        ...
        XCTAssertGreaterThanOrEqual(contrastRatio, 4.5, "DS.Colors.infoBG should meet WCAG AA (≥4.5:1)")
    }
    ...
    func testContrastValidationOnBadgeSuccessColor() {
        ...
        XCTAssertGreaterThanOrEqual(contrastRatio, 4.5, "DS.Colors.successBG should meet WCAG AA")
    }
    ...
    func testAccessibleToggleModifierOnExpandableSection() {
        ...
        let toggle = Button(action: { isExpanded.toggle() }) {
            HStack {
                Text("Section")
        ...
        XCTAssertNotNil(toggle, "Accessibility toggle modifier should work on expandable sections")
    }
    ...
    func testAccessibilityAuditOnInspectorPattern() {
        ...
        XCTAssertTrue(audit.passes || audit.issues.count < 2, "InspectorPattern should be accessibility-compliant")
    }
    ...
    func testKeyboardNavigationInComplexHierarchy() {
        ...
        XCTAssertTrue(AccessibilityHelpers.isValidFocusOrder(elements), "Complex hierarchy should have valid focus order")
    }
    ...
    func testDynamicTypeScalingWithRealTypography() {
        ...
        XCTAssertGreaterThan(scaledSizeAccessibility, scaledSizeXXL, "Accessibility sizes should scale more")
    }

    func testAccessibilitySizeDetectionInComponents() {
        // Test accessibility size detection with real Dynamic Type values
        XCTAssertTrue(AccessibilityHelpers.isAccessibilitySize(.accessibilityMedium), "Should detect accessibility medium")
        XCTAssertTrue(AccessibilityHelpers.isAccessibilitySize(.accessibilityLarge), "Should detect accessibility large")
        XCTAssertFalse(AccessibilityHelpers.isAccessibilitySize(.large), "Should not detect normal size as accessibility")
    }
    ...
    func testHighContrastContextIntegration() {
        ...
        XCTAssertEqual(preferredSpacing, DS.Spacing.l, "Should increase spacing for high contrast")
    }
    ...
    func testAccessibilityContextWithComponents() {
        ...
        XCTAssertFalse(AccessibilityHelpers.shouldAnimate(in: context), "Animations should be disabled")
        XCTAssertGreaterThan(AccessibilityHelpers.preferredSpacing(in: context), DS.Spacing.m, "Spacing should be increased")
    }
