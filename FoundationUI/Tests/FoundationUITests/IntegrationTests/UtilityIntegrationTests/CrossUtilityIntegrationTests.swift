...
import XCTest
    ...
    @MainActor final class CrossUtilityIntegrationTests: XCTestCase {
        ...
        func testCopyableTextWithAccessibilityHelpers() {
            ...
            let audit = AccessibilityHelpers.auditView(
                hasLabel: true, hasHint: true, touchTargetSize: CGSize(width: 44, height: 44),
                contrastRatio: 5.0)

            XCTAssertNotNil(copyableText, "CopyableText should work with accessibility audit")
            XCTAssertTrue(
                audit.passes || !audit.issues.isEmpty,
                "Accessibility audit should run on CopyableText")
        }
        ...
        func testCopyableTextWithKeyboardShortcuts() {
            ...
            let view = VStack(spacing: DS.Spacing.s) {
                ...
                Text("Press \(shortcutHint)").font(DS.Typography.caption).foregroundColor(
                    .secondary)
            }
        ...
        func testAccessibilityHelpersWithKeyboardShortcuts() {
            ...
            let hint = AccessibilityHelpers.voiceOverHint(
                action: "press", target: copyShortcut.displayString)
        ...
        func testAllThreeUtilitiesInInspectorPattern() {
            ...
            let inspector = InspectorPattern(title: "ISO File Inspector") {
                VStack(spacing: DS.Spacing.m) {
                    ...
                    SectionHeader(title: "Details", showDivider: true).accessibleHeading(level: 1)

                    // KeyValueRow with CopyableText
                    KeyValueRow(key: "File Hash", value: "0xABCDEF", copyable: true)

                    // Shortcut hint
                    Text("Press \(KeyboardShortcutType.copy.displayString) to copy").font(
                        DS.Typography.caption)
                }
            }

            XCTAssertNotNil(
                inspector, "All three utilities should work together in InspectorPattern")
        ...
        func testAllThreeUtilitiesInToolbarPattern() {
            ...
            let toolbar = ToolbarPattern(
                items: ToolbarPattern.Items(primary: [
                    ToolbarPattern.Item(
                        id: "copy", iconSystemName: "doc.on.doc",
                        title: "Copy (\(KeyboardShortcutType.copy.displayString))", action: {})
                ]))
        ...
        func testUtilitiesInComplexComponentHierarchy() {
            ...
            let complexView = Card {
                VStack(spacing: DS.Spacing.l) {
                    ...
                    Text("Complex View").font(DS.Typography.headline).accessibleHeading(level: 1)

                    // Content with all utilities
                    VStack(spacing: DS.Spacing.m) {
                        KeyValueRow(key: "ID", value: "12345", copyable: true)
                        Text("Shortcut: \(KeyboardShortcutType.copy.displayString)").font(
                            DS.Typography.caption)
                    }

                    // Action button
                    Button("Copy All") {}.accessibleButton(
                        label: "Copy All Values",
                        hint: AccessibilityHelpers.voiceOverHint(
                            action: "copy", target: "all values"))
                }
            }
        ...
        // MARK: - Platform-Specific Combinations
            ...
            func testMacOSSpecificUtilityCombinations() {
                ...
                let view = VStack {
                    ...
                    Text("⌘C to copy").font(DS.Typography.caption)
                ...
                XCTAssertTrue(
                    KeyboardShortcutType.copy.displayString.contains("⌘"),
                    "Should use Command on macOS")
            }
        #endif
            ...
            func testIOSSpecificUtilityCombinations() {
                ...
                let view = VStack {
                    ...
                    Text("Tap to copy").font(DS.Typography.caption)
            ...
            measure {
                let views = (0..<20).map { index in
                    VStack {
                        ...
                        Text(KeyboardShortcutType.copy.displayString).font(DS.Typography.caption)
        ...
        func testISOInspectorRealWorldScenario() {
            ...
            let inspector = InspectorPattern(title: "ISO Box Details") {
                VStack(spacing: DS.Spacing.l) {
                    // Metadata section
                    SectionHeader(title: "Metadata", showDivider: true).accessibleHeading(level: 1)
