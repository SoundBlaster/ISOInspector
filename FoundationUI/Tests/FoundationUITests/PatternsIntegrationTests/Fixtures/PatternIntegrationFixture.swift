// swift-tools-version: 6.0
    ...
    @MainActor struct PatternIntegrationFixture {
        ...
        init(documents: [Document]) {
            ...
            documentIndexByID = Dictionary(
                uniqueKeysWithValues: documents.enumerated().map { ($1.id, $0) })
        ...
        func makeSidebarPattern() -> SidebarPattern<String, InspectorPattern<PatternContent>> {
            SidebarPattern(
                sections: [
                    SidebarPattern<String, InspectorPattern<PatternContent>>.Section(
                        title: "Library",
                ...
                return InspectorPattern(title: detail.title, material: .regular) {
                    PatternContent(
                        documentTitle: detail.title, subtitle: detail.subtitle,
        ...
        func makeInspectorContent(for identifier: String?) -> (title: String, subtitle: String) {
            guard let identifier, let index = documentIndexByID[identifier] else {
                return (title: "No Selection", subtitle: "Select an item to inspect")
            }

            let document = documents[index]
            return (title: document.title, subtitle: "Document ID: \(document.id)")
        }
        ...
        func makeToolbar(for identifier: String?) -> ToolbarPattern {
            ...
            return ToolbarPattern(
                items: ToolbarPattern.Items(
                    primary: [
                        .init(
                            id: "select.next", iconSystemName: "arrow.down.circle", title: "Next",
                            accessibilityHint: "Select the next document", role: .primaryAction,
                            shortcut: .init(key: "j", modifiers: [.command])
                        ) { [documents, documentIndexByID, binding] in
                            guard let current = binding.wrappedValue,
                    ...
                    secondary: [
                        .init(
                            id: "announce.metadata", iconSystemName: "speaker.wave.2.fill",
                            title: "Announce",
                            accessibilityHint: "Read the current inspector metadata", role: .neutral
                        ) { [selectionStore, binding] in
                            let message =
                                if let identifier = binding.wrappedValue {
                                    "Inspector metadata updated for \(identifier)"
                                } else { "Inspector metadata unavailable" }
                    ...
                    overflow: [
                        .init(
                            ...
                            accessibilityHint: "Archive the current document", role: .destructive,
    ...
    @MainActor private final class SelectionStore {
        ...
        func recordAnnouncement(message: String) { announcements.append(message) }
    }
    ...
    struct PatternContent: View {
        ...
        var body: some View {
            VStack(alignment: .leading, spacing: DS.Spacing.l) {
                Text(documentTitle).font(DS.Typography.title).accessibilityLabel(
                    Text(documentTitle))
                Text(subtitle).font(DS.Typography.body).foregroundStyle(DS.Colors.textSecondary)
                toolbar
            }.padding(DS.Spacing.l).accessibilityElement(children: .contain)
        }
    }
#endif
