...
final class PatternIntegrationTests: XCTestCase {
    ...
    func testSidebarSelectionDrivesInspectorDetailTitle() {
        ...
        XCTAssertEqual(inspector.material, .regular, "Inspector should honor material override for composed content")
    }
    ...
    func testToolbarAccessibilityAnnouncementRecordsSelection() {
        ...
        guard let announceAction = toolbar.items.secondary.first(where: { $0.id == "announce.metadata" }) else {
        ...
        // Then
        XCTAssertEqual(fixture.accessibilityAnnouncements.last, "Inspector metadata updated for document-1")
