// swift-tools-version: 6.0
    ...
    @MainActor final class PatternIntegrationTests: XCTestCase {
        func testSidebarSelectionDrivesInspectorDetailTitle() {
            ...
            XCTAssertEqual(
                String(describing: inspector.material), String(describing: Material.regular),
        ...
        func testToolbarPrimaryActionAdvancesSidebarSelection() {
            ...
            guard let nextAction = toolbar.items.primary.first(where: { $0.id == "select.next" })
            else {
        ...
        func testToolbarAccessibilityAnnouncementRecordsSelection() {
            ...
            guard
                let announceAction = toolbar.items.secondary.first(where: {
                    $0.id == "announce.metadata"
                })
            else {
