...
@MainActor enum AccessibilityTestHelpers {
    ...
    private static func relativeLuminance(of component: CGFloat) -> CGFloat {
        if component <= 0.03928 { component / 12.92 } else { pow((component + 0.055) / 1.055, 2.4) }
    }
    ...
    static func assertMeetsWCAGAA(
        ...
        XCTAssertGreaterThanOrEqual(
            ...
            "\(message). Actual ratio: \(String(format: "%.2f", ratio)):1", file: file, line: line)
    }
    ...
    static func meetsTouchTargetRequirement(size: CGSize, platform _: String = "iOS") -> Bool {
        size.width >= minimumTouchTargetSize && size.height >= minimumTouchTargetSize
    }
    ...
    static func assertMeetsTouchTargetSize(
        size: CGSize, message: String = "Should meet minimum touch target size (≥44×44 pt)",
        file: StaticString = #filePath, line: UInt = #line
    ) {
        XCTAssertGreaterThanOrEqual(
            size.width, minimumTouchTargetSize, "\(message). Width: \(size.width) pt", file: file,
            line: line)
        XCTAssertGreaterThanOrEqual(
            size.height, minimumTouchTargetSize, "\(message). Height: \(size.height) pt",
            file: file, line: line)
    }
    ...
    static func assertValidAccessibilityLabel(
        _ label: String?, context: String = "Component", file: StaticString = #filePath,
        line: UInt = #line
    ) {
        XCTAssertTrue(
            isValidAccessibilityLabel(label),
            "\(context) should have a meaningful accessibility label", file: file, line: line)
    }
