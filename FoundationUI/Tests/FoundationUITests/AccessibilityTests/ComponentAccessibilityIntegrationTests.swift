...
@MainActor final class ComponentAccessibilityIntegrationTests: XCTestCase {
    ...
    func testCardWithSectionHeaderAndKeyValueRows() {
        ...
        XCTAssertNotNil(
            card, "Card containing SectionHeader and KeyValueRows should preserve accessibility")
    }
    ...
    func testComplexComponentHierarchy() {
        ...
        let complexView = Card(elevation: .medium) {
            VStack(alignment: .leading, spacing: DS.Spacing.m) {
                ...
                VStack(alignment: .leading, spacing: DS.Spacing.s) {
                    ...
                    KeyValueRow(
                        key: "Description", value: "A long description that wraps",
                        layout: .vertical)
                }
            }
        }
        ...
        // Then
        XCTAssertNotNil(
            complexView, "Complex nested component hierarchy should maintain accessibility")
    }
    ...
    func testNestedCardsWithMultipleComponents() {
        ...
        // Then
        XCTAssertNotNil(
            outerCard, "Nested cards with multiple components should preserve accessibility")
    }
    ...
    func testBadgeInsideCard() {
        // Given
        let card = Card { Badge(text: "INFO", level: .info) }

        // Then
        XCTAssertNotNil(card, "Badge inside Card should maintain accessibility")
    }
    ...
    func testMultipleBadgesInsideCard() {
        ...
        // Then
        XCTAssertNotNil(card, "Multiple badges inside Card should each maintain accessibility")
    }
    ...
    func testSectionHeaderWithKeyValueRows() {
        ...
        // Then
        XCTAssertNotNil(
            section, "SectionHeader with KeyValueRows should maintain heading semantics")
    }
    ...
    func testMultipleSectionsWithComponents() {
        ...
        // Then
        XCTAssertNotNil(view, "Multiple sections should each maintain heading structure")
    }
    ...
    func testBadgeLevelsHaveColorDefinitions() {
        ...
        for level in allLevels {
            XCTAssertNotNil(
                level.foregroundColor, "Badge level '\(level)' should have foreground color")
            XCTAssertNotNil(
                level.backgroundColor, "Badge level '\(level)' should have background color")
        }
    ...
    func testCopyableKeyValueRowInsideCard() {
        ...
        // Then
        XCTAssertNotNil(
            card, "Copyable KeyValueRows inside Card should maintain interactive accessibility")
    }
    ...
    func testInspectorViewPattern() {
        ...
        let inspectorView = ScrollView {
            VStack(alignment: .leading, spacing: DS.Spacing.l) {
                ...
                Card(elevation: .medium) {
                    VStack(alignment: .leading, spacing: DS.Spacing.m) {
                        HStack {
                            Text("File Inspector").font(.title2)
                            Spacer()
        ...
        // Then
        XCTAssertNotNil(
            inspectorView, "Inspector view pattern should maintain full accessibility hierarchy")
    }
            ...
            ForEach(0..<3) { index in
                Card(elevation: .low) {
                    VStack(alignment: .leading, spacing: DS.Spacing.m) {
                        ...
                        HStack { Badge(text: "TAG \(index + 1)", level: .info) }
                    }.padding()
                }
            }
        }
    ...
    func testComponentsWithDynamicType() {
        ...
        for category in commonSizes {
            ...
            XCTAssertNotNil(view, "Components should work with Dynamic Type size: \(category)")
        }
    }
    ...
    func testColorSchemeInNestedComponents() {
        ...
        // Then
        XCTAssertNotNil(card, "Color scheme should propagate to all nested components")
    }
    ...
    func testComponentsOnCurrentPlatform() {
        ...
        // Then
        XCTAssertNotNil(platformView, "Components should work correctly on \(platform)")
    }
}
