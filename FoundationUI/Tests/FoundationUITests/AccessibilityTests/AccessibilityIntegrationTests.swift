...
final class AccessibilityIntegrationTests: XCTestCase {
    ...
    func testBoxTreeInSidebarWithInspectorDetails_SelectionFlow() {
        ...
        XCTAssertTrue(selectedNodeLabel.contains("selected"), "Selected node announces selection")
        XCTAssertFalse(inspectorLabel.isEmpty, "Inspector should have label")
    ...
    func testKeyboardNavigation_ThroughCompleteInterface() {
        ...
        for (index, element) in tabOrder.enumerated() {
            XCTAssertFalse(
                element.isEmpty,
                "Tab stop \(index + 1) has label: \(element)"
            )
        }
    ...
    func testKeyboardShortcuts_WorkWithVoiceOver() {
        ...
        let shortcuts: [(action: String, shortcut: String, label: String)] = [
            ("Open", "Command O", "Open File"),
            ("Copy", "Command C", "Copy Selected"),
            ("Export", "Command E", "Export Data"),
            ("Refresh", "Command R", "Refresh View")
        ]
    ...
    func testIOSIntegration_VoiceOverGestures() {
        ...
        let gestures: [String] = [
            ...
            "Three-finger swipe down to read from top"
        ]

        for gesture in gestures {
            XCTAssertFalse(
                gesture.isEmpty,
                "VoiceOver gesture supported: \(gesture)"
            )
        }
    }
    ...
    func testIOSIntegration_TouchAccommodations() {
        ...
        XCTAssertTrue(
            true,
            "FoundationUI respects iOS touch accommodations"
        )
    }
    #endif
    ...
    func testAccessibilityState_PropagatesThroughBindings() {
        ...
        XCTAssertTrue(
            selectionLabel.contains("selected"),
        ...
        XCTAssertTrue(
            expandedNodeLabel.contains("expanded"),
            "Expansion state announced to VoiceOver"
        )
    }
