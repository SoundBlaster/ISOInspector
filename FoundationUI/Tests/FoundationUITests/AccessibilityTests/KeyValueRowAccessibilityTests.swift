...
@MainActor final class KeyValueRowAccessibilityTests: XCTestCase {
    ...
    func testKeyValueRowBasicAccessibilityLabel() {
        ...
        XCTAssertEqual(row.key, "Type", "KeyValueRow should preserve key")
        XCTAssertEqual(row.value, "ftyp", "KeyValueRow should preserve value")
    }
    ...
    func testKeyValueRowAccessibilityLabelFormat() {
        ...
        XCTAssertEqual(
            expectedLabel, "File Size, 1024 bytes",
    ...
    func testKeyValueRowWithSpecialCharacters() {
        ...
        // Then
        XCTAssertEqual(
            row.value, "0xDEADBEEF", "KeyValueRow should preserve special characters in value")
    }
    ...
    func testKeyValueRowWithEmptyValue() {
        ...
        // Then
        XCTAssertEqual(row.value, "", "KeyValueRow should accept empty value")
    }
    ...
    func testKeyValueRowCopyableAccessibility() {
        ...
        // Then
        XCTAssertTrue(row.copyable, "KeyValueRow should have copyable enabled")
    ...
    func testKeyValueRowNonCopyableAccessibility() {
        ...
        // Then
        XCTAssertFalse(row.copyable, "KeyValueRow should have copyable disabled")
    }
    ...
    func testKeyValueRowCopyableHasAccessibilityHint() {
        ...
        XCTAssertTrue(copyableRow.copyable, "Copyable row should have copyable property set")
        XCTAssertFalse(
            nonCopyableRow.copyable, "Non-copyable row should not have copyable property set")
    }
    ...
    func testKeyValueRowHorizontalLayout() {
        ...
        // Then
        XCTAssertEqual(row.layout, .horizontal, "KeyValueRow should support horizontal layout")
    }
    ...
    func testKeyValueRowVerticalLayout() {
        ...
        // Then
        XCTAssertEqual(row.layout, .vertical, "KeyValueRow should support vertical layout")
    }
    ...
    func testKeyValueRowDefaultLayout() {
        ...
        // Then
        XCTAssertEqual(row.layout, .horizontal, "KeyValueRow should default to horizontal layout")
    }
    ...
    func testKeyValueRowUsesDesignSystemTypography() {
        ...
        XCTAssertNotNil(bodyFont, "KeyValueRow should use DS.Typography.body for keys")
        XCTAssertNotNil(codeFont, "KeyValueRow should use DS.Typography.code for values")
    }
    ...
    func testKeyValueRowUsesDesignSystemSpacing() {
        ...
        // Then
        XCTAssertGreaterThan(DS.Spacing.s, 0, "KeyValueRow should use DS.Spacing.s (must be > 0)")
        XCTAssertGreaterThan(DS.Spacing.m, 0, "KeyValueRow should use DS.Spacing.m (must be > 0)")
    }
    ...
    func testKeyValueRowKeyTextContrast() {
        ...
        XCTAssertNotNil(
            keyColor, "KeyValueRow keys should use .secondary color (WCAG compliant by system)")
    }
    ...
    func testKeyValueRowValueTextContrast() {
        ...
        XCTAssertNotNil(
            valueColor, "KeyValueRow values should use .primary color (WCAG compliant by system)")
    }
    ...
    func testKeyValueRowCopyableButtonTouchTarget() {
        ...
        XCTAssertTrue(row.copyable, "Copyable row should be interactive")
        ...
        XCTAssertGreaterThan(
            iconSpacing, 0, "Copyable button should have icon spacing for better touch target")
    }
    ...
    func testKeyValueRowOnCurrentPlatform() {
        ...
        // Then
        XCTAssertNotNil(row, "KeyValueRow should be creatable on \(platform)")
        XCTAssertEqual(row.value, platform, "KeyValueRow should work on \(platform)")
    }
    ...
    func testMultipleKeyValueRowsHaveDistinctContent() {
        ...
        // Then
        XCTAssertNotEqual(row1.key, row2.key, "Different rows should have different keys")
        XCTAssertNotEqual(row2.value, row3.value, "Different rows should have different values")
    }
    ...
    func testKeyValueRowInListContext() {
        ...
        for row in rows {
            AccessibilityTestHelpers.assertValidAccessibilityLabel(
                row.key, context: "KeyValueRow key in list")
            AccessibilityTestHelpers.assertValidAccessibilityLabel(
                row.value, context: "KeyValueRow value in list")
        }
    }
    ...
    func testKeyValueRowWithUnicodeValue() {
        ...
        // Then
        XCTAssertEqual(row.value, unicodeValue, "KeyValueRow should support Unicode values")
    }

    func testKeyValueRowFullConfiguration() {
        // Given
        let row = KeyValueRow(
            key: "Full Config", value: "All options set", layout: .vertical, copyable: true)

        // Then
        XCTAssertEqual(row.layout, .vertical, "Should use vertical layout")
        XCTAssertTrue(row.copyable, "Should be copyable")
        AccessibilityTestHelpers.assertValidAccessibilityLabel(
            row.key, context: "Fully configured KeyValueRow")
    }
}
