...
@MainActor final class ComponentHierarchyPerformanceTests: XCTestCase {
    ...
    @MainActor func testCardSectionHeaderHierarchyPerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for i in 0..<DS.PerformanceTest.componentCount {
                let view = Card {
    ...
    @MainActor func testCardSectionKeyValueHierarchyPerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for i in 0..<DS.PerformanceTest.componentCount {
                let view = Card {
    ...
    @MainActor func testSimpleInspectorPanelPerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
    ...
    @MainActor func testMediumInspectorPanelPerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for i in 0..<DS.PerformanceTest.iterationCount {
                let view = Card {
    ...
    @MainActor func testComplexInspectorPanelPerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
                    ...
                    ScrollView {
                        VStack(alignment: .leading, spacing: DS.Spacing.l) {
                            ForEach(0..<5, id: \.self) { sectionIndex in
                                VStack(alignment: .leading, spacing: DS.Spacing.s) {
                                    SectionHeader(
                                        title: "Section \(sectionIndex)", showDivider: true)
    ...
    @MainActor func testCardListPerformance_10Cards() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
                ...
                let view = ScrollView {
                    VStack(spacing: DS.Spacing.m) {
                        ForEach(0..<10, id: \.self) { index in
                            Card {
                                VStack(alignment: .leading, spacing: DS.Spacing.s) {
                                    Text("Card \(index)").font(DS.Typography.title)
                                    Text("Description for card \(index)").font(DS.Typography.body)
                                }.padding(DS.Spacing.m)
                            }
                        }
                    }.padding(DS.Spacing.m)
    ...
    @MainActor func testComplexCardListPerformance_50Cards() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for _ in 0..<DS.PerformanceTest.iterationCount {
                let view = ScrollView {
                    VStack(spacing: DS.Spacing.m) {
                        ForEach(0..<50, id: \.self) { index in
                            Card(elevation: .low) {
                                VStack(alignment: .leading, spacing: DS.Spacing.s) {
                                    ...
                                    KeyValueRow(
                                        key: "Offset",
                                        value: "0x\(String(format: "%08X", index * 32))")
    ...
    @MainActor func testNestedCardsWithComplexContentPerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for i in 0..<DS.PerformanceTest.iterationCount {
                let view = Card {
                    VStack(alignment: .leading, spacing: DS.Spacing.m) {
    ...
    @MainActor func testDeeplyNestedHierarchyPerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for i in 0..<DS.PerformanceTest.iterationCount {
    ...
    @MainActor func testLargeScaleComponentCompositionPerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for _ in 0..<DS.PerformanceTest.iterationCount {
                let view = ScrollView {
                    VStack(spacing: DS.Spacing.l) {
                        ...
                        ForEach(0..<10, id: \.self) { cardIndex in
                            Card {
                                VStack(alignment: .leading, spacing: DS.Spacing.m) {
                                    SectionHeader(title: "Section \(cardIndex)", showDivider: true)

                                    ForEach(0..<10, id: \.self) { rowIndex in
                                        KeyValueRow(
                                            key: "Property \(rowIndex)",
                                            value: "Value \(cardIndex)-\(rowIndex)", copyable: true)
                                    }
    ...
    @MainActor func testISOBoxInspectorPerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for i in 0..<DS.PerformanceTest.iterationCount {
                let view = Card {
                    ScrollView {
                        VStack(alignment: .leading, spacing: DS.Spacing.l) {
                            ...
                            KeyValueRow(
                                key: "Hex Dump", value: "00 00 00 20 66 74 79 70", copyable: true)
                            KeyValueRow(key: "ASCII", value: "....ftyp", copyable: true)
    ...
    @MainActor func testMultiBoxInspectorPerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for _ in 0..<DS.PerformanceTest.iterationCount {
                let view = ScrollView {
                    VStack(spacing: DS.Spacing.l) {
