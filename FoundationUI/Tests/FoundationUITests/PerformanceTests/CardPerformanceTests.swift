...
@MainActor final class CardPerformanceTests: XCTestCase {
    ...
    @MainActor func testCardWithTextContentPerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
    ...
    @MainActor func testAllElevationLevelsPerformance() throws {
    ...
    @MainActor func testAllMaterialBackgroundsPerformance() throws {
        let materials: [Material?] = [
            .thin, .regular, .thick, .ultraThin, .ultraThick, nil,  // No material
        ]
    ...
    @MainActor func testCardWithVaryingCornerRadiusPerformance() throws {
        let radii: [CGFloat] = [
            DS.Radius.small, DS.Radius.card, DS.Radius.chip, 0,  // No radius
            50,  // Large radius
        ]
        ...
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for radius in radii {
                for i in 0..<(DS.PerformanceTest.componentCount / radii.count) {
                    let card = Card(cornerRadius: radius) { Text("Radius: \(radius) - \(i)") }
                    _ = Mirror(reflecting: card).children.count
                }
            }
        }
    ...
    @MainActor func testMultipleCards_10Instances() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            var cards: [Card<Text>] = []
            for index in 0..<DS.PerformanceTest.iterationCount {
                for i in 0..<10 {
                    let card = Card { Text("Card \(index)-\(i)") }
                    cards.append(card)
                }
            }
            _ = cards.count
    ...
    @MainActor func testMultipleCards_50Instances() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            var cards: [Card<Text>] = []
            for index in 0..<DS.PerformanceTest.iterationCount {
                for i in 0..<50 {
                    let elevation = CardElevation.allCases[i % CardElevation.allCases.count]
                    let card = Card(elevation: elevation) { Text("Card \(index)-\(i)") }
                    cards.append(card)
                }
            }
            _ = cards.count
    ...
    @MainActor func testMultipleCards_100Instances() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            var cards: [Card<Text>] = []
            for index in 0..<DS.PerformanceTest.iterationCount {
                for i in 0..<DS.PerformanceTest.componentCount {
    ...
    @MainActor func testNestedCardsPerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for i in 0..<DS.PerformanceTest.iterationCount {
                let card = Card {
                    VStack(spacing: DS.Spacing.m) {
                        ...
                        Card(elevation: .low) { Text("Nested Card 1") }

                        Card(elevation: .low) { Text("Nested Card 2") }
                    }
                }
                _ = Mirror(reflecting: card).children.count
            }
    ...
    @MainActor func testDeeplyNestedCardsPerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for i in 0..<DS.PerformanceTest.iterationCount {
                let card = Card {
                    VStack {
                        ...
                        Card(elevation: .low) {
                            VStack {
                                ...
                                Card(elevation: .none) { Text("Level 3 Card") }
                            }
                        }
                    }
                }
    ...
    @MainActor func testCardWithComplexVStackPerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for i in 0..<DS.PerformanceTest.iterationCount {
                let card = Card {
                    VStack(alignment: .leading, spacing: DS.Spacing.m) {
                        Text("Title \(i)").font(DS.Typography.title)
                        Text("Subtitle").font(DS.Typography.body)
                        Divider()
                        Text("Description text that can be quite long").font(DS.Typography.caption)
                        HStack {
    ...
    @MainActor func testCardWithListContentPerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
    ...
    @MainActor func testCardInVStackPerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for _ in 0..<DS.PerformanceTest.iterationCount {
                let view = VStack(spacing: DS.Spacing.m) {
                    ForEach(0..<10, id: \.self) { index in Card { Text("Card \(index)") } }
                }
    ...
    @MainActor func testCardInScrollViewPerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for _ in 0..<DS.PerformanceTest.iterationCount {
                let view = ScrollView {
                    VStack(spacing: DS.Spacing.m) {
                        ForEach(0..<50, id: \.self) { index in
                            Card(
                                elevation: CardElevation.allCases[
                                    index % CardElevation.allCases.count]
                            ) {
                                VStack(alignment: .leading) {
                                    Text("Card \(index)")
                                    Text("Description").font(DS.Typography.caption)
                                }
                            }
                        }
                    }
    ...
    @MainActor func testNoneElevationCardPerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for i in 0..<DS.PerformanceTest.componentCount {
                let card = Card(elevation: .none) { Text("None \(i)") }
                _ = Mirror(reflecting: card).children.count
            }
        }
    }
    ...
    @MainActor func testLowElevationCardPerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for i in 0..<DS.PerformanceTest.componentCount {
                let card = Card(elevation: .low) { Text("Low \(i)") }
                _ = Mirror(reflecting: card).children.count
            }
        }
    }
    ...
    @MainActor func testMediumElevationCardPerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for i in 0..<DS.PerformanceTest.componentCount {
                let card = Card(elevation: .medium) { Text("Medium \(i)") }
                _ = Mirror(reflecting: card).children.count
            }
        }
    }
    ...
    @MainActor func testHighElevationCardPerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for i in 0..<DS.PerformanceTest.componentCount {
                let card = Card(elevation: .high) { Text("High \(i)") }
                _ = Mirror(reflecting: card).children.count
            }
        }
    }
