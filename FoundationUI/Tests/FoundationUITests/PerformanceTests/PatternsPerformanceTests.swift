import XCTest
    ...
    @MainActor final class PatternsPerformanceTests: XCTestCase {
        ...
        func testBoxTreePatternLargeFlatTreeRenderTime() {
            ...
            measure {
                _ = BoxTreePattern(
                    data: nodes, children: { $0.children }, expandedNodes: .constant(Set<UUID>()),
                    selection: .constant(nil)
                ) { node in Text(node.title) }
            }
        ...
        func testBoxTreePatternDeepNestedTreeRenderTime() {
            ...
            func makeDeepTree(depth: Int, currentDepth: Int = 0) -> [TestTreeNode] {
                ...
                return [
                    TestTreeNode(
                        id: UUID(), title: "Level \(currentDepth)",
            ...
            measure {
                _ = BoxTreePattern(
                    data: deepTree, children: { $0.children }, expandedNodes: .constant(Set<UUID>())
                ) { node in Text(node.title) }
            }
        ...
        func testBoxTreePatternLazyLoadingOptimization() {
            ...
            measure {
                _ = BoxTreePattern(
                    data: rootNodes, children: { $0.children },
        ...
        func testInspectorPatternManySectionsRenderTime() {
            ...
            // When: Measure render time
            measure {
                _ = InspectorPattern(title: "Performance Test") {
                    ForEach(0..<sections.count, id: \.self) { index in sections[index] }
                }
            }
        ...
        func testInspectorPatternMemoryUsage() {
            ...
            let inspector = InspectorPattern(title: "Memory Test") {
                ForEach(0..<content.count, id: \.self) { index in content[index] }
            }
        ...
        func testInspectorPatternScrollPerformance() {
            ...
            // When: Measure render time
            measure {
                _ = InspectorPattern(title: "Scroll Test") {
        ...
        func testSidebarPatternManyItemsRenderTime() {
            ...
            let sidebarItems = (0..<200).map { index in
                SidebarPattern<Int, AnyView>.Item(id: index, title: "Item \(index)")
            }

            // When: Measure render time
            measure {
                _ = SidebarPattern<Int, AnyView>(
                    sections: [
                        SidebarPattern<Int, AnyView>.Section(title: "Items", items: sidebarItems)
                    ], selection: .constant(nil)
                ) { selectedId in
                    AnyView(
                        Group {
            ...
            let sections = (0..<20).map { sectionIndex in
                let items = (0..<20).map { itemIndex in
                    SidebarPattern<String, AnyView>.Item(
                        id: "Section\(sectionIndex)_Item\(itemIndex)", title: "Item \(itemIndex)")
                }
                return SidebarPattern<String, AnyView>.Section(
                    title: "Section \(sectionIndex)", items: items)
            }

            // When: Measure render time
            measure {
                _ = SidebarPattern<String, AnyView>(sections: sections, selection: .constant(nil)) {
                    selectedId in
                    AnyView(
                        Group {
                            if let selectedId { Text(selectedId) } else { Text("No selection") }
                        })
                }
            }
        ...
        func testToolbarPatternManyItemsRenderTime() {
            ...
            let toolbarItems = (0..<30).map { index in
                ToolbarPattern.Item(
                    id: "item\(index)", iconSystemName: "star.fill", title: "Item \(index)",
                    action: {})
            }

            // When: Measure render time
            measure { _ = ToolbarPattern(items: ToolbarPattern.Items(primary: toolbarItems)) }
        ...
        func testCombinedPatternsPerformance() {
            ...
            measure {
                _ = HStack(spacing: 0) {
                    ...
                    SidebarPattern<Int, AnyView>(
                        sections: [
                            SidebarPattern<Int, AnyView>.Section(
                                title: "Items", items: sidebarItems)
                        ], selection: .constant(nil)
                    ) { selectedId in
                        AnyView(
                            Group {
                    ...
                    // Tree
                    BoxTreePattern(
                        data: treeNodes, children: { $0.children },
                        expandedNodes: .constant(Set<UUID>())
                    ) { node in Text(node.title) }.frame(width: 300)
        ...
        func testPatternPerformanceWithAnimations() {
            ...
            measure {
                withAnimation(DS.Animation.medium) { isExpanded.toggle() }

                _ = BoxTreePattern(
                    data: nodes, children: { $0.children }, expandedNodes: .constant(Set<UUID>())
                ) { node in Text(node.title).opacity(isExpanded ? 1.0 : 0.8) }
            }
