...
@MainActor
final class IndicatorPerformanceTests: XCTestCase {
        ...
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for level in levels {
                for _ in 0..<(DS.PerformanceTest.componentCount / levels.count) {
                    let indicator = Indicator(level: level, size: .small, reason: level.accessibilityLabel)
                    _ = Mirror(reflecting: indicator).children.count
                }
            }
        }
        ...
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for size in sizes {
                for _ in 0..<(DS.PerformanceTest.componentCount / sizes.count) {
                    let indicator = Indicator(level: .warning, size: size, reason: "Checksum mismatch")
                    _ = Mirror(reflecting: indicator).children.count
                }
            }
        }
    ...
    func testIndicatorWithTooltipPerformance() {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for _ in 0..<DS.PerformanceTest.componentCount {
                ...
                let indicator = Indicator(level: .warning, size: .small, reason: "Checksum mismatch", tooltip: tooltip)
                _ = Mirror(reflecting: indicator).children.count
            }
        }
    }
