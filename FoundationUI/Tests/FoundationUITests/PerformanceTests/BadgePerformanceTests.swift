...
@MainActor final class BadgePerformanceTests: XCTestCase {
    // MARK: - Render Time Tests

    /// Test render performance for a single Badge component
    ///
    /// Target: <1ms render time
    @MainActor func testSingleBadgeRenderPerformance() throws {
    ...
    @MainActor func testBadgeWithIconRenderPerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for _ in 0..<DS.PerformanceTest.componentCount {
    ...
    @MainActor func testAllBadgeLevelsRenderPerformance() throws {
    ...
    @MainActor func testBadgeWithVaryingTextLengthPerformance() throws {
        let texts = [
            "X",  // 1 char
            "Info",  // 4 chars
    ...
    @MainActor func testMultipleBadges_10Instances() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            var badges: [Badge] = []
    ...
    @MainActor func testMultipleBadges_50Instances() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            var badges: [Badge] = []
            for index in 0..<DS.PerformanceTest.iterationCount {
                for i in 0..<50 {
    ...
    @MainActor func testMultipleBadges_100Instances() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            var badges: [Badge] = []
            for index in 0..<DS.PerformanceTest.iterationCount {
                for i in 0..<DS.PerformanceTest.componentCount {
                    let level = BadgeLevel.allCases[i % BadgeLevel.allCases.count]
                    let badge = Badge(
                        text: "Badge \(index)-\(i)", level: level, showIcon: i % 2 == 0)
                    badges.append(badge)
                }
            }
            _ = badges.count
    ...
    @MainActor func testBadgeInVStackPerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
    ...
    @MainActor func testBadgeInHStackPerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for _ in 0..<DS.PerformanceTest.iterationCount {
    ...
    @MainActor func testBadgeInScrollViewPerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for _ in 0..<DS.PerformanceTest.iterationCount {
                let view = ScrollView {
    ...
    @MainActor func testInfoBadgePerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for i in 0..<DS.PerformanceTest.componentCount {
    ...
    @MainActor func testWarningBadgePerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for i in 0..<DS.PerformanceTest.componentCount {
    ...
    @MainActor func testErrorBadgePerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for i in 0..<DS.PerformanceTest.componentCount {
    ...
    @MainActor func testSuccessBadgePerformance() throws {
        measure(metrics: PerformanceTestHelpers.cpuMetrics) {
            for i in 0..<DS.PerformanceTest.componentCount {
