    ...
    @MainActor final class IndicatorSnapshotTests: XCTestCase {
        ...
        func testIndicatorCatalogDarkMode() throws {
                ...
                throw XCTSkip("macOS snapshot tests disabled due to CI rendering inconsistencies")
            #else
                let view = indicatorCatalog().padding(DS.Spacing.l).preferredColorScheme(.dark)
        ...
        func testIndicatorSingleLightMode() throws {
            ...
            #else
                let view = Indicator(
                    level: .warning, size: .medium, reason: "Preview", tooltip: .text("Preview")
                ...
                ForEach(Array(BadgeLevel.allCases.enumerated()), id: \.offset) { _, level in
                    HStack(spacing: DS.Spacing.m) {
                        ForEach(Array(Indicator.Size.allCases.enumerated()), id: \.offset) {
                            _, size in
                            Indicator(
                                level: level, size: size, reason: "Snapshot",
        ...
        private func assertIndicatorSnapshot(
            _ view: some View, testName: String, file: StaticString = #filePath, line: UInt = #line
        ) {
            #if os(macOS)
                let renderResult = renderImage(for: view, scale: targetDisplayScale)
                logSnapshotMetadata(
                    platformName: snapshotPlatformName, viewSize: renderResult.viewSize,
                    pixelWidth: renderResult.pixelWidth, pixelHeight: renderResult.pixelHeight,
                    scale: renderResult.scale, colorSpace: renderResult.colorSpaceDescription,
                    screenScale: NSScreen.main?.backingScaleFactor)

                assertSnapshot(
                    of: renderResult.image, as: .image, named: snapshotPlatformName,
                    record: shouldRecordSnapshots, file: file, testName: testName, line: line)
            #else
                let traitCollection = UITraitCollection(displayScale: targetDisplayScale)
                logSnapshotMetadata(
                    platformName: snapshotPlatformName, viewSize: nil, pixelWidth: nil,
                    pixelHeight: nil, scale: traitCollection.displayScale,
                    colorSpace: "UIKit default", screenScale: traitCollection.displayScale)

                assertSnapshot(
                    of: view, as: .image(layout: .sizeThatFits, traits: traitCollection),
                    named: snapshotPlatformName, record: shouldRecordSnapshots, file: file,
            ...
            private func renderImage(for view: some View, scale: CGFloat) -> SnapshotRenderResult {
                ...
                guard
                    let context = CGContext(
                        data: nil, width: max(pixelWidth, 1), height: max(pixelHeight, 1),
                        bitsPerComponent: 8, bytesPerRow: 0, space: colorSpace,
                        bitmapInfo: CGImageAlphaInfo.premultipliedLast.rawValue)
                else { fatalError("Unable to allocate CGContext for snapshot.") }
                ...
                return SnapshotRenderResult(
                    image: image, viewSize: size, pixelWidth: pixelWidth, pixelHeight: pixelHeight,
                    scale: scale, colorSpaceDescription: colorSpaceName)
            }
        #endif

        private func logSnapshotMetadata(
            platformName: String, viewSize: CGSize?, pixelWidth: Int?, pixelHeight: Int?,
            scale: CGFloat, colorSpace: String, screenScale: CGFloat?
        ) {
            #if canImport(XCTest)
