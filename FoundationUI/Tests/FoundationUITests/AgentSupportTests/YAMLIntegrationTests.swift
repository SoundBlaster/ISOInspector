...
///
@available(iOS 17.0, macOS 14.0, *) @MainActor final class YAMLIntegrationTests: XCTestCase {
    ...
    func testParseInvalidYAMLGracefully() {
        ...
        } catch { XCTFail("Parsing should succeed even if validation fails: \(error)") }
    }
    ...
    func testAgentDescribableToYAMLToView() throws {
        ...
        let originalDescription = YAMLParser.ComponentDescription(
            componentType: "Badge",
            properties: ["text": "Agent Generated", "level": "success", "showIcon": true],
    ...
    func testLargeYAMLFilePerformance() throws {
        ...
        // Generate (sample - don't generate all 100)
            ...
            for i in 0..<10 { _ = try YAMLViewGenerator.generateView(from: descriptions[i]) }
