...
//
  ...
  final class PatternAgentDescribableTests: XCTestCase {
    ...
    func testInspectorPatternComponentType() {
      let inspector = InspectorPattern(title: "File Details") {
        Text("Content")
      }
    ...
    func testInspectorPatternSemantics() {
      let inspector = InspectorPattern(title: "Metadata") {
        Text("Content")
      }
    ...
    func testSidebarPatternProperties() {
      let sections = [
        SidebarPattern<String, Text>.Section(
          title: "Files",
          items: [
            SidebarPattern<String, Text>.Item(id: "item1", title: "Video", iconSystemName: "film")
          ]
        )
      ]
    ...
    func testBoxTreePatternComponentType() {
      let tree = BoxTreePattern(
        data: [TestNode(id: "root", name: "Root", children: [])],
        children: { $0.children }
      ) { node in
        Text(node.name)
      }
    ...
    func testInspectorPatternEmptyTitle() {
      let inspector = InspectorPattern(title: "") {
        Text("Content")
      }
