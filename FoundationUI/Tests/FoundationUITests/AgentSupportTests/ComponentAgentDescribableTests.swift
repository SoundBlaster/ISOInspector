...
final class ComponentAgentDescribableTests: XCTestCase {
    ...
    func testBadgeSemantics() {
        ...
        XCTAssertTrue(badge.semantics.contains("Error"), "Badge semantics should contain the text")
        XCTAssertTrue(badge.semantics.contains("error"), "Badge semantics should contain the level")
    }

    func testBadgeJSONSerialization() {
        let badge = Badge(text: "Success", level: .success, showIcon: false)

        XCTAssertTrue(badge.isJSONSerializable(), "Badge properties should be JSON serializable")
    }
    ...
    func testCardSemantics() {
        ...
        XCTAssertTrue(card.semantics.contains("high"), "Card semantics should contain elevation level")
    }
    ...
    func testKeyValueRowComponentType() {
        ...
        XCTAssertEqual(row.componentType, "KeyValueRow", "KeyValueRow should have componentType 'KeyValueRow'")
    }
    ...
    func testKeyValueRowSemantics() {
        ...
        XCTAssertTrue(row.semantics.contains("Offset"), "KeyValueRow semantics should contain the key")
        XCTAssertTrue(row.semantics.contains("0x1000"), "KeyValueRow semantics should contain the value")
    }

    func testKeyValueRowJSONSerialization() {
        let row = KeyValueRow(key: "Hash", value: "0xDEADBEEF", layout: .vertical, copyable: true)

        XCTAssertTrue(row.isJSONSerializable(), "KeyValueRow properties should be JSON serializable")
    }
    ...
    func testSectionHeaderComponentType() {
        ...
        XCTAssertEqual(header.componentType, "SectionHeader", "SectionHeader should have componentType 'SectionHeader'")
    }
    ...
    func testSectionHeaderDefaultDivider() {
        ...
        XCTAssertEqual(header.properties["showDivider"] as? Bool, false, "Default showDivider should be false")
    }
    ...
    func testSectionHeaderSemantics() {
        ...
        XCTAssertTrue(header.semantics.contains("Technical Details"), "SectionHeader semantics should contain the title")
    }

    func testSectionHeaderJSONSerialization() {
        let header = SectionHeader(title: "Box Information", showDivider: false)

        XCTAssertTrue(header.isJSONSerializable(), "SectionHeader properties should be JSON serializable")
    }
