...
@MainActor final class PlatformAdaptationTests: XCTestCase {
    ...
    func testSizeClassMapping_NilSizeClass() {
        ...
        #if os(macOS)
            XCTAssertEqual(
                spacing, DS.Spacing.m, "Nil size class should use platform default (macOS)")
        #else
            XCTAssertEqual(
                spacing, DS.Spacing.l, "Nil size class should use platform default (iOS)")
        #endif
    }

    // MARK: - ViewModifier Integration Tests

    @MainActor func testPlatformAdaptiveModifier_AppliesPlatformSpacing() {
        let view = Text("Test").platformAdaptive()

        // Test that modifier can be applied without errors
        XCTAssertNotNil(view, "platformAdaptive() modifier should be applicable")
    }

    @MainActor func testPlatformAdaptiveModifier_WithCustomSpacing() {
        let customSpacing = DS.Spacing.xl
        let view = Text("Test").platformAdaptive(spacing: customSpacing)

        XCTAssertNotNil(view, "platformAdaptive(spacing:) modifier should accept custom spacing")
    }

    @MainActor func testPlatformAdaptiveModifier_WithSizeClass() {
        let view = Text("Test").platformAdaptive(sizeClass: .compact)
    ...
    @MainActor func testViewExtension_PlatformSpacing() {
        let view = Text("Test").platformSpacing()

        XCTAssertNotNil(view, "platformSpacing() extension should be applicable")
    }

    @MainActor func testViewExtension_PlatformSpacingWithValue() {
        let customValue = DS.Spacing.xl
        let view = Text("Test").platformSpacing(customValue)

        XCTAssertNotNil(view, "platformSpacing(_:) extension should accept custom value")
    }

    @MainActor func testViewExtension_PlatformPadding() {
        let view = Text("Test").platformPadding()

        XCTAssertNotNil(view, "platformPadding() extension should be applicable")
    }

    @MainActor func testViewExtension_PlatformPaddingWithEdges() {
        let view = Text("Test").platformPadding(.horizontal)
    ...
    // MARK: - Minimum Touch Target Tests (iOS)
        ...
        func testMinimumTouchTarget_EnsuresAccessibility() {
            ...
            let minTarget = PlatformAdapter.minimumTouchTarget
            XCTAssertEqual(
                minTarget, 44.0, "iOS minimum touch target should be exactly 44pt per HIG")
        }
    #endif

    // MARK: - Integration Tests

    @MainActor func testPlatformAdaptation_IntegrationWithComponents() {
    ...
    func testPlatformAdaptation_ZeroMagicNumbers() {
        ...
        XCTAssertTrue(
            validTokens.contains(spacing), "Platform spacing should only use DS token values")
    }
