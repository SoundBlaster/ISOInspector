...
@MainActor final class PlatformExtensionsTests: XCTestCase {
    // MARK: - macOS Keyboard Shortcut Tests

    #if os(macOS)
        @MainActor func testMacOSKeyboardShortcut_Copy() {
            // Test that copy shortcut (⌘C) can be applied
            let view = Text("Test").platformKeyboardShortcut(.copy)

            XCTAssertNotNil(view, "Copy keyboard shortcut should be applicable on macOS")
        }

        @MainActor func testMacOSKeyboardShortcut_Paste() {
            // Test that paste shortcut (⌘V) can be applied
            let view = Text("Test").platformKeyboardShortcut(.paste)

            XCTAssertNotNil(view, "Paste keyboard shortcut should be applicable on macOS")
        }

        @MainActor func testMacOSKeyboardShortcut_Cut() {
            // Test that cut shortcut (⌘X) can be applied
            let view = Text("Test").platformKeyboardShortcut(.cut)

            XCTAssertNotNil(view, "Cut keyboard shortcut should be applicable on macOS")
        }

        @MainActor func testMacOSKeyboardShortcut_SelectAll() {
            // Test that select all shortcut (⌘A) can be applied
            let view = Text("Test").platformKeyboardShortcut(.selectAll)

            XCTAssertNotNil(view, "Select all keyboard shortcut should be applicable on macOS")
        }

        @MainActor func testMacOSKeyboardShortcut_CustomAction() {
            var actionCalled = false
            let action = { actionCalled = true }

            let view = Text("Test").platformKeyboardShortcut(.copy, action: action)
    ...
    // MARK: - iOS Gesture Tests
    ...
    #if os(iOS)
        @MainActor func testIOSGesture_Tap() {
            var tapCount = 0
            let view = Text("Test").platformTapGesture { tapCount += 1 }

            XCTAssertNotNil(view, "Tap gesture should be applicable on iOS")
        }

        @MainActor func testIOSGesture_DoubleTap() {
            var doubleTapCount = 0
            let view = Text("Test").platformTapGesture(count: 2) { doubleTapCount += 1 }

            XCTAssertNotNil(view, "Double tap gesture should be applicable on iOS")
        }

        @MainActor func testIOSGesture_LongPress() {
            var longPressCount = 0
            let view = Text("Test").platformLongPressGesture { longPressCount += 1 }

            XCTAssertNotNil(view, "Long press gesture should be applicable on iOS")
        }

        @MainActor func testIOSGesture_LongPressWithMinimumDuration() {
            var longPressCount = 0
            let view = Text("Test").platformLongPressGesture(minimumDuration: 0.5) {
                longPressCount += 1
            }
        ...
        @MainActor func testIOSGesture_Swipe() {
            var swipeCount = 0
            let view = Text("Test").platformSwipeGesture(direction: .left) { swipeCount += 1 }
    ...
    #if os(iOS)
        @MainActor func testIPadOSPointerInteraction_HoverEffect() {
            // Test that hover effect can be applied (runtime detection for iPad)
            let view = Text("Test").platformHoverEffect()

            XCTAssertNotNil(view, "Hover effect should be applicable on iOS")
        }

        @MainActor func testIPadOSPointerInteraction_HoverEffectWithStyle() {
            let view = Text("Test").platformHoverEffect(.lift)

            XCTAssertNotNil(view, "Hover effect with style should be applicable")
        }

        @MainActor func testIPadOSPointerInteraction_OnlyAppliesToIPad() {
    ...
    @MainActor func testCrossPlatform_ConditionalCompilation() {
    ...
    @MainActor func testIntegration_MultipleGestures() {
        #if os(iOS)
            var tapCount = 0
            var longPressCount = 0

            let view = Text("Test").platformTapGesture { tapCount += 1 }.platformLongPressGesture {
                longPressCount += 1
            }
    ...
    @MainActor func testIntegration_GesturesWithAnimation() {
        #if os(iOS)
            let view = Text("Test").platformTapGesture {}.animation(DS.Animation.quick, value: true)
    ...
    @MainActor func testEdgeCase_NilAction() {
        #if os(iOS)
            // Test that gesture with no-op action doesn't crash
            let view = Text("Test").platformTapGesture {}

            XCTAssertNotNil(view, "Empty action should not cause issues")
        #endif
    }

    @MainActor func testEdgeCase_ChainedModifiers() {
        #if os(iOS)
            // Test complex modifier chains
            let view = Text("Test").platformTapGesture {}.platformHoverEffect().padding(
                DS.Spacing.m)
