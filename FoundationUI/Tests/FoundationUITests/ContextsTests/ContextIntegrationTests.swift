    ...
    @MainActor final class ContextIntegrationTests: XCTestCase {
        ...
        @MainActor func testSurfaceStylePropagation_NestedComponents() throws {
            struct NestedView: View {
                @Environment(\.surfaceStyle) var surfaceStyle

                var actualStyle: SurfaceMaterial { surfaceStyle }

                var body: some View { Text("Nested") }
            }

            // Test propagation through view hierarchy
            let view = NestedView().environment(\.surfaceStyle, .thick)
        ...
        @MainActor func testMultipleEnvironmentKeys_Propagation() throws {
            struct MultiEnvView: View {
                @Environment(\.surfaceStyle) var surfaceStyle
                @Environment(\.colorScheme) var colorScheme

                var body: some View { Text("Multi-env") }
            }

            let view = MultiEnvView().environment(\.surfaceStyle, .regular).preferredColorScheme(
                .dark)
        ...
        @MainActor func testPlatformAdapter_WithComponents() throws {
            let badge = Text("Test").padding(PlatformAdapter.defaultSpacing)
        ...
        @MainActor func testPlatformSpacing_InComplexHierarchy() throws {
        ...
        @MainActor func testPlatformAdapter_ModifierIntegration() throws {
            let view = Text("Test").platformAdaptive().padding(DS.Spacing.m)
        ...
        @MainActor func testColorSchemeAdapter_WithEnvironment() throws {
            struct AdaptiveView: View {
                @Environment(\.colorScheme) var colorScheme

                var body: some View {
                    let adapter = ColorSchemeAdapter(colorScheme: colorScheme)
                    return Text("Adaptive").foregroundColor(adapter.adaptiveTextColor).background(
        ...
        @MainActor func testColorSchemeAdapter_WithAllComponents() throws {
            let lightAdapter = ColorSchemeAdapter(colorScheme: .light)

            // Test with various component types
            let textView = Text("Test").foregroundColor(lightAdapter.adaptiveTextColor)

            let backgroundView = VStack {}.background(lightAdapter.adaptiveBackground)

            let borderView = RoundedRectangle(cornerRadius: DS.Radius.medium).stroke(
                lightAdapter.adaptiveBorderColor, lineWidth: 1)
        ...
        @MainActor func testCrossContext_AllThreeContexts() throws {
            struct MultiContextView: View {
                ...
                var body: some View {
                    ...
                    return VStack(spacing: PlatformAdapter.defaultSpacing) {
                        Text("Multi-context").foregroundColor(adapter.adaptiveTextColor)
                    }.padding(PlatformAdapter.defaultSpacing).background(adapter.adaptiveBackground)
                }
            }

            let view = MultiContextView().environment(\.surfaceStyle, .regular)
        ...
        @MainActor func testContexts_NoConflicts() throws {
            struct ConflictTestView: View {
                ...
                var body: some View {
                    let adapter = ColorSchemeAdapter(colorScheme: colorScheme)
                    return Text("Test").platformAdaptive().foregroundColor(
                        adapter.adaptiveTextColor)
                }
            }

            // Apply multiple context modifications
            let view = ConflictTestView().environment(\.surfaceStyle, .thick).preferredColorScheme(
                .dark
            ).platformAdaptive()
        ...
        @MainActor func testContexts_OrderIndependence() throws {
            let adapter = ColorSchemeAdapter(colorScheme: .light)

            // Order 1: Platform -> Color -> Surface
            let view1 = Text("Test 1").platformAdaptive().foregroundColor(adapter.adaptiveTextColor)
                .environment(\.surfaceStyle, .regular)

            // Order 2: Surface -> Platform -> Color
            let view2 = Text("Test 2").environment(\.surfaceStyle, .regular).platformAdaptive()
        ...
        func testSizeClass_CompactAdaptation() throws {
            ...
            XCTAssertEqual(
                compactSpacing, DS.Spacing.m, "Compact size class should use medium spacing")
            XCTAssertEqual(compactSpacing, 12.0, "Compact spacing should be 12pt per DS.Spacing.m")
        }
        ...
        func testSizeClass_RegularAdaptation() throws {
            ...
            XCTAssertEqual(
                regularSpacing, DS.Spacing.l, "Regular size class should use large spacing")
            XCTAssertEqual(regularSpacing, 16.0, "Regular spacing should be 16pt per DS.Spacing.l")
        }
        ...
        @MainActor func testSizeClass_ViewIntegration() throws {
            let compactView = Text("Compact").platformAdaptive(sizeClass: .compact)

            let regularView = Text("Regular").platformAdaptive(sizeClass: .regular)
        ...
        @MainActor func testInspectorScreen_AllContexts() throws {
                ...
                var body: some View {
                    ...
                    return HStack(spacing: 0) {
                        ...
                        VStack(spacing: PlatformAdapter.defaultSpacing) {
                            Text("Content").foregroundColor(adapter.adaptiveTextColor)
                        }.frame(maxWidth: .infinity).padding(PlatformAdapter.defaultSpacing)
                            .background(adapter.adaptiveBackground)

                        // Inspector panel
                        VStack(spacing: PlatformAdapter.defaultSpacing) {
                            Text("Inspector").foregroundColor(adapter.adaptiveTextColor)
                        }.frame(width: 250).padding(PlatformAdapter.defaultSpacing).background(
                            adapter.adaptiveSecondaryBackground)
                    }
                }
            }

            let screen = InspectorScreen().environment(\.surfaceStyle, .thick)
        ...
        @MainActor func testSidebarLayout_PlatformAdaptive() throws {
            struct SidebarLayout: View {
                ...
                var body: some View {
                    ...
                    return HStack(spacing: 0) {
                        // Sidebar
                        VStack(spacing: PlatformAdapter.defaultSpacing) { Text("Sidebar") }.frame(
                            width: 200
                        ).platformPadding().background(adapter.adaptiveSecondaryBackground)

                        // Main content
                        VStack(spacing: PlatformAdapter.defaultSpacing) { Text("Main") }
                            .platformPadding().background(adapter.adaptiveBackground)
                    }
                }
            }
        ...
        @MainActor func testEdgeCase_ExtremeNesting() throws {
            struct DeeplyNested: View {
                ...
                var body: some View {
                    ...
                    return VStack(spacing: PlatformAdapter.defaultSpacing) {
                        VStack(spacing: PlatformAdapter.defaultSpacing) {
                            VStack(spacing: PlatformAdapter.defaultSpacing) {
                                Text("Deep").foregroundColor(adapter.adaptiveTextColor)
                            }
                        }
                    }.environment(\.surfaceStyle, .regular)
                }
