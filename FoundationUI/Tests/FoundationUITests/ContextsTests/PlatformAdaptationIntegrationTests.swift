    ...
    @MainActor final class PlatformAdaptationIntegrationTests: XCTestCase {
        ...
        func testMacOS_DefaultSpacing() throws {
                ...
                XCTAssertEqual(spacing, 12, "macOS default spacing should be 12pt")

                // Verify spacing is a valid DS token
                let validTokens: Set<CGFloat> = [
                    DS.Spacing.s, DS.Spacing.m, DS.Spacing.l, DS.Spacing.xl,
                ]
                XCTAssertTrue(
        ...
        @MainActor func testMacOS_InspectorPatternSpacing() throws {
            #if os(macOS)
                ...
                let spacing = PlatformAdapter.defaultSpacing
                XCTAssertEqual(
                    spacing, DS.Spacing.m, "InspectorPattern on macOS should use DS.Spacing.m")
            #else
                throw XCTSkip("This test requires macOS")
            #endif
        }
        ...
        @MainActor func testMacOS_ClipboardIntegration() throws {
            #if os(macOS)
                // Verify platform is macOS
                XCTAssertTrue(PlatformAdapter.isMacOS, "Clipboard test requires macOS")

                // Create CopyableText component
                let copyableText = CopyableText(text: "Test clipboard text").platformAdaptive()
        ...
        @MainActor func testMacOS_KeyboardShortcuts() throws {
            #if os(macOS)
                // Verify platform
                ...
                let buttonWithShortcut = Button("Copy") {}.keyboardShortcut(
                    "c", modifiers: .command
                ).platformAdaptive()
                ...
                let spacing = PlatformAdapter.defaultSpacing
                XCTAssertEqual(
                    spacing, DS.Spacing.m, "macOS keyboard shortcut views should use DS.Spacing.m")
            #else
                throw XCTSkip("This test requires macOS")
            #endif
        }
        ...
        @MainActor func testMacOS_HoverEffects() throws {
            #if os(macOS)
                // Create interactive card with hover effects
                let hoverCard = Card { Text("Hover me") }.interactiveStyle().platformAdaptive()

                XCTAssertNotNil(
                    hoverCard, "macOS should support hover effects via InteractiveStyle")

                // Verify platform spacing
                let spacing = PlatformAdapter.defaultSpacing
                XCTAssertEqual(
                    spacing, DS.Spacing.m, "Interactive elements on macOS should use DS.Spacing.m")
            #else
                throw XCTSkip("This test requires macOS")
            #endif
        }
        ...
        @MainActor func testMacOS_SidebarPatternLayout() throws {
            #if os(macOS)
                ...
                let sidebar = SidebarPattern(
                    sections: [section], selection: selectedItem,
                    detail: { selectedId in Text(selectedId ?? "No selection") })
                ...
                let spacing = PlatformAdapter.defaultSpacing
                XCTAssertEqual(
                    spacing, DS.Spacing.m, "SidebarPattern on macOS should use DS.Spacing.m")
            #else
                throw XCTSkip("This test requires macOS")
            #endif
        }
        ...
        func testIOS_DefaultSpacing() throws {
                ...
                XCTAssertEqual(spacing, 16, "iOS default spacing should be 16pt")
        ...
        func testIOS_MinimumTouchTarget() throws {
                ...
                XCTAssertEqual(minTarget, 44, "iOS minimum touch target must be 44pt per Apple HIG")
                XCTAssertGreaterThanOrEqual(
                    minTarget, 44, "Touch targets must meet accessibility requirements")
        ...
        @MainActor func testIOS_TouchTargetOnBadge() throws {
            #if os(iOS)
                let badge = Badge(text: "Tap Me", level: .info).platformAdaptive().frame(
                    minWidth: PlatformAdapter.minimumTouchTarget,
                ...
                let minTarget = PlatformAdapter.minimumTouchTarget
                XCTAssertEqual(minTarget, 44, "Badge should meet iOS minimum touch target")

                // Verify spacing is iOS-appropriate
                let spacing = PlatformAdapter.defaultSpacing
                XCTAssertEqual(spacing, DS.Spacing.l, "Badge on iOS should use DS.Spacing.l")
            #else
                throw XCTSkip("This test requires iOS")
            #endif
        }
        ...
        @MainActor func testIOS_ClipboardIntegration() throws {
            #if os(iOS)
                ...
                // Verify iOS spacing
                let spacing = PlatformAdapter.defaultSpacing
                XCTAssertEqual(spacing, DS.Spacing.l, "CopyableText on iOS should use DS.Spacing.l")
        ...
        @MainActor func testIOS_GestureSupport() throws {
            #if os(iOS)
                // Create a card with tap gesture
                let gestureCard = Card { Text("Tap or Long Press") }.onTapGesture {}
                    .onLongPressGesture {}.platformAdaptive().frame(
                        minHeight: PlatformAdapter.minimumTouchTarget)
                ...
                // Verify touch target
                let minTarget = PlatformAdapter.minimumTouchTarget
                XCTAssertEqual(minTarget, 44, "Gesture targets should meet iOS minimum")
            #else
                throw XCTSkip("This test requires iOS")
            #endif
        }
        ...
        @MainActor func testIOS_InspectorPatternLayout() throws {
            #if os(iOS)
                let inspector = InspectorPattern(title: "iOS Inspector") {
                    VStack(spacing: PlatformAdapter.defaultSpacing) {
                        Card { Text("Metadata") }
                        Card { Text("Properties") }
                        Badge(text: "Status", level: .success).frame(
                            minHeight: PlatformAdapter.minimumTouchTarget)
                    }.padding(PlatformAdapter.defaultSpacing)
                }
                ...
                let spacing = PlatformAdapter.defaultSpacing
                XCTAssertEqual(
                    spacing, DS.Spacing.l, "InspectorPattern on iOS should use DS.Spacing.l")

                // Verify touch target size
                let minTarget = PlatformAdapter.minimumTouchTarget
                XCTAssertEqual(
                    minTarget, 44, "Interactive elements should meet touch target minimum")
            #else
                throw XCTSkip("This test requires iOS")
            #endif
        }
        ...
        func testIPad_CompactSizeClassSpacing() throws {
                ...
                XCTAssertEqual(
                    compactSpacing, DS.Spacing.m, "Compact size class should use DS.Spacing.m")
                XCTAssertEqual(
                    compactSpacing, 12, "Compact spacing should be 12pt for efficient space usage")
                ...
                XCTAssertTrue(
                    validTokens.contains(compactSpacing), "Compact spacing should use DS tokens")
            #else
        ...
        func testIPad_RegularSizeClassSpacing() throws {
                ...
                XCTAssertEqual(
                    regularSpacing, DS.Spacing.l, "Regular size class should use DS.Spacing.l")
                XCTAssertEqual(
                    regularSpacing, 16,
                ...
                XCTAssertTrue(
                    validTokens.contains(regularSpacing), "Regular spacing should use DS tokens")
            #else
                throw XCTSkip("This test requires iOS/iPadOS")
            #endif
        }
        ...
        @MainActor func testIPad_InspectorSizeClassAdaptation() throws {
            #if os(iOS)
        ...
        @MainActor func testIPad_SidebarAdaptation() throws {
            #if os(iOS)
                // Create sidebar items using the correct structure
                let items = [
                ...
                // Create a section containing the items
                let section = SidebarPattern<String, Text>.Section(title: "Main", items: items)
        ...
        @MainActor func testIPad_PointerInteractionSupport() throws {
            #if os(iOS)
                // Create interactive card for iPad pointer support
                let interactiveCard = Card { Text("Supports Pointer") }.interactiveStyle()
                    .platformAdaptive().frame(
                        minWidth: PlatformAdapter.minimumTouchTarget,
                        minHeight: PlatformAdapter.minimumTouchTarget)
                ...
                let spacing = PlatformAdapter.defaultSpacing
                XCTAssertEqual(
                    spacing, DS.Spacing.l, "iPad should use iOS default spacing (DS.Spacing.l)")
            #else
                throw XCTSkip("This test requires iOS/iPadOS")
            #endif
        }
        ...
        @MainActor func testIPad_SplitViewLayout() throws {
            #if os(iOS)
                struct SplitViewContent: View {
                    ...
                    var body: some View {
                        ...
                        return HStack(spacing: spacing) {
                            VStack(spacing: spacing) { Text("Sidebar") }.frame(maxWidth: 200)

                            VStack(spacing: spacing) { Text("Main Content") }.frame(
                                maxWidth: .infinity)
                        }.padding(spacing)
                    }
                }
                ...
                let compactView = SplitViewContent().environment(\.horizontalSizeClass, .compact)

                XCTAssertNotNil(compactView, "Split view should work in compact mode")

                // Test with regular size class (full screen)
                let regularView = SplitViewContent().environment(\.horizontalSizeClass, .regular)
        ...
        func testCrossPlatform_DSTokenConsistency() throws {
            ...
            XCTAssertTrue(
                validTokens.contains(defaultSpacing), "Platform default spacing must use a DS token"
            )
        }
        ...
        @MainActor func testCrossPlatform_BadgeConsistency() throws {
            ...
            let adaptiveBadge = Badge(text: "Adaptive", level: .info).platformAdaptive()

            XCTAssertNotNil(
                adaptiveBadge, "Badge with platform-adaptive spacing should work everywhere")
            ...
            XCTAssertTrue(
                validTokens.contains(spacing), "Badge spacing should use DS tokens on all platforms"
            )
        }
        ...
        func testCrossPlatform_DarkModeConsistency() throws {
            ...
            XCTAssertNotNil(
                lightAdapter.adaptiveBackground, "Light background should exist on all platforms")
            XCTAssertNotNil(
                darkAdapter.adaptiveBackground, "Dark background should exist on all platforms")

            XCTAssertNotNil(
                lightAdapter.adaptiveTextColor, "Light text color should exist on all platforms")
            XCTAssertNotNil(
                darkAdapter.adaptiveTextColor, "Dark text color should exist on all platforms")

            XCTAssertNotNil(
                lightAdapter.adaptiveBorderColor, "Light border color should exist on all platforms"
            )
            XCTAssertNotNil(
                darkAdapter.adaptiveBorderColor, "Dark border color should exist on all platforms")
            ...
            XCTAssertNotNil(
                darkAdapter.adaptiveDividerColor, "Dark divider color should exist on all platforms"
            )
        }
        ...
        @MainActor func testCrossPlatform_AccessibilityConsistency() throws {
            // Create accessible components
            let badge = Badge(text: "Accessible", level: .info).accessibilityLabel(
                "Info badge: Accessible")

            let card = Card {
                Text("Accessible Content").font(DS.Typography.body)  // Supports Dynamic Type
            ...
            XCTAssertNotNil(
                captionFont, "Caption font should support Dynamic Type on all platforms")
        }
        ...
        @MainActor func testCrossPlatform_EnvironmentPropagation() throws {
            struct NestedView: View {
                ...
                var body: some View {
                    ...
                    return VStack(spacing: PlatformAdapter.defaultSpacing) {
                        Card { Text("Nested Level 1") }
                        Card { VStack { Text("Nested Level 2") } }
                    }.foregroundColor(adapter.adaptiveTextColor)
                }
            }

            let view = NestedView().environment(\.surfaceStyle, .thick).preferredColorScheme(.dark)
            ...
            XCTAssertTrue(
                validTokens.contains(spacing), "Environment propagation should use DS tokens")
        }
        ...
        func testCrossPlatform_ZeroMagicNumbers() throws {
            ...
            XCTAssertTrue(validTokens.contains(defaultSpacing), "Default spacing must use DS token")
            XCTAssertTrue(validTokens.contains(compactSpacing), "Compact spacing must use DS token")
            XCTAssertTrue(validTokens.contains(regularSpacing), "Regular spacing must use DS token")
            XCTAssertTrue(
                validTokens.contains(nilSpacing), "Nil size class spacing must use DS token")

            // Verify specific token mappings
            XCTAssertEqual(compactSpacing, DS.Spacing.m, "Compact should map to DS.Spacing.m")
            XCTAssertEqual(regularSpacing, DS.Spacing.l, "Regular should map to DS.Spacing.l")

            #if os(macOS)
                XCTAssertEqual(defaultSpacing, DS.Spacing.m, "macOS default should be DS.Spacing.m")
            #elseif os(iOS)
                XCTAssertEqual(defaultSpacing, DS.Spacing.l, "iOS default should be DS.Spacing.l")

                // iOS touch target is a documented constant (Apple HIG)
                let minTarget = PlatformAdapter.minimumTouchTarget
                XCTAssertEqual(
                    minTarget, 44, "iOS touch target is 44pt per Apple HIG (documented constant)")
            #endif
        }
        ...
        func testEdgeCase_NilSizeClass() throws {
            ...
            XCTAssertEqual(
                spacing, PlatformAdapter.defaultSpacing,
                "Nil size class should fall back to platform default")

            // Platform default should be a DS token
            #if os(macOS)
                XCTAssertEqual(
                    spacing, DS.Spacing.m, "macOS nil size class should use DS.Spacing.m")
            #elseif os(iOS)
                XCTAssertEqual(spacing, DS.Spacing.l, "iOS nil size class should use DS.Spacing.l")
            #endif
            ...
            XCTAssertTrue(
                validTokens.contains(spacing), "Nil size class fallback should use DS token")
        }
        ...
        /// - Spacing uses DS token
        @MainActor func testEdgeCase_UnknownSizeClass() throws {
            // Create view that uses platformAdaptive with size class
            let view = Text("Test").platformAdaptive(sizeClass: nil)  // nil represents unknown
            ...
            XCTAssertEqual(
                fallbackSpacing, PlatformAdapter.defaultSpacing,
        ...
        /// - All spacing uses DS tokens
        @MainActor func testComplexHierarchy_PlatformAdaptation() throws {
            ...
            XCTAssertTrue(
                validTokens.contains(spacing), "Complex hierarchy spacing should use DS tokens")
        }
        ...
        @MainActor func testPlatformExtensions_UseDSTokens() throws {
            // Test platformSpacing()
            let spacedView = Text("Test").platformSpacing()

            XCTAssertNotNil(spacedView, "platformSpacing() should work")

            // Test platformPadding()
            let paddedView = Text("Test").platformPadding()

            XCTAssertNotNil(paddedView, "platformPadding() should work")

            // Test platformAdaptive()
            let adaptiveView = Text("Test").platformAdaptive()
            ...
            XCTAssertTrue(
                validTokens.contains(defaultSpacing), "All platform extensions should use DS tokens"
            )

            // Test custom spacing with DS tokens
            let customSpacedView = Text("Test").platformSpacing(DS.Spacing.xl)
